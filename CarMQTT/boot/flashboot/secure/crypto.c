...
hi_void crypto_check_decrypt(hi_void)
{
    ...
    if (((burn_encrypt_flag == HI_FALSE) && (upg_encrypt_flag == HI_FALSE)) ||
        ((upg_encrypt_flag == HI_TRUE) && (cfg->file_type == HI_UPG_FILE_BOOT))) {
        /* 解密到ram中 */
        ret = crypto_decrypt(KERNEL_RAM_ADDR, CRYPTO_KERNEL_LENGTH);
        if (ret != HI_ERR_SUCCESS) {
            boot_put_errno(ret);
        }
    } else if ((upg_encrypt_flag == HI_TRUE) && (cfg->file_type == HI_UPG_FILE_KERNEL)) {
#if !defined(CONFIG_COMPRESSION_OTA_SUPPORT)
        /* 解密到ram中 */
        ret = crypto_decrypt(KERNEL_RAM_ADDR, CRYPTO_KERNEL_LENGTH);
        if (ret != HI_ERR_SUCCESS) {
...
static const hi_u8 g_upg_file_magic[IV_BYTE_LENGTH] = { 0xE4, 0xEE, 0x10, 0x0E, 0x43, 0x4D, 0x94, 0x24,
