...
{
    hi_u32 ret;
    hi_u8 flash_encpyt_enable = 0;
    hi_u8 no_encpyt = 1;
    hi_u8 encpyt_cnt_id = 0;

    if (burn_efuse) {
        if (hi_efuse_read(HI_EFUSE_FLASH_ENCPY_CFG_RW_ID, &flash_encpyt_enable, 1) != HI_ERR_SUCCESS) {
            return HI_ERR_FAILURE;
        }

        if (((flash_encpyt_enable & 1) == 1) && (addr == 0) && (erase_size > FLASHBOOT_MAX_SIZE)) {
            encpyt_cnt_id = get_writeable_encpt_cnt();
            if (encpyt_cnt_id == 0) {
                boot_put_errno(HI_PRINT_ERRNO_NO_ENCPT_CNT_LEFT_ERR);
                return HI_ERR_FAILURE;
            }
        }
    }
    ...
    if (burn_efuse) {
        if (((flash_encpyt_enable & 1) == 1) && (addr == 0) && (erase_size > FLASHBOOT_MAX_SIZE)) {
            if (hi_efuse_write((hi_efuse_idx)encpyt_cnt_id, &no_encpyt) != HI_ERR_SUCCESS) {
                return HI_ERR_FAILURE;
            }
        }
    }
