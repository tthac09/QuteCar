...
static hi_u32 codec_set_gain(hi_void)
{
    ...
    ret |= codec_write_reg(ES8311_SYSTEM_REG0E, 0x02);  /* enable analog pga/adc modulator(手册没有bit1、0) */
    /* normal power mode */
...
hi_u32 hi_codec_init(const hi_codec_attribute *codec_attr)
{
    ...
    ret = codec_write_reg(ES8311_DAC_REG31, 0x40);      /* mute DAC状态机 */
    ret |= codec_write_reg(ES8311_RESET_REG00, 0x1F);   /* 复位时钟、从模式 */

    ret |= codec_write_reg(ES8311_GP_REG45, 0x00);      /* 拉高BCLK/LRCK */

    ret |= codec_write_reg(ES8311_CLK_MANAGER_REG01, 0x30);     /* MCLK、BCLK开 */

    ret |= codec_write_reg(ES8311_CLK_MANAGER_REG02, 0x10);     /* AD/DA/全部，用时0x10   dig_mclk = mclk_prediv*4 */
    ...
    ret |= codec_write_reg(ES8311_CLK_MANAGER_REG05, 0x00);     /* ADC,DAC时钟分频系数，决定ad和da的工作频率 */

    ret |= codec_write_reg(ES8311_SYSTEM_REG0B, 0x00);  /* 决定power up的时间 */
    ret |= codec_write_reg(ES8311_SYSTEM_REG0C, 0x00);  /* 决定power up的时间 */
    ret |= codec_write_reg(ES8311_SYSTEM_REG10, 0x1F);  /* bias设置 */
     ...
     */
    ret |= codec_write_reg(ES8311_CLK_MANAGER_REG01, 0x3F);     /* 开采样工作时钟 */

    /* 选择mic通道1，1 – select Mic1p-Mic1n ; ADC PGA gain:4 – 12dB */
    ...
    ret |= codec_set_gain();
    ret |= codec_write_reg(ES8311_DAC_REG31, 0x00);     /* unmute DAC状态机 */
