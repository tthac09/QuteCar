...
static void wpa_supplicant_mesh_deinit(struct wpa_supplicant *wpa_s)
{
	...
	if (wifi_dev == NULL) {
		wpa_error_log0(MSG_ERROR, "wpa_supplicant_mesh_deinit: get wifi dev with mesh mode failed\n");
		return;
	}
					     ...
					     struct wpa_ssid *ssid)
{
	...
	/* hisi mesh do not surpport PMF set */
	conf->ieee80211w = NO_MGMT_FRAME_PROTECTION;
#endif
...
#endif
{
...
#else
	static int default_groups[] = { 28, -1 };
	...
	if ((wifi_dev == NULL) || ((wifi_dev->iftype != HI_WIFI_IFTYPE_STATION) &&
		...
		wpa_error_log0(MSG_ERROR, "wpa_supplicant_mesh_init: get wifi dev or iftype failed\n");
		return -1;
	}
	...
	if (frequency != freq->freq &&
		...
		wpa_warning_log0(MSG_DEBUG, "mesh: pri/sec channels switched");
		frequency = freq->freq;
	}
#endif
	...
	conf = hostapd_config_read2(wpa_s->ifname);
	if (!conf)
		goto out_free;
	...
	if (conf->hw_mode == NUM_HOSTAPD_MODES) {
		wpa_error_log1(MSG_ERROR, "Unsupported mesh mode frequency: %d MHz",
			   frequency);
		goto out_free;
	}
#ifndef HISI_CODE_CROP
...
#else
		bss->conf->sae_groups[0] = default_groups[0];
		bss->conf->sae_groups[1] = default_groups[1];
#endif
#ifndef HISI_CODE_CROP
			     ...
			     struct wpa_ssid *ssid)
{
	...
	if (hi_get_wifi_dev_by_iftype(HI_WIFI_IFTYPE_AP) != NULL) {
		wpa_printf(MSG_ERROR, "wpa_supplicant_join_mesh: AP is in use, should not start mesh.\n");
		ret = -ENOENT;
		goto out;
	}
	if (wpa_mesh_ap_config_set(ssid) == HISI_FAIL) {
		ret = -ENOENT;
		goto out;
	}
...
#endif
	wpa_msg(wpa_s, MSG_ERROR, "joining mesh %s",
...
int wpa_supplicant_leave_mesh(struct wpa_supplicant *wpa_s)
{
	...
	wpa_msg(wpa_s, MSG_ERROR, "leaving mesh");
			    ...
			    size_t len)
{
	...
	wpa_warning_buf(MSG_INFO, "mesh: Created virtual interface %s", ifname, strlen(ifname));
	wpa_warning_log4(MSG_INFO, "addr "
