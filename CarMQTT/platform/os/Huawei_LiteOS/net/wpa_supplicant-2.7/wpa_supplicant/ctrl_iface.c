					     ...
					     char *cmd)
{
	...
	if (cmd == NULL || os_strcmp(cmd, "any") == 0) {
		...
		wpa_warning_buf(MSG_DEBUG, "CTRL_IFACE WPS_PBC: invalid BSSID '%s'",
			   cmd, strlen(cmd));
		return -1;
	}
					     ...
					     size_t buflen)
{
	...
	if (os_strcmp(cmd, "any") == 0)
		...
		wpa_warning_buf(MSG_DEBUG, "CTRL_IFACE WPS_PIN: invalid BSSID '%s'",
			   cmd, strlen(cmd));
		return -1;
	}
	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	...
	const u8 *hisi_mesh = wpa_bss_get_vendor_ie(bss, WPA_OUI_HISI_MESH);
...
static int * freq_range_to_channel_list(struct wpa_supplicant *wpa_s, char *val)
{
	...
	(void)os_memset(&ranges, 0, sizeof(ranges));
...
#ifdef LOS_CONFIG_MESH
static int wpa_mesh_deauth_peer(struct wpa_supplicant *wpa_s, char *addr_txt)
{
...
static int hapd_deauth_peer(struct hostapd_data *hapd, char *addr_txt)
{
	struct sta_info *sta = NULL;
	u8 addr[ETH_ALEN] = { 0 };
	if (hapd == NULL) {
					 ...
					 char *cmd)
{
	...
	if (type & MAC_ADDR_RAND_SCAN) {
		(void)wpas_mac_addr_rand_scan_set(wpa_s, MAC_ADDR_RAND_SCAN,
					    addr, mask);
	}

	if (type & MAC_ADDR_RAND_SCHED_SCAN) {
		(void)wpas_mac_addr_rand_scan_set(wpa_s, MAC_ADDR_RAND_SCHED_SCAN,
					    addr, mask);
	...
	if (type & MAC_ADDR_RAND_PNO) {
		(void)wpas_mac_addr_rand_scan_set(wpa_s, MAC_ADDR_RAND_PNO,
					    addr, mask);
		if (wpa_s->pno) {
			(void)wpas_stop_pno(wpa_s);
			(void)wpas_start_pno(wpa_s);
