			   ...
			   u8 bssid[], unsigned int id)
{
	...
	(void)wpas_dbus_register_bss(wpa_s, bssid, id);
			     ...
			     u8 bssid[], unsigned int id)
{
	...
	(void)wpas_dbus_unregister_bss(wpa_s, bssid, id);
...
void wpas_notify_suspend(struct wpa_global *global)
{
	...
	(void)os_get_time(&global->suspend_time);
	wpa_warning_log0(MSG_DEBUG, "System suspend notification");
...
void wpas_notify_resume(struct wpa_global *global)
{
	...
	if (global->suspend_time.sec == 0)
		...
		(void)os_get_time(&now);
		slept = now.sec - global->suspend_time.sec;
	}
	wpa_warning_log1(MSG_DEBUG, "System resume notification (slept %d seconds)",
		   slept);
			       ...
			       const struct wpabuf *cert)
{
	...
	if (cert) {
		...
		if (cert_hex) {
			(void)wpa_snprintf_hex(cert_hex, len, wpabuf_head(cert),
					 wpabuf_len(cert));
					   ...
					   struct wpa_ssid *ssid)
{
	...
	(void)wpa_drv_roaming(wpa_s, !ssid->bssid_set,
