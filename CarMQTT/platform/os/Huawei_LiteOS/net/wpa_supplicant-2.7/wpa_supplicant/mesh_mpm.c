...
#ifdef LOS_CONFIG_MESH_GTK
	case PLINK_ESTAB:
		/* mgtk rekey retry timer */
		if (sta->mgtk_rekey_flag != WPA_FLAG_ON)
			break;
		if (sta->mgtk_info_retries < conf->dot11MeshMaxRetries) {
			eloop_register_timeout( conf->dot11MeshRetryTimeout / 1000,
				(conf->dot11MeshRetryTimeout % 1000) * 1000,
				plink_timer, wpa_s, sta);
			mesh_mpm_send_plink_action(wpa_s, sta, PLINK_MGTK_INFO, 0);
			sta->mgtk_info_retries++;
			break;
		}
		reason = WLAN_REASON_MESH_GTK_REKEY_TIMEOUT;
		wpa_mesh_set_plink_state(wpa_s, sta, PLINK_HOLDING);
		eloop_register_timeout(conf->dot11MeshHoldingTimeout / 1000,
			(conf->dot11MeshHoldingTimeout % 1000) * 1000,
			plink_timer, wpa_s, sta);
...
int mesh_mpm_close_peer(struct wpa_supplicant *wpa_s, const u8 *addr)
{
	...
	if (!sta) {
		...
		hi_mesh_mpm_send_plink_close_action(wpa_s, addr);
				 ...
				 struct sta_info *sta)
{
	...
	if (conf->security & MESH_CONF_SEC_AMPE) {
		...
		wpa_drv_set_key(wpa_s, wpa_cipher_to_alg(conf->group_cipher),
				...
				sta->mgtk, sta->mgtk_len);
#else
		hisi_set_mesh_mgtk(wpa_s->ifname, sta->addr, sta->mgtk, sta->mgtk_len);
#endif
...
#ifdef LOS_CONFIG_MESH_GTK
		case MGTK_INFO_ACPT:
			if (hisi_set_mesh_mgtk(wpa_s->ifname, sta->addr, sta->mgtk, sta->mgtk_len)) {
				wpa_warning_log0(MSG_DEBUG, "mesh gtk install failed");
				break;
			}
			mesh_mpm_send_plink_action(wpa_s, sta, PLINK_MGTK_ACK, 0);
			break;
		case MGTK_ACK_ACPT:
			if (sta->mgtk_rekey_flag != WPA_FLAG_ON) {
				break;
			...
			sta->mgtk_rekey_flag = WPA_FLAG_OFF;
			sta->mgtk_info_retries = 0;
			eloop_cancel_timeout(plink_timer, wpa_s, sta);
			break;
#endif
		default:
...
#ifdef LOS_CONFIG_MESH_GTK
void mesh_mpm_mgtk_rekey(struct wpa_supplicant *wpa_s, struct sta_info *sta)
{
	struct mesh_conf *conf = NULL;
	if ((wpa_s == NULL) || (wpa_s->ifmsh == NULL) || (sta == NULL) || (wpa_s->ifmsh->mconf == NULL))
		return;
	conf = wpa_s->ifmsh->mconf;
	mesh_mpm_send_plink_action(wpa_s, sta, PLINK_MGTK_INFO, 0);
	sta->mgtk_rekey_flag = WPA_FLAG_ON;
	eloop_register_timeout(conf->dot11MeshRetryTimeout / 1000,
		(conf->dot11MeshRetryTimeout % 1000) * 1000,
		plink_timer, wpa_s, sta);
}
