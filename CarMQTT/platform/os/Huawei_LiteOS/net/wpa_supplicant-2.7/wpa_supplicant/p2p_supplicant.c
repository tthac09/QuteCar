					     ...
					     int freq)
{
	...
	if (wpa_s->conf->p2p_ignore_shared_freq &&
		...
		wpa_warning_log1(MSG_DEBUG, "P2P: Ignore own channel preference %d MHz due to p2p_ignore_shared_freq=1 configuration",
			   freq);
		freq = 0;
	}
				      ...
				      struct wpa_scan_results *scan_res)
{
	...
	wpa_warning_log1(MSG_DEBUG, "P2P: Scan results received (%d BSS)",
		   (int) scan_res->num);
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		if (bss->beacon_ie_len > 0 &&
			...
			wpa_warning_log4(MSG_DEBUG, "P2P: Use P2P IE(s) from Beacon frame since no P2P IE(s) in Probe Response frames received for "
				   "%02x:xx:xx:%02x:%02x:%02x", (bss->bssid)[0], (bss->bssid)[3], (bss->bssid)[4], (bss->bssid)[5]);
			ies = ies + ies_len;
...
static void run_wpas_p2p_disconnect(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_warning_buf(MSG_DEBUG,
		   ...
		   wpa_s->ifname, strlen(wpa_s->ifname));
	...
	for (wpa_s = wpa_s->global->ifaces; wpa_s; wpa_s = wpa_s->next) {
		for (s = wpa_s->conf->ssid; s; s = s->next) {
			...
			wpa_warning_log3(MSG_DEBUG,
				   "P2P: disabled:%d p2p:%d mode:%d",
				   s->disabled, s->p2p_group,
				   s->mode);
				...
				unsigned int *pref_freq)
{
	...
	wpa_warning_log2(MSG_DEBUG, "P2P: Conncap - in:%d role:%d", incoming, role);
	...
	if (p2p_no_group_iface) {
		wpa_warning_log0(MSG_DEBUG, "P2P: p2p_no_group_iface");
				 ...
				 enum p2p_group_removal_reason removal_reason)
{
	...
	if (ssid == NULL) {
		...
		if (ssid == NULL &&
			wpa_s->p2p_group_interface == NOT_P2P_GROUP_INTERFACE)
		{
			wpa_error_log0(MSG_ERROR, "P2P: P2P group interface "
				   "not found");
			return -1;
		}
	}
	...
	if (eloop_cancel_timeout(wpas_p2p_group_freq_conflict, wpa_s, NULL) > 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Cancelled P2P group freq_conflict timeout");
	}
	if (eloop_cancel_timeout(wpas_p2p_group_idle_timeout, wpa_s, NULL) > 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Cancelled P2P group idle timeout");
	}
	if (eloop_cancel_timeout(wpas_p2p_group_formation_timeout,
				 wpa_s->p2pdev, NULL) > 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Cancelled P2P group formation "
			   "timeout");
	...
	if (wpa_s->p2p_group_interface != NOT_P2P_GROUP_INTERFACE) {
		...
		char *ifname;
		enum wpa_driver_if_type type;
		wpa_warning_buf(MSG_DEBUG, "P2P: Remove group interface %s",
			wpa_s->ifname, strlen(wpa_s->ifname));
		global = wpa_s->global;
		...
		hi_free_wifi_dev(hi_get_wifi_dev_by_name(ifname));
		wpa_s = global->ifaces;
		if (wpa_s && ifname)
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Remove temporary group network");
	if (ssid && (ssid->p2p_group ||
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: Temporary group network not "
			   "found");
	}
	if (wpa_s->ap_iface)
				     ...
				     const u8 *ssid, size_t ssid_len)
{
	...
	if (bss == NULL) {
		wpa_warning_log4(MSG_DEBUG, "P2P: Could not figure out whether "
			   "group is persistent - BSS " "%02x:xx:xx:%02x:%02x:%02x" " not found",
			   (bssid)[0], (bssid)[3], (bssid)[4], (bssid)[5]);
		return 0;
	}
	...
	if (p2p == NULL) {
		wpa_warning_log4(MSG_DEBUG, "P2P: Could not figure out whether "
			   "group is persistent - BSS " "%02x:xx:xx:%02x:%02x:%02x"
			   " did not include P2P IE", (bssid)[0], (bssid)[3], (bssid)[4], (bssid)[5]);
	...
	wpa_warning_log1(MSG_DEBUG, "P2P: Checking whether group is persistent: "
		   "group_capab=0x%x", group_capab);
	if (addr) {
		os_memcpy(go_dev_addr, addr, ETH_ALEN);
		wpa_warning_log4(MSG_DEBUG, "P2P: GO Device Address " "%02x:xx:xx:%02x:%02x:%02x",
			   addr[0], addr[3], addr[4], addr[5]);
	} else
		os_memset(go_dev_addr, 0, ETH_ALEN);
	wpabuf_free(p2p);

	wpa_warning_log4(MSG_DEBUG, "P2P: BSS " "%02x:xx:xx:%02x:%02x:%02x",
		   (bssid)[0], (bssid)[3], (bssid)[4], (bssid)[5]);
	wpa_warning_log1(MSG_DEBUG, " group_capab=0x%x ", group_capab);
	wpa_warning_log4(MSG_DEBUG,
		   "go_dev_addr=" "%02x:xx:xx:%02x:%02x:%02x",
		   (go_dev_addr)[0], (go_dev_addr)[3], (go_dev_addr)[4], (go_dev_addr)[5]);
					   ...
					   const u8 *go_dev_addr)
{
	...
	int changed = 0;
#endif
	wpa_warning_log4(MSG_DEBUG, "P2P: Storing credentials for a persistent "
		   "group (GO Dev Addr " "%02x:xx:xx:%02x:%02x:%02x" ")",
		   (go_dev_addr)[0], (go_dev_addr)[3], (go_dev_addr)[4], (go_dev_addr)[5]);
	for (s = wpa_s->conf->ssid; s; s = s->next) {
		if (s->disabled == 2 &&
	...
	if (s) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Update existing persistent group "
			   "entry");
#ifndef HISI_CODE_CROP
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: Create a new persistent group "
			   "entry");
#ifndef HISI_CODE_CROP
		changed = 1;
#endif
	...
	if (changed && wpa_s->conf->update_config &&
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: Failed to update configuration");
	}
#endif
	return s->id;
}
				   ...
				   const char *extra)
{
	...
	/*wpa_printf(MSG_INFO, P2P_EVENT_GROUP_STARTED
		   "%s %s ssid=\"%s\" freq=%d go_dev_addr=" "%02x:xx:xx:%02x:%02x:%02x" "%s%s",
		   wpa_s->ifname, go ? "GO" : "client", ssid_txt, freq,
...
static void wpas_p2p_free_send_action_work(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log3(MSG_DEBUG,
		   "P2P: Free Action frame radio work @%p (freq=%u dst=" " wait_time=%u)",
		   wpa_s->p2p_send_action_work, awork->freq,
		   awork->wait_time);
	wpa_warning_log4(MSG_DEBUG,
		   "%02x:xx:xx:%02x:%02x:%02x",
		   (awork->dst)[0], (awork->dst)[3], (awork->dst)[4], (awork->dst)[5]);
	wpa_warning_log4(MSG_DEBUG,
		  " src=" "%02x:xx:xx:%02x:%02x:%02x",
		   (awork->src)[0], (awork->src)[3], (awork->src)[4], (awork->src)[5]);
	wpa_warning_log4(MSG_DEBUG,
		   " bssid=" "%02x:xx:xx:%02x:%02x:%02x",
		   (awork->bssid)[0], (awork->bssid)[3], (awork->bssid)[4], (awork->bssid)[5]);
					      ...
					      void *timeout_ctx)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Send Action frame radio work timed out");
				 ...
				 size_t len, unsigned int wait_time)
{
	...
	if (wpa_s->p2p_send_action_work) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Cannot schedule new p2p-send-action work since one is already pending");
		return -1;
	}
			    ...
			    size_t len, unsigned int wait_time)
{
	...
	if (listen_freq != (int) freq && send_freq != (int) freq) {
		wpa_warning_log2(MSG_DEBUG, "P2P: Schedule new radio work for Action frame TX (listen_freq=%d send_freq=%d)",
			   listen_freq, send_freq);
		return wpas_send_action_work(wpa_s, freq, dst, src, bssid, buf,
					     len, wait_time);
	}

	wpa_warning_log0(MSG_DEBUG, "P2P: Use ongoing radio work for Action frame TX");
				    ...
				    struct p2p_go_neg_results *res)
{
	...
	wpa_warning_log4(MSG_DEBUG, "P2P: Start WPS Enrollee for peer " "%02x:xx:xx:%02x:%02x:%02x",
		   (res->peer_interface_addr)[0], (res->peer_interface_addr)[3],
		   (res->peer_interface_addr)[4], (res->peer_interface_addr)[5]);
	wpa_warning_log4(MSG_DEBUG,
		   " dev_addr " "%02x:xx:xx:%02x:%02x:%02x",
		   (res->peer_device_addr)[0], (res->peer_device_addr)[3],
		   (res->peer_device_addr)[4], (res->peer_device_addr)[5]);
	wpa_warning_log1(MSG_DEBUG, " wps_method %d", res->wps_method);
...
static void p2p_go_configured(void *ctx, void *data)
{
	...
	if (ssid && ssid->mode == WPAS_MODE_P2P_GO) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Group setup without provisioning");
...
#ifndef HISI_CODE_CROP
	wpa_warning_log0(MSG_DEBUG, "P2P: Setting up WPS for GO provisioning");
	if (wpa_supplicant_ap_mac_addr_filter(wpa_s,
					      params->peer_interface_addr)) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Failed to setup MAC address "
			   "filtering");
		return;
	}
#endif
	if (params->wps_method == WPS_PBC) {
		...
		if (wpa_s->p2pdev->p2p_oob_dev_pw_id !=
		    DEV_PW_NFC_CONNECTION_HANDOVER &&
		    !wpa_s->p2pdev->p2p_oob_dev_pw) {
			wpa_warning_log0(MSG_DEBUG, "P2P: No NFC Dev Pw known");
			return;
		}
					...
					enum wpa_driver_if_type type)
{
	...
	if (wpa_s->pending_interface_name[0]) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Pending virtual interface exists "
			   "- skip creation of a new one");
		if (is_zero_ether_addr(wpa_s->pending_interface_addr)) {
			wpa_warning_buf(MSG_DEBUG, "P2P: Pending virtual address "
				   "unknown?! ifname='%s'",
				   wpa_s->pending_interface_name, strlen(wpa_s->pending_interface_name));
			return -1;
		}
		return 0;
	}
	...
	wpa_warning_buf(MSG_DEBUG, "P2P: Create a new interface %s for the group",
	...
	if (wpa_drv_if_add(wpa_s, type, ifname, NULL, NULL, force_ifname,
		...
		wpa_error_log0(MSG_ERROR, "P2P: Failed to create new group "
			   "interface");
		return -1;
	}
	...
	hisi_ioctl_get_own_mac(ifname, (char *)wpa_s->pending_interface_addr);
#endif /* CONFIG_DRIVER_HISILICON */
#endif /* LOS_CONFIG_P2P */

	if (force_ifname[0]) {
		wpa_warning_buf(MSG_DEBUG, "P2P: Driver forced interface name %s",
			   force_ifname, strlen(force_ifname));
		os_strlcpy(wpa_s->pending_interface_name, force_ifname,
			   sizeof(wpa_s->pending_interface_name));
	} else
		os_strlcpy(wpa_s->pending_interface_name, ifname,
			   sizeof(wpa_s->pending_interface_name));
	wpa_warning_buf(MSG_DEBUG, "P2P: Created pending virtual interface %s addr ",
		wpa_s->pending_interface_name, strlen(wpa_s->pending_interface_name));
	wpa_warning_log4(MSG_DEBUG,
		   "%02x:xx:xx:%02x:%02x:%02x",
		   (wpa_s->pending_interface_addr)[0], (wpa_s->pending_interface_addr)[3],
		   (wpa_s->pending_interface_addr)[4], (wpa_s->pending_interface_addr)[5]);
	return 0;
}
	...
	struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_buf(MSG_DEBUG, "P2P: Removing pending group interface %s",
		   wpa_s->pending_interface_name, strlen(wpa_s->pending_interface_name));
...
wpas_p2p_init_group_interface(struct wpa_supplicant *wpa_s, int go)
{
	...
	if (!wpa_s->pending_interface_name[0]) {
		wpa_error_log0(MSG_ERROR, "P2P: No pending group interface");
	...
	if (group_wpa_s == NULL) {
		wpa_error_log0(MSG_ERROR, "P2P: Failed to create new "
					     ...
					     void *timeout_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;
	(void)timeout_ctx;
	wpa_warning_log0(MSG_DEBUG, "P2P: Group Formation timed out");
...
{
	wpa_warning_log0(MSG_DEBUG, "P2P: Reject group formation due to WPS provisioning failure");
	eloop_cancel_timeout(wpas_p2p_group_formation_timeout,
			     wpa_s->p2pdev, NULL);
...
static void wpas_go_neg_completed(void *ctx, struct p2p_go_neg_results *res)
{
	...
	wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_SUCCESS "role=%s "
			   "freq=%d  peer_dev=" MACSTR " peer_iface=" MACSTR
		       " wps_method=%s",
	...
	if (res->role_go && wpa_s->p2p_persistent_id >= 0) {
		...
		if (ssid && ssid->disabled == 2 &&
			...
			wpa_warning_log0(MSG_DEBUG, "P2P: Override passphrase based "
	...
	if (wpa_s->create_p2p_iface) {
		...
		wpa_s->pending_interface_name[0] = '\0';
#ifdef LOS_CONFIG_P2P
		if (hi_set_p2p_go_gc_wifi_dev(group_wpa_s, res->role_go) != HISI_OK) {
			wpa_error_log0(MSG_ERROR, "hi_set_p2p_go_gc_wifi_dev fail\n");
			goto P2P_GO_NEG_FAIL;
		}
#endif /* LOS_CONFIG_P2P */
...
#else
	eloop_register_timeout(15 + (res->peer_config_timeout * 10) / 100,
						   (res->peer_config_timeout % 100) * 10000,
						   wpas_p2p_group_formation_timeout, wpa_s, NULL);
	return;
P2P_GO_NEG_FAIL:
...
#ifdef LOS_CONFIG_P2P_AUTO_CONNECT
void p2p_connect_time(void *eloop_ctx, void *timeout_ctx)
{
	(void) timeout_ctx;
	hi_wifi_p2p_connect((unsigned char *)eloop_ctx, "pbc", WPA_P2P_DEFAULT_PERSISTENT, WPA_P2P_DEFAULT_GO_INTENT);
}
char g_mac[HI_WIFI_TXT_ADDR_LEN + 1] = {0};
#endif
static void wpas_go_neg_req_rx(void *ctx, const u8 *src, u16 dev_passwd_id,
			       u8 go_intent)
{
#ifndef LOS_CONFIG_P2P_AUTO_CONNECT
	hi_wifi_event wpa_events;
#endif
	struct wpa_supplicant *wpa_s = ctx;
...
#ifndef LOS_CONFIG_P2P_AUTO_CONNECT
	wpa_events.event = HI_WIFI_EVT_P2P_GO_REQUEST;
	(void)os_memcpy(wpa_events.info.p2p_go_request.addr, src, ETH_ALEN);
	if (g_wpa_event_cb != NULL)
		wifi_new_task_event_cb(&wpa_events);
#else
	if (os_snprintf(g_mac, HI_WIFI_TXT_ADDR_LEN + 1, MACSTR, MAC2STR(src)) < 0) {
		printf("wpas_go_neg_req_rx os_snprintf faild\n");
	}
	/*connect the peer after 5s */
	eloop_register_timeout(5, 0, p2p_connect_time, (void *)src, NULL);
#endif
...
static void wpas_find_stopped(void *ctx)
{
	...
	if (wpa_s->p2p_scan_work && wpas_abort_ongoing_scan(wpa_s) < 0)
		wpa_warning_log0(MSG_DEBUG, "P2P: Abort ongoing scan failed");
#endif /* HISI_CODE_CROP */

	wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_FIND_STOPPED);
	wpas_notify_p2p_find_stopped(wpa_s);
	wpa_events.event = HI_WIFI_EVT_P2P_FIND_STOP;
	if (g_wpa_event_cb != NULL)
		wifi_new_task_event_cb(&wpa_events);
}
...
static void wpas_start_listen_cb(struct wpa_radio_work *work, int deinit)
{
	...
	if (wpa_drv_probe_req_report(wpa_s, 1) < 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Failed to request the driver to "
	...
	if (wpa_drv_remain_on_channel(wpa_s, lwork->freq, duration) < 0) {
		wpa_warning_log1(MSG_DEBUG, "P2P: Failed to request the driver "
			     ...
			     const struct wpabuf *probe_resp_ie)
{
	...
	if (wpa_s->p2p_listen_work) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Reject start_listen since p2p_listen_work already exists");
		return -1;
	}
			       ...
			       size_t group_id_len)
{
	...
	if (os_snprintf_error(sizeof(params), res)) {
		wpa_warning_log0(MSG_DEBUG, "P2P: PD Request event truncated");
	}
	params[sizeof(params) - 1] = '\0';

	if (config_methods & WPS_CONFIG_DISPLAY) {
		if (wps_generate_pin(&generated_pin) < 0) {
			wpa_warning_log0(MSG_DEBUG, "P2P: Could not generate PIN");
...
static void wpas_prov_disc_resp(void *ctx, const u8 *peer, u16 config_methods)
{
	...
	if (wpa_s->pending_pd_before_join &&
		...
		wpa_s->pending_pd_before_join = 0;
		wpa_warning_log0(MSG_DEBUG, "P2P: Starting pending "
				  ...
				  int dev_pw_id)
{
	...
	if (!persistent_group) {
		wpa_warning_log4(MSG_DEBUG, "P2P: Invitation from " "%02x:xx:xx:%02x:%02x:%02x"
			   " to join an active group",
			   sa[0], sa[3], sa[4], sa[5]);
		wpa_warning_buf(MSG_DEBUG, " to join an active group (SSID: %s)",
		...
		if (!is_zero_ether_addr(wpa_s->p2p_auth_invite) &&
			...
			wpa_warning_log0(MSG_DEBUG, "P2P: Accept previously "
		...
		if (dev_pw_id >= 0 && wpa_s->p2p_nfc_tag_enabled &&
			...
			wpa_warning_log0(MSG_DEBUG, "P2P: Accept invitation based on local enabled NFC Tag");
	...
	if (grp) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Accept invitation to already "
	...
	if (!is_zero_ether_addr(wpa_s->p2p_auth_invite) &&
	    os_memcmp(sa, wpa_s->p2p_auth_invite, ETH_ALEN) == 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Accept previously initiated "
	...
	if (s->mode == WPAS_MODE_P2P_GO && !wpas_p2p_create_iface(wpa_s)) {
		*go = 1;
		if (wpa_s->wpa_state >= WPA_AUTHENTICATING) {
			wpa_warning_log0(MSG_DEBUG, "P2P: The only available "
		...
		*go = 1;
		...
		{
			wpa_error_log0(MSG_ERROR, "P2P: Failed to allocate a new "
...
accept_inv:
	...
	if (best_freq > 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Trying to prefer a channel already used by one of the interfaces");
		wpas_p2p_set_own_freq_preference(wpa_s, best_freq);

		if (wpa_s->num_multichan_concurrent < 2 ||
		    wpas_p2p_num_unused_channels(wpa_s) < 1) {
			wpa_warning_log0(MSG_DEBUG, "P2P: No extra channels available - trying to force channel to match a channel already used by one of the interfaces");
	...
	if (*force_freq > 0 && wpa_s->num_multichan_concurrent > 1 &&
		...
		if (*go == 0) {
			/* We are the client */
			wpa_warning_log0(MSG_DEBUG, "P2P: Peer was found to be "
			...
			*force_freq = 0;
			 ...
			 * intersection */
			wpa_warning_log1(MSG_DEBUG, "P2P: Forced GO freq %d MHz not "
				     ...
				     int op_freq)
{
	...
	if (status == P2P_SC_SUCCESS) {
		wpa_warning_log4(MSG_DEBUG, "P2P: Invitation from peer " "%02x:xx:xx:%02x:%02x:%02x",
			   sa[0], sa[3], sa[4], sa[5]);
		wpa_warning_log1(MSG_DEBUG,
			   " was accepted; op_freq=%d MHz", op_freq);
		wpa_warning_buf(MSG_DEBUG, "SSID=%s",
			  wpa_ssid_txt(ssid, ssid_len), strlen(wpa_ssid_txt(ssid, ssid_len)));
		if (s) {
	...
	if (status != P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) {
		wpa_warning_log4(MSG_DEBUG, "P2P: Invitation from peer " "%02x:xx:xx:%02x:%02x:%02x", sa[0], sa[3], sa[4], sa[5]);
		wpa_warning_log1(MSG_DEBUG, " was rejected (status %u)", status);
		return;
	}
					...
					const u8 *peer, int inv)
{
	...
	if (i >= ssid->num_p2p_clients || !ssid->p2p_client_list) {
		if (ssid->mode != WPAS_MODE_P2P_GO &&
			...
			wpa_warning_log1(MSG_DEBUG, "P2P: Remove persistent group %d "
	...
	wpa_warning_log4(MSG_DEBUG, "P2P: Remove peer " "%02x:xx:xx:%02x:%02x:%02x",
		   (peer)[0], (peer)[3], (peer)[4], (peer)[5]);
	wpa_warning_log1(MSG_DEBUG, "group %d ", ssid->id);
#ifndef HISI_CODE_CROP
	wpa_warning_buf(MSG_DEBUG,"client list%s",
		   inv ? " due to invitation result" : "",
		   inv ? strlen(" due to invitation result") : strlen(""));
#endif
				   ...
				   int peer_oper_freq)
{
	...
	wpa_warning_log1(MSG_DEBUG, "P2P: Invitation result - status=%d peer=", status);
	...
	if (status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Waiting for peer to start another "
	...
	if (ssid == NULL) {
		wpa_error_log0(MSG_ERROR, "P2P: Could not find persistent group "
	...
	wpa_warning_log1(MSG_DEBUG, "P2P: Persistent group invitation success - op_freq=%d MHz", freq);
	wpa_warning_buf(MSG_DEBUG, "P2P: Persistent group invitation success - SSID=%s",
			wpa_ssid_txt(ssid->ssid, ssid->ssid_len), strlen(wpa_ssid_txt(ssid->ssid, ssid->ssid_len)));
				     ...
				     struct p2p_channels *cli_chan)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Enable operating classes for 2.4 GHz "
		   "band");
	...
	if (chan->reg_class[cla].channels)
		cla++;
#ifndef HISI_CODE_CROP
	wpa_warning_log0(MSG_DEBUG, "P2P: Enable operating classes for lower 5 GHz "
		   "band");
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Enable operating classes for higher 5 GHz "
		   "band");
				   ...
				   struct p2p_channels *cli_chan)
{
	...
	if (wpa_s->hw.modes == NULL) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Driver did not support fetching "
	...
	for (op = 0; global_op_class[op].op_class; op++) {
		...
		for (ch = o->min_chan; ch <= o->max_chan; ch += o->inc) {
			...
			if (res == ALLOWED) {
				if (reg == NULL) {
					wpa_warning_log1(MSG_DEBUG, "P2P: Add operating class %u",
						   o->op_class);
				...
				if (cli_reg == NULL) {
					wpa_warning_log1(MSG_DEBUG, "P2P: Add operating class %u (client only)",
						   o->op_class);
				  ...
				  const char *conf_p2p_dev)
{
...
#else
	ret = os_snprintf(ifname, sizeof(ifname), "%s", wpa_s->ifname);
#endif
	...
	if (ret < 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Failed to create P2P Device interface");
		return ret;
	}
#endif
	os_strlcpy(wpa_s->pending_interface_name, ifname,
	...
	if (!p2pdev_wpa_s) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Failed to add P2P Device interface");
		return -1;
	}
				       ...
				       const u8 *feat_cap, size_t feat_cap_len)
{
	...
	if (ret != (2 * (int) feat_cap_len)) {
		wpa_warning_log0(MSG_WARNING, "P2PS feature_cap bytes truncated");
	}
}
...
static int wpas_prov_disc_resp_cb(void *ctx)
{
	...
	if (persistent_go) {
		wpas_p2p_group_add_persistent(
			wpa_s, persistent_go, 0, 0, 0, NULL,
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	if (global->p2p) {
		wpa_error_log0(MSG_ERROR, "P2P: global->p2p has been inited");
		return 0;
	}
	...
	wpa_warning_log2(MSG_DEBUG, "P2P: Own listen channel: %d:%d",
	...
	if (wpa_s->conf->p2p_oper_reg_class &&
		...
		wpa_warning_log2(MSG_DEBUG, "P2P: Configured operating channel: "
		...
		wpa_warning_log2(MSG_DEBUG, "P2P: Random operating channel: "
			   "%d:%d", p2p.op_reg_class, p2p.op_channel);
	}
...
static void wpas_p2p_check_join_scan_limit(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log1(MSG_DEBUG, "P2P: Join scan attempt %d",
		   wpa_s->p2p_join_scan_count);
	if (wpa_s->p2p_join_scan_count > P2P_MAX_JOIN_SCAN_ATTEMPTS) {
		wpa_warning_log4(MSG_DEBUG, "P2P: Failed to find GO " "%02x:xx:xx:%02x:%02x:%02x"
			   " for join operationg - stop join attempt",
			   (wpa_s->pending_join_iface_addr)[0], (wpa_s->pending_join_iface_addr)[3],
			   (wpa_s->pending_join_iface_addr)[3], (wpa_s->pending_join_iface_addr)[5]);
		eloop_cancel_timeout(wpas_p2p_join_scan, wpa_s, NULL);
...
static int wpas_check_freq_conflict(struct wpa_supplicant *wpa_s, int freq)
{
	...
	for (i = 0; i < num; i++) {
		if (freqs[i].freq == freq) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Frequency %d MHz in use by another virtual interface and can be used",
				   freq);
			res = 0;
			goto exit_free;
		}
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: No valid operating frequencies");
	res = 1;
			    ...
			    const u8 *peer_dev_addr)
{
	...
	if (bss->last_update_idx < wpa_s->bss_update_idx) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Peer BSS entry not updated in the "
			   "last scan");
		return 0;
	}
#endif /* HISI_CODE_CROP */
	updated = os_reltime_before(&wpa_s->p2p_auto_started,
				    &bss->last_update);
	wpa_warning_buf(MSG_DEBUG, "P2P: Current BSS entry for peer updated at "
		   "(%supdated in last scan)",
		   updated ? "": "not ", updated ? strlen(""): strlen("not "));
	wpa_warning_log2(MSG_DEBUG, "P2P: Current BSS entry for peer updated at "
		   "%ld.%06ld",
				   ...
				   struct wpa_scan_results *scan_res)
{
	...
	wpa_warning_buf(MSG_DEBUG, "P2P: Scan results received  %sjoin",
		   wpa_s->p2p_auto_join ? "auto_" : "", wpa_s->p2p_auto_join ? strlen("auto_") : strlen(""));
	if (scan_res)
	...
	if (wpa_s->p2p_auto_pd) {
		...
		if (join == 0 &&
			...
			if (bss) {
				freq = bss->freq;
				wpa_warning_log2(MSG_DEBUG, "P2P: Scan retry %d for " " at %d MHz",
					   wpa_s->auto_pd_scan_retry,
					   freq);
				wpa_warning_log4(MSG_DEBUG,
					   "the peer " "%02x:xx:xx:%02x:%02x:%02x",
						 (wpa_s->pending_join_dev_addr)[0], (wpa_s->pending_join_dev_addr)[3],
						 (wpa_s->pending_join_dev_addr)[4], (wpa_s->pending_join_dev_addr)[5]);
		...
		wpa_warning_log4(MSG_DEBUG, "P2P: Auto PD with " "%02x:xx:xx:%02x:%02x:%02x",
			   (wpa_s->pending_join_dev_addr)[0], (wpa_s->pending_join_dev_addr)[3],
			   (wpa_s->pending_join_dev_addr)[4], (wpa_s->pending_join_dev_addr)[5]);
		wpa_warning_log1(MSG_DEBUG, "join=%d", join);
	...
	if (wpa_s->p2p_auto_join) {
		int join = wpas_p2p_peer_go(wpa_s,
					    wpa_s->pending_join_dev_addr);
		if (join < 0) {
			wpa_warning_log0(MSG_DEBUG, "P2P: Peer was not found to be "
		...
		wpa_warning_buf(MSG_DEBUG, "P2P: Peer was found running GO%s -> "
			   "try to join the group", join ? "" :
			   " in older scan", join ? strlen("") :
			   strlen(" in older scan"));
		if (!join) {
			wpa_msg_global(wpa_s->p2pdev, MSG_INFO,
	...
	if (freq < 0 &&
		...
		wpa_warning_log4(MSG_DEBUG, "P2P: Overwrite pending interface "
			   "address for join from " "%02x:xx:xx:%02x:%02x:%02x"
			   " based on newly discovered P2P peer entry",
			   (wpa_s->pending_join_iface_addr)[0], (wpa_s->pending_join_iface_addr)[3],
			   (wpa_s->pending_join_iface_addr)[4], (wpa_s->pending_join_iface_addr)[5]);
		wpa_warning_log4(MSG_DEBUG, "P2P: Overwrite pending interface "
			   "address for join from "" to " "%02x:xx:xx:%02x:%02x:%02x"
			   " based on newly discovered P2P peer entry",
			   (iface_addr)[0], (iface_addr)[3], (iface_addr)[4], (iface_addr)[5]);
	...
	if (freq >= 0) {
		wpa_warning_log1(MSG_DEBUG, "P2P: Target GO operating frequency "
	...
	if (bss) {
		...
		wpa_warning_buf(MSG_DEBUG, "P2P: Target GO operating frequency "
			   "from BSS table: (SSID %s)",
			   wpa_ssid_txt(bss->ssid, bss->ssid_len), strlen(wpa_ssid_txt(bss->ssid, bss->ssid_len)));
		if (p2p_parse_dev_addr((const u8 *) (bss + 1), bss->ie_len,
		    ...
		    os_memcmp(dev_addr, wpa_s->pending_join_dev_addr,
			      ETH_ALEN) != 0) {
			wpa_warning_log4(MSG_DEBUG,
				   "P2P: Update target GO device address based on BSS entry: " "%02x:xx:xx:%02x:%02x:%02x",
				   (dev_addr)[0], (dev_addr)[3], (dev_addr)[4], (dev_addr)[5]);
			wpa_warning_log4(MSG_DEBUG,
				   "P2P: Update target GO device address based on BSS entry: "" (was " "%02x:xx:xx:%02x:%02x:%02x" ")",
				   (wpa_s->pending_join_dev_addr)[0], (wpa_s->pending_join_dev_addr)[3],
				   (wpa_s->pending_join_dev_addr)[4], (wpa_s->pending_join_dev_addr)[5]);
	...
	if (freq > 0) {
		...
		wpa_warning_log4(MSG_DEBUG, "P2P: Send Provision Discovery Request "
			   "prior to joining an existing group (GO " "%02x:xx:xx:%02x:%02x:%02x",
			   (wpa_s->pending_join_dev_addr)[0], (wpa_s->pending_join_dev_addr)[3],
			   (wpa_s->pending_join_dev_addr)[4], (wpa_s->pending_join_dev_addr)[5]);
		wpa_warning_log1(MSG_DEBUG, "P2P: Send Provision Discovery Request "
			   " freq=%u MHz)", freq);
		...
		if ((p2p_get_provisioning_info(wpa_s->global->p2p,
			...
			wpa_warning_log4(MSG_DEBUG, "P2P: Provision discovery "
				   "with " "%02x:xx:xx:%02x:%02x:%02x" " already done - proceed to "
				   "join",
				   (wpa_s->pending_join_dev_addr)[0], (wpa_s->pending_join_dev_addr)[3],
				   (wpa_s->pending_join_dev_addr)[4], (wpa_s->pending_join_dev_addr)[5]);
			wpa_s->pending_pd_before_join = 0;
		...
		if (p2p_prov_disc_req(wpa_s->global->p2p,
			...
			wpa_warning_log0(MSG_DEBUG, "P2P: Failed to send Provision "
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Failed to find BSS/GO - try again later");
	eloop_cancel_timeout(wpas_p2p_join_scan, wpa_s, NULL);
				   ...
				   const u8 *ssid, size_t ssid_len)
{
	...
	if (ret) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Failed to start scan for join - "
			   "try again later");
		eloop_cancel_timeout(wpas_p2p_join_scan, wpa_s, NULL);
		eloop_register_timeout(1, 0, wpas_p2p_join_scan, wpa_s, NULL);
		wpas_p2p_check_join_scan_limit(wpa_s);
...
static int wpas_p2p_join(struct wpa_supplicant *wpa_s, const u8 *iface_addr,
			 ...
			 const u8 *ssid, size_t ssid_len)
{
	wpa_warning_log4(MSG_DEBUG, "P2P: Request to join existing group (iface "
		   "%02x:xx:xx:%02x:%02x:%02x",
		   (iface_addr)[0], (iface_addr)[3], (iface_addr)[4], (iface_addr)[5]);
	wpa_warning_log4(MSG_DEBUG, "P2P: Request to join existing group (iface "" dev " "%02x:xx:xx:%02x:%02x:%02x",
		   (dev_addr)[0], (dev_addr)[3], (dev_addr)[4], (dev_addr)[5]);
	wpa_warning_log1(MSG_DEBUG, " op_freq=%d", op_freq);
	wpa_warning_buf(MSG_DEBUG, "%s",
			  auto_join ? " (auto_join)" : "", auto_join ? strlen(" (auto_join)") : strlen(""));
	if (ssid && ssid_len) {
		wpa_warning_buf(MSG_DEBUG, "P2P: Group SSID specified: %s",
			   wpa_ssid_txt(ssid, ssid_len), strlen(wpa_ssid_txt(ssid, ssid_len)));
	}
			       ...
			       const u8 *ssid, size_t ssid_len)
{
	...
	if (freq && ssid && ssid_len) {
		...
		if (bss) {
			...
			wpa_warning_log1(MSG_DEBUG, "P2P: Join target GO operating frequency from BSS table: %d MHz",
				   bss->freq);
			wpa_warning_buf(MSG_DEBUG, "P2P: Join target GO operating frequency from BSS table: (SSID %s)",
				   wpa_ssid_txt(bss->ssid, bss->ssid_len), strlen(wpa_ssid_txt(bss->ssid, bss->ssid_len)));
		} else if (ssid && ssid_len) {
			res.ssid_len = ssid_len;
			os_memcpy(res.ssid, ssid, ssid_len);
			wpa_warning_buf(MSG_DEBUG, "P2P: Join target GO (SSID %s)",
				   wpa_ssid_txt(ssid, ssid_len), strlen(wpa_ssid_txt(ssid, ssid_len)));
		}
	}

	if (wpa_s->off_channel_freq || wpa_s->roc_waiting_drv_freq) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Cancel remain-on-channel prior to "
			   "starting client");
		wpa_drv_cancel_remain_on_channel(wpa_s);
		wpa_s->off_channel_freq = 0;
		wpa_s->roc_waiting_drv_freq = 0;
...
#ifndef HISI_CODE_CROP
	unsigned int max_pref_freq = *num_pref_freq;;
#endif
	(void)pref_freq_list;
	*num_pref_freq = 0;
	...
	if (freq > 0) {
		...
		if (num_unused <= 0 && !freq_in_use) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Cannot start P2P group on %u MHz as there are no available channels",
				   freq);
			res = -2;
			goto exit_free;
		}
		wpa_warning_log1(MSG_DEBUG, "P2P: Trying to force us to use the "
	...
	if (best_freq > 0) {
		if (*pref_freq == 0 && num_unused > 0) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Try to prefer a frequency (%u MHz) we are already using",
				   best_freq);
			*pref_freq = best_freq;
		} else {
			wpa_warning_log1(MSG_DEBUG, "P2P: Try to force us to use frequency (%u MHz) which is already in use",
				   best_freq);
			*force_freq = best_freq;
		}
...
int wpas_p2p_connect(struct wpa_supplicant *wpa_s, const u8 *peer_addr,
			 ...
			 int persistent_id, int pd, const u8 *group_ssid,
		     size_t group_ssid_len)
{
	...
	if (pin)
		...
		wpa_warning_buf(MSG_DEBUG, "P2P: Randomly generated PIN: %s",
			   wpa_s->p2p_pin, strlen(wpa_s->p2p_pin));
	...
	if (join || auto_join) {
		...
		if (auth) {
			wpa_warning_log4(MSG_DEBUG, "P2P: Authorize invitation to "
				   "connect a running group from " "%02x:xx:xx:%02x:%02x:%02x",
				   (peer_addr)[0], (peer_addr)[3], (peer_addr)[4], (peer_addr)[5]);
		...
		if (auto_join) {
			...
			wpa_warning_log2(MSG_DEBUG, "P2P: Auto join started at "
				   "%ld.%06ld",
...
#else
		wpa_drv_get_p2p_mac_addr(wpa_s, WPA_IF_P2P_CLIENT, wpa_s->pending_interface_addr);
		wpa_warning_buf(MSG_DEBUG, "P2P: ifname:%s ", wpa_s->ifname, strlen(wpa_s->ifname));
		wpa_warning_log4(MSG_DEBUG, "Own MAC address: %02x:xx:xx:%02x:%02x:%02x",
				   (wpa_s->pending_interface_addr)[0], (wpa_s->pending_interface_addr)[3],
				   (wpa_s->pending_interface_addr)[4], (wpa_s->pending_interface_addr)[5]);
#endif
				   ...
				   unsigned int freq, unsigned int duration)
{
	...
	if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL)
		return;
	wpa_warning_log2(MSG_DEBUG, "P2P: remain-on-channel callback (off_channel_freq=%u pending_listen_freq=%d)",
		   wpa_s->off_channel_freq, wpa_s->pending_listen_freq);
#ifndef HISI_CODE_CROP
	wpa_warning_log3(MSG_DEBUG, "P2P: remain-on-channel callback (roc_waiting_drv_freq=%d freq=%u duration=%u)",
	...
	if (wpa_s->off_channel_freq &&
		...
		wpa_warning_log4(MSG_DEBUG, "P2P: Ignore remain-on-channel callback (off_channel_freq=%u pending_listen_freq=%d freq=%u duration=%u)",
			   wpa_s->off_channel_freq, wpa_s->pending_listen_freq,
			   freq, duration);
#endif /* HISI_CODE_CROP */
	}
...
void wpas_p2p_cancel_remain_on_channel_cb(struct wpa_supplicant *wpa_s,
					  unsigned int freq)
{
	wpa_warning_log2(MSG_DEBUG, "P2P: Cancel remain-on-channel callback "
	...
	if (offchannel_pending_action_tx(wpa_s))
		return;
	if (wpa_s->p2p_long_listen > 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Continuing long Listen state");
...
static int wpas_p2p_select_go_freq(struct wpa_supplicant *wpa_s, int freq)
{
	...
	if (freq == 2) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Request to start GO on 2.4 GHz "
			   "band");
		if (wpa_s->best_24_freq > 0 &&
		    p2p_supported_freq_go(wpa_s->global->p2p,
					  wpa_s->best_24_freq)) {
			freq = wpa_s->best_24_freq;
			wpa_warning_log1(MSG_DEBUG, "P2P: Use best 2.4 GHz band "
				   "channel: %d MHz", freq);
		} else {
			if (os_get_random((u8 *) &r, sizeof(r)) < 0)
				return -1;
			freq = 2412 + (r % 3) * 25;
			wpa_warning_log1(MSG_DEBUG, "P2P: Use random 2.4 GHz band "
				   "channel: %d MHz", freq);
		}
	}

	if (freq == 5) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Request to start GO on 5 GHz "
			   "band");
		if (wpa_s->best_5_freq > 0 &&
		    p2p_supported_freq_go(wpa_s->global->p2p,
				       wpa_s->best_5_freq)) {
			freq = wpa_s->best_5_freq;
			wpa_warning_log1(MSG_DEBUG, "P2P: Use best 5 GHz band "
				   "channel: %d MHz", freq);
		} else {
			if (os_get_random((u8 *) &r, sizeof(r)) < 0)
				return -1;
			...
			if (!p2p_supported_freq_go(wpa_s->global->p2p, freq)) {
				wpa_warning_log0(MSG_DEBUG, "P2P: Could not select "
					   "5 GHz channel for P2P group");
				return -1;
			}
			wpa_warning_log1(MSG_DEBUG, "P2P: Use random 5 GHz band "
				   "channel: %d MHz", freq);
		}
	}
	...
	if (freq > 0 && !p2p_supported_freq_go(wpa_s->global->p2p, freq)) {
		...
		if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_DFS_OFFLOAD) &&
			 ...
			 */
			wpa_warning_log1(MSG_DEBUG, "P2P: "
				   "wpas_p2p_select_go_freq: The forced channel for GO (%u MHz) is DFS, and DFS is offloaded",
					freq);
			return freq;
		}
#endif /* HISI_CODE_CROP */
		wpa_warning_log1(MSG_DEBUG, "P2P: The forced channel for GO "
					    ...
					    const struct p2p_channels *channels)
{
	...
	params->freq = 0;
	wpa_warning_log0(MSG_DEBUG, "P2P: No 2.4, 5, or 60 GHz channel allowed");
	return;
out:
	wpa_warning_log1(MSG_DEBUG, "P2P: Set GO freq %d MHz (no preference known)",
		   params->freq);
}
				   ...
				   const struct p2p_channels *channels)
{
	...
	if (wpa_s->current_ssid &&
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: wpas_p2p_init_go_params called for an active GO");
	...
	if (freq) {
		...
		if (!p2p_supported_freq_go(wpa_s->global->p2p, freq)) {
			...
			if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_DFS_OFFLOAD) &&
				 ...
				 */
				wpa_warning_log1(MSG_DEBUG,
					   "P2P: wpas_p2p_init_go_params: The forced channel for GO (%u MHz) requires DFS and DFS is offloaded",
					   freq);
			} else {
#endif /* HISI_CODE_CROP */
			wpa_warning_log1(MSG_DEBUG,
	...
	if (wpa_s->conf->p2p_oper_reg_class == 81 &&
		...
		wpa_warning_log1(MSG_DEBUG, "P2P: Set GO freq based on configured "
	...
	if (wpa_s->conf->p2p_oper_channel == 0 &&
		...
		wpa_warning_log1(MSG_DEBUG, "P2P: Set GO freq based on best overall "
	...
	if (wpa_s->conf->p2p_oper_channel == 0 &&
		...
		wpa_warning_log1(MSG_DEBUG, "P2P: Set GO freq based on best 2.4 GHz "
			   "channel %d MHz", params->freq);
		goto success;
	}
	...
	if (wpa_s->conf->p2p_oper_channel == 0 &&
		...
		wpa_warning_log1(MSG_DEBUG, "P2P: Set GO freq based on best 5 GHz "
			   "channel %d MHz", params->freq);
		goto success;
	}
	...
	if (cand && wpas_p2p_supported_freq_go(wpa_s, channels, cand)) {
		params->freq = cand;
		wpa_warning_log1(MSG_DEBUG, "P2P: Set GO freq %d MHz from preferred "
	...
	if (params->freq == 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: did not find a freq for GO use");
		goto fail;
	}
			   ...
			   int freq)
{
	...
	/* Make sure we are not running find during connection establishment */
	wpa_warning_log0(MSG_DEBUG, "P2P: Stop any on-going P2P FIND");
	...
	if (wpas_p2p_init_go_params(wpa_s, &params, freq, NULL))
		return -1;
				 ...
				 int freq, int force_scan)
{
	...
	if (wpa_s == NULL)
		return -1;
#ifdef LOS_CONFIG_P2P
	if (hi_set_p2p_go_gc_wifi_dev(wpa_s, 0) != HISI_OK) {
		wpa_warning_log0(MSG_ERROR, "hi_set_p2p_go_gc_wifi_dev fail\n");
	}
#endif /* LOS_CONFIG_P2P */
#ifndef HISI_CODE_CROP
	if (force_scan)
				  ...
				  int connection_timeout, int force_scan)
{
	...
	if (wpas_get_p2p_group(wpa_s, ssid->ssid, ssid->ssid_len, &go) &&
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: Requested persistent group is "
			   "already running");
		if (go == 0 &&
		    eloop_cancel_timeout(wpas_p2p_group_formation_timeout,
					 wpa_s->p2pdev, NULL)) {
	...
	if (wpas_p2p_init_go_params(wpa_s, &params, freq, channels))
		return -1;
	...
	if (ssid->passphrase) {
		if (os_strlen(ssid->passphrase) >= sizeof(params.passphrase)) {
			wpa_error_log0(MSG_ERROR, "P2P: Invalid passphrase in "
				   "persistent group");
			return -1;
		}
	...
	if (wpa_s == NULL)
		return -1;
#ifdef LOS_CONFIG_P2P
	if (hi_set_p2p_go_gc_wifi_dev(wpa_s, 1) != HISI_OK) {
		wpa_warning_log0(MSG_ERROR, "hi_set_p2p_go_gc_wifi_dev fail\n");
	}
#endif /* LOS_CONFIG_P2P */
...
static void wpas_p2p_idle_update(void *ctx, int idle)
{
	...
	if (!wpa_s->ap_iface)
		return;
	wpa_warning_buf(MSG_DEBUG, "P2P: GO - group %sidle", idle ? "" : "not ", idle ? strlen("") : strlen("not "));
	if (idle) {
			  ...
			  int registrar)
{
	...
	if (!wpa_s->p2p_in_provisioning) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Ignore WPS success event - P2P "
...
{
	if (!wpa_s->p2p_in_provisioning) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Ignore WPS fail event - P2P "
			   "provisioning not in progress");
		return;
	}
	...
	if (wpa_s == wpa_s->global->p2p_group_formation) {
		 ...
		 */
		wpa_warning_log0(MSG_DEBUG, "P2P: WPS step failed during group formation - reject connection from timeout");
		       ...
		       struct p2ps_provision *p2ps_prov)
{
	...
	if (p2ps_prov && use == WPAS_P2P_PD_FOR_ASP) {
...
#ifndef HISI_CODE_CROP
		wpa_warning_log3(MSG_DEBUG,
			   "P2P: wpas_p2p_prov_disc conncap: %d - ASP parsed: %x %x %d",
			   p2ps_prov->conncap,
			   p2ps_prov->adv_id, p2ps_prov->conncap,
			   p2ps_prov->status);
#endif
		wpa_warning_buf(MSG_DEBUG,
			   "P2P: wpas_p2p_prov_disc conncap:%s",
			   p2ps_prov->info, strlen(p2ps_prov->info));
		config_methods = 0;
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: Unknown config method");
		os_free(p2ps_prov);
		return -1;
	}
	...
	if (use == WPAS_P2P_PD_AUTO) {
		...
		wpa_s->p2p_join_scan_count = 0;
		os_get_reltime(&wpa_s->p2p_auto_started);
		wpa_warning_log2(MSG_DEBUG, "P2P: Auto PD started at %ld.%06ld",
			   wpa_s->p2p_auto_started.sec,
			   wpa_s->p2p_auto_started.usec);
...
static void wpas_p2p_clear_pending_action_tx(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Drop pending Action TX due to new "
		   "operation request");
...
{
	(void)scan_res;
	wpa_warning_log0(MSG_DEBUG, "P2P: Ignore scan results");
		    ...
		    int pref_freq)
{
	...
	if (ssid->mode == WPAS_MODE_P2P_GO) {
		...
		if (peer_addr == NULL) {
			wpa_warning_log0(MSG_DEBUG, "P2P: Missing peer "
		...
		if (wpas_p2p_create_iface(wpa_s)) {
			if (wpas_p2p_add_group_interface(wpa_s,
							 WPA_IF_P2P_GO) < 0) {
				wpa_error_log0(MSG_ERROR, "P2P: Failed to "
	...
	if (wpa_s->parent->conf->p2p_ignore_shared_freq &&
	    ...
	    wpas_p2p_num_unused_channels(wpa_s) > 0) {
		wpa_warning_log1(MSG_DEBUG, "P2P: Ignore own channel preference %d MHz for invitation due to p2p_ignore_shared_freq=1 configuration",
			   pref_freq);
		pref_freq = 0;
	}
			  ...
			  const u8 *peer_addr, const u8 *go_dev_addr)
{
	...
	if (wpa_s == NULL) {
		wpa_warning_buf(MSG_DEBUG, "P2P: Interface '%s' not found", ifname, strlen(ifname));
		return -1;
	}

	ssid = wpa_s->current_ssid;
	if (ssid == NULL) {
		wpa_warning_log0(MSG_DEBUG, "P2P: No current SSID to use for "
			   "invitation");
		return -1;
	}
	...
	if (ssid->mode == WPAS_MODE_P2P_GO) {
		...
		if (wpa_s->wpa_state < WPA_ASSOCIATED) {
			wpa_warning_log0(MSG_DEBUG, "P2P: Not associated - cannot "
...
static void wpas_p2p_group_idle_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_s->conf->p2p_group_idle == 0 && !wpas_p2p_is_client(wpa_s)) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Ignore group idle timeout - "
			   "disabled");
		return;
	}

	wpa_warning_log0(MSG_DEBUG, "P2P: Group idle timeout reached - terminate "
		   "group");
...
static void wpas_p2p_set_group_idle_timeout(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->p2p_in_provisioning) {
		 ...
		 */
		wpa_warning_log0(MSG_DEBUG, "P2P: Do not use P2P group idle timeout "
			   "during provisioning");
		return;
	}
	...
	wpa_warning_log1(MSG_DEBUG, "P2P: Set P2P group idle timeout to %u seconds",
		   timeout);
			  ...
			  int locally_generated)
{
	...
	if (reason_code == WLAN_REASON_DEAUTH_LEAVING && !locally_generated &&
	    ...
	    wpa_s->current_ssid->p2p_group &&
	    wpa_s->current_ssid->mode == WPAS_MODE_INFRA) {
		wpa_warning_log0(MSG_DEBUG, "P2P: GO indicated that the P2P Group "
			   "session is ending");
...
void wpas_p2p_update_config(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_OPER_CHANNEL) {
		...
		ret = p2p_set_oper_channel(p2p, op_reg_class, op_channel,
					   cfg_op_channel);
		if (ret)
			wpa_error_log1(MSG_ERROR, "P2P: Own oper channel update "
				   "failed: %d", ret);
	}
	...
	if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_PREF_CHAN) {
		if (p2p_set_pref_chan(p2p, wpa_s->conf->num_p2p_pref_chan,
			...
			wpa_error_log0(MSG_ERROR, "P2P: Preferred channel list "
				   "update failed");
		}

		if (p2p_set_no_go_freq(p2p, &wpa_s->conf->p2p_no_go_freq) < 0) {
			wpa_error_log0(MSG_ERROR, "P2P: No GO channel list "
				   "update failed");
		}
	}
...
static void wpas_p2p_cross_connect_setup(struct wpa_supplicant *wpa_s)
{
	...
	for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
		...
		wpa_warning_two_buf(MSG_DEBUG, "P2P: Enable cross connection from "
			   "%s to %s whenever uplink is available",
			   wpa_s->ifname, strlen(wpa_s->ifname), wpa_s->cross_connect_uplink, strlen(wpa_s->cross_connect_uplink));
...
int wpas_p2p_notif_pbc_overlap(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Terminate connection due to WPS PBC "
		   "session overlap");
	if (wpa_s != wpa_s->p2pdev)
				  ...
				  enum wpas_p2p_channel_update_trig trig)
{
	...
	if (wpas_p2p_setup_channels(wpa_s, &chan, &cli_chan)) {
		wpa_error_log0(MSG_ERROR, "P2P: Failed to update supported "
			   "channel list");
		return;
	}
...
int wpas_p2p_cancel(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Request to cancel group formation");
	...
	if (peer) {
		wpa_warning_log4(MSG_DEBUG, "P2P: Unauthorize pending GO Neg peer "
			   "%02x:xx:xx:%02x:%02x:%02x", peer[0], peer[3], peer[4], peer[5]);
		p2p_unauthorize(global->p2p, peer);
		found = 1;
	}

	if (wpa_s->scan_res_handler == wpas_p2p_scan_res_join) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Stop pending scan for join");
		wpa_s->scan_res_handler = wpas_p2p_scan_res_ignore;
		found = 1;
	}

	if (wpa_s->pending_pd_before_join) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Stop pending PD before join");
		wpa_s->pending_pd_before_join = 0;
		found = 1;
	}
	...
	for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
		if (wpa_s == global->p2p_group_formation &&
		    (wpa_s->p2p_in_provisioning ||
		     ...
		     WPA_IF_P2P_CLIENT)) {
			wpa_warning_buf(MSG_DEBUG, "P2P: Interface %s in group "
			...
			wpa_warning_buf(MSG_DEBUG, "P2P: Interface %s in invitation found - cancelling",
				   wpa_s->ifname, strlen(wpa_s->ifname));
			found = 1;
			wpas_p2p_group_formation_failed(wpa_s, 0);
			break;
	...
	if (!found) {
		wpa_warning_log0(MSG_DEBUG, "P2P: No ongoing group formation found");
		return -1;
	}
...
void wpas_p2p_interface_unavailable(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Remove group due to driver resource not "
			      ...
			      struct wpa_ssid *ssid)
{
	if (wpa_s->p2p_in_provisioning && ssid->p2p_group &&
		 ...
		 */
		wpa_warning_log0(MSG_DEBUG, "P2P: Canceled group formation due to "
				       ...
				       const u8 *addr)
{
	if (eloop_cancel_timeout(wpas_p2p_group_formation_timeout,
		 ...
		 * provisioning step.
		 */
		wpa_warning_log0(MSG_DEBUG, "P2P: Canceled P2P group formation timeout on data connection");
				...
				int iface_addr)
{
	...
	(void)wpas_p2p_remove_psk_entry(wpa_s, s, addr, iface_addr);
#ifndef HISI_CODE_CROP
...
static void wpas_p2p_group_freq_conflict(void *eloop_ctx, void *timeout_ctx)
{
	...
	(void)timeout_ctx;
	wpa_warning_log0(MSG_DEBUG, "P2P: Frequency conflict - terminate group");
					...
					struct wpa_ssid *ssid)
{
	...
	for (iface = wpa_s->global->ifaces; iface; iface = iface->next) {
		...
		if (!wpas_is_p2p_prioritized(iface)) {
			...
			wpa_warning_log0(MSG_DEBUG, "P2P: Removing P2P connection due to single channel concurrent mode frequency conflict");
					     ...
					     struct wpabuf *p2p)
{
	...
	if (p2p == NULL) {
		wpabuf_free(wsc);
		wpa_warning_log0(MSG_DEBUG, "P2P: No p2p buffer for handover");
		return NULL;
	}
	...
	if (ndef && ret) {
		...
		wpabuf_free(ret);
		if (tmp == NULL) {
			wpa_warning_log0(MSG_DEBUG, "P2P: Failed to NDEF encapsulate handover request");
			return NULL;
		}
		ret = tmp;
	}
					  ...
					  int ndef)
{
	...
	if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL) {
		wpa_warning_log0(MSG_DEBUG, "P2P: P2P disabled - cannot build handover request");
		return NULL;
	}
...
{
	wpa_warning_log1(MSG_DEBUG, "P2P: Initiate join-group based on NFC "
				  ...
				  struct p2p_nfc_params *params, int tag)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Authorize join-group based on NFC "
		   "connection handover");
	for (wpa_s = wpa_s->global->ifaces; wpa_s; wpa_s = wpa_s->next) {
		ssid = wpa_s->current_ssid;
		if (ssid == NULL)
	...
	if (wpa_s == NULL) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Could not find GO interface");
		return -1;
	}
	...
	if (!tag) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Negotiated handover - wait for peer to join without invitation");
		return 0;
	}
...
static int wpas_p2p_nfc_init_go_neg(struct wpa_supplicant *wpa_s,
				    ...
				    int forced_freq)
{
	wpa_warning_log0(MSG_DEBUG, "P2P: Initiate GO Negotiation based on NFC "
		   "connection handover");
	return wpas_p2p_connect(wpa_s, params->peer->p2p_device_addr, NULL,
				    ...
				    int forced_freq)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Authorize GO Negotiation based on NFC "
		   "connection handover");
					    ...
					    int sel, int tag, int forced_freq)
{
	...
	if (end - pos < 2) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Not enough data for Length of WSC "
			   "attributes");
		return -1;
	}
	...
	pos += 2;
	if (len > end - pos) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Not enough data for WSC "
			   "attributes");
		return -1;
	}
	params.wsc_attr = pos;
	...
	if (end - pos < 2) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Not enough data for Length of P2P "
			   "attributes");
		return -1;
	}
	len = WPA_GET_BE16(pos);
	pos += 2;
	if (len > end - pos) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Not enough data for P2P "
			   "attributes");
		return -1;
	}
	params.p2p_attr = pos;
	...
	if (params.oob_dev_pw_len < WPS_OOB_PUBKEY_HASH_LEN + 2) {
		wpa_warning_log0(MSG_DEBUG, "P2P: No peer OOB Dev Pw "
			   "received");
		return -1;
	}

	id = WPA_GET_BE16(params.oob_dev_pw + WPS_OOB_PUBKEY_HASH_LEN);
	wpa_warning_log1(MSG_DEBUG, "P2P: Peer OOB Dev Pw %u", id);
	...
	if (tag) {
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: Static handover - use peer OOB "
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: Using abbreviated WPS handshake "
				 ...
				 const struct wpabuf *sel, int forced_freq)
{
	...
	wpa_warning_log0(MSG_DEBUG, "NFC: P2P connection handover reported");
	...
	if (forced_freq)
		wpa_warning_log1(MSG_DEBUG, "NFC: Forced freq %d", forced_freq);
	tmp = ndef_parse_p2p(init ? sel : req);
	if (tmp == NULL) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Could not parse NDEF");
		return -1;
	}
...
int wpas_p2p_nfc_tag_enabled(struct wpa_supplicant *wpa_s, int enabled)
{
	...
	if (!enabled) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Disable use of own NFC Tag");
		for (iface = wpa_s->global->ifaces; iface; iface = iface->next)
		{
			if (!iface->ap_iface)
				continue;
	...
	if (wpa_s->conf->wps_nfc_dh_pubkey == NULL ||
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: NFC password token not configured "
			   "to allow static handover cases");
		return -1;
	}

	wpa_warning_log0(MSG_DEBUG, "P2P: Enable use of own NFC Tag");
...
static int wpas_p2p_move_go_csa(struct wpa_supplicant *wpa_s)
{
	 ...
	 */
	if (wpas_p2p_init_go_params(wpa_s, &params, 0, NULL)) {
		wpa_dbg(wpa_s, MSG_DEBUG,
...
#else
	csa_settings.freq_params.sec_channel_offset = 0;
	csa_settings.freq_params.ht_enabled = conf->ieee80211n;
	csa_settings.freq_params.bandwidth = 20;
	ret = -1;
#endif /* HISI_CODE_CROP */
out:
		      ...
		      unsigned int count)
{
	...
	if (wpa_s->global->p2p == NULL ||
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: Listen offload not supported");
		return -1;
	}

	if (!p2p_supported_freq(wpa_s->global->p2p, freq)) {
		wpa_error_log1(MSG_ERROR, "P2P: Input channel not supported: %u",
			   freq);
		return -1;
	}
	...
	dev_types_len = (wpa_s->conf->num_sec_device_types + 1) *
		WPS_DEV_TYPE_LEN;
#endif
	dev_types_len = WPS_DEV_TYPE_LEN;
