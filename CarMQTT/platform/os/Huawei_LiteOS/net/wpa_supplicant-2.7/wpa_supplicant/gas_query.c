			    ...
			    size_t len)
{
	if (wpabuf_resize(&query->resp, len) < 0) {
		wpa_warning_log0(MSG_DEBUG, "GAS: No memory to store the response");
		return -1;
	}
				...
				enum offchannel_send_action_result result)
{
	...
	if (gas->current == NULL) {
#ifndef HISI_CODE_CROP
		wpa_warning_log2(MSG_DEBUG, "GAS: Unexpected TX status: freq=%u"
			   " result=%d - no query in progress",
			   freq, result);
#endif /* HISI_CODE_CROP */
		wpa_warning_log4(MSG_DEBUG, "dst="
	...
	wpa_warning_log1(MSG_DEBUG, "GAS: TX status: freq=%u",
		   freq);
#endif /* HISI_CODE_CROP */
	wpa_warning_log4(MSG_DEBUG, "dst=" "%02x:xx:xx:%02x:%02x:%02x",
		   dst[0], dst[3], dst[4], dst[5]);
#ifndef HISI_CODE_CROP
	wpa_warning_log4(MSG_DEBUG,
		   " result=%d query=%p dialog_token=%u dur=%d ms",
		   result, query, query->dialog_token, dur);
#endif /* HISI_CODE_CROP */
	if (os_memcmp(dst, query->addr, ETH_ALEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "GAS: TX status for unexpected destination");
		return;
	}
	...
	if (result == OFFCHANNEL_SEND_ACTION_SUCCESS ||
		...
		if (result == OFFCHANNEL_SEND_ACTION_NO_ACK) {
			wpa_warning_log0(MSG_DEBUG, "GAS: No ACK to GAS request");
			...
			struct wpabuf *req, unsigned int wait_time)
{
	...
	wpa_warning_log4(MSG_DEBUG, "GAS: Send action frame to " "%02x:xx:xx:%02x:%02x:%02x",
		   (query->addr)[0], (query->addr)[3], (query->addr)[4], (query->addr)[5]);
	wpa_warning_log3(MSG_DEBUG," len=%u " "freq=%d prot=%d ",
		   (unsigned int) wpabuf_len(req), query->freq, prot);
	wpa_warning_log4(MSG_DEBUG, "using src addr " "%02x:xx:xx:%02x:%02x:%02x",
		  (query->sa)[0], (query->sa)[3], (query->sa)[4], (query->sa)[5]);
	if (prot) {
				      ...
				      struct gas_query_pending *query)
{
	...
	if (gas_query_tx(gas, query, req, wait_time) < 0) {
		wpa_warning_log4(MSG_DEBUG, "GAS: Failed to send Action frame to "
			   "%02x:xx:xx:%02x:%02x:%02x", (query->addr)[0], (query->addr)[3], (query->addr)[4], (query->addr)[5]);
		gas_query_done(gas, query, GAS_QUERY_INTERNAL_ERROR);
	}
...
static void gas_query_tx_comeback_timeout(void *eloop_data, void *user_ctx)
{
	...
	wpa_warning_log4(MSG_DEBUG, "GAS: Comeback timeout for request to " "%02x:xx:xx:%02x:%02x:%02x",
		   (query->addr)[0], (query->addr)[3], (query->addr)[4], (query->addr)[5]);
					    ...
					    u16 comeback_delay)
{
	...
	wpa_warning_log4(MSG_DEBUG, "GAS: Send comeback request to " "%02x:xx:xx:%02x:%02x:%02x",
		(query->addr)[0], (query->addr)[3], (query->addr)[4], (query->addr)[5]);
	wpa_warning_log2(MSG_DEBUG, " in %u secs %u usecs", secs, usecs);
	eloop_cancel_timeout(gas_query_tx_comeback_timeout, gas, query);
...
{
	wpa_warning_log4(MSG_DEBUG, "GAS: Received initial response from "
		   "%02x:xx:xx:%02x:%02x:%02x",
		   (query->addr)[0], (query->addr)[3], (query->addr)[4], (query->addr)[5]);
	wpa_warning_log2(MSG_DEBUG, " (dialog_token=%u comeback_delay=%u)",
			query->dialog_token, comeback_delay);
...
static void gas_query_rx_comeback(struct gas_query *gas,
				  ...
				  u16 comeback_delay)
{
	wpa_warning_log4(MSG_DEBUG, "GAS: Received comeback response from "
		   "%02x:xx:xx:%02x:%02x:%02x",
		   (query->addr)[0], (query->addr)[3], (query->addr)[4], (query->addr)[5]);
	wpa_warning_log4(MSG_DEBUG, " (dialog_token=%u frag_id=%u more_frags=%u "
		   "comeback_delay=%u)",
		   query->dialog_token, frag_id,
	...
	if ((size_t) 2 + adv_proto[1] != wpabuf_len(query->adv_proto) ||
		...
		wpa_warning_log4(MSG_DEBUG, "GAS: Advertisement Protocol changed "
	...
	if (comeback_delay) {
		if (frag_id) {
			wpa_warning_log4(MSG_DEBUG, "GAS: Invalid comeback response "
				   "with non-zero frag_id and comeback_delay "
				   "from " "%02x:xx:xx:%02x:%02x:%02x", (query->addr)[0], (query->addr)[3], (query->addr)[4], (query->addr)[5]);
			gas_query_done(gas, query, GAS_QUERY_PEER_ERROR);
			return;
		}
		gas_query_tx_comeback_req_delay(gas, query, comeback_delay);
	...
	if (frag_id != query->next_frag_id) {
		wpa_warning_log4(MSG_DEBUG, "GAS: Unexpected frag_id in response "
			   "from " "%02x:xx:xx:%02x:%02x:%02x", (query->addr)[0], (query->addr)[3], (query->addr)[4], (query->addr)[5]);
		if (frag_id + 1 == query->next_frag_id) {
			wpa_warning_log0(MSG_DEBUG, "GAS: Drop frame as possible "
		 ...
		 int freq)
{
	...
	if (prot && !pmf) {
		wpa_warning_log0(MSG_DEBUG, "GAS: Drop unexpected protected GAS frame when PMF is disabled");
		return 0;
	}
	if (!prot && pmf) {
		wpa_warning_log0(MSG_DEBUG, "GAS: Drop unexpected unprotected GAS frame when PMF is enabled");
		return 0;
	}

	query = gas_query_get_pending(gas, sa, dialog_token);
	if (query == NULL) {
		wpa_warning_log4(MSG_DEBUG, "GAS: No pending query found for " "%02x:xx:xx:%02x:%02x:%02x", sa[0], sa[3], sa[4], sa[5]);
		wpa_warning_log1(MSG_DEBUG, " dialog token %u", dialog_token);
		return -1;
	}
#ifndef HISI_CODE_CROP
	wpa_warning_log1(MSG_DEBUG, "GAS: Response in %d ms from ",
		   ms_from_time(&query->last_oper));
#endif
	wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x",
		   sa[0], sa[3], sa[4], sa[5]);

	if (query->wait_comeback && action == WLAN_PA_GAS_INITIAL_RESP) {
		wpa_warning_log4(MSG_DEBUG, "GAS: Unexpected initial response from "
			   "%02x:xx:xx:%02x:%02x:%02x", sa[0], sa[3], sa[4], sa[5]);
		wpa_warning_log1(MSG_DEBUG, " dialog token %u when waiting for comeback "
			   "response", dialog_token);
		return 0;
	}

	if (!query->wait_comeback && action == WLAN_PA_GAS_COMEBACK_RESP) {
		wpa_warning_log4(MSG_DEBUG, "GAS: Unexpected comeback response from "
			   "%02x:xx:xx:%02x:%02x:%02x", sa[0], sa[3], sa[4], sa[5]);
		wpa_warning_log1(MSG_DEBUG, " dialog token %u when waiting for initial "
			   "response", dialog_token);
		return 0;
	}
	...
	if (query->status_code == WLAN_STATUS_QUERY_RESP_OUTSTANDING &&
	    action == WLAN_PA_GAS_COMEBACK_RESP) {
		wpa_warning_log0(MSG_DEBUG, "GAS: Allow non-zero status for outstanding comeback response");
	} else if (query->status_code != WLAN_STATUS_SUCCESS) {
		wpa_warning_log2(MSG_DEBUG, "GAS: Query to " " dialog token "
			   "%u failed - status code %u",
			   dialog_token, query->status_code);
		wpa_warning_log4(MSG_DEBUG, "GAS: Query to " "%02x:xx:xx:%02x:%02x:%02x",
			   sa[0], sa[3], sa[4], sa[5]);
	...
	if (pos + 2 > data + len || pos + 2 + pos[1] > data + len) {
		wpa_warning_log4(MSG_DEBUG, "GAS: No room for Advertisement "
			   "Protocol element in the response from " "%02x:xx:xx:%02x:%02x:%02x",
			   sa[0], sa[3], sa[4], sa[5]);
		return 0;
	}

	if (*pos != WLAN_EID_ADV_PROTO) {
		wpa_warning_log1(MSG_DEBUG, "GAS: Unexpected Advertisement "
			   "Protocol element ID %u in response from ",
			   *pos);
		wpa_warning_log4(MSG_DEBUG, "GAS: Unexpected Advertisement "
				"%02x:xx:xx:%02x:%02x:%02x",
			   sa[0], sa[3], sa[4], sa[5]);
		return 0;
	}
	...
	if (pos + 2 > data + len) {
		wpa_warning_log0(MSG_DEBUG, "GAS: No room for GAS Response Length");
		return 0;
	}
	...
	left = data + len - pos;
	if (resp_len > left) {
		wpa_warning_log4(MSG_DEBUG, "GAS: Truncated Query Response in "
			   "response from " "%02x:xx:xx:%02x:%02x:%02x", sa[0], sa[3], sa[4], sa[5]);
		return 0;
	}

	if (resp_len < left) {
		wpa_warning_log1(MSG_DEBUG, "GAS: Ignore %u octets of extra data ",
			   left - resp_len);
		wpa_warning_log4(MSG_DEBUG,
			   "after Query Response from " "%02x:xx:xx:%02x:%02x:%02x",
			   sa[0], sa[3], sa[4], sa[5]);
	}
...
static void gas_query_timeout(void *eloop_data, void *user_ctx)
{
	...
	wpa_warning_log4(MSG_DEBUG, "GAS: No response received for query to " "%02x:xx:xx:%02x:%02x:%02x",
		   (query->addr)[0], (query->addr)[3], (query->addr)[4], (query->addr)[5]);
	wpa_warning_log1(MSG_DEBUG, " dialog token %u", query->dialog_token);
				     ...
				     struct gas_query_pending *query)
{
	...
	wpa_warning_log1(MSG_DEBUG, "GAS: Starting query timeout for dialog token %u",
		   query->dialog_token);
	eloop_register_timeout(GAS_QUERY_TIMEOUT_PERIOD, 0,
			       gas_query_timeout, gas, query);
}
			    ...
			    struct gas_query_pending *query)
{
	...
	if (wpa_s->conf->gas_rand_mac_addr == gas->last_rand_sa_type &&
		...
		wpa_warning_log4(MSG_DEBUG,
			   ...
			   "%02x:xx:xx:%02x:%02x:%02x", (gas->rand_addr)[0], (gas->rand_addr)[3], (gas->rand_addr)[4], (gas->rand_addr)[5]);
	...
	if (wpa_s->conf->gas_rand_mac_addr == 1 &&
		...
		wpa_error_log0(MSG_ERROR, "GAS: Failed to get random address");
		return -1;
	}
	...
	wpa_warning_log4(MSG_DEBUG, "GAS: Use a new random transmitter address "
		   "%02x:xx:xx:%02x:%02x:%02x", (gas->rand_addr)[0], (gas->rand_addr)[3], (gas->rand_addr)[4], (gas->rand_addr)[5]);
	os_memcpy(query->sa, gas->rand_addr, ETH_ALEN);
