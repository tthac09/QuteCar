...
static u8 wmm_ac_direction_to_idx(u8 direction)
{
	...
	default:
		wpa_error_log1(MSG_ERROR, "Invalid direction: %d", direction);
			 ...
			 const struct wmm_tspec_element *tspec)
{
	...
	wpa_warning_log0(MSG_DEBUG, "Traffic stream was created successfully");
			      ...
			      enum ts_dir_idx dir)
{
	...
	wpa_warning_log2(MSG_DEBUG, "WMM AC: Del TS ac=%d tsid=%d", ac, tsid);
				     ...
				     const struct wmm_ac_addts_request *req)
{
	...
	wpa_warning_log4(MSG_DEBUG, "Sending ADDTS Request to " "%02x:xx:xx:%02x:%02x:%02x",
		   (req->address)[0], (req->address)[3], (req->address)[4], (req->address)[5]);

	/* category + action code + dialog token + status + sizeof(tspec) */
	buf = wpabuf_alloc(4 + sizeof(req->tspec));
	if (!buf) {
		wpa_error_log0(MSG_ERROR, "WMM AC: Allocation error");
		return -1;
	}
			     ...
			     const u8 *address)
{
	...
	wpa_warning_log4(MSG_DEBUG, "Sending DELTS to " "%02x:xx:xx:%02x:%02x:%02x", (address)[0], (address)[3], (address)[4], (address)[5]);
	...
	ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, address,
				  ...
				  wpabuf_head(buf), wpabuf_len(buf), 0);
	if (ret)
		wpa_warning_log0(MSG_WARNING, "Failed to send DELTS frame");
			  ...
			  long min_val, long max_val)
{
	if (value < min_val || (max_val >= 0 && value > max_val)) {
		wpa_warning_buf(MSG_DEBUG,
			   ...
			   name, strlen(name));
		wpa_warning_log3(MSG_DEBUG,
			   "WMM AC: (%ld) is out of range (%ld-%ld)",
...
#undef PARAM_IN_RANGE
	...
	if (!(params->direction == WMM_TSPEC_DIRECTION_UPLINK ||
		...
		wpa_warning_log1(MSG_DEBUG, "WMM AC: invalid TS direction: %d",
			   params->direction);
		return 0;
	}
	...
	if (!wpa_s->wmm_ac_assoc_info->ac_params[req_ac].acm) {
		wpa_warning_log1(MSG_DEBUG, "WMM AC: AC %d is not ACM", req_ac);
		return 0;
	}
			  ...
			  size_t ies_len)
{
	...
	if (ieee802_11_parse_elems(ies, ies_len, &elems, 1) == ParseFailed) {
		wpa_warning_log0(MSG_DEBUG, "WMM AC: could not parse assoc ies");
		return NULL;
	}

	if (!elems.wmm) {
		wpa_warning_log0(MSG_DEBUG, "WMM AC: No WMM IE");
		return NULL;
	}

	if (elems.wmm_len != sizeof(*wmm_params)) {
		wpa_warning_log0(MSG_DEBUG, "WMM AC: Invalid WMM ie length");
		return NULL;
	}
		       ...
		       size_t ies_len, const struct wmm_params *wmm_params)
{
	...
	if (wpa_s->wmm_ac_assoc_info) {
		wpa_error_log0(MSG_ERROR, "WMM AC: Already initialized");
		return -1;
	}

	if (!ies) {
		wpa_error_log0(MSG_ERROR, "WMM AC: Missing IEs");
		return -1;
	}

	if (!(wmm_params->info_bitmap & WMM_PARAMS_UAPSD_QUEUES_INFO)) {
		wpa_warning_log0(MSG_DEBUG, "WMM AC: Missing U-APSD configuration");
		return -1;
	}
	...
	wpa_warning_log1(MSG_DEBUG, "WMM AC: U-APSD queues=0x%x",
...
void wmm_ac_notify_disassoc(struct wpa_supplicant *wpa_s)
{
	...
	wmm_ac_deinit(wpa_s);
	wpa_warning_log0(MSG_DEBUG, "WMM AC: WMM AC is disabled");
}
...
int wpas_wmm_ac_delts(struct wpa_supplicant *wpa_s, u8 tsid)
{
	...
	if (ac < 0) {
		wpa_warning_log0(MSG_DEBUG, "WMM AC: TS does not exist");
		return -1;
	}
		      ...
		      struct wmm_ac_ts_setup_params *params)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WMM AC: TS setup request (addr=" "%02x:xx:xx:%02x:%02x:%02x",
		   (wpa_s->bssid)[0], (wpa_s->bssid)[3], (wpa_s->bssid)[4], (wpa_s->bssid)[5]);
	wpa_warning_log3(MSG_DEBUG, "WMM AC: TS setup request "
		   " tsid=%u user priority=%u direction=%d",
		   params->tsid, params->user_priority, params->direction);
				...
				const struct wmm_tspec_element *tspec)
{
	...
	wpa_warning_log1(MSG_DEBUG,
		   "WMM AC: DELTS frame has been received TSID=%u addr=", tsid);
	wpa_warning_log4(MSG_DEBUG,
		   "WMM AC: DELTS frame has been received addr="
	...
	wpa_warning_log1(MSG_DEBUG,
		   "TS was deleted successfully (tsid=%u" ")",
		   tsid);
	wpa_warning_log4(MSG_DEBUG,
		   "TS was deleted successfully address=" "%02x:xx:xx:%02x:%02x:%02x" ")",
		   sa[0], sa[3], sa[4], sa[5]);
}
		...
		const struct wmm_tspec_element *tspec)
{
	...
	if (status_code != WMM_ADDTS_STATUS_ADMISSION_ACCEPTED) {
		wpa_warning_log0(MSG_INFO, "WMM AC: ADDTS request was rejected");
		goto err_msg;
	}
	...
	wpa_warning_log1(MSG_DEBUG, "ts idx replace bitmap: 0x%x", replace_tspecs);
	...
	wpa_warning_log4(MSG_DEBUG,
		   "WMM AC: adding a new TS with TSID=%u"
		   " medium time=%u access category=%d dir=%d ",
		   tsid, le_to_host16(tspec->medium_time), ac, dir);
	wpa_warning_log4(MSG_DEBUG,
		   "WMM AC: address=""%02x:xx:xx:%02x:%02x:%02x",
			sa[0], sa[3], sa[4], sa[5]);
			...
			const u8 *sa, const u8 *data, size_t len)
{
	...
	if (os_memcmp(da, wpa_s->own_addr, ETH_ALEN) != 0) {
		wpa_warning_log4(MSG_DEBUG, "WMM AC: frame destination addr=""%02x:xx:xx:%02x:%02x:%02x"
			   " is other than ours, ignoring frame", da[0], da[3], da[4], da[5]);
		return;
	}

	if (os_memcmp(sa, wpa_s->bssid, ETH_ALEN) != 0) {
		wpa_warning_log4(MSG_DEBUG, "WMM AC: ignore frame with sa " "%02x:xx:xx:%02x:%02x:%02x"
			   " different other than our bssid", da[0], da[3], da[4], da[5]);
		return;
	}
	...
	if (ieee802_11_parse_elems(data + 2, len - 2, &elems, 1) != ParseOK) {
		wpa_warning_log4(MSG_DEBUG,
			   "WMM AC: Could not parse WMM AC action from " "%02x:xx:xx:%02x:%02x:%02x",
			   sa[0], sa[3], sa[4], sa[5]);
		return;
	}
	...
	if (elems.wmm_tspec_len != sizeof(struct wmm_tspec_element) - 2) {
		wpa_warning_log0(MSG_DEBUG, "WMM AC: missing or wrong length TSPEC");
		return;
	}

	tspec = (struct wmm_tspec_element *)(elems.wmm_tspec - 2);

	wpa_warning_log4(MSG_DEBUG, "WMM AC: RX WMM AC Action from " "%02x:xx:xx:%02x:%02x:%02x",
		   sa[0], sa[3], sa[4], sa[5]);
...
void wmm_ac_save_tspecs(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WMM AC: Save last configured tspecs");
	...
	if (!tspecs_count) {
		wpa_warning_log0(MSG_DEBUG, "WMM AC: No configured TSPECs");
		return;
	}

	wpa_warning_log0(MSG_DEBUG, "WMM AC: Saving tspecs");

	wmm_ac_clear_saved_tspecs(wpa_s);
	wpa_s->last_tspecs = os_calloc(tspecs_count,
				       sizeof(*wpa_s->last_tspecs));
	if (!wpa_s->last_tspecs) {
		wpa_error_log0(MSG_ERROR, "WMM AC: Failed to save tspecs!");
		return;
	}
	...
	wpa_warning_log1(MSG_DEBUG, "WMM AC: Successfully saved %d TSPECs",
...
{
	if (wpa_s->last_tspecs) {
		wpa_warning_log0(MSG_DEBUG, "WMM AC: Clear saved tspecs");
...
int wmm_ac_restore_tspecs(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WMM AC: Restore %u saved tspecs",
		   wpa_s->last_tspecs_count);
