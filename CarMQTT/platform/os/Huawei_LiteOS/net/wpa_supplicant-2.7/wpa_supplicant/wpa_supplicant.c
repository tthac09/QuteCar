				    ...
				    struct wpa_ssid *ssid)
{
	...
	case WPA_CIPHER_CCMP:
		(void)os_memcpy(key, ssid->psk, 16);
		keylen = 16;
		alg = WPA_ALG_CCMP;
		break;
	...
	case WPA_CIPHER_TKIP:
		...
		(void)os_memcpy(key, ssid->psk, 16 + 8);
		(void)os_memcpy(key + 16 + 8, ssid->psk + 16, 8);
		keylen = 32;
		alg = WPA_ALG_TKIP;
		break;
	default:
				       ...
				       struct wpa_ssid *ssid)
{
	...
	(void)wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_RSN_ENABLED, 0);
	(void)wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_KEY_MGMT, wpa_s->key_mgmt);
	(void)wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_PAIRWISE,
			 wpa_s->pairwise_cipher);
	(void)wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_GROUP, wpa_s->group_cipher);
#ifdef CONFIG_IEEE80211W
	(void)wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_MGMT_GROUP,
			      ...
			      enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		if ((g_wpa_event_cb != NULL) && (wpa_s->ifmsh == NULL) && (ssid != NULL) &&
			...
			if ((ssid->ssid != NULL) && (memcpy_s(wpa_events.info.wifi_connected.ssid, HI_WIFI_MAX_SSID_LEN + 1,
						 ssid->ssid, ssid->ssid_len) != EOK)) {
				wpa_error_log0(MSG_DEBUG, "wpa_supplicant_set_state: memcpy_s failed");
			}
			(void)memcpy_s(wpa_events.info.wifi_connected.bssid, HI_WIFI_MAC_LEN, wpa_s->bssid, HI_WIFI_MAC_LEN);
			(void)memcpy_s(wpa_events.info.wifi_connected.ifname, WIFI_IFNAME_MAX_SIZE,
			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
	...
	if (bss) {
		...
		bss_hisi_mesh = wpa_bss_get_vendor_ie(bss, WPA_OUI_HISI_MESH);
#ifdef CONFIG_HS20
	...
	(void)wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_PROTO, proto);
	(void)wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_RSN_ENABLED,
#ifdef CONFIG_HS20
...
#ifndef HISI_CODE_CROP
	wpa_printf(MSG_DEBUG, "WPA: using GTK %s",
...
#ifndef HISI_CODE_CROP
	wpa_printf(MSG_DEBUG, "WPA: using PTK %s",
...
#ifndef HISI_CODE_CROP
		wpa_warning_log0(MSG_DEBUG, "RSN: using KEY_MGMT FT/SAE");
#endif /* HISI_CODE_CROP */
#endif /* HISI_WPA_KEY_MGMT_CROP */
#endif /* CONFIG_SAE */
#ifdef CONFIG_IEEE80211W
...
int wpas_update_random_addr(struct wpa_supplicant *wpa_s, int style)
{
	...
	(void)os_get_reltime(&wpa_s->last_mac_addr_change);
#ifndef HISI_CODE_CROP
...
#ifdef CONFIG_MESH
		if (wpa_s->ifmsh) {
			if (bss && (g_mesh_connect_flag == WPA_FLAG_ON)) {
				hi_wpa_mesh_peer(wpa_s, bss);
			}
			return;
		}
		if (bss)
	...
	enum wpa_drv_update_connect_params_mask *mask)
{
	...
	if (bss && (wpa_bss_get_vendor_ie(bss, WPA_IE_VENDOR_TYPE) ||
		    wpa_bss_get_ie(bss, WLAN_EID_RSN) || wpa_bss_get_vendor_ie(bss, WPA_OUI_HISI_MESH)) &&
				   ...
				   int reason_code)
{
	...
	if (addr) {
		(void)wpa_drv_deauthenticate(wpa_s, addr, reason_code);
			     ...
			     const u8 *buf, size_t len)
{
	...
	(void)os_memcpy(wpa_s->last_eapol_src, src_addr, ETH_ALEN);
	if (!wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt) &&
#ifndef HISI_WPA_KEY_MGMT_CROP
	    wpa_s->key_mgmt != WPA_KEY_MGMT_OWE &&
	    wpa_s->key_mgmt != WPA_KEY_MGMT_DPP &&
...
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE))
		(void)wpa_sm_rx_eapol(wpa_s->wpa, src_addr, buf, len);
#ifndef HISI_WPA_KEY_MGMT_CROP
...
int wpa_supplicant_update_mac_addr(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->driver->get_mac_addr) {
		...
		const u8 *addr = wpa_drv_get_mac_addr(wpa_s);
		if (addr)
			(void)os_memcpy(wpa_s->own_addr, addr, ETH_ALEN);
	}
#endif /* HISI_CODE_CROP */
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	(void)os_memcpy(wpa_s->perm_addr, wpa_s->own_addr, ETH_ALEN);
#endif /* HISI_CODE_CROP */
	wpa_sm_set_own_addr(wpa_s->wpa, wpa_s->own_addr);
...
#endif /* HISI_CODE_CROP */
	(void)wpa_drv_flush_pmkid(wpa_s);
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
...
#ifndef HISI_CODE_CROP
	if (rn)
		(void)os_strlcpy(radio->name, rn, sizeof(radio->name));
#endif
...
#endif /* HISI_CODE_CROP */
		(void)os_strlcpy(wpa_s->ifname, ifname, sizeof(wpa_s->ifname));
	}
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
					...
					int notify, int terminate)
{
	...
	if (wpa_s == wpa_s->parent)
		(void)wpas_p2p_group_remove(wpa_s, "*");
	...
	while (iface) {
		...
		prev = iface;
		iface = iface->next;
		(void)wpa_supplicant_remove_iface(global, prev, terminate);
	}
...
#ifdef CONFIG_DRIVER_HISILICON
#ifndef HISI_CODE_CROP
	wpa_msg(wpa_s, MSG_DEBUG, WPA_EVENT_ADD_IFACE);
#endif /* HISI_CODE_CROP */
#endif
...
#ifndef HISI_CODE_CROP
	(void)eloop_register_signal_terminate(wpa_supplicant_terminate, global);
	(void)eloop_register_signal_reconfig(wpa_supplicant_reconfig, global);
#endif /* HISI_CODE_CROP */
...
void wpas_connection_failed(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	if (count > 3 && wpa_s->current_ssid) {
		wpa_warning_log0(MSG_DEBUG, "Continuous association failures - "
					      ...
					      const char *value)
{
	...
	wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: IEEE 802.1X not included");
	return -1;
#endif /* IEEE8021X_EAPOL */
}
				   ...
				   const char *value, int value_len)
{
	...
	case WPA_CTRL_REQ_EXT_CERT_CHECK:
		...
		break;
	default:
		wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: Unknown type %d", rtype);
		return -1;
	}
...
void wpas_auth_failed(struct wpa_supplicant *wpa_s, char *reason)
{
	...
	if (ssid == NULL) {
		wpa_warning_log0(MSG_DEBUG, "Authentication failure but no known "
			   "SSID block");
		return;
	}
