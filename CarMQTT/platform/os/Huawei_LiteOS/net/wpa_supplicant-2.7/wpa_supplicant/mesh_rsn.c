...
#define MESH_AUTH_TIMEOUT 15
#define MESH_AUTH_RETRY 1
...
void mesh_auth_timer(void *eloop_ctx, void *user_data)
{
	...
	wpa_error_log0(MSG_ERROR, "mesh_auth_timer: enter");

	if (sta->sae->state != SAE_ACCEPTED) {
		wpa_warning_log4(MSG_DEBUG, "AUTH: Re-authenticate with " "%02x:xx:xx:%02x:%02x:%02x",
			   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
		wpa_warning_log1(MSG_DEBUG, " (attempt %d) ", sta->sae_auth_retry);
		...
		if (sta->sae_auth_retry < MESH_AUTH_RETRY) {
			...
			if (sta->sae_auth_retry >= MESH_AUTH_RETRY) {
				wpa_error_log0(MSG_ERROR, "mesh_auth_timer: sae_auth_retry free sta");
				hi_mesh_mpm_send_plink_close_action(wpa_s, sta->addr);
				hi_at_printf("+NOTICE:MESH DISCONNECTED\n\r");
				hi_mesh_report_connect_results(sta, false, WLAN_REASON_MESH_AUTH_FAIL);
			...
			const char *txt)
{
	...
	(void)txt;
	if (addr) {
		wpa_warning_log4(MSG_DEBUG, "AUTH: " "%02x:xx:xx:%02x:%02x:%02x",
			   addr[0], addr[3], addr[4], addr[5]);
		wpa_warning_buf(MSG_DEBUG," - %s",
			   txt, strlen(txt));
	} else {
		wpa_warning_buf(MSG_DEBUG, "AUTH: %s", txt, strlen(txt));
	}
}
			      ...
			      size_t *psk_len)
{
	...
	if (psk_len)
		*psk_len = PMK_LEN;
	wpa_warning_log4(MSG_DEBUG, "AUTH: auth_get_psk (addr=" "%02x:xx:xx:%02x:%02x:%02x",
		   addr[0], addr[3], addr[4], addr[5]);
			...
			const u8 *addr, int idx, u8 *key, size_t key_len)
{
	...
	if (addr) {
		wpa_error_log2(MSG_DEBUG, "AUTH: auth_set_key(alg=%d"
			   " key_idx=%d)", alg, idx);
		wpa_error_log4(MSG_DEBUG, "addr=" "%02x:xx:xx:%02x:%02x:%02x",
			   addr[0], addr[3], addr[4], addr[5]);
	} else {
		wpa_error_log2(MSG_DEBUG, "AUTH: auth_set_key(alg=%d key_idx=%d)",
			   alg, idx);
	}
				...
				enum mfp_options ieee80211w)
{
	...
	u8 seq[6] = {};
#endif
	wpa_warning_log0(MSG_DEBUG, "AUTH: Initializing group state machine");
	...
	if (rsn->auth == NULL) {
		wpa_warning_log0(MSG_DEBUG, "AUTH: wpa_init() failed");
		return -1;
	}
	...
	if (random_get_bytes(rsn->mgtk, rsn->mgtk_len) < 0)
		return -1;
#else
	struct wpa_group *hapd_group = g_hapd->wpa_auth->group;

	rsn->mgtk_len = wpa_cipher_key_len(conf.wpa_group);
	os_memcpy(rsn->mgtk, hapd_group->GTK[hapd_group->GN - 1], rsn->mgtk_len);
...
#ifndef HISI_CODE_CROP
	/* hisi mesh driver has no need to install GTK since AP GTK has been installed before.  */
				    ...
				    struct mesh_conf *conf)
{
	...
	/* hisi mesh use wpa_mesh_auth */
	bss->wpa_mesh_auth = mesh_rsn->auth;
	...
	while ((entry = dl_list_last(&wpa_s->mesh_external_pmksa_cache,
		...
		ret = wpa_auth_pmksa_add_entry(bss->wpa_mesh_auth,
					       entry->pmksa_cache);
...
#else
	if (sae_set_group(sta->sae, wpa_s->ifmsh->bss[0]->conf->sae_groups[0]) < 0) {
		wpa_dbg(wpa_s, MSG_DEBUG, "SME: Selected SAE group %d failed", sta->sae->group);
		return -1;
	}
#endif
			  ...
			  struct sta_info *sta)
{
...
#ifndef LOS_CONFIG_NO_PMKSA
	pmksa = wpa_auth_pmksa_get(hapd->wpa_mesh_auth, sta->addr, NULL);
	if (pmksa) {
		if (!sta->wpa_sm)
			sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_mesh_auth,
							sta->addr, NULL);
		if (!sta->wpa_sm) {
		...
		wpa_error_log4(MSG_DEBUG,
			   "AUTH: Mesh PMKSA cache entry found for " "%02x:xx:xx:%02x:%02x:%02x"
			   " - try to use PMKSA caching instead of new SAE authentication",
			   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
		wpa_auth_pmksa_set_to_sm(pmksa, sta->wpa_sm, hapd->wpa_mesh_auth,
	...
	eloop_register_timeout(MESH_AUTH_TIMEOUT + rnd, 0, mesh_auth_timer,
			       wpa_s, sta);
	wpa_error_log1(MSG_DEBUG, "### mesh_auth_timer start, timeout = %d s", MESH_AUTH_TIMEOUT + rnd);
	return 0;
}
...
void mesh_rsn_init_ampe_sta(struct wpa_supplicant *wpa_s, struct sta_info *sta)
{
	if (random_get_bytes(sta->my_nonce, WPA_NONCE_LEN) < 0) {
		wpa_warning_log0(MSG_INFO, "mesh: Failed to derive random nonce");
			   ...
			   const u8 *cat, struct wpabuf *buf)
{
	...
	u8 *pcat			= (u8 *)cat + 2;
	size_t pcat_len	 = ie - cat - 2;
	if (cat[1] == PLINK_CONFIRM) {
		pcat		+= 2;
		pcat_len	-= 2;
	}
	/*  aes_siv_encrypt begin from MESHID ies */
	const u8 *aad[] = { rsn->wpa_s->own_addr, sta->addr, (const u8 *)pcat};
	size_t aad_len[] = { ETH_ALEN, ETH_ALEN, pcat_len};
	int ret = 0;
	size_t len;

	len = sizeof(*ampe);
#ifdef LOS_CONFIG_MESH_GTK
	if ((cat[1] == PLINK_OPEN) || (cat[1] == PLINK_MGTK_INFO))
#else
	if (cat[1] == PLINK_OPEN)
...
#ifdef LOS_CONFIG_MESH_GTK
	if ((cat[1] == PLINK_MGTK_INFO) || (cat[1] == PLINK_MGTK_ACK))
		len += WPA_REPLAY_COUNTER_LEN;
#endif /* LOS_CONFIG_MESH_GTK */

#ifdef CONFIG_IEEE80211W_AP
#ifdef LOS_CONFIG_MESH_GTK
	if (((cat[1] == PLINK_OPEN) || (cat[1] == PLINK_MGTK_INFO)) && rsn->igtk_len)
#else
	...
	if (2 + AES_BLOCK_SIZE + 2 + len > wpabuf_tailroom(buf)) {
		wpa_error_log0(MSG_ERROR, "protect frame: buffer too small");
		return -EINVAL;
	}

	ampe_ie = os_zalloc(2 + len);
	if (!ampe_ie) {
		wpa_error_log0(MSG_ERROR, "protect frame: out of memory");
		return -ENOMEM;
	}
	...
	/* Key Replay Counter[8] optionally for
	 * Mesh Group Key Inform/Acknowledge frames */
#ifdef LOS_CONFIG_MESH_GTK
	if ((cat[1] == PLINK_MGTK_INFO) || (cat[1] == PLINK_MGTK_ACK)) {
		if (cat[1] == PLINK_MGTK_INFO)
			inc_byte_array(sta->mesh_replay_counter, WPA_REPLAY_COUNTER_LEN);
		os_memcpy(pos, sta->mesh_replay_counter, WPA_REPLAY_COUNTER_LEN);
		pos += WPA_REPLAY_COUNTER_LEN;
	}
	if ((cat[1] != PLINK_OPEN) && (cat[1] != PLINK_MGTK_INFO))
#else
	if (cat[1] != PLINK_OPEN)
...
skip_keys:
	...
	if (aes_siv_encrypt(sta->aek, sizeof(sta->aek), ampe_ie, 2 + len, 3,
				(const u8 **)aad, aad_len, mic_payload)) {
		wpa_error_log0(MSG_ERROR, "protect frame: failed to encrypt");
		ret = -ENOMEM;
	}
			  ...
			  const u8 *start, size_t elems_len)
{
	...
	size_t crypt_len;
	/*  aes_siv_decrypt begin from MESHID ies */
#ifndef HISI_CODE_CROP
	const u8 *aad[] = { sta->addr, wpa_s->own_addr, cat };
	const size_t aad_len[] = { ETH_ALEN, ETH_ALEN,
				   (elems->mic - 2) - cat };
#else
	const u8 *aad[] = { sta->addr, wpa_s->own_addr, elems->mesh_id - 2};
	const size_t aad_len[] = { ETH_ALEN, ETH_ALEN, elems->mic - elems->mesh_id};
#endif
	size_t key_len;
	...
	if (!sta->sae) {
		...
		if (!wpa_auth_pmksa_get(hapd->wpa_mesh_auth, sta->addr, NULL)) {
			wpa_warning_log0(MSG_INFO,
				   "Mesh RSN: SAE is not prepared yet");
			return -1;
		}
#endif /* LOS_CONFIG_NO_PMKSA */
		if (mesh_rsn_auth_sae_sta(wpa_s, sta))
			return -1;
	}
	...
	if (!crypt) {
		wpa_error_log0(MSG_ERROR, "Mesh RSN: out of memory");
		ret = -ENOMEM;
		goto free;
	}
	...
	if (aes_siv_decrypt(sta->aek, sizeof(sta->aek), crypt, crypt_len, 3,
		...
		wpa_error_log0(MSG_ERROR, "Mesh RSN: frame verification failed!");
		ret = -2;
		goto free;
	}
	 ...
	 * frames */
#ifdef LOS_CONFIG_MESH_GTK
	if (((cat[1] == PLINK_MGTK_INFO) && (os_memcmp(sta->mesh_replay_counter, pos, WPA_REPLAY_COUNTER_LEN) >= 0)) ||
	    ((cat[1] == PLINK_MGTK_ACK) && (os_memcmp(sta->mesh_replay_counter, pos, WPA_REPLAY_COUNTER_LEN) != 0))) {
		wpa_error_log0(MSG_DEBUG, "Mesh RSN: invalid replay count");
		ret = -1;
		goto free;
	}
	if (cat[1] == PLINK_MGTK_INFO)
		os_memcpy(sta->mesh_replay_counter, pos, WPA_REPLAY_COUNTER_LEN);
	if ((cat[1] == PLINK_MGTK_INFO) || (cat[1] == PLINK_MGTK_ACK))
		pos += WPA_REPLAY_COUNTER_LEN;
#endif /* LOS_CONFIG_MESH_GTK */
	...
	if (end - pos >= (int) (2 + 6 + key_len)) {
		...
		wpa_warning_log1(MSG_DEBUG, "mesh: IGTKdata - Key ID %u",
			   sta->igtk_key_id);
		pos += 2;
