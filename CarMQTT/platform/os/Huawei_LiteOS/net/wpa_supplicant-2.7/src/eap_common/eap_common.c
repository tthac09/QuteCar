...
int eap_hdr_len_valid(const struct wpabuf *msg, size_t min_payload)
{
	...
	if (wpabuf_len(msg) < sizeof(*hdr)) {
		wpa_warning_log0(MSG_INFO, "EAP: Too short EAP frame");
		return 0;
	}

	len = be_to_host16(hdr->length);
	if (len < sizeof(*hdr) + min_payload || len > wpabuf_len(msg)) {
		wpa_warning_log0(MSG_INFO, "EAP: Invalid EAP length");
		return 0;
	}
			    ...
			    const struct wpabuf *msg, size_t *plen)
{
	...
	if (*pos == EAP_TYPE_EXPANDED) {
		...
		u32 exp_type;
		if (len < sizeof(*hdr) + 8) {
			wpa_warning_log0(MSG_INFO, "EAP: Invalid expanded EAP "
				   "length");
			return NULL;
		}
		pos++;
		...
		if (exp_vendor != vendor || exp_type != (u32) eap_type) {
			wpa_warning_log0(MSG_INFO, "EAP: Invalid expanded frame "
				   "type");
			return NULL;
		}
		...
		*plen = len - sizeof(*hdr) - 8;
		...
		if (vendor != EAP_VENDOR_IETF || *pos != eap_type) {
			wpa_warning_log0(MSG_INFO, "EAP: Invalid frame type");
			return NULL;
		}
		   ...
		   int stop_at_keyname)
{
	...
	while (pos < end) {
		...
		case EAP_ERP_TV_RMSK_LIFETIME:
			/* 4-octet TV */
			if (pos + 4 > end) {
				wpa_warning_log0(MSG_DEBUG, "EAP: Too short TV");
				return -1;
			}
			pos += 4;
			break;
		...
		case EAP_ERP_TLV_NAS_IPV6_ADDRESS:
			if (pos >= end) {
				wpa_warning_log0(MSG_DEBUG, "EAP: Too short TLV");
				return -1;
			}
			tlv_len = *pos++;
			if (tlv_len > (unsigned) (end - pos)) {
				wpa_warning_log0(MSG_DEBUG, "EAP: Truncated TLV");
				return -1;
			}
		...
		default:
			...
			wpa_warning_log1(MSG_DEBUG, "EAP: Unknown TV/TLV type %u",
				   tlv_type);
			pos = end;
			break;
		}
