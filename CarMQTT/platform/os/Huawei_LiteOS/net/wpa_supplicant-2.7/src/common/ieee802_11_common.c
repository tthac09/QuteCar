					    ...
					    int show_errors)
{
	...
	if (elen < 4) {
		if (show_errors) {
			wpa_msgdump_log1(MSG_MSGDUMP, "short vendor specific "
	...
	case OUI_MICROSOFT:
		...
		case WMM_OUI_TYPE:
			...
			if (elen < 5) {
				wpa_msgdump_log1(MSG_MSGDUMP, "short WMM "
			...
			case WMM_OUI_SUBTYPE_TSPEC_ELEMENT:
				...
				break;
			default:
				wpa_msgdump_log2(MSG_EXCESSIVE, "unknown WMM "
					   "information element ignored "
					   "(subtype=%d len=%lu)",
		...
		default:
				...
				wpa_msgdump_log2(MSG_EXCESSIVE, "Unknown Microsoft "
					   "information element ignored "
					   "(type=%d len=%lu)",
...
#endif /* CONFIG_MBO */
		default:
			wpa_msgdump_log2(MSG_MSGDUMP, "Unknown WFA "
				   "information element ignored "
				   "(type=%d len=%lu)",
				   pos[3], (unsigned long) elen);
			return -1;
	...
	case OUI_BROADCOM:
		...
		case VENDOR_VHT_TYPE:
			...
			break;
		default:
			wpa_msgdump_log2(MSG_EXCESSIVE, "Unknown Broadcom "
				   "information element ignored "
				   "(type=%d len=%lu)",
				   pos[3], (unsigned long) elen);
			return -1;
	...
	case OUI_QCA:
		...
		case QCA_VENDOR_ELEM_P2P_PREF_CHAN_LIST:
			...
			break;
		default:
			wpa_msgdump_log2(MSG_EXCESSIVE,
...
#endif /* LOS_CONFIG_80211_IES_CROP */
#ifdef CONFIG_MESH
	case WPA_WLAN_OUI_HUAWEI:
		if ((elen == WPA_MESH_HISI_OPTIMIZATION_VAL_LEN) && (pos[3] == WPA_OUITYPE_MESH) &&
			(pos[4] == WPA_OUISUBTYPE_MESH_HISI_OPTIMIZATION)) {
			elems->mesh_bcn_priority = pos[5];
			elems->mesh_is_mbr = pos[6];
			wpa_error_log2(MSG_ERROR, "get hisi mesh bcn_priority : %d, is_mbr : %d",
						elems->mesh_bcn_priority, elems->mesh_is_mbr);
		}
		break;
#endif
	default:
		wpa_msgdump_log4(MSG_EXCESSIVE, "unknown vendor specific "
				...
				int show_errors)
{
	...
	while (left >= 2) {
		...
		if (elen > left) {
			if (show_errors) {
				wpa_warning_log3(MSG_DEBUG, "IEEE 802.11 element "
		...
		default:
			...
			if (!show_errors)
				break;
#ifndef CONFIG_PRINT_NOUSE
				wpa_msgdump_log2(MSG_MSGDUMP, "IEEE 802.11 element parse "
			  ...
			  const char *name, const char *val)
{
	...
	if (os_strncmp(pos, "be_", 3) == 0) {
		...
		wpa_error_buf(MSG_ERROR, "Unknown WMM name '%s'", pos, strlen(pos));
		return -1;
	}
	...
	if (os_strcmp(pos, "aifs") == 0) {
		v = atoi(val);
		if (v < 1 || v > 255) {
			wpa_error_log1(MSG_ERROR, "Invalid AIFS value %d", v);
			return -1;
		}
		ac->aifs = v;
		...
		v = atoi(val);
		if (v < 0 || v > 15) {
			wpa_error_log1(MSG_ERROR, "Invalid cwMin value %d", v);
			return -1;
		}
		ac->cwmin = v;
		...
		v = atoi(val);
		if (v < 0 || v > 15) {
			wpa_error_log1(MSG_ERROR, "Invalid cwMax value %d", v);
			return -1;
		}
		ac->cwmax = v;
		...
		v = atoi(val);
		if (v < 0 || v > 0xffff) {
			wpa_error_log1(MSG_ERROR, "Invalid txop value %d", v);
			return -1;
		}
		ac->txop_limit = v;
		...
		v = atoi(val);
		if (v < 0 || v > 1) {
			wpa_error_log1(MSG_ERROR, "Invalid acm value %d", v);
			return -1;
		}
		ac->admission_control_mandatory = v;
	} else {
		wpa_error_buf(MSG_ERROR, "Unknown wmm_ac_ field '%s'", pos, strlen(pos));
		return -1;
	}
...
#else
	return ieee80211_freq_to_channel_ext(freq, 0,
#endif /* HISI_CODE_CROP */
					     &op_class, channel);
}
		       ...
		       size_t ies_len)
{
	...
	while (ies_buf && ies_len >= 2 &&
		...
		if (ies_buf[0] == WLAN_EID_MULTI_BAND) {
			wpa_warning_log1(MSG_DEBUG, "MB IE of %zu bytes found", len);
...
#else
enum phy_type ieee80211_get_phy_type(int freq, int ht)
#endif /* HISI_CODE_CROP */
{
#ifndef HISI_CODE_CROP
	if (vht)
	...
	while (pos) {
		...
		*nei_pos++ = WLAN_EID_NEIGHBOR_REPORT;
		...
		if (hwaddr_aton(pos, nei_pos)) {
			wpa_warning_log0(MSG_DEBUG, "Invalid BSSID");
			return -1;
		}
		nei_pos += ETH_ALEN;
		pos += 17;
		if (*pos != ',') {
			wpa_warning_log0(MSG_DEBUG, "Missing BSSID Information");
			return -1;
		}
		pos++;
		...
		nei_pos += 4;
		if (*endptr != ',') {
			wpa_warning_log0(MSG_DEBUG, "Missing Operating Class");
			return -1;
		}
		pos = endptr + 1;
		...
		*nei_pos++ = atoi(pos); /* Operating Class */
		pos = os_strchr(pos, ',');
		if (pos == NULL) {
			wpa_warning_log0(MSG_DEBUG, "Missing Channel Number");
			return -1;
		}
		pos++;
		...
		*nei_pos++ = atoi(pos); /* Channel Number */
		pos = os_strchr(pos, ',');
		if (pos == NULL) {
			wpa_warning_log0(MSG_DEBUG, "Missing PHY Type");
			return -1;
		}
		pos++;
