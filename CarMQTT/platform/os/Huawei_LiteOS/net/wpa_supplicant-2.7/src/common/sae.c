...
int sae_set_group(struct sae_data *sae, int group)
{
	...
	if (tmp->ec) {
		wpa_warning_log1(MSG_DEBUG, "SAE: Selecting supported ECC group %d",
			   group);
		sae->group = group;
...
#ifndef CONFIG_SAE_NO_FFC
	/* Not an ECC group, check FFC */
	tmp->dh = dh_groups_get(group);
	if (tmp->dh) {
		wpa_warning_log1(MSG_DEBUG, "SAE: Selecting supported FFC group %d",
			   group);
		sae->group = group;
...
{
	wpa_warning_log4(MSG_DEBUG, "SAE: PWE derivation - addr1=" "%02x:xx:xx:%02x:%02x:%02x",
		(addr1)[0], (addr1)[3], (addr1)[4], (addr1)[5]);
	wpa_warning_log4(MSG_DEBUG, "SAE: PWE derivation"
		   " addr2=" "%02x:xx:xx:%02x:%02x:%02x", (addr2)[0], (addr2)[3], (addr2)[4], (addr2)[5]);
			     ...
			     struct crypto_bignum **qnr)
{
	...
	*qnr = NULL;
	...
	while (!(*qr) || !(*qnr)) {
		...
		int res;
		rand_count++;
		if (rand_count >= GET_RANDOM_QR_QNR) {
			wpa_warning_log0(MSG_DEBUG, "get_random_qr_qnr out!");
			return -1;
		}
			      ...
			      size_t password_len, const char *identifier)
{
...
#ifndef CONFIG_SAE_NO_PW_ID
	if (identifier) {
		wpa_warning_buf(MSG_DEBUG, "SAE: password identifier: %s",
	...
	for (counter = 1; counter <= k || !found; counter++) {
		...
		if (counter > 200) {
			...
			wpa_warning_log0(MSG_DEBUG, "SAE: Failed to derive PWE");
			break;
		}

		wpa_warning_log1(MSG_DEBUG, "SAE: counter = %03u", counter);
	...
	if (res < 0) {
		 ...
		 * is a result.
		 */
		wpa_warning_log0(MSG_DEBUG, "SAE: Could not solve y");
	}
			      ...
			      size_t password_len, const char *identifier)
{
	...
	for (counter = 1; counter <= k || !found; counter++) {
		...
		wpa_warning_log1(MSG_DEBUG, "SAE: counter = %02u", counter);
		if (hmac_sha256_vector(addrs, sizeof(addrs), num_elem,
				       addr, len, pwd_seed) < 0)
			break;
					 ...
					 struct crypto_bignum *mask)
{
	...
	if (crypto_ec_point_mul(sae->tmp->ec, sae->tmp->pwe_ecc, mask,
	    ...
	    crypto_ec_point_invert(sae->tmp->ec,
				   sae->tmp->own_commit_element_ecc) < 0) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Could not compute commit-element");
		return -1;
	}
...
static int sae_derive_commit(struct sae_data *sae)
{
	...
	do {
		...
		if (mask == NULL) {
			wpa_warning_log0(MSG_DEBUG, "SAE: Could not get rand/mask");
			return -1;
		}
...
static int sae_derive_k_ecc(struct sae_data *sae, u8 *k)
{
	...
	if (crypto_ec_point_mul(sae->tmp->ec, sae->tmp->pwe_ecc,
		...
		wpa_warning_log0(MSG_DEBUG, "SAE: Failed to calculate K and k");
		goto fail;
	}
		      ...
		      const struct wpabuf *token, const char *identifier)
{
	...
	if (identifier) {
		...
		wpa_warning_buf(MSG_DEBUG, "SAE: own Password Identifier: %s",
			   identifier, strlen(identifier));
	}
#else
	(void)identifier;
...
u16 sae_group_allowed(struct sae_data *sae, int *allowed_groups, u16 group)
{
	if (allowed_groups) {
		...
		if (allowed_groups[i] != group) {
			wpa_warning_log1(MSG_DEBUG, "SAE: Proposed group %u not "
	...
	if (sae->state == SAE_COMMITTED && group != sae->group) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Do not allow group to be changed");
		return WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED;
	}

	if (group != sae->group && sae_set_group(sae, group) < 0) {
		wpa_warning_log1(MSG_DEBUG, "SAE: Unsupported Finite Cyclic Group %u",
			   group);
		return WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED;
	}

	if (sae->tmp == NULL) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Group information not yet initialized");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}

#ifndef CONFIG_SAE_NO_FFC
	if (sae->tmp->dh && !allowed_groups) {
		wpa_warning_log1(MSG_DEBUG, "SAE: Do not allow FFC group %u without "
				   ...
				   const u8 *end)
{
	...
	if (sae->tmp->prime_len > end - *pos) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Not enough data for scalar");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	...
	if (sae->state == SAE_ACCEPTED && sae->peer_commit_scalar &&
		...
		wpa_warning_log0(MSG_DEBUG, "SAE: Do not accept re-use of previous "
			   "peer-commit-scalar");
	...
	if (crypto_bignum_is_zero(peer_scalar) ||
		...
		wpa_warning_log0(MSG_DEBUG, "SAE: Invalid peer scalar");
		crypto_bignum_deinit(peer_scalar, 0);
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
					...
					const u8 *end)
{
	...
	if (2 * sae->tmp->prime_len > end - *pos) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Not enough data for "
			   "commit-element");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	...
	if (os_memcmp(*pos, prime, sae->tmp->prime_len) >= 0 ||
	    os_memcmp(*pos + sae->tmp->prime_len, prime,
		      sae->tmp->prime_len) >= 0) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Invalid coordinates in peer "
			   "element");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	...
	if (!crypto_ec_point_is_on_curve(sae->tmp->ec,
					 sae->tmp->peer_commit_element_ecc)) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Peer element is not on curve");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
					...
					const u8 *end)
{
	...
	if (!res || !one ||
		...
		wpa_warning_log0(MSG_DEBUG, "SAE: Invalid peer element");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	crypto_bignum_deinit(one, 0);
	...
	if (crypto_bignum_exptmod(sae->tmp->peer_commit_element_ffc,
		...
		wpa_warning_log0(MSG_DEBUG, "SAE: Invalid peer element (scalar-op)");
		crypto_bignum_deinit(res, 0);
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	crypto_bignum_deinit(res, 0);
	...
	if (!sae_is_password_id_elem(pos, end)) {
		if (sae->tmp->pw_id) {
			wpa_warning_buf(MSG_DEBUG,
				   ...
				   sae->tmp->pw_id, strlen(sae->tmp->pw_id));
...
int sae_check_confirm(struct sae_data *sae, const u8 *data, size_t len)
{
	...
	if (len < 2 + SHA256_MAC_LEN) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Too short confirm message");
		return -1;
	}

	wpa_warning_log1(MSG_DEBUG, "SAE: peer-send-confirm %u", WPA_GET_LE16(data));

	if (!sae->tmp || !sae->peer_commit_scalar ||
		!sae->tmp->own_commit_scalar) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Temporary data not yet available");
		return -1;
	}
	...
	if (os_memcmp_const(verifier, data + 2, SHA256_MAC_LEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Confirm mismatch");
