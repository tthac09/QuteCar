					     ...
					     u8 id)
{
	...
	req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_IDENTITY, req_data_len,
			    EAP_CODE_REQUEST, id);
	if (req == NULL) {
		wpa_error_log0(MSG_ERROR, "EAP-Identity: Failed to allocate "
			   "memory for request");
		data->state = FAILURE;
		return NULL;
	}
				  ...
				  struct wpabuf *respData)
{
	...
	pos = eap_hdr_validate(EAP_VENDOR_IETF, EAP_TYPE_IDENTITY,
			       respData, &len);
	if (pos == NULL) {
		wpa_warning_log0(MSG_INFO, "EAP-Identity: Invalid frame");
		return TRUE;
	}
				 ...
				 struct wpabuf *respData)
{
	...
	if (data->pick_up) {
		if (eap_identity_check(sm, data, respData)) {
			wpa_warning_log0(MSG_DEBUG, "EAP-Identity: failed to pick "
