...
{
	wpa_warning_two_buf(MSG_DEBUG, "EAP-pwd: %s -> %s",
		   eap_pwd_state_txt(data->state), strlen(eap_pwd_state_txt(data->state)), eap_pwd_state_txt(state), strlen(eap_pwd_state_txt(state)));
...
static void * eap_pwd_init(struct eap_sm *sm)
{
	...
	if (sm->user == NULL || sm->user->password == NULL ||
		...
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): Password is not "
			   "configured");
		return NULL;
	}
	...
	wpa_warning_log1(MSG_DEBUG, "EAP-pwd: Selected group number %d",
		   data->group_num);
	...
	if (data->password == NULL) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD: Memory allocation password "
			   "fail");
...
static void eap_pwd_build_id_req(struct eap_sm *sm, struct eap_pwd_data *data,
				 u8 id)
{
	wpa_warning_log0(MSG_DEBUG, "EAP-pwd: ID/Request");
	/*
				     ...
				     struct eap_pwd_data *data, u8 id)
{
	...
	wpa_warning_log0(MSG_DEBUG, "EAP-pwd: Commit/Request");
	/*
	...
	if (!data->private_value || !data->my_element || !data->my_scalar ||
	    !mask) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): scalar allocation "
			   "fail");
		goto fin;
	}
	...
	if (crypto_ec_point_mul(data->grp->group, data->grp->pwe, mask,
				data->my_element) < 0) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): element allocation "
			   "fail");
		eap_pwd_state(data, FAILURE);
		goto fin;
	}
	...
	if (crypto_ec_point_invert(data->grp->group, data->my_element) < 0) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): element inversion "
			   "fail");
		goto fin;
	}
	...
	if (!scalar || !element) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): data allocation fail");
		goto fin;
	}

	if (crypto_ec_point_to_bin(data->grp->group, data->my_element, element,
				   element + prime_len) < 0) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): point assignment "
			   "fail");
		goto fin;
	}
				      ...
				      struct eap_pwd_data *data, u8 id)
{
	...
	wpa_warning_log0(MSG_DEBUG, "EAP-pwd: Confirm/Request");
	/*
	...
	if (!cruft) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): debug allocation "
			   "fail");
		goto fin;
	}
	...
	if (crypto_ec_point_to_bin(data->grp->group, data->my_element, cruft,
				   cruft + prime_len) < 0) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): confirm point "
			   "assignment fail");
		goto fin;
	}
...
eap_pwd_build_req(struct eap_sm *sm, void *priv, u8 id)
{
	 ...
	 */
	if (data->in_frag_pos) {
		wpa_warning_log0(MSG_DEBUG, "EAP-pwd: ACKing a fragment!!");
	...
	case PWD_Confirm_Req:
		...
		break;
	default:
		wpa_warning_log1(MSG_INFO, "EAP-pwd: Unknown state %d in build_req",
			   data->state);
		eap_pwd_state(data, FAILURE);
	...
	if ((len + EAP_PWD_HDR_SIZE) > data->mtu) {
		...
		if (data->out_frag_pos == 0) {
			...
			len -= sizeof(u16);
			wpa_warning_log1(MSG_DEBUG, "EAP-pwd: Fragmenting output, "
				   "total length = %d", totlen);
		}
		wpa_warning_log1(MSG_DEBUG, "EAP-pwd: Send a %d byte fragment",
			   (int) len);
	}
			     ...
			     struct wpabuf *respData)
{
	...
	if (pos == NULL || len < 1) {
		wpa_warning_log0(MSG_INFO, "EAP-pwd: Invalid frame");
		return TRUE;
	}

	wpa_warning_log2(MSG_DEBUG, "EAP-pwd: Received frame: exch = %d, len = %d",
	...
	wpa_warning_log2(MSG_INFO, "EAP-pwd: Unexpected opcode=%d in state=%d",
		   *pos, data->state);
				    ...
				    const u8 *payload, size_t payload_len)
{
	...
	if (payload_len < sizeof(struct eap_pwd_id)) {
		wpa_warning_log0(MSG_INFO, "EAP-pwd: Invalid ID response");
		return;
	}
	...
	if ((data->group_num != be_to_host16(id->group_num)) ||
		...
		wpa_warning_log0(MSG_INFO, "EAP-pwd: peer changed parameters");
		eap_pwd_state(data, FAILURE);
		return;
	}
	if (data->id_peer || data->grp) {
		wpa_warning_log0(MSG_INFO, "EAP-pwd: data was already allocated");
		return;
	}
	data->id_peer = os_malloc(payload_len - sizeof(struct eap_pwd_id));
	if (data->id_peer == NULL) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD: memory allocation id fail");
		return;
	}
	...
	if (data->grp == NULL) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD: failed to allocate memory for "
			   "group");
		return;
	}
	...
	if (res) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): unable to compute "
			   "PWE");
		return;
	}
	wpa_warning_log1(MSG_DEBUG, "EAP-PWD (server): computed %d bit PWE...",
			    ...
			    const u8 *payload, size_t payload_len)
{
	...
	wpa_warning_log0(MSG_DEBUG, "EAP-pwd: Received commit response");
	...
	if (!data->k || !cofactor || !K) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): peer data allocation "
			   "fail");
		goto fin;
	}

	if (crypto_ec_cofactor(data->grp->group, cofactor) < 0) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): unable to get "
			   "cofactor for curve");
		goto fin;
	}
	...
	if (!data->peer_element) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): setting peer element "
			   "fail");
		goto fin;
	}
	ptr += prime_len * 2;
	...
	if ((crypto_ec_point_mul(data->grp->group, data->grp->pwe,
	    ...
	    (crypto_ec_point_mul(data->grp->group, K, data->private_value,
				 K) < 0)) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): computing shared key "
			   "fail");
		goto fin;
	}
	...
	if (!crypto_bignum_is_one(cofactor)) {
		if (crypto_ec_point_mul(data->grp->group, K, cofactor,
					K) != 0) {
			wpa_warning_log0(MSG_INFO, "EAP-PWD (server): cannot "
	...
	if (crypto_ec_point_is_at_infinity(data->grp->group, K)) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): shared key point is "
			   "at infinity");
		goto fin;
	}
	if (crypto_ec_point_x(data->grp->group, K, data->k)) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): unable to extract "
			     ...
			     const u8 *payload, size_t payload_len)
{
	...
	*ptr = EAP_PWD_DEFAULT_PRF;
	...
	cruft = os_malloc(prime_len * 2);
	if (!cruft) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (peer): allocation fail");
		goto fin;
	}
	...
	if (os_memcmp_const(conf, ptr, SHA256_MAC_LEN)) {
		wpa_warning_log0(MSG_INFO, "EAP-PWD (server): confirm did not "
			   "verify");
		goto fin;
	}

	wpa_warning_log0(MSG_DEBUG, "EAP-pwd (server): confirm verified");
			    ...
			    struct wpabuf *respData)
{
	...
	pos = eap_hdr_validate(EAP_VENDOR_IETF, EAP_TYPE_PWD, respData, &len);
	if ((pos == NULL) || (len < 1)) {
		wpa_warning_buf(MSG_INFO, "Bad EAP header! pos %s",
			   (pos == NULL) ? "is NULL" : "is not NULL",
			   (pos == NULL) ? strlen("is NULL") : strlen("is not NULL"));
		wpa_warning_log1(MSG_INFO, "Bad EAP header! len = %d", (int) len);
		return;
	}
	...
	if (data->out_frag_pos) {
		if (len > 1)
			wpa_warning_log0(MSG_INFO, "EAP-pwd: Bad response! "
				   "Fragmenting but not an ACK");
		else
			wpa_warning_log0(MSG_DEBUG, "EAP-pwd: received ACK from "
				   "peer");
		return;
	}
	/*
	...
	if (EAP_PWD_GET_LENGTH_BIT(lm_exch)) {
		...
		wpa_warning_log1(MSG_DEBUG, "EAP-pwd: Incoming fragments, total "
			   "length = %d", tot_len);
		if (tot_len > 15000)
			return;
		if (data->inbuf) {
		...
		if (data->inbuf == NULL) {
			wpa_warning_log0(MSG_INFO, "EAP-pwd: Out of memory to "
				   "buffer fragments!");
			return;
		}
		data->in_frag_pos = 0;
	...
	if (EAP_PWD_GET_MORE_BIT(lm_exch) || data->in_frag_pos) {
		...
		if ((data->in_frag_pos + len) > wpabuf_size(data->inbuf)) {
			wpa_warning_log3(MSG_DEBUG, "EAP-pwd: Buffer overflow "
	...
	if (EAP_PWD_GET_MORE_BIT(lm_exch)) {
		wpa_warning_log1(MSG_DEBUG, "EAP-pwd: Got a %d byte fragment",
			   (int) len);
		return;
	}
	/*
	...
	if (data->in_frag_pos && data->inbuf)  {
		...
		wpa_warning_log1(MSG_DEBUG, "EAP-pwd: Last fragment, %d bytes",
			   (int) len);
	}
