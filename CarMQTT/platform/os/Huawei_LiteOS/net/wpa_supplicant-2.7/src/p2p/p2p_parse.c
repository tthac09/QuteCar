			       ...
			       struct p2p_message *msg)
{
	...
	case P2P_ATTR_CAPABILITY:
		if (len < 2) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Capability "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->capability = data;
		wpa_warning_log2(MSG_DEBUG, "P2P: * Device Capability %02x "
			   "Group Capability %02x",
			   data[0], data[1]);
		break;
	case P2P_ATTR_DEVICE_ID:
		if (len < ETH_ALEN) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Device ID "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->device_id = data;
		wpa_warning_log4(MSG_DEBUG, "P2P: * Device ID " "%02x:xx:xx:%02x:%02x:%02x",
			   (msg->device_id)[0], (msg->device_id)[3], (msg->device_id)[4], (msg->device_id)[5]);
		break;
	case P2P_ATTR_GROUP_OWNER_INTENT:
		if (len < 1) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short GO Intent "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->go_intent = data;
		wpa_warning_log2(MSG_DEBUG, "P2P: * GO Intent: Intent %u "
			   "Tie breaker %u", data[0] >> 1, data[0] & 0x01);
		break;
	case P2P_ATTR_STATUS:
		if (len < 1) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Status "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->status = data;
		wpa_warning_log1(MSG_DEBUG, "P2P: * Status: %d", data[0]);
		break;
	case P2P_ATTR_LISTEN_CHANNEL:
		if (len == 0) {
			wpa_warning_log0(MSG_DEBUG, "P2P: * Listen Channel: Ignore "
				   "null channel");
			break;
		}
		if (len < 5) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Listen Channel "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->listen_channel = data;
		wpa_warning_log2(MSG_DEBUG, "P2P: * Listen Channel: "
			   "Country %c%c Regulatory ", data[0], data[1]);
		wpa_warning_log3(MSG_DEBUG, "P2P: * Listen Channel: "
			   "Country (0x%02x) Regulatory "
			   "Class %d Channel Number %d",
			   data[2], data[3], data[4]);
		break;
	case P2P_ATTR_OPERATING_CHANNEL:
		if (len == 0) {
			wpa_warning_log0(MSG_DEBUG, "P2P: * Operating Channel: "
				   "Ignore null channel");
			break;
		}
		if (len < 5) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Operating "
				   "Channel attribute (length %d)", len);
			return -1;
		}
		msg->operating_channel = data;
		wpa_warning_log2(MSG_DEBUG, "P2P: * Operating Channel: "
			   "Country %c%c Regulatory ", data[0], data[1]);
		wpa_warning_log3(MSG_DEBUG, "P2P: * Operating Channel: "
			   "Country (0x%02x) Regulatory "
			   "Class %d Channel Number %d",
			   data[2], data[3], data[4]);
		break;
	case P2P_ATTR_CHANNEL_LIST:
		if (len < 3) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Channel List "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->channel_list = data;
		msg->channel_list_len = len;
		wpa_warning_log3(MSG_DEBUG, "P2P: * Channel List: Country String "
	...
	case P2P_ATTR_GROUP_INFO:
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: * Group Info");
		break;
	case P2P_ATTR_DEVICE_INFO:
		if (len < ETH_ALEN + 2 + 8 + 1) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Device Info "
				   "attribute (length %d)", len);
			return -1;
		}
		...
		if (msg->num_sec_dev_types * 8 > data + len - pos) {
			wpa_warning_log0(MSG_DEBUG, "P2P: Device Info underflow");
			return -1;
		}
		pos += msg->num_sec_dev_types * 8;
		if (data + len - pos < 4) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Invalid Device Name "
		...
		if (nlen > data + len - pos || nlen > WPS_DEV_NAME_MAX_LEN) {
			wpa_warning_log2(MSG_DEBUG, "P2P: Invalid Device Name "
		...
		/*wpa_printf(MSG_DEBUG, "P2P: * Device Info: addr " "%02x:xx:xx:%02x:%02x:%02x"
	...
	case P2P_ATTR_CONFIGURATION_TIMEOUT:
		if (len < 2) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Configuration "
				   "Timeout attribute (length %d)", len);
			return -1;
		}
		msg->config_timeout = data;
		wpa_warning_log0(MSG_DEBUG, "P2P: * Configuration Timeout");
		break;
	case P2P_ATTR_INTENDED_INTERFACE_ADDR:
		if (len < ETH_ALEN) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Intended P2P "
				   "Interface Address attribute (length %d)",
				   len);
			return -1;
		}
		msg->intended_addr = data;
		wpa_warning_log4(MSG_DEBUG, "P2P: * Intended P2P Interface Address: "
			   "%02x:xx:xx:%02x:%02x:%02x", (msg->intended_addr)[0], (msg->intended_addr)[3], 
			   (msg->intended_addr)[4], (msg->intended_addr)[5]);
		break;
	case P2P_ATTR_GROUP_BSSID:
		if (len < ETH_ALEN) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short P2P Group BSSID "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->group_bssid = data;
		wpa_warning_log4(MSG_DEBUG, "P2P: * P2P Group BSSID: " "%02x:xx:xx:%02x:%02x:%02x",
			   (msg->group_id)[0], (msg->group_id)[3], (msg->group_id)[4], (msg->group_id)[5]);
		break;
	case P2P_ATTR_GROUP_ID:
		if (len < ETH_ALEN || len > ETH_ALEN + SSID_MAX_LEN) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Invalid P2P Group ID "
				   "attribute length %d", len);
			return -1;
		}
		msg->group_id = data;
		msg->group_id_len = len;
		wpa_warning_log4(MSG_DEBUG, "P2P: * P2P Group ID: Device Address "
			   "%02x:xx:xx:%02x:%02x:%02x", (msg->group_id)[0], (msg->group_id)[3], (msg->group_id)[4], (msg->group_id)[5]);
	...
	case P2P_ATTR_INVITATION_FLAGS:
		if (len < 1) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Invitation "
				   "Flag attribute (length %d)", len);
			return -1;
		}
		msg->invitation_flags = data;
		wpa_warning_log1(MSG_DEBUG, "P2P: * Invitation Flags: bitmap 0x%x",
			   data[0]);
		break;
	case P2P_ATTR_MANAGEABILITY:
		if (len < 1) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Manageability "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->manageability = data;
		wpa_warning_log1(MSG_DEBUG, "P2P: * Manageability: bitmap 0x%x",
			   data[0]);
		break;
	case P2P_ATTR_NOTICE_OF_ABSENCE:
		if (len < 2) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Notice of "
				   "Absence attribute (length %d)", len);
			return -1;
		}
		msg->noa = data;
		msg->noa_len = len;
		wpa_warning_log0(MSG_DEBUG, "P2P: * Notice of Absence");
		break;
	case P2P_ATTR_EXT_LISTEN_TIMING:
		if (len < 4) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Extended Listen "
				   "Timing attribute (length %d)", len);
			return -1;
		}
		msg->ext_listen_timing = data;
		wpa_warning_log2(MSG_DEBUG, "P2P: * Extended Listen Timing "
	...
	case P2P_ATTR_MINOR_REASON_CODE:
		if (len < 1) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short Minor Reason "
				   "Code attribute (length %d)", len);
			return -1;
		}
		msg->minor_reason_code = data;
		wpa_warning_log1(MSG_DEBUG, "P2P: * Minor Reason Code: %u",
			   *msg->minor_reason_code);
		break;
	case P2P_ATTR_OOB_GO_NEG_CHANNEL:
		if (len < 6) {
			wpa_warning_log1(MSG_DEBUG, "P2P: Too short OOB GO Neg "
				   "Channel attribute (length %d)", len);
			return -1;
		}
		msg->oob_go_neg_channel = data;
		wpa_warning_log3(MSG_DEBUG, "P2P: * OOB GO Neg Channel: "
			   "Country %c%c(0x%02x)", data[0], data[1], data[2]);
		wpa_warning_log3(MSG_DEBUG, "P2P: * OOB GO Neg Channel: "
			   "Operating Class %d " "Channel Number %d Role %d",
	...
	case P2P_ATTR_SESSION_INFORMATION_DATA:
		...
		wpa_warning_log2(MSG_DEBUG, "P2P: * Service Instance: %u bytes - %p",
			   len, data);
		break;
	case P2P_ATTR_CONNECTION_CAPABILITY:
		...
		msg->conn_cap = data;
		wpa_warning_log1(MSG_DEBUG, "P2P: * Connection Capability: 0x%x",
			   *msg->conn_cap);
		break;
	case P2P_ATTR_ADVERTISEMENT_ID:
		...
		wpa_warning_log1(MSG_DEBUG, "P2P: * Advertisement ID %x",
			   WPA_GET_LE32(data));
		break;
	case P2P_ATTR_ADVERTISED_SERVICE:
		...
		if (len <= 255 + 8) {
			...
			str[namelen] = '\0';
			wpa_warning_log1(MSG_DEBUG, "P2P: * Service Instance: %x",
				   WPA_GET_LE32(data));
			wpa_warning_buf(MSG_DEBUG, "P2P: * Service Instance: %s",
				   str, strlen(str));
		} else {
			wpa_warning_log1(MSG_DEBUG, "P2P: * Service Instance: %p",
				   data);
		}
		break;
	case P2P_ATTR_SESSION_ID:
		...
		wpa_warning_log1(MSG_DEBUG, "P2P: * Session ID: %x ",
			   WPA_GET_LE32(data));
		wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x",
			   (msg->session_mac)[0], (msg->session_mac)[3], (msg->session_mac)[4], (msg->session_mac)[5]);
		break;
	case P2P_ATTR_FEATURE_CAPABILITY:
		...
		wpa_warning_log1(MSG_DEBUG, "P2P: * Feature Cap (length=%u)", len);
		break;
	case P2P_ATTR_PERSISTENT_GROUP:
	{
		...
		wpa_warning_log4(MSG_DEBUG, "P2P: * Persistent Group: " "%02x:xx:xx:%02x:%02x:%02x",
			   (msg->persistent_dev)[0], (msg->persistent_dev)[3], (msg->persistent_dev)[4], (msg->persistent_dev)[5]);
		wpa_warning_buf(MSG_DEBUG, " %s",
			   wpa_ssid_txt(msg->persistent_ssid,
					msg->persistent_ssid_len), strlen(wpa_ssid_txt(msg->persistent_ssid,
					msg->persistent_ssid_len)));
		break;
	}
	default:
		wpa_warning_log2(MSG_DEBUG, "P2P: Skipped unknown attribute %d "
			   "(length %d)", id, len);
		break;
	}
...
int p2p_parse_p2p_ie(const struct wpabuf *buf, struct p2p_message *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Parsing P2P IE");
	...
	while (pos < end) {
		...
		if (end - pos < 3) {
			wpa_warning_log0(MSG_DEBUG, "P2P: Invalid P2P attribute");
			return -1;
		}
		id = *pos++;
		attr_len = WPA_GET_LE16(pos);
		pos += 2;
		wpa_warning_log2(MSG_DEBUG, "P2P: Attribute %d length %u",
			   id, attr_len);
		if (attr_len > end - pos) {
			wpa_warning_log2(MSG_DEBUG, "P2P: Attribute underflow "
				   "(len=%u left=%d)",
...
static int p2p_parse_wps_ie(const struct wpabuf *buf, struct p2p_message *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Parsing WPS IE");
	...
	if (attr.config_methods) {
		...
		wpa_warning_log1(MSG_DEBUG, "P2P: Config Methods (WPS): 0x%x",
			   msg->wps_config_methods);
	}
	if (attr.dev_password_id) {
		msg->dev_password_id = WPA_GET_BE16(attr.dev_password_id);
		wpa_warning_log1(MSG_DEBUG, "P2P: Device Password ID: %d",
			   msg->dev_password_id);
...
#ifndef HISI_CODE_CROP
		wpa_warning_buf(MSG_DEBUG, "P2P: Primary Device Type (WPS): %s",
			   wps_dev_type_bin2str(msg->wps_pri_dev_type, devtype,
						sizeof(devtype)), strlen(wps_dev_type_bin2str(msg->wps_pri_dev_type, devtype,
						sizeof(devtype))));
#endif
	}
...
int p2p_parse_ies(const u8 *data, size_t len, struct p2p_message *msg)
{
	...
	if (msg->p2p_attributes &&
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: Failed to parse P2P IE data");
		if (msg->p2p_attributes)
...
int p2p_parse(const u8 *data, size_t len, struct p2p_message *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: Parsing the received message");
	if (len < 1) {
		wpa_warning_log0(MSG_DEBUG, "P2P: No Dialog Token in the message");
		return -1;
	}
	msg->dialog_token = data[0];
	wpa_warning_log1(MSG_DEBUG, "P2P: * Dialog Token: %d", msg->dialog_token);
