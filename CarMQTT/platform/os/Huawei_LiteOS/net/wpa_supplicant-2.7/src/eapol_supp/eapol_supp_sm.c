...
#else
/*
 * Trimmed version of struct eapol_sm. The main modifications:
 * 1, delete all statistics params.
 * 2, delete all constant timer or count params, include heldPeriod,
 *	startPeriod, authPeriod, and maxStart.
 * 3, change the type of all timers from "unsigned int" to "unsigned char",
 *	include authWhile, heldWhile, startWhen, and idleWhile.
 * 4, change the type of all Boolean variables to "unsigned int:1".
 * 5, change some enum type to "unsigned int:n", include sPortMode,
 *	portControl, suppPortStatus.
 * 7, change timer_tick_enabled from int to "unsigned int:1".
 * 8, change startCount from int to "unsigned int:8".
 * 9, delete unused params, like cached_pmk.
 */
struct eapol_sm {
	/* Timers */
	unsigned char authWhile;
	unsigned char heldWhile;
	unsigned char startWhen;
	unsigned char idleWhile; /* for EAP state machine */

	unsigned int timer_tick_enabled:1;
	unsigned int eapFail:1;
	unsigned int eapolEap:1;
	unsigned int eapSuccess:1;
	unsigned int initialize:1;
	unsigned int keyDone:1;
	unsigned int keyRun:1;
	unsigned int portEnabled:1;
	unsigned int portValid:1;
	unsigned int suppAbort:1;
	unsigned int suppFail:1;
	unsigned int suppStart:1;
	unsigned int suppSuccess:1;
	unsigned int suppTimeout:1;
	unsigned int userLogoff:1;
	unsigned int logoffSent:1;
	unsigned int eapRestart:1;
	unsigned int rxKey:1;
	unsigned int eapNoResp:1;
	unsigned int eapReq:1;
	unsigned int eapResp:1;
	unsigned int changed:1;
	unsigned int initial_req:1;
	unsigned int altAccept:1; /* for EAP */
	unsigned int altReject:1; /* for EAP */
	unsigned int eapTriggerStart:1;
	unsigned int replay_counter_valid:1;
	unsigned int unicast_key_received:1;
	unsigned int broadcast_key_received:1;
	unsigned int force_authorized_update:1;
	unsigned int sPortMode:2;
	unsigned int portControl:2;
	unsigned int suppPortStatus:2;  /* dot1xSuppControlledPortStatus */
	unsigned int startCount:8;
...
static void eapol_port_timers_tick(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (sm->startWhen > 0) {
		sm->startWhen--;
		if (sm->startWhen == 0) {
			wpa_error_log0(MSG_DEBUG, "EAPOL: startWhen --> 0");
		}
	}
	if (sm->idleWhile > 0) {
		sm->idleWhile--;
	...
	if (sm->authWhile | sm->heldWhile | sm->startWhen | sm->idleWhile) {
		(void)eloop_register_timeout(1, 0, eapol_port_timers_tick, eloop_ctx,
				       sm);
	} else {
...
#else
		sm->startWhen = 30; /* default startPeriod */
#endif /* HISI_EAP_TRIM */
		sm->startCount++;
	} else {
		/*
...
#else
	sm->heldWhile = 60; /* default heldPeriod */
#endif /* HISI_EAP_TRIM */
	eapol_enable_timer_tick(sm);
	eapol_sm_set_port_unauthorized(sm);
...
#else
		if (sm->startWhen == 0 && sm->startCount < 3)
#endif /* HISI_EAP_TRIM */
			SM_ENTER(SUPP_PAE, CONNECTING);
...
#else
	sm->authWhile = 30; /* default authPeriod */
#endif /* HISI_EAP_TRIM */
	eapol_enable_timer_tick(sm);
	sm->eapolEap = FALSE;
	sm->eapNoResp = FALSE;
...
static void eapol_sm_txLogoff(struct eapol_sm *sm)
{
	wpa_error_log0(MSG_DEBUG, "EAPOL: txLogoff");
...
static void eapol_sm_txStart(struct eapol_sm *sm)
{
	wpa_error_log0(MSG_DEBUG, "EAPOL: txStart");
	sm->ctx->eapol_send(sm->ctx->eapol_send_ctx,
...
static void eapol_sm_processKey(struct eapol_sm *sm)
{
	...
	struct eap_key_data keydata = {{0}, {0}};
	...
	wpa_error_log0(MSG_DEBUG, "EAPOL: processKey");
...
#ifndef HISI_EAP_TRIM
	wpa_error_log4(MSG_DEBUG, "EAPOL: RX IEEE 802.1X ver=%d type=%d len=%d "
		   "EAPOL-Key: type=%d",
		   hdr->version, hdr->type, be_to_host16(hdr->length),
		   key->type);
	wpa_error_log2(MSG_DEBUG, "key_length=%d key_index=0x%x",
	...
	if (res < 0) {
		wpa_error_log0(MSG_DEBUG, "EAPOL: Could not get master key for "
	...
	if (res == 16) {
		...
		if (res) {
			wpa_error_log0(MSG_DEBUG, "EAPOL: Could not get LEAP "
				   "master key for decrypting EAPOL-Key keys");
			return;
		}
		sign_key_len = 16;
		encr_key_len = 16;
		(void)os_memcpy(keydata.sign_key, keydata.encr_key, 16);
	} else if (res) {
		wpa_error_log1(MSG_DEBUG, "EAPOL: Could not get enough master key "
	...
	(void)os_memcpy(orig_key_sign, key->key_signature, IEEE8021X_KEY_SIGN_LEN);
	(void)os_memset(key->key_signature, 0, IEEE8021X_KEY_SIGN_LEN);
	(void)hmac_md5(keydata.sign_key, sign_key_len,
	...
	if (os_memcmp_const(orig_key_sign, key->key_signature,
		...
		wpa_printf(MSG_DEBUG, "EAPOL: Invalid key signature in "
			   "EAPOL-Key packet");
#endif /* CONFIG_PRINT_NOUSE */
		(void)os_memcpy(key->key_signature, orig_key_sign,
	...
	if (key_len == rx_key_length) {
		...
		wpa_error_log0(MSG_ERROR, "EAPOL: RC4 not supported in the build");
		return;
#else /* CONFIG_NO_RC4 */
		(void)os_memcpy(ekey, key->key_iv, IEEE8021X_KEY_IV_LEN);
		(void)os_memcpy(ekey + IEEE8021X_KEY_IV_LEN, keydata.encr_key,
			  encr_key_len);
		(void)os_memcpy(datakey, key + 1, key_len);
		(void)rc4_skip(ekey, IEEE8021X_KEY_IV_LEN + encr_key_len, 0,
			 datakey, key_len);
#ifndef CONFIG_PRINT_NOUSE
		...
		(void)os_memcpy(datakey, keydata.encr_key, key_len);
#ifndef CONFIG_PRINT_NOUSE
	...
	(void)os_memcpy(sm->last_replay_counter, key->replay_counter,
		  IEEE8021X_REPLAY_COUNTER_LEN);
#ifndef HISI_EAP_TRIM
...
static void eapol_sm_txSuppRsp(struct eapol_sm *sm)
{
	...
	wpa_error_log0(MSG_DEBUG, "EAPOL: txSuppRsp");
	...
	if (sm->use_eap_proxy) {
		...
		if (resp == NULL) {
			wpa_warning_log0(MSG_WARNING, "EAPOL: txSuppRsp - EAP Proxy "
...
void eapol_sm_step(struct eapol_sm *sm)
{
	...
	if (sm->changed) {
		...
		(void)eloop_cancel_timeout(eapol_sm_step_timeout, NULL, sm);
		(void)eloop_register_timeout(0, 0, eapol_sm_step_timeout, NULL, sm);
	}
		      ...
		      size_t len)
{
	...
	if (sm->conf.wps && sm->conf.workaround &&
		...
		if (elen > plen && elen <= len - sizeof(*hdr)) {
			 ...
			 */
			wpa_error_log2(MSG_DEBUG, "EAPOL: Workaround - fix EAPOL "
	...
	case IEEE802_1X_TYPE_EAP_PACKET:
...
#ifndef CONFIG_PRINT_NOUSE
				wpa_warning_log0(MSG_DEBUG, "EAPOL: Ignore EAP packet with unknown code 10");
#endif /* CONFIG_PRINT_NOUSE */
				break;
			}
		}
		...
		if (sm->eapReqData) {
			wpa_error_log0(MSG_DEBUG, "EAPOL: Received EAP-Packet "
				   "frame");
			sm->eapolEap = TRUE;
#ifdef CONFIG_EAP_PROXY
			if (sm->use_eap_proxy) {
				...
				wpa_warning_log0(MSG_DEBUG, "EAPOL: eap_proxy "
					   "EAP Req updated");
			}
#endif /* CONFIG_EAP_PROXY */
			eapol_sm_step(sm);
	...
	case IEEE802_1X_TYPE_EAPOL_KEY:
		if (plen < sizeof(*key)) {
			wpa_error_log0(MSG_DEBUG, "EAPOL: Too short EAPOL-Key "
				   "frame received");
			break;
		}
		...
		if (key->type == EAPOL_KEY_TYPE_WPA ||
			...
			wpa_error_log0(MSG_DEBUG, "EAPOL: Ignoring WPA EAPOL-Key "
				   "frame in EAPOL state machines");
			res = 0;
			break;
		}
		if (key->type != EAPOL_KEY_TYPE_RC4) {
			wpa_error_log1(MSG_DEBUG, "EAPOL: Ignored unknown "
		...
		if (sm->last_rx_key) {
			wpa_error_log0(MSG_DEBUG, "EAPOL: Received EAPOL-Key "
				   "frame");
			(void)os_memcpy(sm->last_rx_key, buf, data_len);
	...
	case IEEE802_1X_TYPE_EAPOL_MKA:
		wpa_msgdump_log1(MSG_EXCESSIVE,
			   "EAPOL type %d will be handled by MKA",
			   hdr->type);
		break;
#endif /* CONFIG_MACSEC */
	default:
		wpa_error_log1(MSG_DEBUG, "EAPOL: Received unknown EAPOL type %d",
			   hdr->type);
#ifndef HISI_CODE_CROP
		sm->dot1xSuppInvalidEapolFramesRx++;
#endif /* HISI_CODE_CROP */
...
int eapol_sm_get_key(struct eapol_sm *sm, u8 *key, size_t len)
{
	...
	if (sm && sm->use_eap_proxy) {
		...
		if (sm == NULL || !eap_proxy_key_available(sm->eap_proxy)) {
			wpa_warning_log0(MSG_DEBUG, "EAPOL: EAP key not available");
			return -1;
		}
		eap_key = eap_proxy_get_eapKeyData(sm->eap_proxy, &eap_len);
		if (eap_key == NULL) {
			wpa_warning_log0(MSG_DEBUG, "EAPOL: Failed to get "
				   "eapKeyData");
			return -1;
		}
		goto key_fetched;
	...
	if (sm == NULL || !eap_key_available(sm->eap)) {
		wpa_error_log0(MSG_DEBUG, "EAPOL: EAP key not available");
		return -1;
	}
	eap_key = eap_get_eapKeyData(sm->eap, &eap_len);
	if (eap_key == NULL) {
		wpa_error_log0(MSG_DEBUG, "EAPOL: Failed to get eapKeyData");
		return -1;
	}
#ifdef CONFIG_EAP_PROXY
key_fetched:
#endif /* CONFIG_EAP_PROXY */
	if (len > eap_len) {
		wpa_error_log2(MSG_DEBUG, "EAPOL: Requested key length (%lu) not "
			   "available (len=%lu)",
			   (unsigned long) len, (unsigned long) eap_len);
		return eap_len;
	}
	(void)os_memcpy(key, eap_key, len);
	wpa_error_log1(MSG_DEBUG, "EAPOL: Successfully fetched key (len=%lu)",
		   (unsigned long) len);
	return 0;
}
...
{
	if (sm == NULL || !eap_key_available(sm->eap)) {
		wpa_error_log0(MSG_DEBUG, "EAPOL: EAP Session-Id not available");
		return NULL;
	}
...
void eapol_sm_notify_cached(struct eapol_sm *sm)
{
	if (sm == NULL)
		return;
	wpa_error_log0(MSG_DEBUG, "EAPOL: PMKSA caching was used - skip EAPOL");
	sm->eapSuccess = TRUE;
	eap_notify_success(sm->eap);
	eapol_sm_step(sm);
}
...
void eapol_sm_notify_pmkid_attempt(struct eapol_sm *sm)
{
	if (sm == NULL)
		return;
	wpa_error_log0(MSG_DEBUG, "RSN: Trying to use cached PMKSA");
#ifndef HISI_CODE_CROP
	sm->cached_pmk = TRUE;
#endif /* HISI_CODE_CROP */
}
...
static void eapol_sm_abort_cached(struct eapol_sm *sm)
{
	wpa_warning_log0(MSG_DEBUG, "RSN: Authenticator did not accept PMKID, "
...
void eapol_sm_notify_portControl(struct eapol_sm *sm, PortControl portControl)
{
	if (sm == NULL)
		return;
#ifndef CONFIG_PRINT_NOUSE
	wpa_warning_buf(MSG_DEBUG, "EAPOL: External notification - "
		   "portControl=%s", eapol_port_control(portControl), strlen(eapol_port_control(portControl)));
#endif /* CONFIG_PRINT_NOUSE */
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log0(MSG_DEBUG, "EAPOL: received control response (user "
...
static void eapol_sm_notify_pending(void *ctx)
{
	...
	if (sm->eapReqData && !sm->eapReq) {
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log0(MSG_DEBUG, "EAPOL: received notification from EAP "
				      ...
				      const char *txt)
{
	struct eapol_sm *sm = ctx;
#ifndef CONFIG_PRINT_NOUSE
	wpa_warning_log0(MSG_DEBUG, "EAPOL: EAP parameter needed");
#endif /* CONFIG_PRINT_NOUSE */
...
struct eapol_sm *eapol_sm_init(struct eapol_ctx *ctx)
{
	...
	if (sm->eap_proxy == NULL) {
		wpa_error_log0(MSG_ERROR, "Unable to initialize EAP Proxy");
	}
#endif /* CONFIG_EAP_PROXY */
