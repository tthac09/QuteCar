				...
				const u8 *buf, size_t len)
{
	...
	(void)eapol_sm_rx_eapol(sm->preauth_eapol, src_addr, buf, len);
}
		     ...
		     struct eap_peer_config *eap_conf)
{
	...
	if (sm->l2_preauth == NULL) {
		wpa_warning_log0(MSG_WARNING, "RSN: Failed to initialize L2 packet "
	...
	if (sm->bridge_ifname) {
		...
		if (sm->l2_preauth_br == NULL) {
			wpa_warning_log0(MSG_WARNING, "RSN: Failed to initialize L2 "
	...
	if (ctx == NULL) {
		wpa_warning_log0(MSG_WARNING, "Failed to allocate EAPOL context.");
		ret = -4;
		goto fail;
	}
	ctx->ctx = sm->ctx->ctx;
	...
	if (sm->preauth_eapol == NULL) {
		os_free(ctx);
		wpa_warning_log0(MSG_WARNING, "RSN: Failed to initialize EAPOL "
	...
	(void)os_memcpy(sm->preauth_bssid, dst, ETH_ALEN);

	eapol_sm_notify_portValid(sm->preauth_eapol, TRUE);
	/* 802.1X::portControl = Auto */
	eapol_sm_notify_portEnabled(sm->preauth_eapol, TRUE);

	(void)eloop_register_timeout(sm->dot11RSNAConfigSATimeout, 0,
...
void rsn_preauth_deinit(struct wpa_sm *sm)
{
	...
	(void)eloop_cancel_timeout(rsn_preauth_timeout, sm, NULL);
	eapol_sm_deinit(sm->preauth_eapol);
	sm->preauth_eapol = NULL;
	(void)os_memset(sm->preauth_bssid, 0, ETH_ALEN);
...
void rsn_preauth_candidate_process(struct wpa_sm *sm)
{
	...
	dl_list_for_each_safe(candidate, n, &sm->pmksa_candidates,
		...
		if (os_memcmp(sm->bssid, candidate->bssid, ETH_ALEN) != 0 &&
			...
			(void)rsn_preauth_init(sm, candidate->bssid,
					 sm->eap_conf_ctx);
			os_free(candidate);
			return;
		}
			 ...
			 int prio, int preauth)
{
	...
	if (!preauth) {
		wpa_warning_log0(MSG_DEBUG, "RSN: Ignored PMKID candidate without "
			   "preauth flag");
		return;
	}
	...
	if (cand) {
		...
		if (cand == NULL)
			return;
		(void)os_memcpy(cand->bssid, bssid, ETH_ALEN);
		cand->priority = prio;
	}
