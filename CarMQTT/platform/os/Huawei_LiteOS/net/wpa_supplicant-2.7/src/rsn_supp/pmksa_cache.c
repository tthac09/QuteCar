...
static void pmksa_cache_set_expiration(struct rsn_pmksa_cache *pmksa)
{
	...
	(void)eloop_cancel_timeout(pmksa_cache_reauth, pmksa, NULL);
	if (pmksa->pmksa == NULL)
		return;
	(void)os_get_reltime(&now);
	...
	if (entry) {
		...
		if (sec < 0)
			sec = 0;
		(void)eloop_register_timeout(sec, 0, pmksa_cache_reauth, pmksa,
				       NULL);
	}
}
#endif
		...
		const u8 *cache_id)
{
	...
	if (entry == NULL)
		return NULL;
	(void)os_memcpy(entry->pmk, pmk, pmk_len);
		...
		if (os_memcmp(entry->aa, pos->aa, ETH_ALEN) == 0) {
			if (pos->pmk_len == entry->pmk_len &&
			    ...
			    os_memcmp_const(pos->pmkid, entry->pmkid,
					    PMKID_LEN) == 0) {
				wpa_warning_log0(MSG_DEBUG, "WPA: reusing previous "
					   "PMKSA entry");
				os_free(entry);
				return pos;
			}
			 ...
			 */
			wpa_warning_log0(MSG_DEBUG, "RSN: Replace PMKSA entry for "
			      ...
			      const u8 *aa, int akmp)
{
	...
	while (entry) {
		if (entry->network_ctx == network_ctx &&
			...
			if (entry) {
				wpa_warning_log0(MSG_DEBUG, "RSN: added "
			    ...
			    int akmp)
{
	...
	wpa_warning_log0(MSG_DEBUG, "RSN: No PMKSA cache entry found");
	return -1;
}
#endif
#ifndef HISI_CODE_CROP
