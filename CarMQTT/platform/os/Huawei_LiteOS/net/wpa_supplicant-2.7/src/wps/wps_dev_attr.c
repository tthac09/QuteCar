...
{
	size_t len;
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Manufacturer");
...
{
	size_t len;
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Model Name");
...
{
	size_t len;
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Model Number");
...
{
	size_t len;
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Serial Number");
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Primary Device Type");
				  ...
				  struct wpabuf *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Secondary Device Type");
...
{
	size_t len;
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Device Name");
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * OS Version");
...
{
	if (str == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Manufacturer received");
		return -1;
	}
...
{
	if (str == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Model Name received");
		return -1;
	}
...
{
	if (str == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Model Number received");
		return -1;
	}
...
{
	if (str == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Serial Number received");
		return -1;
	}
...
{
	if (str == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Device Name received");
		return -1;
	}
					...
					const u8 *dev_type)
{
	...
	if (dev_type == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Primary Device Type received");
		return -1;
	}

	os_memcpy(dev->pri_dev_type, dev_type, WPS_DEV_TYPE_LEN);
	/*wpa_warning_buf(MSG_DEBUG, "WPS: Primary Device Type: %s",
		   wps_dev_type_bin2str(dev->pri_dev_type, devtype,
					sizeof(devtype)), strlen(wps_dev_type_bin2str(dev->pri_dev_type, devtype,
					sizeof(devtype))));*/
...
{
	if (ver == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No OS Version received");
		return -1;
	}

	dev->os_version = WPA_GET_BE32(ver);
	wpa_warning_log1(MSG_DEBUG, "WPS: OS Version %08x", dev->os_version);
...
{
	if (bands == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No RF Bands received");
		return -1;
	}

	dev->rf_bands = *bands;
	wpa_warning_log1(MSG_DEBUG, "WPS: Enrollee RF Bands 0x%x", dev->rf_bands);
