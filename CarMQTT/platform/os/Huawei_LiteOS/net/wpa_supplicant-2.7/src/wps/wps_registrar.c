...
{
	int i;
	wpa_warning_log4(MSG_DEBUG, "WPS: Add authorized MAC " "%02x:xx:xx:%02x:%02x:%02x",
		   addr[0], addr[3], addr[4], addr[5]);
	for (i = 0; i < WPS_MAX_AUTHORIZED_MACS; i++)
		if (os_memcmp(reg->authorized_macs[i], addr, ETH_ALEN) == 0) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Authorized MAC was "
				   "already in the list");
...
{
	int i;
	wpa_warning_log4(MSG_DEBUG, "WPS: Remove authorized MAC " "%02x:xx:xx:%02x:%02x:%02x",
		   addr[0], addr[3], addr[4], addr[5]);
	for (i = 0; i < WPS_MAX_AUTHORIZED_MACS; i++) {
	...
	if (i == WPS_MAX_AUTHORIZED_MACS) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Authorized MAC was not in the "
			   "list");
					     ...
					     const u8 *p2p_dev_addr)
{
	...
	while (pbc) {
		if (os_memcmp(pbc->uuid_e, uuid_e, WPS_UUID_LEN) == 0 ||
			...
			tmp = pbc;
			pbc = pbc->next;
			wpa_warning_log4(MSG_DEBUG, "WPS: Removing PBC session for "
				   "addr=" "%02x:xx:xx:%02x:%02x:%02x",
				   (tmp->addr)[0], (tmp->addr)[3], (tmp->addr)[4], (tmp->addr)[5]);
			      ...
			      const u8 *addr, const u8 *uuid_e)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Checking active PBC sessions for overlap");

	if (uuid_e) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Add one for the requested UUID");
	...
	for (pbc = reg->pbc_sessions; pbc; pbc = pbc->next) {
		wpa_warning_log4(MSG_DEBUG, "WPS: Consider PBC session with " "%02x:xx:xx:%02x:%02x:%02x",
			   (pbc->addr)[0], (pbc->addr)[3], (pbc->addr)[4], (pbc->addr)[5]);
		wpa_hexdump(MSG_DEBUG, "WPS: UUID-E",
			    pbc->uuid_e, WPS_UUID_LEN);
		if (os_reltime_expired(&now, &pbc->timestamp,
				       WPS_PBC_WALK_TIME)) {
			wpa_warning_log0(MSG_DEBUG, "WPS: PBC walk time has expired");
			break;
		}
		if (first &&
		    os_memcmp(pbc->uuid_e, first->uuid_e, WPS_UUID_LEN) == 0) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Same Enrollee");
			continue; /* same Enrollee */
		}
		if (uuid_e == NULL ||
		    os_memcmp(uuid_e, pbc->uuid_e, WPS_UUID_LEN)) {
			wpa_warning_log0(MSG_DEBUG, "WPS: New Enrollee");
			count++;
		}
		if (first == NULL)
			first = pbc;
	...
	wpa_warning_log1(MSG_DEBUG, "WPS: %u active PBC session(s) found", count);
...
{
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Wi-Fi Protected Setup State (%d)",
		   wps->wps_state);
	...
	while (p) {
		if (p->type == WPS_M2 || p->type == WPS_M2D) {
			if (prev == NULL)
				...
				prev->next = p->next;
			wpa_warning_log0(MSG_DEBUG, "WPS UPnP: Drop pending M2/M2D");
			p2 = p;
			p = p->next;
			wpabuf_free(p2->msg);
			os_free(p2);
				     ...
				     struct wpabuf *msg)
{
	if (wps->ap_setup_locked && wps->ap_setup_locked != 2) {
		wpa_warning_log0(MSG_DEBUG, "WPS:  * AP Setup Locked");
					...
					struct wpabuf *msg)
{
	if (!reg->sel_reg_union)
		return 0;
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Selected Registrar");
					     ...
					     struct wpabuf *msg)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Device Password ID (%d)", id);
					    ...
					    struct wpabuf *msg)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Selected Registrar Config Methods (%x)",
		   methods);
					  ...
					  struct wpabuf *msg)
{
	...
	methods &= ~(WPS_CONFIG_VIRT_PUSHBUTTON |
		     WPS_CONFIG_PHY_PUSHBUTTON);
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Config Methods (%x)", methods);
...
void wps_registrar_deinit(struct wps_registrar *reg)
{
	if (reg == NULL)
		return;
	(void)eloop_cancel_timeout(wps_registrar_pbc_timeout, reg, NULL);
	(void)eloop_cancel_timeout(wps_registrar_set_selected_timeout, reg, NULL);
...
static void wps_registrar_invalidate_unused(struct wps_registrar *reg)
{
	...
	dl_list_for_each(pin, &reg->pins, struct wps_uuid_pin, list) {
		if (pin->wildcard_uuid == 1 && !(pin->flags & PIN_LOCKED)) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Invalidate previously "
			  ...
			  int timeout)
{
	...
	if (timeout) {
		p->flags |= PIN_EXPIRES;
		(void)os_get_reltime(&p->expiration);
	...
	wpa_warning_log1(MSG_DEBUG, "WPS: A new PIN configured (timeout=%d)",
		   timeout);
	...
	(void)eloop_cancel_timeout(wps_registrar_set_selected_timeout, reg, NULL);
	(void)eloop_register_timeout(WPS_PBC_WALK_TIME, 0,
					...
					const u8 *uuid, size_t *pin_len)
{
	...
	if (!found) {
		...
		dl_list_for_each(pin, &reg->pins, struct wps_uuid_pin, list) {
			if (pin->wildcard_uuid == 1 ||
			    pin->wildcard_uuid == 2) {
				wpa_warning_log0(MSG_DEBUG, "WPS: Found a wildcard "
	...
	if (found->flags & PIN_LOCKED) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Selected PIN locked - do not "
	...
	dl_list_for_each(pin, &reg->pins, struct wps_uuid_pin, list) {
		if (os_memcmp(pin->uuid, uuid, WPS_UUID_LEN) == 0) {
			if (pin->wildcard_uuid == 3) {
				wpa_warning_log0(MSG_DEBUG, "WPS: Invalidating used "
					   "wildcard PIN");
...
static void wps_registrar_pbc_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	(void)timeout_ctx;
	wpa_warning_log0(MSG_DEBUG, "WPS: PBC timed out - disable PBC mode");
				...
				const u8 *p2p_dev_addr)
{
	if (p2p_dev_addr == NULL &&
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: PBC overlap - do not start PBC "
			   "mode");
		wps_pbc_overlap_event(reg->wps);
		return -2;
	}
	wpa_warning_log0(MSG_DEBUG, "WPS: Button pushed - PBC mode started");
	...
	(void)eloop_cancel_timeout(wps_registrar_set_selected_timeout, reg, NULL);
	(void)eloop_cancel_timeout(wps_registrar_pbc_timeout, reg, NULL);
	(void)eloop_register_timeout(WPS_PBC_WALK_TIME, 0, wps_registrar_pbc_timeout,
			       reg, NULL);
	return 0;
}
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS: PBC completed - stopping PBC mode");
	(void)eloop_cancel_timeout(wps_registrar_pbc_timeout, reg, NULL);
	wps_registrar_stop_pbc(reg);
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS: PIN completed using internal Registrar");
	(void)eloop_cancel_timeout(wps_registrar_set_selected_timeout, reg, NULL);
	reg->selected_registrar = 0;
	wps_registrar_selected_registrar_changed(reg, 0);
}
...
#ifdef WPS_WORKAROUNDS
		(void)os_get_reltime(&registrar->pbc_ignore_start);
#endif /* WPS_WORKAROUNDS */
...
{
	if (reg->pbc) {
		wpa_warning_log0(MSG_DEBUG, "WPS: PBC is set - cancelling it");
		wps_registrar_pbc_timeout(reg, NULL);
		(void)eloop_cancel_timeout(wps_registrar_pbc_timeout, reg, NULL);
		return 1;
	} else if (reg->selected_registrar) {
		/* PIN Method */
		wpa_warning_log0(MSG_DEBUG, "WPS: PIN is set - cancelling it");
		wps_registrar_pin_completed(reg);
		(void)wps_registrar_invalidate_wildcard_pin(reg, NULL, 0);
		return 1;
	}
	return 0;
}
				...
				int p2p_wildcard)
{
	...
	if (attr.config_methods == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Config Methods attribute in "
			   "Probe Request");
		return;
	}

	if (attr.dev_password_id == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Device Password Id attribute "
			   "in Probe Request");
		return;
	}
	...
	wpa_warning_log4(MSG_DEBUG, "WPS: Probe Request for PBC received from "
		   "%02x:xx:xx:%02x:%02x:%02x", addr[0], addr[3], addr[4], addr[5]);
	if (attr.uuid_e == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Invalid Probe Request WPS IE: No "
			   "UUID-E included");
		return;
	}
	...
	if (reg->pbc_ignore_start.sec &&
		...
		if (dur.sec >= 0 && dur.sec < 5) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Ignore PBC activation "
	...
	if (wps_registrar_pbc_overlap(reg, addr, attr.uuid_e)) {
		wpa_warning_log0(MSG_DEBUG, "WPS: PBC session overlap detected");
...
static void wps_cb_set_sel_reg(struct wps_registrar *reg)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WPS: wps_cb_set_sel_reg: sel_reg=%d "
...
static int wps_set_ie(struct wps_registrar *reg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Build Beacon IEs");
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Build Probe Response IEs");
	...
	if (reg->static_wep_only) {
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Add Microsoft Provisioning IE "
...
static int wps_get_dev_password(struct wps_data *wps)
{
	...
	if (wps->pbc) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Use default PIN for PBC");
		...
		{
			wpa_warning_log0(MSG_DEBUG, "WPS: Using NFC connection "
				   "handover and abbreviated WPS handshake "
				   "without Device Password");
			return 0;
		}
		wpa_warning_log0(MSG_DEBUG, "WPS: Use OOB Device Password from NFC "
			   "Password Token");
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Use OOB Device Password from own NFC Password Token");
		...
		if (pin && wps->dev_pw_id >= 0x10) {
			wpa_warning_log0(MSG_DEBUG, "WPS: No match for OOB Device "
	...
	if (pin == NULL) {
		wpa_warning_log2(MSG_DEBUG, "WPS: No Device Password available for "
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * UUID-R");
...
static int wps_build_r_hash(struct wps_data *wps, struct wpabuf *msg)
{
	...
	if (wps->dh_pubkey_e == NULL || wps->dh_pubkey_r == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: DH public keys not available for "
			   "R-Hash derivation");
		return -1;
	}

	wpa_warning_log0(MSG_DEBUG, "WPS:  * R-Hash1");
	...
	(void)hmac_sha256_vector(wps->authkey, WPS_AUTHKEY_LEN, 4, addr, len, hash);
	wpa_hexdump(MSG_DEBUG, "WPS: R-Hash1", hash, SHA256_MAC_LEN);

	wpa_warning_log0(MSG_DEBUG, "WPS:  * R-Hash2");
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * R-SNonce1");
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * R-SNonce2");
...
{
	(void)cred;
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Network Index (1)");
...
static int wps_build_cred_ssid(struct wpabuf *msg,
			       const struct wps_credential *cred)
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * SSID");
...
static int wps_build_cred_encr_type(struct wpabuf *msg,
				    const struct wps_credential *cred)
{
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Encryption Type (0x%x)",
		   cred->encr_type);
...
static int wps_build_cred_network_key(struct wpabuf *msg,
				      const struct wps_credential *cred)
{
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Network Key (len=%d)",
		   (int) cred->key_len);
...
int wps_build_cred(struct wps_data *wps, struct wpabuf *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Credential");
	if (wps->use_cred) {
	...
	if (wps->auth_type & WPS_AUTH_WPA2PSK)
		...
		wpa_warning_log1(MSG_DEBUG, "WPS: Unsupported auth_type 0x%x",
			   wps->auth_type);
		return -1;
	}
	...
	if (wps->auth_type == WPS_AUTH_WPA2PSK ||
		...
		if (wps->encr_type & WPS_ENCR_AES)
			...
			wpa_warning_log0(MSG_DEBUG, "WPS: No suitable encryption "
				   "type for WPA/WPA2");
			return -1;
		}
	} else {
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->wps->ap &&
		...
		char hex[65];
		wpa_warning_log0(MSG_DEBUG, "WPS: Use PSK format for Network Key");
		(void)wpa_snprintf_hex(hex, sizeof(hex), wps->wps->psk, 32);
...
#else
		random_get_bytes(wps->new_psk, wps->new_psk_len);
#endif /* CONFIG_NO_RANDOM_POOL */
		wpa_hexdump_key(MSG_DEBUG, "WPS: Generated per-device PSK",
				wps->new_psk, wps->new_psk_len);
		(void)wpa_snprintf_hex(hex, sizeof(hex), wps->new_psk,
				 wps->new_psk_len);
...
use_provided:
	...
	if (cred) {
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Add dummy credential");
...
skip_cred_build:
	if (wps->wps->registrar->extra_cred) {
		wpa_warning_log0(MSG_DEBUG, "WPS:  * Credential (pre-configured)");
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * AP Settings");
...
static struct wpabuf * wps_build_m2(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message M2");
	msg = wpabuf_alloc(1000);
	if (msg == NULL)
		return NULL;
...
static struct wpabuf * wps_build_m2d(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message M2D");
	msg = wpabuf_alloc(1000);
	if (msg == NULL)
		return NULL;
...
static struct wpabuf * wps_build_m4(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message M4");
...
static struct wpabuf * wps_build_m6(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message M6");
...
static struct wpabuf * wps_build_m8(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message M8");
				      ...
				      enum wsc_op_code *op_code)
{
	...
	if (!wps->int_reg && wps->wps->wps_upnp) {
		...
		if (p) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Use pending message from "
				   "UPnP");
			if (prev)
				prev->next = NULL;
			else
	...
	if (wps->ext_reg) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Using external Registrar, but no "
	...
	case SEND_WSC_NACK:
		...
		*op_code = WSC_NACK;
		break;
	default:
		wpa_warning_log1(MSG_DEBUG, "WPS: Unsupported state %d for building "
			   "a message", wps->state);
		msg = NULL;
		break;
	}
...
{
	if (e_nonce == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Enrollee Nonce received");
		return -1;
	}
...
{
	if (r_nonce == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Registrar Nonce received");
		return -1;
	}

	if (os_memcmp(wps->nonce_r, r_nonce, WPS_NONCE_LEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Invalid Registrar Nonce received");
		return -1;
	}
...
{
	if (uuid_e == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No UUID-E received");
		return -1;
	}
...
{
	if (pw_id == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Device Password ID received");
		return -1;
	}

	wps->dev_pw_id = WPA_GET_BE16(pw_id);
	wpa_warning_log1(MSG_DEBUG, "WPS: Device Password ID %d", wps->dev_pw_id);
...
{
	if (e_hash1 == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No E-Hash1 received");
		return -1;
	}
...
{
	if (e_hash2 == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No E-Hash2 received");
		return -1;
	}
...
static int wps_process_e_snonce1(struct wps_data *wps, const u8 *e_snonce1)
{
	...
	if (e_snonce1 == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No E-SNonce1 received");
		return -1;
	}
	...
	if (os_memcmp_const(wps->peer_hash1, hash, WPS_HASH_LEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: E-Hash1 derived from E-S1 does "
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Enrollee proved knowledge of the first "
...
static int wps_process_e_snonce2(struct wps_data *wps, const u8 *e_snonce2)
{
	...
	if (e_snonce2 == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No E-SNonce2 received");
		return -1;
	}
	...
	if (os_memcmp_const(wps->peer_hash2, hash, WPS_HASH_LEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: E-Hash2 derived from E-S2 does "
			   "not match with the pre-committed value");
		(void)wps_registrar_invalidate_pin(wps->wps->registrar, wps->uuid_e);
		wps->config_error = WPS_CFG_DEV_PASSWORD_AUTH_FAILURE;
		wps_pwd_auth_fail_event(wps->wps, 0, 2, wps->mac_addr_e);
		return -1;
	}

	wpa_warning_log0(MSG_DEBUG, "WPS: Enrollee proved knowledge of the second "
		   "half of the device password");
	wps->wps_pin_revealed = 0;
	(void)wps_registrar_unlock_pin(wps->wps->registrar, wps->uuid_e);
...
{
	if (mac_addr == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No MAC Address received");
		return -1;
	}

	wpa_warning_log4(MSG_DEBUG, "WPS: Enrollee MAC Address " "%02x:xx:xx:%02x:%02x:%02x",
...
{
	if (pk == NULL || pk_len == 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Public Key received");
		return -1;
	}
...
static int wps_process_auth_type_flags(struct wps_data *wps, const u8 *auth)
{
	...
	if (auth == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Authentication Type flags "
			   "received");
		return -1;
	}
	...
	wpa_warning_log1(MSG_DEBUG, "WPS: Enrollee Authentication Type flags 0x%x",
		   auth_types);
#ifdef WPS_WORKAROUNDS
	/*
	...
	if (wps->auth_type == 0) {
		wpa_warning_log2(MSG_DEBUG, "WPS: No match in supported "
		 ...
		 */
		wpa_warning_log0(MSG_DEBUG, "WPS: Workaround - assume Enrollee "
...
static int wps_process_encr_type_flags(struct wps_data *wps, const u8 *encr)
{
	...
	if (encr == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Encryption Type flags "
			   "received");
		return -1;
	}
	...
	wpa_warning_log1(MSG_DEBUG, "WPS: Enrollee Encryption Type flags 0x%x",
		   encr_types);
...
static int wps_process_conn_type_flags(struct wps_data *wps, const u8 *conn)
{
	(void)wps;
	if (conn == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Connection Type flags "
			   "received");
		return -1;
	}

	wpa_warning_log1(MSG_DEBUG, "WPS: Enrollee Connection Type flags 0x%x",
		   *conn);
...
static int wps_process_config_methods(struct wps_data *wps, const u8 *methods)
{
	...
	if (methods == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Config Methods received");
		return -1;
	}

	m = WPA_GET_BE16(methods);

	wpa_warning_log1(MSG_DEBUG, "WPS: Enrollee Config Methods 0x%x", m);
	wpa_warning_two_buf(MSG_DEBUG,
		   "%s%s",
		   m & WPS_CONFIG_USBA ? " [USBA]" : "", m & WPS_CONFIG_USBA ? strlen(" [USBA]") : strlen(""),
		   m & WPS_CONFIG_ETHERNET ? " [Ethernet]" : "", m & WPS_CONFIG_ETHERNET ? strlen(" [Ethernet]") : strlen(""));
	wpa_warning_two_buf(MSG_DEBUG,
		   "%s%s",
		   m & WPS_CONFIG_LABEL ? " [Label]" : "", m & WPS_CONFIG_LABEL ? strlen(" [Label]") : strlen(""),
		   m & WPS_CONFIG_DISPLAY ? " [Display]" : "", m & WPS_CONFIG_DISPLAY ? strlen(" [Display]") : strlen(""));
	wpa_warning_two_buf(MSG_DEBUG,
		   "%s%s",
		   m & WPS_CONFIG_EXT_NFC_TOKEN ? " [Ext NFC Token]" : "", m & WPS_CONFIG_EXT_NFC_TOKEN ? strlen(" [Ext NFC Token]") : strlen(""),
		   m & WPS_CONFIG_INT_NFC_TOKEN ? " [Int NFC Token]" : "",m & WPS_CONFIG_INT_NFC_TOKEN ? strlen(" [Int NFC Token]") : strlen(""));
	wpa_warning_two_buf(MSG_DEBUG,
		   "%s%s",
		   m & WPS_CONFIG_NFC_INTERFACE ? " [NFC]" : "", m & WPS_CONFIG_NFC_INTERFACE ? strlen(" [NFC]") : strlen(""),
		   m & WPS_CONFIG_PUSHBUTTON ? " [PBC]" : "", m & WPS_CONFIG_PUSHBUTTON ? strlen(" [PBC]") : strlen(""));
	wpa_warning_buf(MSG_DEBUG,
		   "%s",
		   m & WPS_CONFIG_KEYPAD ? " [Keypad]" : "", m & WPS_CONFIG_KEYPAD ? strlen(" [Keypad]") : strlen(""));
	if (!(m & WPS_CONFIG_DISPLAY) && !wps->use_psk_key) {
		 ...
		 */
		wpa_warning_log0(MSG_DEBUG, "WPS: Prefer PSK format key due to "
...
static int wps_process_wps_state(struct wps_data *wps, const u8 *state)
{
	(void)wps;
	if (state == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Wi-Fi Protected Setup State "
			   "received");
		return -1;
	}

	wpa_warning_log1(MSG_DEBUG, "WPS: Enrollee Wi-Fi Protected Setup State %d",
		   *state);
...
static int wps_process_assoc_state(struct wps_data *wps, const u8 *assoc)
{
	...
	(void)a;
	if (assoc == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Association State received");
		return -1;
	}

	a = WPA_GET_BE16(assoc);
	wpa_warning_log1(MSG_DEBUG, "WPS: Enrollee Association State %d", a);
...
static int wps_process_config_error(struct wps_data *wps, const u8 *err)
{
	...
	(void)e;
	if (err == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Configuration Error received");
		return -1;
	}

	e = WPA_GET_BE16(err);
	wpa_warning_log1(MSG_DEBUG, "WPS: Enrollee Configuration Error %d", e);
...
static int wps_registrar_p2p_dev_addr_match(struct wps_data *wps)
{
	...
	if (os_memcmp(reg->p2p_dev_addr, wps->p2p_dev_addr, ETH_ALEN) != 0) {
		wpa_warning_log4(MSG_DEBUG, "WPS: No match on P2P Device Address "
			   "filtering for PBC: expected " "%02x:xx:xx:%02x:%02x:%02x",
			   (reg->p2p_dev_addr)[0], (reg->p2p_dev_addr)[3], (reg->p2p_dev_addr)[4], (reg->p2p_dev_addr)[5]);
		wpa_warning_log4(MSG_DEBUG, " was "
			   "%02x:xx:xx:%02x:%02x:%02x" " - indicate PBC session overlap",
			   (wps->p2p_dev_addr)[0], (wps->p2p_dev_addr)[3], (wps->p2p_dev_addr)[4], (wps->p2p_dev_addr)[5]);
		return 0;
	}
#else
	(void)wps;
...
static int wps_registrar_skip_overlap(struct wps_data *wps)
{
	...
	if (os_memcmp(reg->p2p_dev_addr, wps->p2p_dev_addr, ETH_ALEN) == 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Skip PBC overlap due to selected "
			   "Enrollee match");
		return 1;
	}
#else
...
static enum wps_process_res wps_process_m1(struct wps_data *wps,
					   struct wps_parse_attr *attr)
{
	wpa_warning_log0(MSG_DEBUG, "WPS: Received M1");

	if (wps->state != RECV_M1) {
		wpa_warning_log1(MSG_DEBUG, "WPS: Unexpected state (%d) for "
	...
	if (wps->dev_pw_id < 0x10 &&
		...
		wpa_warning_log1(MSG_DEBUG, "WPS: Unsupported Device Password ID %d",
			   wps->dev_pw_id);
		wps->state = SEND_M2D;
		return WPS_CONTINUE;
	}
	...
	if (wps->dev_pw_id >= 0x10 ||
		...
		if (token && token->peer_pk_hash_known) {
			...
			wpa_warning_log0(MSG_DEBUG, "WPS: Found matching NFC "
				   "Password Token");
			dl_list_del(&token->list);
			...
			if (os_memcmp_const(hash,
				...
				wpa_error_log0(MSG_ERROR, "WPS: Public Key hash "
					   "mismatch");
				wps->state = SEND_M2D;
				wps->config_error =
			...
			wps->nfc_pw_token = token;
			   ...
			   wps->wps->ap_nfc_dev_pw_id == wps->dev_pw_id &&
			   wps->wps->ap_nfc_dev_pw) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Found match with own NFC Password Token");
		}
	}
#endif /* CONFIG_WPS_NFC */
	...
	if (wps->dev_pw_id == DEV_PW_PUSHBUTTON) {
		if ((wps->wps->registrar->force_pbc_overlap ||
			...
			wpa_warning_log0(MSG_DEBUG, "WPS: PBC overlap - deny PBC "
				   "negotiation");
			wps->state = SEND_M2D;
	...
	if (!wps->use_psk_key &&
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Workaround - Force Network Key in "
			   "PSK format");
		wps->use_psk_key = 1;
	}
#endif /* WPS_WORKAROUNDS */
...
static enum wps_process_res wps_process_m3(struct wps_data *wps,
					   ...
					   struct wps_parse_attr *attr)
{
	wpa_warning_log0(MSG_DEBUG, "WPS: Received M3");
	...
	if (wps->pbc && wps->wps->registrar->force_pbc_overlap &&
	    !wps_registrar_skip_overlap(wps)) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Reject negotiation due to PBC "
			   "session overlap");
		wps->state = SEND_WSC_NACK;
		wps->config_error = WPS_CFG_MULTIPLE_PBC_DETECTED;
		return WPS_CONTINUE;
					   ...
					   struct wps_parse_attr *attr)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Received M5");
	...
	if (decrypted == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Failed to decrypted Encrypted "
			   "Settings attribute");
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Processing decrypted Encrypted Settings "
		   "attribute");
...
static void wps_sta_cred_cb(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Update local configuration based on the "
		   "AP configuration");
	if (wps->wps->cred_cb)
		(void)wps->wps->cred_cb(wps->wps->cb_ctx, &wps->cred);
}
				     ...
				     struct wps_parse_attr *attr)
{
	...
	wpa_warning_log0(MSG_INFO, "WPS: Received old AP configuration from AP");

	if (wps->new_ap_settings) {
		wpa_warning_log0(MSG_INFO, "WPS: Update AP configuration based on "
			   "new settings");
					   ...
					   struct wps_parse_attr *attr)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Received M7");
	...
	decrypted = wps_decrypt_encr_settings(wps, attr->encr_settings,
					      attr->encr_settings_len);
	if (decrypted == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Failed to decrypt Encrypted "
			   "Settings attribute");
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}
						...
						const struct wpabuf *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Received WSC_MSG");

	if (wps_parse_msg(msg, &attr) < 0)
		return WPS_FAILURE;

	if (attr.msg_type == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Message Type attribute");
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}
	...
	if (*attr.msg_type != WPS_M1 &&
	    (attr.registrar_nonce == NULL ||
	     os_memcmp(wps->nonce_r, attr.registrar_nonce,
		       WPS_NONCE_LEN) != 0)) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Mismatch in registrar nonce");
		return WPS_FAILURE;
	}
	...
	case WPS_M7:
		...
		break;
	default:
		wpa_warning_log1(MSG_DEBUG, "WPS: Unsupported Message Type %d",
			   *attr.msg_type);
		return WPS_FAILURE;
	}
						...
						const struct wpabuf *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Received WSC_ACK");
	...
	if (*attr.msg_type != WPS_WSC_ACK) {
		wpa_warning_log1(MSG_DEBUG, "WPS: Invalid Message Type %d",
			   *attr.msg_type);
		return WPS_FAILURE;
	}
	...
	if (wps->wps->wps_upnp && wps->ext_reg && wps->state == RECV_M2D_ACK &&
		...
		if (wps->wps->upnp_msgs)
			return WPS_CONTINUE;
		wpa_warning_log0(MSG_DEBUG, "WPS: Wait for response from an "
			   "external Registrar");
		return WPS_PENDING;
	}
#endif /* CONFIG_WPS_UPNP */
	...
	if (attr.enrollee_nonce == NULL ||
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Mismatch in enrollee nonce");
		return WPS_FAILURE;
	}
	...
	if (wps->state == RECV_M2D_ACK) {
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: No more registrars available - "
						 ...
						 const struct wpabuf *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Received WSC_NACK");
	...
	if (wps->wps->wps_upnp && wps->ext_reg) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Negotiation using external "
	...
	if (attr.config_error == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Configuration Error attribute "
			   "in WSC_NACK");
		return WPS_FAILURE;
	}

	config_error = WPA_GET_BE16(attr.config_error);
	wpa_warning_log1(MSG_DEBUG, "WPS: Enrollee terminated negotiation with "
						 ...
						 const struct wpabuf *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Received WSC_Done");
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Negotiation completed successfully");
	(void)wps_device_store(wps->wps->registrar, &wps->peer_dev,
			 wps->uuid_e);
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->new_psk &&
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Moving to Configured state based "
		...
		wpa_hexdump_ascii_key(MSG_DEBUG,
				      ...
				      wps->new_psk, wps->new_psk_len);
		if (wps->wps->cred_cb)
			(void)wps->wps->cred_cb(wps->wps->cb_ctx, &cred);
	...
	if (wps->new_psk) {
		if (wps_cb_new_psk(wps->wps->registrar, wps->mac_addr_e,
				   ...
				   wps->new_psk_len)) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Failed to configure the "
				   "new PSK");
		}
		os_free(wps->new_psk);
		wps->new_psk = NULL;
...
#ifdef WPS_WORKAROUNDS
		(void)os_get_reltime(&wps->wps->registrar->pbc_ignore_start);
#endif /* WPS_WORKAROUNDS */
					       ...
					       const struct wpabuf *msg)
{
	...
	wpa_warning_log2(MSG_DEBUG, "WPS: Processing received message (len=%lu "
		   "op_code=%d)",
	    ...
	    (op_code == WSC_MSG || op_code == WSC_Done || op_code == WSC_NACK))
	{
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Skip internal processing - using "
			   "external Registrar");
		return WPS_CONTINUE;
	}
#endif /* CONFIG_WPS_UPNP */
	...
	case WSC_Done:
		...
		return ret;
	default:
		wpa_warning_log1(MSG_DEBUG, "WPS: Unsupported op_code %d", op_code);
		return WPS_FAILURE;
	}
}
					       ...
					       void *timeout_ctx)
{
	struct wps_registrar *reg = eloop_ctx;
	(void)timeout_ctx;
	wpa_warning_log0(MSG_DEBUG, "WPS: Selected Registrar timeout - "
...
static void wps_registrar_sel_reg_union(struct wps_registrar *reg)
{
	...
	dl_list_for_each(s, &reg->wps->wps_upnp->subscriptions,
		...
		if (s->selected_registrar)
			...
			wpa_warning_log0(MSG_DEBUG, "WPS: External Registrar not "
				   "selected");
		}
	}
#else
...
void wps_registrar_selected_registrar_changed(struct wps_registrar *reg,
					      u16 dev_pw_id)
{
	wpa_warning_log0(MSG_DEBUG, "WPS: Selected registrar information changed");
	...
	if (reg->selected_registrar) {
		...
		wpa_warning_log1(MSG_DEBUG, "WPS: Internal Registrar selected "
			   "(pbc=%d)", reg->pbc);
		reg->sel_reg_config_methods_override = methods;
	} else {
		wpa_warning_log0(MSG_DEBUG, "WPS: Internal Registrar not selected");
	}
...
int wps_registrar_config_ap(struct wps_registrar *reg,
			    struct wps_credential *cred)
{
	wpa_warning_log1(MSG_DEBUG, "WPS: encr_type=0x%x", cred->encr_type);
	if (!(cred->encr_type & (WPS_ENCR_NONE | WPS_ENCR_TKIP |
				 WPS_ENCR_AES))) {
		if (cred->encr_type & WPS_ENCR_WEP) {
			wpa_warning_log0(MSG_INFO, "WPS: Reject new AP settings "
				   "due to WEP configuration");
			return -1;
		}

		wpa_warning_log1(MSG_INFO, "WPS: Reject new AP settings due to "
	...
	if ((cred->encr_type & (WPS_ENCR_TKIP | WPS_ENCR_AES)) ==
	    WPS_ENCR_TKIP) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Upgrade encr_type TKIP -> "
			   "TKIP+AES");
		cred->encr_type |= WPS_ENCR_AES;
	}
	...
	if ((cred->auth_type & (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK)) ==
	    WPS_AUTH_WPAPSK) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Upgrade auth_type WPAPSK -> "
			   "WPAPSK+WPA2PSK");
		cred->auth_type |= WPS_AUTH_WPA2PSK;
	}
				   ...
				   int pk_hash_provided_oob)
{
	...
	if (pw_id == DEV_PW_NFC_CONNECTION_HANDOVER &&
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Unexpected NFC Password Token "
	...
	wpa_warning_log1(MSG_DEBUG, "WPS: Added NFC Device Password %u to Registrar",
		   pw_id);
					 ...
					 size_t oob_dev_pw_len)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WPS: Add NFC Password Token for Password ID %u",
		   id);
