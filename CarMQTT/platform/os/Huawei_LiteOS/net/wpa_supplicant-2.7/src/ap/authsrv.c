...
static int hostapd_setup_radius_srv(struct hostapd_data *hapd)
{
	...
	if (hapd->radius_srv == NULL) {
		wpa_error_log0(MSG_ERROR, "RADIUS server initialization failed.");
		return -1;
	}
			      ...
			      union tls_event_data *data)
{
	...
	case TLS_CERT_CHAIN_SUCCESS:
		wpa_warning_log0(MSG_DEBUG, "authsrv: remote certificate verification success");
		break;
...
int authsrv_init(struct hostapd_data *hapd)
{
	...
	if (hapd->conf->eap_server &&
		...
		if (hapd->ssl_ctx == NULL) {
			wpa_error_log0(MSG_ERROR, "Failed to initialize TLS");
			authsrv_deinit(hapd);
			return -1;
		}
		...
		if (tls_global_set_params(hapd->ssl_ctx, &params)) {
			wpa_error_log0(MSG_ERROR, "Failed to set TLS parameters");
			authsrv_deinit(hapd);
			return -1;
		}
		...
		if (tls_global_set_verify(hapd->ssl_ctx,
					  hapd->conf->check_crl)) {
			wpa_error_log0(MSG_ERROR, "Failed to enable check_crl");
			authsrv_deinit(hapd);
			return -1;
		}
	}
	...
	if (hapd->conf->eap_sim_db) {
		...
		if (hapd->eap_sim_db_priv == NULL) {
			wpa_error_log0(MSG_ERROR, "Failed to initialize EAP-SIM "
				   "database interface");
			authsrv_deinit(hapd);
			return -1;
		}
