				      ...
				      struct sta_info *sta)
{
	...
	wpa_warning_log4(MSG_DEBUG, "hostapd_notify_assoc_fils_finish FILS: Finish association with " "%02x:xx:xx:%02x:%02x:%02x",
	...
	if (!elems.fils_session) {
		wpa_warning_log0(MSG_DEBUG, "hostapd_notify_assoc_fils_finish failed to find FILS Session element");
		return;
	}
			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
	...
	(void)ieee802_11_parse_elems(req_ies, req_ies_len, &elems, 0);
	if (elems.wps_ie) {
		ie = elems.wps_ie - 2;
		ielen = elems.wps_ie_len + 2;
		wpa_warning_log0(MSG_DEBUG, "STA included WPS IE in (Re)AssocReq");
	} else if (elems.rsn_ie) {
		ie = elems.rsn_ie - 2;
		ielen = elems.rsn_ie_len + 2;
		wpa_warning_log0(MSG_DEBUG, "STA included RSN IE in (Re)AssocReq");
	} else if (elems.wpa_ie) {
		ie = elems.wpa_ie - 2;
		ielen = elems.wpa_ie_len + 2;
		wpa_warning_log0(MSG_DEBUG, "STA included WPA IE in (Re)AssocReq");
#ifndef HISI_CODE_CROP
#ifdef CONFIG_HS20
	} else if (elems.osen) {
		ie = elems.osen - 2;
		ielen = elems.osen_len + 2;
		wpa_warning_log0(MSG_DEBUG, "STA included OSEN IE in (Re)AssocReq");
#endif /* CONFIG_HS20 */
#endif
	} else {
		ie = NULL;
	...
	if (sta) {
		...
		if (sta == NULL) {
#ifndef HISI_CODE_CROP
			(void)hostapd_drv_sta_disassoc(hapd, addr,
	...
	if (hapd->iface->drv_max_acl_mac_addrs == 0 &&
		...
		wpa_warning_log4(MSG_INFO, "STA " "%02x:xx:xx:%02x:%02x:%02x" " not allowed to connect",
	...
	if (hapd->conf->wpa) {
		if (ie == NULL || ielen == 0) {
			...
			wpa_warning_log0(MSG_DEBUG, "No WPA/RSN IE from STA");
...
skip_wpa_check:
	...
	if (!p) {
		wpa_warning_log0(MSG_DEBUG, "FT: Failed to write AssocResp IEs");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
#endif /* CONFIG_IEEE80211R_AP */
	...
	if (sta->auth_alg == WLAN_AUTH_FILS_SK ||
		...
		if (sta) {
			wpa_warning_log0(MSG_DEBUG, "FILS: HLP callback cleanup");
			eloop_cancel_timeout(fils_hlp_timeout, hapd, sta);
			os_free(sta->fils_pending_assoc_req);
			sta->fils_pending_assoc_req = NULL;
			sta->fils_pending_assoc_req_len = 0;
	...
	if (reassoc && (sta->auth_alg == WLAN_AUTH_FT))
		...
		(void)wpa_auth_sm_event(sta->wpa_sm, WPA_ASSOC);
...
unsigned int hostapd_notif_disassoc(struct hostapd_data *hapd, const u8 *addr)
{
	struct sta_info *sta;
	...
	sta->flags &= ~(WLAN_STA_AUTH | WLAN_STA_ASSOC);
	(void)wpa_auth_sm_event(sta->wpa_sm, WPA_DISASSOC);
...
void hostapd_event_sta_low_ack(struct hostapd_data *hapd, const u8 *addr)
{
	...
	(void)hostapd_drv_sta_disassoc(hapd, addr, WLAN_REASON_DISASSOC_LOW_ACK);
			     ...
			     int offset, int width, int cf1, int cf2)
{
	...
	case HOSTAPD_MODE_IEEE80211A:
		...
		break;
	default:
		(void)ieee80211_freq_to_chan(cf1, &seg0_idx);
		(void)ieee80211_freq_to_chan(cf2, &seg1_idx);
		break;
	}
				  ...
				  struct acs_selected_channels *acs_res)
{
	...
	if (hapd->iconf->channel) {
		wpa_warning_log1(MSG_INFO, "ACS: Channel was already set to %d",
			   hapd->iconf->channel);
		return;
	}
	...
	if (acs_res->sec_channel == 0)
		...
		wpa_error_log0(MSG_ERROR, "Invalid secondary channel!");
		err = 1;
		goto out;
	}
...
fail:
	(void)hostapd_sta_auth(hapd, rx_auth->peer, rx_auth->auth_transaction + 1,
			      ...
			      struct rx_mgmt *drv_mgmt)
{
	...
	if (sta == NULL) {
		wpa_printf(MSG_DEBUG, "%s: station not found", __FUNCTION__);
		return;
	}
#ifdef CONFIG_IEEE80211R_AP
					      ...
					      struct survey_results *survey_res)
{
	...
	if (survey->channel_time > iface->last_channel_time &&
		...
		wpa_warning_log1(MSG_DEBUG, "Channel Utilization: %d",
			      ...
			      struct survey_results *survey_results)
{
	...
	if (dl_list_empty(&survey_results->survey_list)) {
		wpa_warning_log0(MSG_DEBUG, "No survey data received");
		return;
	}
...
{
	wpa_warning_log1(MSG_DEBUG, "DFS radar detected on %d MHz", radar->freq);
...
static void hostapd_event_dfs_pre_cac_expired(struct hostapd_data *hapd,
					      struct dfs_event *radar)
{
	wpa_warning_log1(MSG_DEBUG, "DFS Pre-CAC expired on %d MHz", radar->freq);
...
static void hostapd_event_dfs_cac_finished(struct hostapd_data *hapd,
					   struct dfs_event *radar)
{
	wpa_warning_log1(MSG_DEBUG, "DFS CAC finished on %d MHz", radar->freq);
	(void)hostapd_dfs_complete_cac(hapd->iface, 1, radar->freq, radar->ht_enabled,
				 radar->chan_offset, radar->chan_width,
				 radar->cf1, radar->cf2);
}
...
static void hostapd_event_dfs_cac_aborted(struct hostapd_data *hapd,
					  struct dfs_event *radar)
{
	wpa_warning_log1(MSG_DEBUG, "DFS CAC aborted on %d MHz", radar->freq);
	(void)hostapd_dfs_complete_cac(hapd->iface, 0, radar->freq, radar->ht_enabled,
				 radar->chan_offset, radar->chan_width,
				 radar->cf1, radar->cf2);
}
...
static void hostapd_event_dfs_nop_finished(struct hostapd_data *hapd,
					   struct dfs_event *radar)
{
	wpa_warning_log1(MSG_DEBUG, "DFS NOP finished on %d MHz", radar->freq);
	(void)hostapd_dfs_nop_finished(hapd->iface, radar->freq, radar->ht_enabled,
				 radar->chan_offset, radar->chan_width,
				 radar->cf1, radar->cf2);
}
...
static void hostapd_event_dfs_cac_started(struct hostapd_data *hapd,
					  struct dfs_event *radar)
{
	wpa_warning_log1(MSG_DEBUG, "DFS offload CAC started on %d MHz", radar->freq);
	(void)hostapd_dfs_start_cac(hapd->iface, radar->freq, radar->ht_enabled,
			      radar->chan_offset, radar->chan_width,
			      radar->cf1, radar->cf2);
}
...
/* LiteOS wpa_supplicant_event->hostapd_event */
...
#endif
{
	...
	if ((wifi_dev == NULL) && ((g_hapd == NULL) || (g_hapd != ctx))){
		return;
	}
	if (wifi_dev != NULL) {
		if (wifi_dev->iftype == HI_WIFI_IFTYPE_STATION) {
			wpa_supplicant_event(wifi_dev->priv, event, data);
			return;
		}
#ifdef CONFIG_MESH
	...
	(void)level;
	wpa_warning_buf(MSG_DEBUG, "hostapd Event %s received",
		event_to_string(event), strlen(event_to_string(event)));
	wpa_warning_log1(MSG_DEBUG, "hostapd Event (%d) received", event);
#endif /* CONFIG_NO_STDOUT_DEBUG */

	wpa_warning_log1(MSG_DEBUG, "hostapd Event (%d)", event);
	switch (event) {
#ifndef HISI_CODE_CROP
	case EVENT_MICHAEL_MIC_FAILURE:
		(void)michael_mic_failure(hapd, data->michael_mic_failure.src, 1);
		break;
	...
	case EVENT_WPS_BUTTON_PUSHED:
		(void)hostapd_wps_button_pushed(hapd, NULL);
		break;
#ifdef NEED_AP_MLME
	...
	case EVENT_RX_PROBE_REQ:
		if (data->rx_probe_req.sa == NULL ||
			...
			break;
		(void)hostapd_probe_req_rx(hapd, data->rx_probe_req.sa,
		...
		break;
	case EVENT_NEW_STA:
		(void)hostapd_event_new_sta(hapd, data->new_sta.addr);
		break;
#endif /* HISI_CODE_CROP */
	case EVENT_EAPOL_RX:
	...
	case EVENT_ASSOC:
		if (!data)
			return;
		(void)hostapd_notif_assoc(hapd, data->assoc_info.addr,
	...
	case EVENT_DISASSOC:
		if (data) {
			flags = hostapd_notif_disassoc(hapd, data->disassoc_info.addr);
#ifdef LOS_CONFIG_P2P
			wifi_dev = hi_get_wifi_dev_by_iftype(HI_WIFI_IFTYPE_P2P_GO);
			if ((wifi_dev != NULL) && ((flags & WLAN_STA_AUTHORIZED) != 0)) {
	...
	case EVENT_DEAUTH:
		if (data)
			(void)hostapd_notif_disassoc(hapd, data->deauth_info.addr);
		break;
	...
	default:
#ifndef HISI_CODE_CROP
		wpa_warning_log1(MSG_DEBUG, "hostapd Unknown event %d", event);
#endif /* HISI_CODE_CROP */
		break;
	}
}
#ifndef HISI_CODE_CROP
#ifdef CONFIG_LITEOS_WPA /* LiteOS wpa_supplicant_event_global->hostapd_event_global */
