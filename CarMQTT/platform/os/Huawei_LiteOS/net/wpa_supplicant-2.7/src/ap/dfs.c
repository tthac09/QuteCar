				       ...
				       int chan_width, int cf1, int cf2)
{
	...
	(void)ieee80211_freq_to_chan(frequency, &radar_chan);
			     ...
			     int cf1, int cf2)
{
	...
	if (success) {
		...
		if (iface->drv_flags & WPA_DRIVER_FLAGS_DFS_OFFLOAD) {
			...
			(void)set_dfs_state(iface, freq, ht_enabled, chan_offset,
				      chan_width, cf1, cf2,
				      HOSTAPD_CHAN_DFS_AVAILABLE);
			/*
...
static int hostapd_dfs_start_channel_switch_cac(struct hostapd_iface *iface)
{
	...
	(void)hostapd_setup_interface_complete(iface, err);
	return err;
}
			       ...
			       int cf1, int cf2)
{
	...
	(void)set_dfs_state(iface, freq, ht_enabled, chan_offset, chan_width,
