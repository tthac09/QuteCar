				    ...
				    struct hostapd_acl_query_data *query)
{
	...
	if (!radius_msg_add_attr(msg, RADIUS_ATTR_USER_NAME, (u8 *) buf,
				 os_strlen(buf))) {
		wpa_warning_log0(MSG_DEBUG, "Could not add User-Name");
		goto fail;
	}
	...
	if (!radius_msg_add_attr_user_password(
		...
		wpa_warning_log0(MSG_DEBUG, "Could not add User-Password");
		goto fail;
	}
	...
	if (!radius_msg_add_attr(msg, RADIUS_ATTR_CALLING_STATION_ID,
		...
		wpa_warning_log0(MSG_DEBUG, "Could not add Calling-Station-Id");
		goto fail;
	}
	...
	if (!radius_msg_add_attr(msg, RADIUS_ATTR_CONNECT_INFO,
				 (u8 *) buf, os_strlen(buf))) {
		wpa_warning_log0(MSG_DEBUG, "Could not add Connect-Info");
		goto fail;
	}
			    ...
			    int is_probe_req)
{
	...
	if (hapd->conf->macaddr_acl == USE_EXTERNAL_RADIUS_AUTH) {
		...
		if (query == NULL) {
			wpa_error_log0(MSG_ERROR, "malloc for query data failed");
			return HOSTAPD_ACL_REJECT;
		}
		os_get_reltime(&query->timestamp);
		os_memcpy(query->addr, addr, ETH_ALEN);
		if (hostapd_radius_acl_query(hapd, addr, query)) {
			wpa_warning_log0(MSG_DEBUG, "Failed to send Access-Request "
				   "for ACL query.");
			...
			void *data)
{
	...
	if (radius_msg_verify(msg, shared_secret, shared_secret_len, req, 0)) {
		wpa_warning_log0(MSG_INFO, "Incoming RADIUS packet did not have "
	...
	if (hdr->code != RADIUS_CODE_ACCESS_ACCEPT &&
		...
		wpa_warning_log1(MSG_DEBUG, "Unknown RADIUS message code %d to ACL "
			   "query", hdr->code);
		return RADIUS_RX_UNKNOWN;
	}
	...
	if (cache == NULL) {
		wpa_warning_log0(MSG_DEBUG, "Failed to add ACL cache entry");
		goto done;
	}
	...
	wpa_warning_log0(MSG_DEBUG, "Re-sending authentication frame after "
