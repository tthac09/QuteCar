			       ...
			       struct wpabuf **assocresp_ret)
{
	...
	*beacon_ret = *proberesp_ret = *assocresp_ret = NULL;
	...
	(void)add_buf(&beacon, hapd->conf->vendor_elements);
	(void)add_buf(&proberesp, hapd->conf->vendor_elements);
	(void)add_buf(&assocresp, hapd->conf->assocresp_elements);
#endif /* HISI_CODE_CROP */
...
{
	wpa_error_log0(MSG_ERROR, "hostapd_drv_send_mlme.");
			  ...
			  int center_segment0, int center_segment1)
{
	...
	if (!iface->conf->ieee80211h) {
		wpa_error_log0(MSG_ERROR, "Can't start DFS CAC, DFS functionality "
			   "is not enabled");
		return -1;
	}
	...
	if (hostapd_set_freq_params(&data, mode, freq, channel, ht_enabled,
		...
		wpa_error_log0(MSG_ERROR, "Can't set freq params");
		return -1;
	}
	...
	if (!res) {
		iface->cac_started = 1;
		(void)os_get_reltime(&iface->dfs_cac_start);
	}
...
void hostapd_get_ext_capa(struct hostapd_iface *iface)
{
	...
	(void)hapd->driver->get_ext_capab(hapd->drv_priv, WPA_IF_AP_BSS,
				    &iface->extended_capa,
