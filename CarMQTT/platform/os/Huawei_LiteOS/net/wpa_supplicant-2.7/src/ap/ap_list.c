...
static void ap_ap_hash_del(struct hostapd_iface *iface, struct ap_info *ap)
{
	...
	if (s->hnext != NULL) {
		...
		wpa_warning_log4(MSG_INFO, "AP: could not remove AP " "%02x:xx:xx:%02x:%02x:%02x"
			   " from hash table",  (ap->addr)[0], (ap->addr)[3], (ap->addr)[4], (ap->addr)[5]);
}
}
...
static struct ap_info * ap_ap_add(struct hostapd_iface *iface, const u8 *addr)
{
	...
	if (iface->num_ap > iface->conf->ap_table_max_size && ap != ap->prev) {
		wpa_warning_log4(MSG_DEBUG, "Removing the least recently used AP "
			   "%02x:xx:xx:%02x:%02x:%02x" " from AP table", (ap->prev->addr)[0], (ap->prev->addr)[3], (ap->prev->addr)[4], (ap->prev->addr)[5]);
			    ...
			    struct hostapd_frame_info *fi)
{
	...
	(void)merge_byte_arrays(ap->supported_rates, WLAN_SUPP_RATES_MAX,
	...
	(void)os_get_reltime(&ap->last_beacon);
	...
	if (!iface->olbc &&
		...
		iface->olbc = 1;
		wpa_warning_log4(MSG_DEBUG, "OLBC AP detected: " "%02x:xx:xx:%02x:%02x:%02x",
			   (ap->addr)[0], (ap->addr)[3], (ap->addr)[4], (ap->addr)[5]);
		wpa_warning_log1(MSG_DEBUG, " (channel %d) - enable protection", ap->channel);
		set_beacon++;
	}
	...
	if (!iface->olbc_ht && !ap->ht_support &&
		...
		iface->olbc_ht = 1;
		(void)hostapd_ht_operation_update(iface);
		wpa_warning_log4(MSG_DEBUG, "OLBC HT AP detected: " "%02x:xx:xx:%02x:%02x:%02x",
			   (ap->addr)[0], (ap->addr)[3], (ap->addr)[4], (ap->addr)[5]);
		wpa_warning_log1(MSG_DEBUG, " (channel %d) - enable protection", ap->channel);
		set_beacon++;
	}
	...
	if (set_beacon)
		(void)ieee802_11_update_beacons(iface);
}
...
void ap_list_timer(struct hostapd_iface *iface)
{
	...
	if (iface->olbc || iface->olbc_ht) {
		...
		if (!olbc && iface->olbc) {
			wpa_warning_log0(MSG_DEBUG, "OLBC not detected anymore");
			iface->olbc = 0;
			set_beacon++;
		}
#ifdef CONFIG_IEEE80211N
		if (!olbc_ht && iface->olbc_ht) {
			wpa_warning_log0(MSG_DEBUG, "OLBC HT not detected anymore");
			iface->olbc_ht = 0;
			(void)hostapd_ht_operation_update(iface);
			set_beacon++;
		}
