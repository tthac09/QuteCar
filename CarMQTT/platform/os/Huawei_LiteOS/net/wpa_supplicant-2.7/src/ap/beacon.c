...
void sta_track_expire(struct hostapd_iface *iface, int force)
{
	...
	while ((info = dl_list_first(&iface->sta_seen, struct hostapd_sta_info, list))) {
		if (!force &&
		...
		wpa_msgdump_buf(MSG_MSGDUMP, "%s",
			   iface->bss[0]->conf->iface, strlen(iface->bss[0]->conf->iface));
		wpa_msgdump_log4(MSG_MSGDUMP, "Expire STA tracking entry for "
			   "%02x:xx:xx:%02x:%02x:%02x", (info->addr)[0], (info->addr)[3], (info->addr)[4], (info->addr)[5]);
...
void sta_track_add(struct hostapd_iface *iface, const u8 *addr, int ssi_signal)
{
	...
	if (info) {
		...
		(void)os_get_reltime(&info->last_seen);
	...
	wpa_msgdump_buf(MSG_MSGDUMP, "%s", iface->bss[0]->conf->iface, strlen(iface->bss[0]->conf->iface));
	wpa_msgdump_log4(MSG_MSGDUMP, "%02x:xx:xx:%02x:%02x:%02x", addr[0], addr[3], addr[4], addr[5]);
	dl_list_add_tail(&iface->sta_seen, &info->list);
	iface->num_sta_seen++;
}
		      ...
		      int ssi_signal)
{
	...
	if (ieee802_11_parse_elems(ie, ie_len, &elems, 0) == ParseFailed) {
		wpa_warning_log4(MSG_DEBUG, "Could not parse ProbeReq from " "%02x:xx:xx:%02x:%02x:%02x",
			   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
		return;
	}

	if ((!elems.ssid || !elems.supp_rates)) {
		wpa_warning_log4(MSG_DEBUG, "STA " "%02x:xx:xx:%02x:%02x:%02x" " sent probe request "
	...
	if (hapd->p2p && hapd->p2p_group && elems.wps_ie) {
		...
		if (wps && !p2p_group_match_dev_type(hapd->p2p_group, wps)) {
			wpa_msgdump_log0(MSG_MSGDUMP, "P2P: Ignore Probe Request "
	...
	if (hapd->conf->ignore_broadcast_ssid && elems.ssid_len == 0 &&
		...
		wpa_msgdump_log4(MSG_MSGDUMP, "Probe Request from " "%02x:xx:xx:%02x:%02x:%02x" " for "
			   "broadcast SSID ignored", (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
		return;
	}
	...
	if (res == NO_SSID_MATCH) {
		if (!(mgmt->da[0] & 0x01)) {
			wpa_msgdump_log4(MSG_MSGDUMP, "Probe Request from " "%02x:xx:xx:%02x:%02x:%02x",
				   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
			wpa_msgdump_buf(MSG_MSGDUMP, " for foreign SSID '%s' (DA ",
				   wpa_ssid_txt(elems.ssid, elems.ssid_len), strlen(wpa_ssid_txt(elems.ssid, elems.ssid_len)));
			wpa_msgdump_log4(MSG_MSGDUMP, "%02x:xx:xx:%02x:%02x:%02x",
				   (mgmt->da)[0], (mgmt->da)[3], (mgmt->da)[4], (mgmt->da)[5]);
			wpa_msgdump_buf(MSG_MSGDUMP, ")%s", elems.ssid_list ? " (SSID list)" : "", elems.ssid_list ? strlen(" (SSID list)") : strlen(""));
		}
		return;
	}
#ifndef HISI_CODE_CROP
	...
	if ((hapd->conf->p2p & P2P_GROUP_OWNER) &&
		...
		wpa_msgdump_log4(MSG_EXCESSIVE, "P2P: Ignore Probe Request from "
			   "%02x:xx:xx:%02x:%02x:%02x" " with only 802.11b rates",
			   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
		return;
	}
#endif /* CONFIG_P2P */
	...
	if (hapd->conf->no_probe_resp_if_seen_on &&
		...
		wpa_msgdump_buf(MSG_MSGDUMP, "%s: Ignore Probe Request from ",
			   hapd->conf->iface, strlen(hapd->conf->iface));
		wpa_msgdump_log4(MSG_MSGDUMP, "%02x:xx:xx:%02x:%02x:%02x",
			  (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
		wpa_msgdump_buf(MSG_MSGDUMP, " since STA has been seen on %s",
			   hapd->conf->no_probe_resp_if_seen_on, strlen(hapd->conf->no_probe_resp_if_seen_on));
		return;
	}
	...
	if (hapd->iconf->ignore_probe_probability > 0.0 &&
		...
		wpa_warning_log4(MSG_INFO,
			   "TESTING: ignoring probe request from " "%02x:xx:xx:%02x:%02x:%02x",
			   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
		return;
	}
#endif /* CONFIG_TESTING_OPTIONS */
	...
	if (ret < 0)
		wpa_warning_log0(MSG_INFO, "handle_probe_req: send failed");

	os_free(resp);

	wpa_msgdump_log4(MSG_EXCESSIVE, "STA " "%02x:xx:xx:%02x:%02x:%02x"
		   "SSID", (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
	wpa_msgdump_buf(MSG_EXCESSIVE, " sent probe request for %s "
		   "SSID", elems.ssid_len == 0 ? "broadcast" : "our", elems.ssid_len == 0 ? strlen("broadcast") : strlen("our"));
}
					...
					size_t *resp_len)
{
	...
	if (hapd->conf->wps_state && hapd->wps_probe_resp_ie &&
		...
		wpa_warning_log0(MSG_WARNING, "Device is trying to offload WPS "
	...
	if ((hapd->conf->p2p & P2P_ENABLED) && hapd->p2p_probe_resp_ie &&
		...
		wpa_warning_log0(MSG_WARNING, "Device is trying to offload P2P "
			   "Probe Response while not supporting this");
#endif  /* CONFIG_P2P */
#ifndef HISI_CODE_CROP
	if (hapd->conf->interworking &&
		...
		wpa_warning_log0(MSG_WARNING, "Device is trying to offload "
...
#define BEACON_TAIL_BUF_SIZE 512
	...
	if (head == NULL || tail == NULL) {
		wpa_error_log0(MSG_ERROR, "Failed to set beacon data");
		os_free(head);
		os_free(tail);
		return -1;
	}
	...
	*pos++ = WLAN_EID_SSID;
	...
	if (hapd->conf->wpa != WPA_PROTO_WPA)
		tailpos = hostapd_eid_wpa(hapd, tailpos,
					  ...
					  tailpos);
	/* fill DS ies to beacon tail ,so that tail len will not be 0 in 11b open AP mode. */
	/* DS Params */
	tailpos = hostapd_eid_ds_params(hapd, tailpos);
...
#endif /* CONFIG_IEEE80211N */
#ifdef CONFIG_MESH
	tailpos = hi_hostapd_eid_mesh_id(hapd, tailpos);
	tailpos = hi_hostapd_eid_mesh_cfg(hapd, tailpos);
#endif /* CONFIG_MESH */
#ifndef HISI_CODE_CROP
...
#ifndef HISI_CODE_CROP
		params->mesh_ssid = hapd->iface->mconf->meshid;
		params->mesh_ssid_len = hapd->iface->mconf->meshid_len;
#else
		params->ssid = hapd->iface->mconf->meshid;
		params->ssid_len = hapd->iface->mconf->meshid_len;
#endif /* HISI_CODE_CROP */
	}
#endif
...
int ieee802_11_set_beacon(struct hostapd_data *hapd)
{
	...
	if (hapd->csa_in_progress) {
		wpa_error_log0(MSG_ERROR, "Cannot set beacons during CSA period");
		return -1;
	}
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	...
	if (iface->current_mode &&
		hostapd_set_freq_params(&freq, iconf->hw_mode, iface->freq,
					iconf->channel, iconf->ieee80211n) == 0)
#endif /* HISI_CODE_CROP */
		params.freq = &freq;
	wpa_error_log0(MSG_ERROR, "before hostapd_drv_set_ap.");
	res = hostapd_drv_set_ap(hapd, &params);
	wpa_error_log0(MSG_ERROR, "after hostapd_drv_set_ap.");
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
	hostapd_free_ap_extra_ies(hapd, beacon, proberesp, assocresp);
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	if (res) {
		wpa_error_log1(MSG_ERROR, "Failed to set beacon parameters, res = %d", res);
	} else {
		ret = 0;
	}
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
