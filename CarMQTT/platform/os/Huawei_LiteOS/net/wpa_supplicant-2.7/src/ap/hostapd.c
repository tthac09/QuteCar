...
static void hostapd_set_own_neighbor_report(struct hostapd_data *hapd)
{
	...
	if (ht) {
		...
		ieee80211_freq_to_chan(hapd->iface->freq +
				       ...
				       &center_freq1_idx);
#else
		ieee80211_freq_to_chan(hapd->iface->freq, &center_freq1_idx);
#endif /* HISI_CODE_CROP */
	}
						 ...
						 int err)
{
	...
	if ((iface->drv_flags & WPA_DRIVER_FLAGS_DFS_OFFLOAD) &&
		 ...
		 * for CAC to complete.
		 */
		wpa_printf(MSG_DEBUG, "%s: Wait for CAC to complete", __FUNCTION__);
				      ...
				      struct hostapd_freq_params *old_params)
{
	...
	(void)ieee80211_freq_to_chan(params->center_freq1,
			       &conf->vht_oper_centr_freq_seg0_idx);
	(void)ieee80211_freq_to_chan(params->center_freq2,
			       &conf->vht_oper_centr_freq_seg1_idx);
