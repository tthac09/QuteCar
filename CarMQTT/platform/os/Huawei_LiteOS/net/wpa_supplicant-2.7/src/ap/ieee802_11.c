...
void sae_accept_sta(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	if (hapd->conf->mesh & MESH_ENABLED)
		...
		wpa_auth_pmksa_add_sae(hapd->wpa_auth, sta->addr, sta->sae->pmk, sta->sae->pmkid);
			...
			const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	wpa_warning_log1(MSG_DEBUG, "authentication: STA=" "seq_ctrl=0x%x", seq_ctrl);
				   ...
				   size_t len)
{
	...
	if (((u8 *) mgmt) + len < end) {
		wpa_warning_log1(MSG_DEBUG, "IEEE 802.11: Too short SA Query Action "
...
#endif /* HISI_CODE_CROP */
	if (sta->auth_alg == WLAN_AUTH_FT)
		(void)wpa_auth_sm_event(sta->wpa_sm, WPA_ASSOC_FT);
	else
		       ...
		       const u8 *buf, size_t len, int ack)
{
	...
	(void)ieee802_1x_tx_status(hapd, sta, buf, len, ack);
}
			     ...
			     const u8 *data, size_t len, int ack)
{
	...
	(void)ieee802_1x_eapol_tx_status(hapd, sta, data, len, ack);
}
