				      ...
				      const char *txtaddr)
{
	...
	if (os_strstr(txtaddr, " tx=0"))
		(void)hostapd_drv_sta_remove(hapd, addr);
	else
				    ...
				    const char *txtaddr)
{
	...
	if (os_strstr(txtaddr, " tx=0"))
		...
		(void)hostapd_drv_sta_disassoc(hapd, addr, reason);
	sta = ap_get_sta(hapd, addr);
	if (sta)
			       ...
			       struct csa_settings *settings)
{
	...
	if (pos == end) {
		wpa_error_log0(MSG_ERROR, "chanswitch: invalid cs_count provided");
		return -1;
	}

	settings->freq_params.freq = atoi(end);
	if (settings->freq_params.freq == 0) {
		wpa_error_log0(MSG_ERROR, "chanswitch: invalid freq provided");
		return -1;
	}
