				  ...
				  const u8 *addr, const u8 *trans_id)
{
	...
	wpa_warning_log4(MSG_DEBUG, "IEEE 802.11: Sending SA Query Request to "
	...
	if (hostapd_drv_send_mlme(hapd, &mgmt, end - (u8 *) &mgmt, 0) < 0) {
		wpa_warning_log0(MSG_INFO, "ieee802_11_send_sa_query_req: send failed");
}
}
					  ...
					  const u8 *sa, const u8 *trans_id)
{
	...
	wpa_warning_log4(MSG_DEBUG, "IEEE 802.11: Received SA Query Request from "
	...
	if (sta == NULL || !(sta->flags & WLAN_STA_ASSOC)) {
		wpa_warning_log4(MSG_DEBUG, "IEEE 802.11: Ignore SA Query Request "
			   "from unassociated STA " "%02x:xx:xx:%02x:%02x:%02x", sa[0], sa[3], sa[4], sa[5]);
		return;
	}

	wpa_warning_log4(MSG_DEBUG, "IEEE 802.11: Sending SA Query Response to "
		   "%02x:xx:xx:%02x:%02x:%02x", sa[0], sa[3], sa[4], sa[5]);
	...
	if (hostapd_drv_send_mlme(hapd, &resp, end - (u8 *) &resp, 0) < 0) {
		wpa_warning_log0(MSG_INFO, "ieee80211_mgmt_sa_query_request: send failed");
}
}
				...
				const u8 action_type, const u8 *trans_id)
{
	...
	if (action_type != WLAN_SA_QUERY_RESPONSE) {
		wpa_warning_log1(MSG_DEBUG, "IEEE 802.11: Unexpected SA Query "
			   "Action %d", action_type);
		return;
	}

	wpa_warning_log4(MSG_DEBUG, "IEEE 802.11: Received SA Query Response from "
		   "%02x:xx:xx:%02x:%02x:%02x", sa[0], sa[3], sa[4], sa[5]);
	wpa_hexdump(MSG_DEBUG, "IEEE 802.11: SA Query Transaction ID",
		    trans_id, WLAN_SA_QUERY_TR_ID_LEN);
	...
	if (sta == NULL || sta->sa_query_trans_id == NULL) {
		wpa_warning_log0(MSG_DEBUG, "IEEE 802.11: No matching STA with "
	...
	if (i >= sta->sa_query_count) {
		wpa_warning_log0(MSG_DEBUG, "IEEE 802.11: No matching SA Query "
