...
static void wpabuf_overflow(const struct wpabuf *buf, size_t len)
{
	...
	if (trace->magic != WPABUF_MAGIC) {
		wpa_error_log1(MSG_ERROR, "wpabuf: invalid magic %x",
			   trace->magic);
	}
#endif /* WPA_TRACE */
	wpa_error_log4(MSG_ERROR, "wpabuf %p (size=%lu used=%lu) overflow len=%lu",
...
void wpabuf_free(struct wpabuf *buf)
{
	...
	trace = wpabuf_get_trace(buf);
	if (trace->magic != WPABUF_MAGIC) {
		wpa_error_log1(MSG_ERROR, "wpabuf_free: invalid magic %x",
			   trace->magic);
