...
static inline void hisi_driver_event_mesh_close_process(hisi_driver_data_stru *drv, int8 *data_ptr)
{
	hisi_mesh_close_peer_stru *mesh_close_peer = (hisi_mesh_close_peer_stru *)data_ptr;
	if (is_zero_ether_addr((const uint8 *)mesh_close_peer->macaddr)) {
		wpa_error_log0(MSG_ERROR, "hisi_driver_event_process:mesh close: macaddr all zero");
		return;
	}
	if ((eloop_is_running(ELOOP_TASK_WPA) != HISI_OK) || (drv->ctx == NULL))
		return;
	wpas_mesh_peer_remove(drv->ctx, mesh_close_peer->macaddr);
}
...
int hisi_mesh_init(void *priv)
{
	...
	if (priv == NULL)
		return -HISI_EFAIL;
	wpa_s = (struct wpa_supplicant *)priv;
	if ((wpa_s->drv_priv == NULL) || (wpa_s->ifmsh == NULL))
		return -HISI_EFAIL;
	drv = wpa_s->drv_priv;
	if (drv == NULL)
		return -HISI_EFAIL;
	...
	rc = memset_s(&set_mode, sizeof(hisi_set_mode_stru), 0, sizeof(hisi_set_mode_stru));
	rc |= memcpy_s(set_mode.bssid, ETH_ADDR_LEN, drv->own_addr, ETH_ADDR_LEN);
	if (rc != EOK)
		return -HISI_EFAIL;
	int_save = LOS_IntLock();
	g_rx_mgmt_count = 0;
	LOS_IntRestore(int_save);
	set_mode.iftype = HI_WIFI_IFTYPE_MESH_POINT;
	drv->hapd = hapd;
	ret = hisi_ioctl_set_mode(drv->iface, &set_mode);
	if (ret != HISI_SUCC) {
		wpa_error_log0(MSG_ERROR, "hisi_mesh_init: hisi_ioctl_set_mode failed!");
		return -HISI_EFAIL;
	}
...
int32 hisi_mesh_sta_add(void *priv, struct hostapd_sta_add_params *params)
{
	int32 ret;
	hisi_driver_data_stru *drv = NULL;
	hisi_mesh_usr_params_stru mesh_usr = {0};

	if ((priv == HISI_PTR_NULL) || (params == HISI_PTR_NULL) || (params->addr == HISI_PTR_NULL))
		return -HISI_EFAIL;

	drv = (hisi_driver_data_stru *)priv;
	mesh_usr.set = (uint8)(params->set);
	mesh_usr.plink_state = (uint8)(params->plink_state);
	mesh_usr.mesh_bcn_priority = (uint8)(params->mesh_bcn_priority);
	mesh_usr.mesh_is_mbr = (uint8)(params->mesh_is_mbr);
	mesh_usr.mesh_initiative_peering = (uint8)(params->mesh_initiative_peering);
	mesh_usr.addr = hisi_dup_macaddr(params->addr, ETH_ADDR_LEN);
	if (mesh_usr.addr == NULL) {
		wpa_error_log0(MSG_ERROR, "hisi_mesh_sta_add failed.");
		return -HISI_EFAIL;
	}
	wpa_error_log2(MSG_ERROR, "hisi_mesh_sta_add mesh_initiative_peering = %d, set = %d.",
	               mesh_usr.mesh_initiative_peering, mesh_usr.set);
	ret = hisi_ioctl_mesh_usr_add(drv->iface, &mesh_usr);
	wpa_warning_log4(MSG_ERROR, "hisi_mesh_sta_add : ""%02x:xx:xx:%02x:%02x:%02x",
	                 (mesh_usr.addr)[0], (mesh_usr.addr)[3], (mesh_usr.addr)[4], (mesh_usr.addr)[5]);
	os_free(mesh_usr.addr);
	mesh_usr.addr = NULL;
	return ret;
}
...
int32 hisi_set_mesh_mgtk(const char *ifname, const uint8 *addr, const uint8 *mgtk, size_t mgtk_len)
{
	int32 ret;
	errno_t rc;
	hisi_set_mesh_user_gtk_stru *gtk_params = NULL;

	if ((ifname == NULL) || (mgtk == NULL) || (mgtk_len > WPA_TK_MAX_LEN) ||
	    (addr == NULL) || (is_broadcast_ether_addr(addr)))
		return -HISI_EFAIL;
	gtk_params = os_zalloc(sizeof(hisi_set_mesh_user_gtk_stru));
	if (gtk_params == NULL)
		return -HISI_EFAIL;

	gtk_params->gtk = (uint8 *)os_zalloc(mgtk_len);
	if (gtk_params->gtk == NULL) {
		ret = -HISI_EFAIL;
		goto HISI_SET_MGTK_FAIL;
	}
	rc = memcpy_s(gtk_params->gtk, WPA_TK_MAX_LEN, mgtk, mgtk_len);
	if (rc != EOK) {
		ret = -HISI_EFAIL;
		goto HISI_SET_MGTK_FAIL;
	}
	gtk_params->gtk_len = mgtk_len;
	gtk_params->addr = (uint8 *)os_zalloc(ETH_ADDR_LEN);
	if (gtk_params->addr == NULL) {
		ret = -HISI_EFAIL;
		goto HISI_SET_MGTK_FAIL;
	}
	(void)memcpy_s(gtk_params->addr, ETH_ADDR_LEN, addr, ETH_ADDR_LEN);
	ret = hisi_ioctl_mesh_set_mgtk((int8 *)ifname, gtk_params);
HISI_SET_MGTK_FAIL:
	if (gtk_params->addr) {
		os_free(gtk_params->addr);
		gtk_params->addr = NULL;
	}
	if (gtk_params->gtk) {
		os_free(gtk_params->gtk);
		gtk_params->gtk = NULL;
	}
	os_free(gtk_params);
	return ret;
