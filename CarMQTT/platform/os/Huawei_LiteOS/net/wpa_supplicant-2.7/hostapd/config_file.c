					...
					struct hostapd_bss_config *conf)
{
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		if (user == NULL) {
			wpa_error_log0(MSG_ERROR, "EAP user allocation failed");
			goto failed;
		}
		...
		if (buf[0] == '*') {
			...
			if (user->identity == NULL) {
				wpa_error_log0(MSG_ERROR, "Failed to allocate "
...
static int hostapd_config_parse_key_mgmt(int line, const char *value)
{
	...
	while (*start != '\0') {
		...
		*end = '\0';
		if (os_strcmp(start, "WPA-PSK") == 0)
			...
			wpa_error_buf(MSG_ERROR, "invalid key_mgmt '%s'",
				   start, strlen(start));
	...
	if (val == 0) {
#ifndef CONFIG_PRINT_NOUSE
		wpa_error_log1(MSG_ERROR, "Line %d: no key_mgmt values "
			   "configured.", line);
#endif /* CONFIG_PRINT_NOUSE */
		return -1;
	}
...
static int hostapd_config_parse_cipher(int line, const char *value)
{
	...
	if (val < 0) {
		wpa_error_buf(MSG_ERROR, "invalid cipher '%s'.",
			   value, strlen(value));
		return -1;
	}
	if (val == 0) {
		wpa_error_log1(MSG_ERROR, "Line %d: no cipher values configured.",
			   line);
		return -1;
	}
	return val;
...
static int hostapd_config_bss(struct hostapd_config *conf, const char *ifname)
{
	...
	if (all == NULL) {
		wpa_error_log0(MSG_ERROR, "Failed to allocate memory for "
			   "multi-BSS entry");
		return -1;
	}
	conf->bss = all;
	...
	(void)os_memcpy(bss->ssid.vlan, bss->iface, IFNAMSIZ + 1);
#endif
	return 0;
}
				   ...
				   const char *name, const char *val)
{
	...
	if (os_strncmp(pos, "data", 4) == 0 &&
		...
		wpa_warning_buf(MSG_INFO, "DEPRECATED: '%s' not used", name, strlen(name));
		return 0;
	} else {
		wpa_error_buf(MSG_ERROR, "Unknown tx_queue name '%s'", pos, strlen(pos));
		return -1;
	}
	...
	if (os_strcmp(pos, "aifs") == 0) {
		...
		if (queue->aifs < 0 || queue->aifs > 255) {
			wpa_error_log1(MSG_ERROR, "Invalid AIFS value %d",
				   queue->aifs);
			return -1;
		}
		...
		if (!valid_cw(queue->cwmin)) {
			wpa_error_log1(MSG_ERROR, "Invalid cwMin value %d",
				   queue->cwmin);
			return -1;
		}
		...
		if (!valid_cw(queue->cwmax)) {
			wpa_error_log1(MSG_ERROR, "Invalid cwMax value %d",
				   queue->cwmax);
			return -1;
		}
		...
		wpa_error_buf(MSG_ERROR, "Unknown tx_queue field '%s'", pos, strlen(pos));
		return -1;
	}
...
static int parse_nai_realm(struct hostapd_bss_config *bss, char *buf, int line)
{
	...
	while (pos && *pos) {
		...
		if (realm->eap_method_count >= MAX_NAI_EAP_METHODS) {
			wpa_error_log0(MSG_ERROR, "Too many EAP methods");
			goto fail;
		}
		...
		for (;;) {
			...
			if (eap->num_auths >= MAX_NAI_AUTH_TYPES) {
				wpa_error_log0(MSG_ERROR, "Too many auth params");
				goto fail;
			}
	...
	while (*rpos) {
		if (i >= MAX_NAI_REALMS) {
			wpa_error_log0(MSG_ERROR, "Too many realms");
			goto fail;
		}
		realm->realm[i++] = rpos;
		rpos = os_strchr(rpos, ';');
					...
					char *pos, int line)
{
	if (bss->last_osu == NULL) {
		wpa_error_log1(MSG_ERROR, "Line %d: Unexpected OSU field", line);
		return -1;
	}
				      ...
				      int line)
{
	...
	if (hostapd_parse_intlist(&bss->last_osu->method_list, pos)) {
		wpa_error_log1(MSG_ERROR, "Line %d: Invalid osu_method_list", line);
		return -1;
	}
			    ...
			    const char *val)
{
	...
	if (!elems) {
		wpa_error_two_buf(MSG_ERROR, "Invalid %s '%s'",
			       ...
			       const char *buf, char *pos, int line)
{
	if (os_strcmp(buf, "interface") == 0) {
		(void)os_strlcpy(conf->bss[0]->iface, pos,
			   sizeof(conf->bss[0]->iface));
	} else if (os_strcmp(buf, "bridge") == 0) {
		(void)os_strlcpy(bss->bridge, pos, sizeof(bss->bridge));
	} else if (os_strcmp(buf, "vlan_bridge") == 0) {
		(void)os_strlcpy(bss->vlan_bridge, pos, sizeof(bss->vlan_bridge));
	} else if (os_strcmp(buf, "wds_bridge") == 0) {
		(void)os_strlcpy(bss->wds_bridge, pos, sizeof(bss->wds_bridge));
		...
		wpa_warning_log1(MSG_DEBUG, "Line %d: DEPRECATED: 'debug' configuration variable is not used anymore",
			   line);
		...
		wpa_warning_log1(MSG_INFO, "Line %d: DEPRECATED: 'dump_file' configuration variable is not used anymore",
			   line);
	} else if (os_strcmp(buf, "ssid") == 0) {
		bss->ssid.ssid_len = os_strlen(pos);
		if (bss->ssid.ssid_len > SSID_MAX_LEN ||
		    bss->ssid.ssid_len < 1) {
			wpa_error_buf(MSG_ERROR, "invalid SSID '%s'",
				   pos, strlen(pos));
			return 1;
		}
		...
		wpa_warning_log1(MSG_DEBUG, "eapol_version=%d", bss->eapol_version);
#ifdef EAP_SERVER
	} else if (os_strcmp(buf, "eap_authenticator") == 0) {
		bss->eap_server = atoi(pos);
		wpa_error_log1(MSG_ERROR, "Line %d: obsolete eap_authenticator used; this has been renamed to eap_server", line);
		...
		if (bss->pac_opaque_encr_key == NULL) {
			...
			wpa_error_log1(MSG_ERROR, "Line %d: Invalid pac_opaque_encr_key",
				   line);
			return 1;
		}
		...
		if (idlen & 1) {
			wpa_error_log1(MSG_ERROR, "Line %d: Invalid eap_fast_a_id",
				   line);
			return 1;
		}
		...
		if (bss->eap_fast_a_id == NULL ||
			...
			wpa_error_log1(MSG_ERROR, "Line %d: Failed to parse eap_fast_a_id",
				   line);
			os_free(bss->eap_fast_a_id);
		...
		if (bss->eap_req_id_text == NULL) {
			wpa_error_log1(MSG_ERROR, "Line %d: Failed to allocate memory for eap_req_id_text",
				   line);
			return 1;
		}
		...
		if (bss->wep_rekeying_period < 0) {
			wpa_error_log2(MSG_ERROR, "Line %d: invalid period %d",
		...
		if (len == 0) {
			/* RFC 2865, Ch. 3 */
			wpa_error_log1(MSG_ERROR, "Line %d: empty shared secret is not allowed",
				   line);
			return 1;
		}
		...
		if (hostapd_parse_das_client(bss, pos) < 0) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid DAS client",
				   line);
			return 1;
		}
		...
		if (bss->auth_algs == 0) {
			wpa_error_log1(MSG_ERROR, "Line %d: no authentication algorithms allowed",
				   line);
			return 1;
		}
		...
		int len = os_strlen(pos);
		if (len < 8 || len > 63) {
			wpa_error_log2(MSG_ERROR, "Line %d: invalid WPA passphrase length %d (expected 8..63)",
				   line, len);
			return 1;
		}
		...
		if (hexstr2bin(pos, bss->ssid.wpa_psk->psk, PMK_LEN) ||
		    pos[PMK_LEN * 2] != '\0') {
			wpa_error_buf(MSG_ERROR, "Invalid PSK '%s'.",
				   pos, strlen(pos));
			hostapd_config_clear_wpa_psk(&bss->ssid.wpa_psk);
			return 1;
		}
		...
		if (!bss->ssid.wpa_psk_file) {
			wpa_error_log1(MSG_ERROR, "Line %d: allocation failed",
				   line);
			return 1;
		}
		...
		if (bss->wpa_pairwise &
			...
			wpa_error_buf(MSG_ERROR, "unsupported pairwise cipher suite '%s'",
				   pos, strlen(pos));
			return 1;
		}
		...
		if (bss->group_cipher != WPA_CIPHER_TKIP &&
			...
			wpa_error_buf(MSG_ERROR,
				   "unsupported group cipher suite '%s'",
				   pos, strlen(pos));
			return 1;
		}
		...
		bss->ctrl_interface_gid_set = 1;
		wpa_warning_log1(MSG_DEBUG, "ctrl_interface_group=%d",
		...
		if (os_strcmp(pos, "a") == 0)
			...
			wpa_error_buf(MSG_ERROR, "unknown hw_mode '%s'",
				   pos, strlen(pos));
			return 1;
		}
...
#ifndef CONFIG_ACS
			wpa_error_log1(MSG_ERROR, "Line %d: tries to enable ACS but CONFIG_ACS disabled",
				   line);
			return 1;
#else /* CONFIG_ACS */
			conf->acs = 1;
		...
		if (hostapd_parse_chanlist(conf, pos)) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid channel list",
				   line);
			return 1;
		}
		...
		if (val < 15 || val > 65535) {
			wpa_error_log2(MSG_ERROR, "Line %d: invalid beacon_int %d (expected 15..65535)",
				   line, val);
			return 1;
		}
		conf->beacon_int = val;
		...
		if (hostapd_config_parse_acs_chan_bias(conf, pos)) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid acs_chan_bias",
				   line);
			return -1;
		}
#endif /* CONFIG_ACS */
		...
		if (val < 1 || val > 255) {
			wpa_error_log2(MSG_ERROR, "Line %d: invalid dtim_period %d",
				   line, val);
			return 1;
		}
		bss->dtim_period = val;
		...
		int val = atoi(pos);
		if (val != 0 && val != 1) {
			wpa_error_log2(MSG_ERROR, "Line %d: invalid send_probe_response %d (expected 0 or 1)",
				   line, val);
			return 1;
		}
		...
		if (hostapd_parse_intlist(&conf->supported_rates, pos)) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid rate list",
				   line);
			return 1;
		}
		...
		if (hostapd_config_read_wep(&bss->ssid.wep,
					    buf[7] - '0', pos)) {
			wpa_error_buf(MSG_ERROR, "invalid WEP key '%s'",
				   buf, strlen(buf));
			return 1;
		}
#ifndef CONFIG_NO_VLAN
		...
		if (hostapd_config_tx_queue(conf, buf, pos)) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid TX queue item",
				   line);
			return 1;
		}
		...
		if (hostapd_config_wmm_ac(conf->wmm_ac_params, buf, pos)) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid WMM ac item",
				   line);
			return 1;
		}
	} else if (os_strcmp(buf, "bss") == 0) {
		if (hostapd_config_bss(conf, pos)) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid bss item",
				   line);
			return 1;
		}
	} else if (os_strcmp(buf, "bssid") == 0) {
		if (hwaddr_aton(pos, bss->bssid)) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid bssid item",
				   line);
			return 1;
		}
		...
		if (os_strcmp(pos, "AES-128-CMAC") == 0) {
			...
			wpa_error_buf(MSG_ERROR, "invalid group_mgmt_cipher: %s",
				   pos, strlen(pos));
			return 1;
		}
		...
		if (bss->assoc_sa_query_max_timeout == 0) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid assoc_sa_query_max_timeout",
				   line);
			return 1;
		}
		...
		if (bss->assoc_sa_query_retry_timeout == 0) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid assoc_sa_query_retry_timeout",
				   line);
			return 1;
		}
#endif /* CONFIG_IEEE80211W */
		...
		if (hostapd_config_ht_capab(conf, pos) < 0) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid ht_capab",
				   line);
			return 1;
		}
		...
		if (hostapd_config_vht_capab(conf, pos) < 0) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid vht_capab",
				   line);
			return 1;
		}
		...
		if (bss->wps_state < 0 || bss->wps_state > 2) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid wps_state",
				   line);
			return 1;
		}
		...
		if (os_strlen(pos) > WPS_DEV_NAME_MAX_LEN) {
			wpa_error_log1(MSG_ERROR, "Line %d: Too long "
				   "device_name", line);
			return 1;
		}
		...
		if (os_strlen(pos) > 64) {
			wpa_error_log1(MSG_ERROR, "Line %d: Too long manufacturer",
				   line);
			return 1;
		}
		...
		if (os_strlen(pos) > 32) {
			wpa_error_log1(MSG_ERROR, "Line %d: Too long model_name",
				   line);
			return 1;
		}
		...
		if (os_strlen(pos) > 32) {
			wpa_error_log1(MSG_ERROR, "Line %d: Too long model_number",
				   line);
			return 1;
		}
		...
		if (os_strlen(pos) > 32) {
			wpa_error_log1(MSG_ERROR, "Line %d: Too long serial_number",
				   line);
			return 1;
		}
		...
		if (hexstr2bin(pos, bss->os_version, 4)) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid os_version",
				   line);
			return 1;
		}
		...
		if (bss->extra_cred == NULL) {
			wpa_error_buf(MSG_ERROR, "could not read Credentials from '%s'",
				   pos, strlen(pos));
			return 1;
		}
	}
		...
		if (bss->ap_settings == NULL) {
			wpa_error_buf(MSG_ERROR, "could not read AP Settings from '%s'",
				   pos, strlen(pos));
			return 1;
		}
		...
		if (bss->wps_nfc_dev_pw_id < 0x10 ||
			...
			wpa_error_log1(MSG_ERROR, "Line %d: Invalid wps_nfc_dev_pw_id value",
				   line);
			return 1;
		}
		...
		if (tz_len < 4 || tz_len > 255) {
			wpa_warning_log1(MSG_DEBUG, "Line %d: invalid time_zone",
				   line);
			return 1;
		}
		...
		if (hwaddr_aton(pos, bss->hessid)) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid hessid", line);
			return 1;
		}
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	PARSE_TEST_PROBABILITY(corrupt_gtk_rekey_mic_probability)
		...
		pos = os_strchr(pos, ':');
		if (pos == NULL) {
			wpa_error_log1(MSG_ERROR, "Line %d: Invalid bss_load_test",
				   line);
			return 1;
		}
		pos++;
		...
		if (parse_sae_password(bss, pos) < 0) {
			wpa_error_log1(MSG_ERROR, "Line %d: Invalid sae_password",
				   line);
			return 1;
		}
#endif /* CONFIG_SAE */
		...
		int val = atoi(pos);
		if (val < 0 || val > 255) {
			wpa_error_log2(MSG_ERROR, "Line %d: Invalid local_pwr_constraint %d (expected 0..255)",
				   line, val);
			return 1;
		}
		      ...
		      char *value)
{
	...
	if (hostapd_config_check(conf, 0)) {
		wpa_error_log0(MSG_ERROR, "Configuration check failed");
		return -1;
	}
