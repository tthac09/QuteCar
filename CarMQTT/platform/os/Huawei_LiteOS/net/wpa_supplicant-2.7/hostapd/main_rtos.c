...
static int hostapd_bss_init(struct hostapd_bss_config *bss, const hostapd_conf *conf, const char *ifname)
{
	...
	if ((bss->ssid.ssid_len > SSID_MAX_LEN) || (bss->ssid.ssid_len < 1)) {
		...
		if (memcpy_s(bss->ssid.ssid, SSID_MAX_LEN, conf->ssid, SSID_MAX_LEN) != EOK) {
			wpa_error_log0(MSG_ERROR, "os_memcpy failed");
			errors++;
		}
		bss->ssid.ssid_set = 1;
	}
...
void hi_hostapd_exit(void)
{
	size_t i;
	if (g_interfaces == NULL)
		return;

	wpa_error_log0(MSG_ERROR, "hi_hostapd_exit enter\n");
	hostapd_global_ctrl_iface_deinit(g_interfaces);
	/* Deinitialize all interfaces */
	for (i = 0; i < g_interfaces->count; i++) {
		if (g_interfaces->iface[i] == NULL)
			continue;
		hostapd_interface_deinit_free(g_interfaces->iface[i]);
		g_interfaces->iface[i] = NULL;
	}
	wpa_error_log0(MSG_ERROR, "hi_hostapd_exit ing\n");

	hi_free_wifi_dev(hi_get_wifi_dev_by_priv(g_hapd));
	hostapd_global_deinit();
