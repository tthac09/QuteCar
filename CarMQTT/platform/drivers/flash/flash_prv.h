...
#define HI_FLASH_SUPPORT_REF_VBAT   /* VBAT can be used as ref valtage 配置文件 */
...
typedef struct {
    hi_u32 cmd:     8;  /* 命令 */
    hi_u32 iftype:  3;  /* 通信类型 */
    hi_u32 dummy:   3;  /* dummy字节数 */
    hi_u32 size:    18; /* CHIP SIZE */
} spi_flash_operation;
...
typedef enum {
    ...
    HI_FLASH_SUPPORT_SUSPEND_RESUME = 0x80,       /* 支持Flash擦写中挂起和恢复 */
...
typedef struct spi_flash_ctrl {
    hi_bool init;
    hi_bool sus_enable;
    hi_bool limit_0_to_1;    /* 是否限制 flash 0 写 1 的情况，TRUE: 0写1报错，FALSE: 0写1不报错 */
    hi_u8 reserve;
...
/* flash protect */
#define PROTECT_TIMEOUT_1    6000      /* 易失性写Flash状态寄存器，超时值秒级 */
#define PROTECT_TIMEOUT_2    3600000   /* 非易失性写Flash状态寄存器，超时值小时级 */
#define PROTECT_TIMEOUT_AUTO 0xFFFFFFFF /* 具体类型对应超时值由内部代码封装，用户可调整 */
#define PROTECT_FLASH_ALL    0x29  /* 0x29: 101001,保护范围0x10000-0x1fffff;0x3c:111100,0x8000-0x1fffff */
...
typedef enum {
    HI_FLASH_PROTECT_TYPE_1 = 0x0,    /**< 本类型支持0x50命令，即同时支持易失和非易失操作状态寄存器*/
    HI_FLASH_PROTECT_TYPE_2,        /**< 本类型不支持0x50命令，即不支持易失性操作状态寄存器*/
    HI_FLASH_PROTECT_NONE,          /**< 本类型不支持保护*/
...
/* 如果需要测试flash 擦写次数，请打开该宏 #define HI_FLASH_SUPPORT_COUNT_DEBUG */
