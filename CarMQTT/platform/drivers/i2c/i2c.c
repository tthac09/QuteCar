...
typedef struct {
    ...
    hi_u16 timeout_us;
    hi_u8 scl_h : 3;  /*  时钟线高电平占比 */
    hi_u8 scl_l : 3;  /* 时钟线低电平占比 */
    hi_bool init : 1;
    hi_bool pad : 1;
    hi_u8 err_times;
} hi_i2c_ctrl;
...
/* 公式: (1/baudrate)*占空比 = 1/apb_clk * (scl_h+1)*2，此处占空比为1/2
    scl_h = apb_clk/(baudrate*2) *占空比 -1  */
hi_void i2c_cfg_clk(hi_i2c_idx id, hi_u32 i2c_rate)
{
    hi_u32 reg_val = 0;
    hi_u32 reg_clk_val;
    ...
    reg_clk_val = (I2C_CLK / (i2c_rate * 2)) * /* 波特率*2 */
...
hi_u32 i2c_start(hi_i2c_idx id)
{
    hi_u32 reg_val = 0;
    hi_u32 time_out = 0;
 ...
 *
 */
hi_u32 i2c_send_byte(hi_i2c_idx id, hi_u8 send_val)
{
 ...
 *
 */
hi_u32 i2c_receive_byte(hi_i2c_idx id, hi_u8 *p_data)
{
    hi_u32 ret;
    hi_u32 reg_val = 0;
    hi_u32 time_out = 0;
 ...
 *
 */
hi_void i2c_set_addr(hi_i2c_idx id, hi_u16 addr)
{
    hi_reg_write32((i2c_base(id) + I2C_TXR), addr);
}

hi_bool i2c_is_10bit_addr(hi_u16 uc_addr)
{
    if ((uc_addr & 0xF000) == 0xF000) {   // 某第三方代码中是第一bit空出，&0xFC==0x78判断，待确认
        return HI_TRUE;
    }
    return HI_FALSE;
}

hi_u32 i2c_10bit_send_addressing(hi_i2c_idx id, hi_u16 device_addr)
{
    hi_u32 ret;
    /* 10bit: */
    ...
    hi_u8 addr = device_addr >> 8;   /* 先传高8位地址，整体右移8位 */
    ...
    addr = (hi_u8)device_addr;             /* 强制类型转换取低8位地址 */
...
hi_u32 i2c_10bit_receive_addressing(hi_i2c_idx id, hi_u16 device_addr)
{
    hi_u32 ret;
    /* 10bit: */
    ...
    addr = device_addr >> 8;   /* 先传高8位地址，整体右移8位 */
...
hi_u32 i2c_receive_byte_proc(hi_i2c_idx id, const hi_i2c_data *i2c_data)
{
    hi_u32 reg_val = 0;
    ...
    for (i = 0; i < (i2c_data->receive_len); i++) {
        if (i == (i2c_data->receive_len - 1)) {
            ...
            if (i2c_data->receive_buf != HI_NULL) {
                *(i2c_data->receive_buf + i) = (hi_u8)reg_val;
            }
            ret = i2c_wait(id);
            if (ret != HI_ERR_SUCCESS) {
...
hi_u32 i2c_restart_receive_data(hi_i2c_idx id, hi_u8 device_addr, const hi_i2c_data *i2c_data)
{
    hi_u32 i = 0;
    hi_u32 ret;
...
hi_u32 i2c_send_data(hi_i2c_idx id, hi_u16 device_addr, const hi_i2c_data *i2c_data)
{
    hi_u32 i = 0;
    hi_u32 ret;
...
hi_u32 i2c_receive_data(hi_i2c_idx id, hi_u16 device_addr, const hi_i2c_data *i2c_data)
{
    hi_u32 ret;
...
hi_u32 hi_i2c_writeread(hi_i2c_idx id, hi_u16 device_addr, const hi_i2c_data *i2c_data)
{
    ...
    ret = hi_sem_wait(g_i2c_drv_sem, HI_SYS_WAIT_FOREVER);
    if (ret != HI_ERR_SUCCESS) {
...
hi_u32 hi_i2c_read(hi_i2c_idx id, hi_u16 device_addr, const hi_i2c_data *i2c_data)
{
    ...
    (hi_void) hi_sem_signal(g_i2c_drv_sem);
...
hi_u32 hi_i2c_init(hi_i2c_idx id, hi_u32 baudrate)
{
    ...
    if (g_i2c_ctrl[id].init == HI_FALSE) {
        ...
        ret = hi_sem_bcreate(&g_i2c_drv_sem, HI_TRUE);
        if (ret) {
            return ret;
        }
