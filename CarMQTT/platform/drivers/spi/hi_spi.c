...
hi_u32 spi_write_data(hi_spi_idx id, hi_u32 timeout_ms)
{
    hi_u32 event_bit;
    hi_u32 ret;
...
/*****************************************************************************
 功能描述     : SPI从RX FIFO读数据
 输入参数     : p_sw_dev:SPI接口软件配置
                p_hw_dev:SPI接口硬件配置
                p_trans_data:传输配置
 返   回  值  : HI_ERR_XXX
*****************************************************************************/
hi_u32 spi_read_data(hi_spi_idx id, hi_u32 timeout_ms)
{
    hi_u32 event_bit = 0;
    hi_u32 ret;
...
hi_u32 spi_host_transfer(hi_spi_idx id, const spi_trans_attr *trans_attr, hi_u32 total_len, hi_u32 read_write)
{
    hi_u32 ret;
...
hi_u32 spi_slave_transfer(hi_spi_idx id, const spi_trans_attr *trans_attr, hi_u32 read_write)
{
    hi_u32 ret;
    hi_u16 threshold = (id == HI_SPI_ID_0 ? SPI0_FIFO_THRESHOLD : SPI1_FIFO_THRESHOLD);
...
hi_u32 spi_slave_dma_transfer(hi_spi_idx id, const spi_trans_attr *trans_attr, hi_u32 read_write)
{
...
hi_u32 spi_host_writeread(hi_spi_idx id, hi_pvoid write_data, hi_pvoid read_data, hi_u32 byte_len, hi_u32 options)
{
    hi_u32 data_len;
    hi_u32 ret;
...
hi_u32 hi_spi_slave_write(hi_spi_idx id, hi_pvoid write_data, hi_u32 byte_len, hi_u32 time_out_ms)
{
    hi_u32 ret;
    hi_u32 data_len;
    if ((hi_u32)id > HI_SPI_ID_1 || write_data == HI_NULL || byte_len == 0) {
        return HI_ERR_SPI_PARAMETER_WRONG;
    }
    if (g_spi_ctrl[id] == HI_NULL) {
        return HI_ERR_SPI_NOT_INIT;
...
hi_u32 hi_spi_slave_read(hi_spi_idx id, hi_pvoid read_data, hi_u32 byte_len, hi_u32 time_out_ms)
{
    hi_u32 ret;
    hi_u32 data_len;
    if ((hi_u32)id > HI_SPI_ID_1 || read_data == HI_NULL || byte_len == 0) {
        return HI_ERR_SPI_PARAMETER_WRONG;
    }
    if (g_spi_ctrl[id] == HI_NULL) {
        return HI_ERR_SPI_NOT_INIT;
...
hi_u32 hi_spi_set_basic_info(hi_spi_idx id, const hi_spi_cfg_basic_info *param)
{
    ...
    if (((hi_u32)id > HI_SPI_ID_1) || (param == HI_NULL)) {
        return HI_ERR_SPI_PARAMETER_WRONG;
    }
    ...
    /* write reg */
    spi_config(g_spi_ctrl[id]);
    return HI_ERR_SUCCESS;
}

hi_void spi_ctrl_init(hi_spi_idx id, hi_spi_cfg_init_param init_param)
{
...
{
    if ((id > HI_SPI_ID_1) || (param == HI_NULL)) {
        return HI_ERR_SPI_PARAMETER_WRONG;
    }
    hi_u32 ret;
    if (g_spi_ctrl[id] != HI_NULL) {
