...
spi_ctrl *g_spi_ctrl[SPI_NUM] = { 0 }; /* SPI全局控制接口 */
...
{
    hi_u16 reg_val = 0;
    /* host增加busy的判断,busy对数据的敏感,接收fifo中有数据时暂不关闭 */
...
hi_u32 spi_config(const spi_ctrl *spi_hw_ctrl)
{
    ...
    if (memcpy_s(&inner_cfg, sizeof(spi_inner_cfg), &spi_hw_ctrl->spi_cfg, sizeof(spi_inner_cfg)) == EOK) {
        ...
        hi_reg_write16(spi_hw_ctrl->reg_base + REG_SPI_CR1, inner_cfg.cr1); /* 关闭了SPI使能 */
...
hi_void spi_isr_enable(hi_u32 reg_base, hi_u16 enable_bits) /* 去屏蔽SPI中断(按bit操作) */
{
    hi_u16 reg_val = 0;
    hi_u32 int_save;
    int_save = hi_int_lock();
...
hi_void spi_isr_disable(hi_u32 reg_base, hi_u16 disable_bits) /* 屏蔽SPI中断(按bit操作) */
{
    hi_u16 reg_val = 0;
    hi_u32 int_save;
    int_save = hi_int_lock();
...
/*****************************************************************************
    功能描述     : SPI RX FIFO 中断响应
    输入参数     : p_hw_dev:SPI接口硬件配置
                 : is_timeout_isr:是否是timeout中断
    返   回  值  : HI_ERR_XXX
*****************************************************************************/
hi_u16 spi_isr_read_fifo(spi_ctrl *spi_dev_ctrl, hi_bool fifo_irq)
{
    hi_pvoid read_buf = spi_dev_ctrl->rx_buf.buf;
    ...
    /* 将if和else分支2段类似代码实现两份的目:提供代码性能，减少for循环中的判断
     * time out 中断需要判断FIFO是否为空再去读数，water line中断直接根据水线阀值读数
    */
...
/*****************************************************************************
    功能描述     : SPI TX FIFO 中断响应
    输入参数     : p_hw_dev:SPI接口硬件配置
    返   回  值  : HI_ERR_XXX
*****************************************************************************/
hi_u16 spi_isr_write_fifo(spi_ctrl *spi_dev_ctrl)
{
    hi_pvoid write_buf = spi_dev_ctrl->tx_buf.buf;
    hi_u32 i;
    hi_u16 fix_data = SPI_UNUSE_DATA;
...
/*****************************************************************************
    功能描述     : SPI中断响应
    输入参数     : p_hw_dev:SPI接口硬件配置
    说    明     : 1.从软件buffer中获取数据, 写到FIFO中/从FIFO读数, 放到软件BUFFER
                   2.软件buffer空则关闭TX水线中断/软件BUFFER满则关闭RX水线中断
                   3.发送事件上层接口, 软件BUFFER有空间填写/通知上层接口可以读数据了
    返   回  值  : HI_ERR_XXX
*****************************************************************************/
hi_void spi_isr(spi_ctrl *spi_dev_ctrl)
{
    ...
    hi_reg_read16(reg_base + REG_SPI_MIS, int_val); /* 读取中断状态 */
    ...
    if (int_val & SPI_INT_BIT_RX_FIFO_TIME_OUT) {
        ...
        event_bits |= HI_EVENT_BIT_RX_DATA;
        clear_int_bits |= SPI_INT_BIT_RTIC; /* 超时中断手动清除 */
    }
    ...
    hi_event_send(spi_dev_ctrl->event_id, event_bits); /* 发送事件通知上层收数或发数 */
}

/*****************************************************************************
    功能描述     : SPI发送准备，具体内容由客户实现
    输入参数     : fd:设备句柄
                 : data:传输配置
                 : p_sw_dev:SPI接口软件配置
                 : p_hw_dev:SPI接口硬件配置
...
hi_void spi_trans_restore(spi_ctrl *spi_hw_ctrl, const spi_trans_attr *trans_attr)
{
    ...
    if ((trans_attr->trans_opt & SPI_OPT_TASKED_SIGNAL) && (trans_attr->trans_opt & SPI_OPT_FREE_SIGNAL)) {
        ...
        hi_sem_signal(spi_hw_ctrl->sem_id);
    }
