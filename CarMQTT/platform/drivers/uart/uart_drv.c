...
hi_u32 uart_init_circ_buf(uart_driver_data_t *udd, unsigned int rx_buf_size, unsigned int tx_buf_size)
{
  ...
  free_tx_circ_buf:
    LOS_MemFree(m_aucSysMem0, (VOID *) udd->tx_transfer);
    udd->tx_transfer = HI_NULL;

  free_rx_circ_buf:
    LOS_MemFree(m_aucSysMem0, (VOID *) udd->rx_transfer->data);
    udd->rx_transfer->data = HI_NULL;
  free_rx_transfer:
    LOS_MemFree(m_aucSysMem0, (VOID *) udd->rx_transfer);
...
hi_void uart_deinit_circ_buf(uart_driver_data_t *udd)
{
    if (udd->rx_transfer != HI_NULL) {
        if (udd->rx_transfer->data != HI_NULL) {
            LOS_MemFree(m_aucSysMem0, udd->rx_transfer->data);
            udd->rx_transfer->data = HI_NULL;
        }

        LOS_MemFree(m_aucSysMem0, udd->rx_transfer);
        udd->rx_transfer = HI_NULL;
    }

    if (udd->tx_transfer != HI_NULL) {
        if (udd->tx_transfer->data != HI_NULL) {
            LOS_MemFree(m_aucSysMem0, udd->tx_transfer->data);
            udd->tx_transfer->data = HI_NULL;
        }

        LOS_MemFree(m_aucSysMem0, udd->tx_transfer);
        udd->tx_transfer = HI_NULL;
    }
}
...
hi_void uart_dma_para_cfg(const uart_driver_data_t *udd, hi_dma_para *dma_para, const hi_char *buf)
{
...
hi_u32 uart_dma_recv_irq(uart_driver_data_t *udd, hi_u32 status, hi_char *buf)
{
    hi_u32 ret;
...
hi_void uart_nodma_recv_irq(uart_driver_data_t *udd, hi_u32 status, hi_char *buf)
{
    hi_u32 count = 0;
...
hi_void uart_send_irq(uart_driver_data_t *udd, hi_char *buf)
{
#ifdef CONFIG_UART_DEBUG_INFO
...
hi_u32 uart_drv_irq_start_tx(struct uart_driver_data *udd, const hi_char *buf, hi_u32 count)
{
