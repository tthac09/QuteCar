...
/* 256 bytes is too large for the stack, so using a global variable. */
static u8_t g_nat64_dname[DNS_MAX_NAME_LENGTH];
                                  ...
                                  u16_t *index)
{
  ...
  do {
    ...
    if ((((u8_t)n) & 0xC0) == 0xC0) {
      ...
      if (pointer >= qlen) {
        return nat64_dns64_dname_offset_nochange(p4, pointer, qlen, index);
      } else {
        g_nat64_dname[(*index)++] = (u8_t)n;
        g_nat64_dname[(*index)++] = (u8_t)m;
        return 1;
      }
      ...
      if ((*index > (DNS_MAX_NAME_LENGTH - n - 1)) ||
          (pbuf_copy_partial(p4, &g_nat64_dname[*index], (u16_t)(n + 1), (u16_t)(offset - 1)) != n + 1)) {
        return -1;
      }
                              ...
                              u16_t nameoffset, u16_t qlen, u16_t *offset6, struct pbuf *p6)
{
  ...
  if (memset_s(g_nat64_dname, DNS_MAX_NAME_LENGTH, 0, DNS_MAX_NAME_LENGTH) != EOK) {
    LWIP_DEBUGF(NAT64_DEBUG, ("dns64: memset_s g_nat64_dname failed\n"));
    return ERR_VAL;
  }
  /* find the compression offset, only when all offset is less then */
  ret = nat64_dns64_dname_offset_nochange(p4, nameoffset, qlen, &name_index);
  if (ret < 0) {
    LWIP_DEBUGF(NAT64_DEBUG, ("dns64: nat64_dns64_dname_offset_nochange failed\n"));
    return ERR_VAL;
  }

  if (pbuf_take_at(p6, g_nat64_dname, name_index, *offset6) != ERR_OK) {
