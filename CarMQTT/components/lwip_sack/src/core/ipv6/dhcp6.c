...
static err_t dhcp6_form_duid(struct netif *netif_p, struct dhcp6 *dhcp6_p, u8_t type);
static err_t dhcp6_create_iaid(struct netif *netif_p, struct dhcp6 *dhcp6_p, u8_t idx);
static err_t dhcp6_stateful_init(struct netif *netif_p, struct dhcp6 *dhcp6_p);
...
#define dhcp6_set_option(dhcp6, idx, start, len) do { \
...
dhcp6_set_struct(struct netif *netif, struct dhcp6 *dhcp6)
{
  ...
  (void)memset_s(dhcp6, sizeof(struct dhcp6), 0, sizeof(struct dhcp6));
...
dhcp6_get_struct(struct netif *netif, const char *dbg_requester)
{
  ...
  if (dhcp6->pcb == NULL) {
    ...
    dhcp6->pcb->ifindex = netif->ifindex;
#endif
...
dhcp6_stateful_enabled(struct dhcp6 *dhcp6)
{
  ...
  if (dhcp6_stateless_enabled(dhcp6) != 0) {
    return 0;
  }
  return 1;
}
 ...
 * Requests are sent after state of linklocal address is
 * IP6_ADDR_PREFERRED.
 *
...
dhcp6_enable_stateful(struct netif *netif)
{
  ...
  if (dhcp6_stateful_enabled(dhcp6) != 0) {
...
dhcp6_release_stateful(struct netif *netif)
{
  ...
  LWIP_DEBUGF(DHCP_DEBUG | LWIP_DBG_TRACE | LWIP_DBG_STATE, ("dhcp6_release_stateful(netif=%p) %c%c%"U16_F"\n",
                                                             (void *)netif,
                                                             netif->name[0],
                                                             netif->name[1],
                                                             (u16_t)netif->num));
  ...
  if ((dhcp6->state == DHCP6_STATE_STATEFUL_RENEW) ||
    ...
    netif_ip6_addr_set_parts(netif, 1, 0, 0, 0, 0);
                 ...
                 u16_t opt_len_alloc, u16_t *options_out_len)
{
  ...
  LWIP_ERROR("dhcp6_create_msg: opt_len_alloc to large", (opt_len_alloc <= (0xffff - sizeof(struct dhcp6_msg))),
             return NULL);
  p_out = pbuf_alloc(PBUF_TRANSPORT, (u16_t)(sizeof(struct dhcp6_msg) + opt_len_alloc), PBUF_RAM);
  if (p_out == NULL) {
    DHCP6_STATS_INC(dhcp6.memerr);
  ...
  if (dhcp6->tries == 0) {
    dhcp6->xid = (u32_t)LWIP_RAND() & 0xFFFFFF;
  }
  ...
  (void)memset_s(msg_out, sizeof(struct dhcp6_msg) + opt_len_alloc, 0, sizeof(struct dhcp6_msg) + opt_len_alloc);

  msg_out->msgtype = message_type;
  /* set dhcp6 transaction id which is three byte */
                           ...
                           u16_t num_req_options, u16_t max_len)
{
  ...
  LWIP_ASSERT("dhcp6_option_optionrequest: options_out_len + sizeof(struct dhcp6_msg) + addlen <= max_len",
              ((sizeof(struct dhcp6_msg) + options_out_len + DHCP6_OPT_HEADER_LEN +
                (sizeof(u16_t) * num_req_options)) <= max_len));
  LWIP_UNUSED_ARG(max_len);

  ret = dhcp6_option_short(options_out_len, options, DHCP6_OPTION_ORO);
  ret = dhcp6_option_short(ret, options, (u16_t)(sizeof(u16_t) * num_req_options));
...
#if LWIP_DHCP6_PROVIDE_DNS_SERVERS
static err_t
dhcp6_set_dns_servers(struct dhcp6 *dhcp6_p, struct pbuf *p_msg_in)
{
  ip_addr_t dns_addr;
  ...
  (void)memset_s(&dns_addr, sizeof(dns_addr), 0, sizeof(dns_addr));
...
#if LWIP_DHCP6_GET_NTP_SRV
static err_t
dhcp6_set_ntp_srv(struct dhcp6 *dhcp6_p, struct pbuf *p_msg_in)
{
...
dhcp6_information_request(struct netif *netif, struct dhcp6 *dhcp6)
{
  ...
  /* 255 : max dhcp6 information request times */
  if (dhcp6->tries < 255) {
    dhcp6->tries++;
  }
  /* 6 : Critical threshold 60: if more than Critical threshold we always get 60 */
  msecs = (u16_t)((dhcp6->tries < 6 ? 1 << dhcp6->tries : 60) * MS_PER_SECOND);
...
dhcp6_handle_config_reply(struct netif *netif, struct pbuf *p_msg_in)
{
  ...
  if (dhcp6_option_given(dhcp6, DHCP6_OPTION_IDX_DNS_SERVER)) {
    if (dhcp6_set_dns_servers(dhcp6, p_msg_in) != ERR_OK) {
  ...
  if (dhcp6_option_given(dhcp6, DHCP6_OPTION_IDX_NTP_SERVER)) {
    if (dhcp6_set_ntp_srv(dhcp6, p_msg_in) != ERR_OK) {
      DHCP6_STATS_INC(dhcp6.err);
      DHCP6_STATS_INC(dhcp6.drop);
      return;
    }
...
static err_t
dhcp6_form_duid(struct netif *netif_p, struct dhcp6 *dhcp6_p, u8_t type)
{
  duid_t *duid_p = NULL;
  u16_t len;
  err_t ret = ERR_OK;
...
static err_t
dhcp6_create_iaid(struct netif *netif_p, struct dhcp6 *dhcp6_p, u8_t idx)
{
  iaid_t *iaid_p = NULL;
...
static err_t
dhcp6_stateful_init(struct netif *netif_p, struct dhcp6 *dhcp6_p)
{
  err_t ret;
  u8_t i;
...
static u16_t
dhcp6_option_append(u16_t options_out_len, u8_t *options, u8_t option_type, u16_t op_len, u8_t *val)
{
...
dhcp6_set_timeout_params(struct dhcp6 *dhcp6_p)
{
  ...
  switch (dhcp6_p->state) {
    ...
    case DHCP6_STATE_STATEFUL_RENEW:
      ...
      if (dhcp6_p->ia[1].iaid.ia.dh6_ia_t2 == 0) {
        dhcp6_p->MRD = 900000; /* 900000: 15 min */
      } else if (dhcp6_p->ia[1].iaid.ia.dh6_ia_t2 == DHCP6_DURATION_INFINITE) {
        /* Renew forever, never hit Rebind */
        dhcp6_p->MRD = 0;
      } else {
        /* Remaining util T2 */
        dhcp6_p->MRD = (dhcp6_p->ia[1].iaid.ia.dh6_ia_t2 - dhcp6_p->ia[1].iaid.ia.dh6_ia_t1) * MS_PER_SECOND;
      }
      break;
    case DHCP6_STATE_STATEFUL_REBIND:
      dhcp6_p->IRT = REB_TIMEOUT * MS_PER_SECOND;
      dhcp6_p->MRT = REB_MAX_RT * MS_PER_SECOND;
      dhcp6_p->MRD = (dhcp6_p->ia[1].iaaddr.dh6_iaaddr_valid - dhcp6_p->ia[1].iaid.ia.dh6_ia_t2) * MS_PER_SECOND;
      break;
...
dhcp6_stateful_timeout(struct netif *netif_p, struct dhcp6 *dhcp6_p)
{
  ...
  switch (dhcp6_p->state) {
    ...
    case DHCP6_STATE_STATEFUL_IDLE:
      ...
      break;
    case DHCP6_STATE_STATEFUL_RENEW:
      if (((dhcp6_p->MRD != 0) && (dhcp6_retransmission_duration(dhcp6_p->duration_start) >= dhcp6_p->MRD)) ||
          ((dhcp6_p->MRC != 0) && (dhcp6_p->tries >= dhcp6_p->MRC))) {
    ...
    case DHCP6_STATE_STATEFUL_REBIND:
      if (((dhcp6_p->MRD != 0) && (dhcp6_retransmission_duration(dhcp6_p->duration_start) >= dhcp6_p->MRD)) ||
        ...
        /* the ip address is still in used? */
...
dhcp6_find_server(struct dhcp6 *dhcp6_p, u8_t *serv_duid, u16_t serv_duid_len)
{
  dhcp6_serverinfo_t *dhcp6_serverinfo_p = dhcp6_p->serv_list;
  opt_comm_t *opt_comm_p = NULL;

  while (dhcp6_serverinfo_p != NULL) {
    opt_comm_p = dhcp6_server_find_opt(dhcp6_serverinfo_p, DHCP6_OPTION_SERVERID);
    if (opt_comm_p != NULL) {
      if ((opt_comm_p->opt_len == serv_duid_len) && (memcmp(serv_duid, opt_comm_p->opt_start, serv_duid_len) == 0)) {
        break;
      }
    }
...
dhcp6_serverinfo_parse(dhcp6_serverinfo_t *dhcp6_serverinfo_p, u8_t *buff, u16_t buff_len)
{
  ...
  while (idx <= (buff_len - DHCP6_OPT_HEADER_LEN)) {
    op = (u16_t)((buff[idx] << 8) | buff[idx + 1]);
    len = (u16_t)((buff[idx + 2] << 8) | buff[idx + 3]);
...
static dhcp6_serverinfo_t *
dhcp6_serverinfo_new(struct pbuf *p_msg_in)
{
  ...
  if (buff_len > DHCP6_SERVERINFO_MAX_LEN) {
    DHCP6_STATS_INC(dhcp6.lenerr);
    LWIP_DEBUGF(DHCP6_DEBUG | LWIP_DBG_TRACE, ("drop too large dhcp6 message\n"));
    return NULL;
  }
  ...
  if (dhcp6_serverinfo_parse(dhcp6_serverinfo_p, dhcp6_serverinfo_p->buff, dhcp6_serverinfo_p->buff_len) != ERR_OK) {
...
static ia_t *
dhcp6_get_ia_by_iaid(struct dhcp6 *dhcp6_p, const u8_t *iaid)
{
  u8_t i;
  ia_t *ia_p = NULL;
  ...
  for (i = 0; i < LWIP_IPV6_NUM_ADDRESSES; i++) {
    ia_p = &(dhcp6_p->ia[i]);
    if (memcmp(&(ia_p->iaid.ia.dh6_ia_iaid), iaid, DHCP6_IAID_SIZE) == 0) {
      return ia_p;
    }
  }
...
{
  (void)memset_s(&dhcp6_p->ia[1].iaaddr, sizeof(dhcp6_p->ia[1].iaaddr), 0, sizeof(dhcp6_p->ia[1].iaaddr));
  dhcp6_p->ia[1].iaid.ia.dh6_ia_t1 = 0;
  dhcp6_p->ia[1].iaid.ia.dh6_ia_t2 = 0;
}

static void
dhcp6_rebind_failure_cleanup(struct netif *netif_p, struct dhcp6 *dhcp6_p)
{
  netif_ip6_addr_set_state(netif_p, 1, IP6_ADDR_INVALID);
  netif_ip6_addr_set_parts(netif_p, 1, 0, 0, 0, 0);
  dhcp6_clear_ia(dhcp6_p);
  (void)memset_s(dhcp6_rx_options, sizeof(dhcp6_rx_options), 0, sizeof(dhcp6_rx_options));
...
static void
dhcp6_handle_advertise(struct netif *netif_p, struct pbuf *p_msg_in)
{
  ...
  pbuf_p = pbuf_alloc(PBUF_RAW, LWIP_MEM_ALIGN_SIZE(PBUF_POOL_BUFSIZE), PBUF_POOL);
  if (pbuf_p == NULL) {
    DHCP6_STATS_INC(dhcp6.memerr);
    DHCP6_STATS_INC(dhcp6.drop);
    LWIP_DEBUGF(DHCP_DEBUG | LWIP_DBG_TRACE, ("dhcp6_handle_advertise(): pbuf_alloc failed"));
    goto exit;
  }
  ...
  u8_t *option_p = (u8_t *)(pbuf_p->payload);
  ...
  if ((op_len > pbuf_p->len) || (pbuf_copy_partial(p_msg_in, option_p, op_len, op_start) != op_len)) {
    /*
     * condition 1 : copy length must not be greater than the length of buffer
     * condition 2 : pbuf length mismatch
     */
    DHCP6_STATS_INC(dhcp6.lenerr);
    DHCP6_STATS_INC(dhcp6.drop);
      ...
      while (idx <= (op_len - DHCP6_OPT_HEADER_LEN)) {
        ...
        if (op == DHCP6_OPTION_IAADDR) {
          ...
          if (len > 24) {
            ...
            /* idx + 4 + 24 : go to del with Status code */
            sub_idx = (u16_t)(idx + 4 + 24);
            if ((sub_idx < idx) || (sub_idx > (op_len - 4))) {
              /* length mismatch */
              DHCP6_STATS_INC(dhcp6.lenerr);
              DHCP6_STATS_INC(dhcp6.drop);
            ...
            if ((u16_t)(sub_idx + 4 + sub_len) < sub_idx || (u16_t)(sub_idx + 4 + sub_len) > op_len) {
              /* length mismatch */
              DHCP6_STATS_INC(dhcp6.lenerr);
              DHCP6_STATS_INC(dhcp6.drop);
            ...
            if ((sub_op == DHCP6_OPTION_STATUS_CODE) && (sub_len >= 2)) {
              status_code = (u16_t)((option_p[sub_idx + 4] << 8) | option_p[sub_idx + 5]);
              if (status_code == DHCP6_STATUS_NOADDRSAVAIL) {
                /* NoAddrsAvail */
               ...
               */
              if (sub_op == DHCP6_OPTION_STATUS_CODE) {
                DHCP6_STATS_INC(dhcp6.proterr);
              } else {
                DHCP6_STATS_INC(dhcp6.lenerr);
              }
          ...
          /* length of option Status Code must be not less than 2 byte */
          if (len < 2) {
            DHCP6_STATS_INC(dhcp6.lenerr);
            DHCP6_STATS_INC(dhcp6.drop);
            LWIP_DEBUGF(DHCP6_DEBUG | LWIP_DBG_TRACE, ("invalid option STATUS_CODE len %u\n", len));
          ...
          status_code = (u16_t)((option_p[idx + 4] << 8) | option_p[idx + 5]);
          if (status_code == DHCP6_STATUS_NOADDRSAVAIL) {
            /* NoAddrsAvail */
  ...
  if ((op_len != dhcp6_p->duid.duid_len) || (op_len > pbuf_p->len) ||
      (pbuf_copy_partial(p_msg_in, option_p, op_len, op_start) != op_len) ||
      (memcmp(option_p, dhcp6_p->duid.duid_id, op_len) != 0)) {
    /*
     * condition 1 : op_len must be equal to the netif's duid_len
     * condition 2 : copy length must not be greater than the length of buffer
     * condition 3 : pbuf length mismatch
     * condition 4 : the content of option CLI_ID must be the same with the netif's duid
     */
    DHCP6_STATS_INC(dhcp6.err);
    DHCP6_STATS_INC(dhcp6.drop);
...
exit:
  if (pbuf_p != NULL) {
    (void)pbuf_free(pbuf_p);
  }
  return;
}

static void
dhcp6_handle_reply(struct netif *netif_p, struct pbuf *p_msg_in)
{
  struct dhcp6 *dhcp6_p = netif_dhcp6_data(netif_p);
  u16_t op_start, op_len;
  u8_t state;
  ...
  state = dhcp6_p->state;
  /* not support rapid_commit option yet */
  ...
  if (pbuf_p == NULL) {
    DHCP6_STATS_INC(dhcp6.memerr);
    DHCP6_STATS_INC(dhcp6.drop);
    LWIP_DEBUGF(DHCP_DEBUG | LWIP_DBG_TRACE, ("dhcp6_handle_reply(): pbuf_alloc failed"));
    goto exit;
  }

  option_p = (u8_t *)(pbuf_p->payload);
  ...
  /* todo : validate authentication (luminais mark) */
#if LWIP_DHCP6_PROVIDE_DNS_SERVERS
  if (dhcp6_option_given(dhcp6_p, DHCP6_OPTION_IDX_DNS_SERVER)) {
    if (dhcp6_set_dns_servers(dhcp6_p, p_msg_in) != ERR_OK) {
      DHCP6_STATS_INC(dhcp6.err);
      DHCP6_STATS_INC(dhcp6.drop);
      goto exit;
    }
  ...
  if (dhcp6_option_given(dhcp6_p, DHCP6_OPTION_IDX_NTP_SERVER)) {
    if (dhcp6_set_ntp_srv(dhcp6_p, p_msg_in) != ERR_OK) {
      DHCP6_STATS_INC(dhcp6.err);
      DHCP6_STATS_INC(dhcp6.drop);
      goto exit;
    }
  ...
  if (state != DHCP6_STATE_STATEFUL_RELEASE) {
    if (dhcp6_option_given(dhcp6_p, DHCP6_OPTION_IDX_IA_NA)) {
      ...
      op_start = dhcp6_get_option_start(dhcp6_p, DHCP6_OPTION_IDX_IA_NA);
      op_len = dhcp6_get_option_length(dhcp6_p, DHCP6_OPTION_IDX_IA_NA);
      /* the length of IA_NA field : 12 */
      if ((op_len > pbuf_p->len) || (op_len < 12) ||
          (pbuf_copy_partial(p_msg_in, option_p, op_len, op_start) != op_len)) {
        /*
         * condition 1 : copy length must not be greater than the length of buffer
         * condition 2 : opt length must not be less than the length of IA_NA field
         * condition 3 : pbuf length mismatch
         */
        DHCP6_STATS_INC(dhcp6.lenerr);
        DHCP6_STATS_INC(dhcp6.drop);
      ...
      ia_p = dhcp6_get_ia_by_iaid(dhcp6_p, &(option_p[0]));
      if (ia_p == NULL) {
        DHCP6_STATS_INC(dhcp6.proterr);
        DHCP6_STATS_INC(dhcp6.drop);
        LWIP_DEBUGF(DHCP6_DEBUG | LWIP_DBG_TRACE, ("no such iaid of netif\n"));
        goto exit;
      }
      /* validate t1/t2 for stateful? [RFC3315 22.4] */
      ...
      while (idx <= (op_len - 4)) {
        ...
        if (op == DHCP6_OPTION_IAADDR) {
          ...
          iaaddr_p = (struct dhcp6opt_iaaddr *)(&(option_p[idx + 4]));
          ...
          netif_ip6_addr_set_parts(netif_p, 1,
          ...
          netif_ip6_addr_set_state(netif_p, 1, IP6_ADDR_TENTATIVE);
          /* NoAddrsAvail */
  ...
  if (dhcp6_option_given(dhcp6_p, DHCP6_OPTION_IDX_SERVER_ID) &&
    ...
    if (dhcp6_serverinfo_p != NULL) {
      ...
      op_start = dhcp6_get_option_start(dhcp6_p, DHCP6_OPTION_IDX_UNICAST);
      if ((op_len != sizeof(ip6_addr_t)) || (op_len > pbuf_p->len) ||
          (pbuf_copy_partial(p_msg_in, option_p, op_len, op_start) != op_len)) {
        /*
         * condition 1 : opt length must match ipv6 addr length
         * condition 2 : copy length must not be greater than the length of buffer
         * condition 3 : pbuf length mismatch
         */
        DHCP6_STATS_INC(dhcp6.lenerr);
  ...
  if (state == DHCP6_STATE_STATEFUL_RELEASE) {
    ...
    dhcp6_p->request_timeout =
      (u16_t)((ia_p->iaid.ia.dh6_ia_t1 * MS_PER_SECOND + DHCP6_TIMER_MSECS - 1) / DHCP6_TIMER_MSECS);
    LWIP_DEBUGF(DHCP6_DEBUG | LWIP_DBG_TRACE | LWIP_DBG_STATE,
                ("dhcp6_handle_reply(): set request timeout %"U32_F" msecs\n",
                 ia_p->iaid.ia.dh6_ia_t1 * MS_PER_SECOND));
  }
...
void
dhcp6_dad_handle(struct netif *netif_p, ip6_addr_t *target_address)
{
  struct dhcp6 *dhcp6_p = netif_dhcp6_data(netif_p);
  ia_t *ia_p = NULL;
...
dhcp6_stateful_send(struct netif *netif_p, struct dhcp6 *dhcp6_p, u8_t message_type)
{
    ...
    switch (option_list[i]) {
      ...
      case DHCP6_OPTION_IA_NA:
        ...
        if (dhcp6_p->current_serv != NULL) {
          ...
          ia_option = (u8_t *)&((dhcp6_p->ia[1]).iaid.ia); /* NOPAD */
          ia_option_length = (dhcp6_p->ia[1]).iaid.iaid_len;
        }
  ...
  if (p_out != NULL) {
    ...
    if (dhcp6_p->tries == 0) {
      ...
      /*
       * when (dhcp6_p->tries == 255), elapsed_time MUST be greater than 0x0000ffff, but be smaller than (u32_t)(-1).
    ...
    if (dhcp6_p->MRD && (elapsed_time < dhcp6_p->MRD)) {
      ...
      /* what if elapsed_time is greater than dhcp6_p->MRD ? */
      remaining_timeout = 0;
    }

    i = 0;
    while (option_list[i] != 0) {
      switch (option_list[i]) {
        case DHCP6_OPTION_ELAPSED_TIME:
          /* 2: options len */
...
#if DHCP6_ENABLE_UNICAST_SUPPORT
    if ((dhcp6_p->current_serv != NULL) && (dhcp6_p->current_serv->unicast == true)) {
      ip_addr_t src_ip;
      IP_ADDR6(&src_ip,
               dhcp6_p->ia[1].iaaddr.dh6_iaaddr_addr.addr[0],
               dhcp6_p->ia[1].iaaddr.dh6_iaaddr_addr.addr[1],
               dhcp6_p->ia[1].iaaddr.dh6_iaaddr_addr.addr[2],
               dhcp6_p->ia[1].iaaddr.dh6_iaaddr_addr.addr[3]);
      DHCP6_STATS_INC(dhcp6.xmit);
      err = udp_sendto_if_src(dhcp6_p->pcb, p_out, &dhcp6_p->current_serv->unicast_addr, DHCP6_SERVER_PORT, netif_p,
                              &src_ip);
    } else {
#endif
      DHCP6_STATS_INC(dhcp6.xmit);
  ...
  /* 255 :retries of current request <= 255 */
  if (dhcp6_p->tries < 255) {
    dhcp6_p->tries++;
  }
  if (dhcp6_p->tries == 1) {
    ...
    dhcp6_p->RT = 2 * dhcp6_p->RT + (u32_t)(dhcp6_p->RT * DHCPV6_RT_RAND);
  }
  if ((dhcp6_p->MRT != 0) && (dhcp6_p->RT > dhcp6_p->MRT)) {
    /* [RFC3315 Section 14 ] */
    dhcp6_p->RT = dhcp6_p->MRT + (u32_t)(dhcp6_p->MRT * DHCPV6_RT_RAND);
  }
  if ((dhcp6_p->MRD != 0) && (remaining_timeout != 0) && (dhcp6_p->RT > remaining_timeout)) {
...
dhcp6_nd6_ra_trigger(struct netif *netif, u8_t managed_addr_config, u8_t other_config)
{
  ...
  if (dhcp6 != NULL) {
    if (dhcp6_stateless_enabled(dhcp6)) {
      LWIP_DEBUGF(DHCP6_DEBUG | LWIP_DBG_TRACE, ("receive RA m=%02x, o=%02x\n", managed_addr_config, other_config));
      if (other_config != 0) {
        (void)dhcp6_request_config(netif, dhcp6);
      } else {
...
dhcp6_parse_reply(struct pbuf *p, struct dhcp6 *dhcp6)
{
  ...
  while ((offset + DHCP6_OPT_HEADER_LEN <= offset_max)) {
    ...
    op = (u16_t)((op_len[0] << 8) | op_len[1]);
    len = (u16_t)((op_len[2] << 8) | op_len[3]);
    offset = (u16_t)(val_offset + len);
...
dhcp6_recv(void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *addr, u16_t port)
{
  ...
  xid = (u32_t)(reply_msg->transaction_id[0] << 16);
  xid |= (u32_t)(reply_msg->transaction_id[1] << 8);
