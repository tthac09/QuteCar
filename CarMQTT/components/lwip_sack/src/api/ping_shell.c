...
lwip_ping_usage(u32_t is_v6)
{
  ...
  PRINTK("Usage:"\
         "\n  %s"
         "\n  %s [-6] [-n cnt] [-w interval] [-l data_len] destination"
         "\n  %s [-I iface/ipaddress] [-t] [-w interval] destination"
         "\n  %s -k", ping, ping, ping, ping);
...
#ifdef CUSTOM_AT_COMMAND
    (void)hi_at_printf("Ping : recv id unmatch\n");
#else
    PRINTK("Ping: recv id unmatch %u %u\n", ctx->iecho_resp->id, ctx->iecho->id);
#endif
    return -1;
  }
...
#ifndef CUSTOM_AT_COMMAND
    perror("Ping: recv seqno unmatch\n");
#endif
    return -1;
  }
...
#endif
  ctx->timout_ms = LWIP_SHELL_CMD_PING_TIMEOUT;
...
#else
  PRINTK("Ping: destination unreachable ...\n");
#endif
  return 0;
}
...
ping_result_print(struct ping_run_ctx *ctx)
{
  ...
  if (ctx->succ_cnt > 0) {
    ...
    (void)hi_at_printf(", rtt min/avg/max = %d/%d/%d ms\r\n", ctx->rtt_min, rtt_ave, ctx->rtt_max);
    (void)hi_at_printf("\r\nOK\r\n");
  } else {
    (void)hi_at_printf("\r\nERROR\r\n");
  }
...
LWIP_STATIC int ping_setsockopt(struct ping_run_ctx *ctx, struct ping_cfg *cfg)
{
  ...
  if (cfg->src_type == LWIP_PING_HOSTNAME_ARG) {
    ...
    if (ret == -1) {
      ...
      (void)hi_at_printf("ping: bind icmp socket: cannot assign requested address\r\n");
#else
      PRINTK("ping: bind icmp socket: cannot assign requested address\n");
#endif
      return -1;
    }
  ...
  if (ctx->is_v6) {
    ...
    if (ret == -1) {
      PRINTK("ping : setsockopt: errno %d\n", errno);
      return -1;
    }
  }
...
os_ping_func(struct ping_cfg *cfg)
{
  ...
  while ((ping_kill == 0) && ((ctx.forever != 0) || (ctx.i < ctx.cnt))) {
    ...
    if (ret < 0) {
#ifdef CUSTOM_AT_COMMAND
      (void)hi_at_printf("Ping: sending ICMP echo request failed errno %d\n", errno);
#else
      PRINTK("Ping: sending ICMP echo request failed errno %d\n", errno);
#endif
      goto FAILURE;
    }
...
fail:
#if CUSTOM_AT_COMMAND
  (void)hi_at_printf("ping : Host : %s can't be resolved to IP address\n", daddr);
#else
  PRINTK("ping : Host : %s can't be resolved to IP address\n", daddr);
#endif
  return -1;
}
      ...
      if (strcmp(argv[i], handle[j].key) == 0) {
        ...
        if (handle[j].arg_num == 0) {
          *(handle[j].data) = 1;
          ...
          if (ret != 0) {
#ifdef CUSTOM_AT_COMMAND
            (void)hi_at_printf("Ping %s arg error\n", handle[j].key);
#else
            PRINTK("Ping %s arg error\n", handle[j].key);
#endif
            return -1;
          }
        } else {
...
LWIP_STATIC u32_t ping_exec(struct ping_cfg *cfg)
{
  ...
  if ((cfg->count == 0) || (cfg->count > LWIP_SHELL_CMD_PING_RETRY_TIMES)) {
    ...
    if ((ret == LOS_NOK) || (ping_taskid == 0)) {
      PRINTK("ping_task create failed\n");
      cfg->count = LWIP_SHELL_CMD_PING_RETRY_TIMES;
    } else {
      return LOS_OK;
    }
...
os_shell_ping(int argc, const char **argv)
{
  ...
  if (cfg == NULL) {
#ifdef CUSTOM_AT_COMMAND
    (void)hi_at_printf("%s: mem_malloc faild\n", __FUNCTION__);
#else
    PRINTK("%s: mem_malloc faild\n", __FUNCTION__);
#endif
    return LOS_NOK;
  }
  cfg->count = 0;
...
u32_t os_shell_ping6(int argc, const char **argv)
{
    ...
    if (argv6 == NULL) {
#ifdef CUSTOM_AT_COMMAND
      (void)hi_at_printf("%s: mem_malloc failed\n", __FUNCTION__);
#else
      PRINTK("%s: mem_malloc failed\n", __FUNCTION__);
#endif
      return LOS_NOK;
    }
