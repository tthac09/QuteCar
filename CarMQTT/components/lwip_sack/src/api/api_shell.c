...
#define PRINT_ERRCODE(x) PRINTK("ERR:%u\n", (x))
#endif
...
#ifdef CUSTOM_AT_COMMAND
  (void)hi_at_printf("\t%-8"DHCP_NUM_F"%-16s%-16s%-8hhu%-8hu%-8hhu%-8hu\n", dhcp_state->idx, macaddr,
...
#else
  PRINTK("\t%-8"DHCP_NUM_F"%-16s%-16s%-8hhu%-8hu%-8hhu%-8hu\n", dhcp_state->idx, macaddr,
         (dhcp_state->offered_ip_addr == 0) ? "0.0.0.0" : ip4addr_ntoa(&cli_ip),
         state, dhcp_state->lease_used, dhcp_state->tries, dhcp_state->request_timeout);
#endif
...
dhcp_clients_info_show(struct netif *netif_p)
{
  ...
  (void)hi_at_printf("\t%-8s%-16s%-16s%-8s%-8s%-8s%-8s\n", "mac_idx", "mac", "addr", "state", "lease", "tries", "rto");
#else
  PRINTK("clients <%"DHCP_NUM_F"> :\n", dhcp->cli_cnt);
  PRINTK("\t%-8s%-16s%-16s%-8s%-8s%-8s%-8s\n", "mac_idx", "mac", "addr", "state", "lease", "tries", "rto");
#endif
