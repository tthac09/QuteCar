...
/*****************************************************************************
 函 数 名  : at_strtoaddr
 功能描述  : 字符串转MAC地址
 输入参数  : param: MAC地址字符串, 格式 xx:xx:xx:xx:xx:xx
...
hi_u32 cmd_strtoaddr(const hi_char *param, hi_uchar *mac_addr, hi_u32 addr_len)
{
    ...
    for (cnt = 0; cnt < 17; cnt++) { /* MAC地址字符串占用17 byte */
    ...
    for (cnt = 0; cnt < (addr_len - 1); cnt++) {
        ...
        mac_addr[cnt] = (hi_uchar)strtoul(tmp2, &tmp3, 16); /* 16进制 */
    }

    if (tmp1 == NULL) {
        return HI_ERR_APP_INVALID_PARAMETER;
    }
    mac_addr[cnt] = (hi_uchar)strtoul(tmp1, &tmp3, 16); /* 16进制 */
    return HI_ERR_SUCCESS;
}
...
int at_hex2byte(const char *hex)
{
    ...
    return ((hi_u32)a << 4) | (hi_u32)b;    /* 左移4位 */
}
...
{
    static char ssid_txt[AT_SSID_MAX_LEN * 4 + 1];  /* 乘以4 */
