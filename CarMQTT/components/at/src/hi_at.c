...
#define SOFT_REBOOT_MAX_BUSY_CNT    20 /* AT连续返回busy次数超过20，则系统软复位 */

hi_u32 g_at_event = 0;
static hi_uart_idx g_at_uart_port = HI_UART_IDX_1;
static const hi_uart_attribute g_at_uart_cfg  = {115200, 8, 1, 0, 0};
...
hi_u8 g_busy_count = 0; /* AT连续返回busy次数计数 */
...
hi_u32 hi_at_register_input_func(hi_at_input_func at_input_func)
{
    ...
    if (last_uart_input == HI_TRUE) {
        return hi_uart_quit_read(g_at_uart_port);
    }
...
hi_u32 at_key_filter(hi_char ch, hi_u32 *index, hi_s32 *key_value)
{
    if (ch == '\b') { /* backspace */
        if (((*index) > 0) && ((*index) < (CMD_MAX_LEN - 1))) {
            g_at_buf[(*index) - 1] = '\0';
            (*index)--;
            (hi_void)serial_putc_at('\b');
            (hi_void)serial_putc_at(' ');
            (hi_void)serial_putc_at('\b');
        }
