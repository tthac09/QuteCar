...
hi_s32 cmd_mesh_auto_connect(hi_s32 argc, const hi_char* argv[])
{
    ...
    /* get auth_type */ /* 2 参数：认证类型 */
    if ((integer_check(argv[2]) == HI_ERR_FAILURE) || /* 参数2 */
        ((atoi(argv[2]) != HI_MESH_OPEN) && (atoi(argv[2]) != HI_MESH_AUTH)) || /* 参数2 */
        ((atoi(argv[2]) == HI_MESH_OPEN) && (argc != 3))) { /* 参数2为open场景时，固定3个参数个数 */
        return HI_ERR_FAILURE;
    }
...
hi_s32 cmd_mesh_set_rssi_threshold(hi_s32 argc, const hi_char* argv[])
{
    ...
    if ((usr_rssi_config < -127) || (usr_rssi_config > 10)) { /* rssi有效范围-127-10 */
        return HI_ERR_FAILURE;
    }
...
hi_s32 cmd_mesh_set_autonetwork_bw(hi_s32 argc, const hi_char* argv[])
{
    ...
    if ((argv[0] != HI_NULL) && (integer_check(argv[0]) != HI_ERR_SUCCESS)) { /* 0:第一个参数 */
        return HI_ERR_FAILURE;
    }

    if ((argv[0] != HI_NULL) && (atoi(argv[0]) == 5)) {           /* 5M ,0:第一个参数 */
        bw_value = HI_WIFI_BW_HIEX_5M;
    } else if ((argv[0] != HI_NULL) && (atoi(argv[0]) == 10)) {   /* 10M ,0:第一个参数 */
