...
hi_void wake_gpio_callback(hi_void *arg)
{
    /*  用户根据业务场景增加处理代码 */
    hi_unref_param(arg);
    hi_watchdog_feed(); /* 避免某些异常场景下,GPIO中断频繁上报触发看门狗，此处喂狗 */
...
hi_u32 check_io_status(hi_gpio_idx io_num)
{
    ...
    /* hi_io_get_func 传入枚举值io_num与预期一致 */
...
hi_u32 wake_gpio_init(hi_gpio_idx io_num)
{
    ...
    /* hi_io_set_func 传入枚举值io_num与预期一致 */
...
hi_u32 at_set_powersave(hi_s32 argc, const hi_char **argv)
{
    ...
    if (argc == 1) {
        ...
        if ((wifi_sleep_time < 33) || (wifi_sleep_time > 4000)) { /* 33 4000 wifi 睡眠时间上下限 */
            return HI_ERR_FAILURE;
        }
    }
