...
typedef struct {
    ...
    char custom_en : 1; /**< get beacon/probe response flag. CNcomment: 使能后STA扫描时接收上报beacon/probe response包.CNend */
...
*
* @attention  NULL
* @param  iftype        [IN]   Type  #hi_wifi_iftype, interface type,usually use HI_WIFI_IFTYPE_STATION or HI_WIFI_IFTYPE_AP.
                                      CNcomment:接口类型,一般只采用HI_WIFI_IFTYPE_STATION或者HI_WIFI_IFTYPE_AP。CNend
* @param  fram_type     [IN]   Type  #hi_wifi_frame_type, frame type，HI_WIFI_IFTYPE_STATION iftype only supports Probe Request.
*                                     CNcomment:帧类型, HI_WIFI_IFTYPE_STATION 接口类型只支持Probe Request帧。CNend
* @param  usr_ie_type   [IN]   Type  #usr_ie_type, user IE type, default set zero.CNcomment:用户IE类型，默认设为0。CNend
* @param  ie            [IN]   Type  #const unsigned char *, user IE value.CNcomment:用户IE字段内容。CNend
* @param  ie_len        [IN]   Type  #unsigned short, user IE length.CNcomment:用户IE字段内容长度。CNend
* @retval #HISI_OK      Execute successfully.
* @retval #HISI_FAIL    Execute failed.
* @par Dependency:
*            @li hi_wifi_api.h: WiFi API
...
*             2.Channel set to 0 indicates no channel specified,otherwise STA will scan on the specified channel firstly.\n
*             CNcomment:2.不指定信道时channel参数可设置为0,如果指定信道则优先在指定信道扫描，指定信道扫描不到则全信道扫描.CNend
*             3.When start anchor function, it will report beacon.CNcomment:3.调用了开始锚点的函数则会开始上报对应AP的Beacon帧.CNend
*             4.When anchor AP parameter changed and need to restart anchor , it should restart STA and then restart anchor.\n
*             CNcomment:4.当需要重启锚点功能或者AP锚点参数发生变化(信道等),应该重启STA并再次调用该锚点 API.CNend
*             5.If anchored AP is not started, this API cannot report Beacon.\n
*             CNcomment:5.如果锚点AP没有开启，此API不能上报Beacon帧.CNend
*
* @param  bssid        [IN]     Type #unsigned char*, ap's bssid. CNcomment:ap的bssid.CNend
* @param  bssid_len    [IN]     Type #unsigned char, bssid length.CNcomment:bssid长度.CNend
* @param  channel      [IN]     Type #unsigned char, ap's channel.CNcomment:ap的信道.CNend
* @param  cb           [IN]     Type #hi_wifi_anchor_cb, callback function.CNcomment:用户实现供驱动调用的接收beacon处理函数.CNend
*
* @retval #HISI_OK  Excute successfully
* @retval #Other    Error code
*
