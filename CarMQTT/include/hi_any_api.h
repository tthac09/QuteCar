...
* Description: header file for ANY api.CNcomment:描述：ANY api接口头文件。CNend\n
* Author: Hisilicon \n
* Create: 2019-01-03
*/
 ...
 */
typedef struct {
    unsigned char mac[WIFI_ANY_MAC_LEN];  /**< 收发操作的对端MAC地址 */
    unsigned char channel;                /**< 对端所在信道号，驱动对该信息仅做存储，0xFF表示当前信道 */
    bool          has_key;                /**< 是否配置通信密钥，是则填true，并在key中传入密钥，否则填false */
    unsigned char key[WIFI_ANY_KEY_LEN];  /**< 用于加密通信的密钥，填充的密钥固定为16字节长度 */
 ...
 */
typedef struct {
    unsigned char bssid[WIFI_ANY_MAC_LEN];      /**< BSSID,若对端为STA则为其MAC地址 */
    unsigned char channel;                      /**< 信道号，取值范围1-14，不同区域取值范围有差异 */
    unsigned char sta_flag;                     /**< true表示对端是STA，否则为普通AP */
    unsigned char ssid[WIFI_ANY_MAX_SSID_LEN];  /**< SSID */
    unsigned char ssid_len;                     /**< SSID字符串长度 */
    char          rssi;                         /**< 信号强度 */
 ...
 */
typedef struct {
    hi_wifi_any_send_complete_cb send_cb; /**< ANY发送完成驱动回调函数，用于将驱动发送结果反馈给上层 */
    hi_wifi_any_recv_cb recv_cb;          /**< ANY接收数据驱动回调函数，用于将驱动接收的数据传递给上层 */
