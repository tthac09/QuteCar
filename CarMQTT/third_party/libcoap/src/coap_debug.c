...
#ifdef WITH_LWIP
# define fprintf(fd, fmt, args...) LWIP_PLATFORM_PRINT(fmt, ##args)
# define fflush(...)
#endif
...
static coap_log_t maxlog = 9;        /* default maximum log level */
...
static unsigned char pdu_buf[1024]; /* need some space for output creation */
...
coap_show_pdu(coap_log_t level, const coap_pdu_t *pdu) {
  ...
  while ((option = coap_option_next(&opt_iter))) {
    ...
    case COAP_SIGNALING_OPTION_MAX_MESSAGE_SIZE:
      buf_len = snprintf((char *)pdu_buf, sizeof(pdu_buf), "%u",
    ...
    case COAP_OPTION_CONTENT_FORMAT:
      ...
      buf_len = print_content_format(content_format, pdu_buf, sizeof(pdu_buf));
      break;
    ...
    case COAP_OPTION_BLOCK2:
      ...
      buf_len = snprintf((char *)pdu_buf, sizeof(pdu_buf), "%u/%c/%u",
  ...
  if (coap_get_data(pdu, &data_len, &data)) {
    ...
    if (is_binary(content_format)) {
      ...
      if (print_readable(data, data_len, pdu_buf, sizeof(pdu_buf), 0)) {
        outbuflen = strlen(outbuf);
        snprintf(&outbuf[outbuflen], sizeof(outbuf)-outbuflen,  "'%s'", pdu_buf);
      }
    }
  }
