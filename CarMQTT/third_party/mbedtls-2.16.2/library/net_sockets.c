...
#include "lwip/sockets.h"
    ...
    !defined(__HAIKU__) && !defined(__LITEOS__)
...
#else
#if !defined(EFIX64) && !defined(EFI32) && !defined (__LITEOS__)
                         ...
                         const char *port, int proto )
{
    ...
    for( cur = addr_list; cur != NULL; cur = cur->ai_next )
    {
        ...
        {
              struct timeval tv;
              tv.tv_sec = 10;   /* according to cloud, timeout set 10s. */
              tv.tv_usec = 0;

              if ( setsockopt(ctx->fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(struct timeval)) != 0 )
              {
#if defined (__LITEOS__)
                lwip_close( ctx->fd );
#else
...
int mbedtls_net_set_block( mbedtls_net_context *ctx )
{
    ...
    u_long n = 0;
#if defined (__LITEOS__)
    return( lwip_ioctl( ctx->fd, FIONBIO, &n ) );
#else
