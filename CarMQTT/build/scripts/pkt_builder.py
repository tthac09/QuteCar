    ...
    if args >= 6 and sys.argv[1] in list:
        ...
        if type == 'boot_ota':
            ...
            if os.path.exists(factory_bin):
                list = ['%s|0|0|0'%loader_bin, '%s|0|0|3'%efuse_bin, '%s|0|%d|1'%(target, 0x200000), '%s|%d|%d|6'%(factory_bin, 0x14D000, 0x96000), '%s|%d|%d|1'%(boot_b, 0x200000 - boot_b_size, boot_b_size)] if efuse_bin!=None else ['%s|0|0|0'%loader_bin, '%s|0|%d|1'%(target, 0x200000), '%s|%d|%d|6'%(factory_bin, 0x14D000, 0x96000), '%s|%d|%d|1'%(boot_b, 0x200000 - boot_b_size, boot_b_size)]
