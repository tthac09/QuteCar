...
static inline void *
pbuf_header_pointer(struct pbuf *pbuf, u16_t offset, u16_t len, void *buffer)
{
...
static inline void *
load_pointer(struct pbuf *pbuf, u16_t k, u16_t size, void *buffer)
{
 ...
 */
u32_t
sock_run_filter(struct pbuf *pbuf, struct sock_filter *filter, u16_t len)
{
  void *ptr = NULL;
  u32_t X = 0;
  u32_t A = 0;
 ...
 */
s32_t
sock_filter(struct netconn *conn, struct pbuf *pbuf)
{
  s32_t err = ERR_OK;
  u32_t pkt_len;
 ...
 */
s32_t
sock_check_filter(struct sock_filter *filter, s32_t len)
{
  struct sock_filter *entry = NULL;
  s32_t pc;

  if ((len < 1) || (len > LSF_MAXINSNS) || (filter == NULL)) {
    return EINVAL;
  }
 ...
 */
s32_t
sock_attach_filter(struct sock_fprog *fprog, struct netconn *conn)
{
  if ((fprog == NULL) || (fprog->filter == NULL) || (fprog->len == 0) || (conn == NULL)) {
    return EINVAL;
  }
...
s32_t
sock_detach_filter(struct netconn *conn)
{
  s32_t ret = ENOENT;
