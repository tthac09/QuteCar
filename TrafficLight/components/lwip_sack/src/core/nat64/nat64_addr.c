...
int
nat64_addr_mac_to6(const linklayer_addr_t *mac, ip6_addr_t *ip6addr)
{
  ip6_addr_t prefix;
  uint8_t len = 0;
  err_t err;

  if ((mac == NULL) || (ip6addr == NULL)) {
    return -1;
  }
  (void)memset_s(&prefix, sizeof(ip6_addr_t), 0, sizeof(ip6_addr_t));
  err = lwip_rpl_get_default_prefix(&prefix, &len);
  ...
  (void)len;
  ip6_addr_copy_ptr(ip6addr, &prefix);
  lwip_rpl_set_addr_iid(ip6addr, mac);
...
nat64_entry_to6(const nat64_entry_t *entry, ip6_addr_t *ip6addr)
{
  ...
  (void)len;
  ip6_addr_copy_ptr(ip6addr, &prefix);
  lwip_rpl_set_ip6_iid(&entry->mac, entry->mnid, ip6addr);
