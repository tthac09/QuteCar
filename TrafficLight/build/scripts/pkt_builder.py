...
class ImageBuild:
    ...
    def dual_partion_ota_work(self, signature = None):
        self.BuildUpgBoot()
        #制作升级文件和烧写文件先后顺序不能调整
        #The sequence of preparing upgrade files and burning files cannot be adjusted.
        #self.set_encrypt_flag(0xFF) #顺序不能调整,需要在编译flupgbin结束后设置
        #The sequence cannot be adjusted. You need to set the sequence after the Flugbin compilation is complete.
        self.BuildUpgBin()
        self.BuildHiburnBin()
    ...
    def compress_ota_work(self, signature = None):
        ...
        #The sequence cannot be adjusted. You need to set the sequence after the Upgbin compilation is complete.
    ...
    def BuildCompressUpgBin(self, compress_ota_bin = None, ota_file = None):
        ...
        #制作压缩升级文件依赖文件
        #Creating a Dependent File for Compressing Upgrade Files
...
if __name__ == '__main__':
    ...
    if args >= 6 and sys.argv[1] in list:
        ...
        if type == 'boot_ota':
            ...
            #证书安全存储示例
            #Example of Secure Certificate Storage
            ...
            #tee_cert_key_bin_max必须为4KB证书倍，需匹配分区表确定烧写地址和大小
            #The tee_cert_key_bin_max must be 4 KB-aligned. The burning address and size must be determined by matching the partition table.
            tee_cert_key_bin_max = 12*1024;
            tee_total_file_cnt = 3; #3: two certificates and one key.
            burn_tee_cert = False
            ...
            if (burn_tee_cert == True):
                ...
                if (total_cert_key_size > tee_cert_key_bin_max - 4 - 4 - 4*tee_total_file_cnt):
                    ...
                    cert_key_bin[0:4] = start_flag_bytes #Fill Magic Number
                    tee_total_file_cnt_bytes = tee_total_file_cnt.to_bytes(4, byteorder = 'little', signed = True)
                    cert_key_bin[4:8] = tee_total_file_cnt_bytes #Fill total number of files
                    #填充各文件的大小
                    #Fill in the size of each file.
