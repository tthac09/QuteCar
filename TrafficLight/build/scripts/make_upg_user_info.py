...
#The upgrade file header provides 32-byte reserved fields for users.
#示例为默认填充了8字节,用于存储芯片产品类型;如不需要,屏蔽即可;或者填充其他内容。
#In the example, 8 bytes are filled by default to store the chip product type.
#如不使用用例所示填充,屏蔽即可;或者填充其他内容。
...
def fill_user_info(chip_name):
    ...
    info.chip_product = bytes(chip_name, encoding='utf8')#If the chip product field does not need to be filled, this line should be shielded.
