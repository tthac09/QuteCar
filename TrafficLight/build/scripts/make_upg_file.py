...
def print_upg_info(image_file, upg_bin):
    ...
    #Displays the key information, signature information, and signature information of the common section.
    ...
    #打印section段固定头信息
    #Prints the fixed header information of the SECTION section.
...
def make_sha256_unsecure_signature(common_head_content, section_content):
    ...
    #section段非安全签名
    #Non-secure signature of section field
...
def make_upg_only(image_id, file_ver, encrypt_flag, file_type, section_bin_list, section_compress_list, alg_dir_path, max_size, sign_alg, chip_name, file_attr):
    ...
    #Common key area size offset and fixed header signature size offset
    #(The maximum value is used for different signature algorithms (the key is 288 bytes/signature is 256 bytes).
    # Set this parameter to the actual length, and set the extra part to 0).
    if sign_alg < 0x10:
        #RSA2048
    ...
    #Signature size offset and key information filling. The padding size varies according to the signature algorithm.
    if sign_alg < 0x10:
    ...
    #Size and offset of the padding field: AES encryption requires 16-byte alignment.
    ...
    #填充section段固定头
    #Fill in the section field header.
    ...
    #填充common段key
    #Fill in the key in the common field.
    ...
    #Fill in the common field signature and section field signature.
    #(The maximum value of each signature algorithm is 256 bytes. The actual length is filled in bytes and the extra part is filled with 0.)
    if sign_alg == 0x0:
...
def make_hupg(image_id, file_ver, encrypt_flag, kernel_file, normal_nv_file, image_file, alg_dir_path, max_size, sign_alg, chip_name, file_attr):
    ...
    if encrypt_flag != 0x42:
        ...
        #双分区升级文件需要加密
        #The dual-partition upgrade file needs to be encrypted.
...
def make_compress_hupg(image_id, file_ver, encrypt_flag, upg_file, image_file, alg_dir_path, kernel_upg_max_size, sign_alg, lzma_tool, build_tmp, chip_name, file_attr):
    ...
    #计算压缩文件大小上限
    #Calculate the maximum size of a compressed file.
...
def make_hbin(flash_boot_file, factory_nv_file, normal_nv_file, upg_file, image_file):
    ...
    #Obtaining Information from the Partition Table
    boot_st_addr = 0 #BOOT 32KB
    boot_size = 0x8000
    ftm1_st_addr = 0x8000 #factory NV 4KB+4KB
    ftm1_size = 0x1000
    ftm2_st_addr = 0x9000
    ftm2_size = 0x1000
    nv_file_st_addr = 0xa000 #NV Workspace 4KB+4KB
    nv_file_size = 0x1000
    nv_file_origin_st_addr = 0xc000 #Original backup in the NV zone
    ...
    #boot_nv_kernel_bin[ftm2_st_addr:ftm2_st_addr + ftm2_size] = factory_nv_bin Currently, only ftm1 is stored.
    boot_nv_kernel_bin[nv_file_st_addr:nv_file_st_addr + nv_file_size] = normal_nv_bin
    boot_nv_kernel_bin[nv_file_origin_st_addr:nv_file_origin_st_addr + nv_file_size] = normal_nv_bin #Original backup in the NV zone
...
def make_upg_boot_only(image_id, file_ver, encrypt_flag, file_type, section_bin, alg_dir_path, max_size, sign_alg, chip_name, file_attr):
    ...
    print('[upg_file_sign_offset=%d][upg_file_sign_size=%d]'%(upg_file_sign_offset, upg_file_sign_size))
    #填充大小和偏移
    #Size and offset of the padding field
    ...
    #镜像大小和偏移
    #Size and offset of each image
    (section_offset, section_size) = make_part_offset_size(temp_offset, len(section_bin))#size of the boot part.
    ...
    #填充common段固定头
    #Fill in the fixed head in the common field.
    common_head_bin = upg_bin[common_head_offset:common_head_offset + common_head_size]
    common_head = hi_upg_common_head.from_buffer(common_head_bin)
    common_head.image_id = image_id
    common_head.struct_version = 0
    ...
    #填充section段固定头
    #Fill in the fixed head in the section field.
    section_head_bin = upg_bin[section_head_offset:section_head_offset+section_head_size]
    section_head = hi_upg_section_head.from_buffer(section_head_bin)
    section_head.image_id = image_id
    section_head.struct_version = 0
