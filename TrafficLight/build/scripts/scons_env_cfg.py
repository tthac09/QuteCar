...
class SconsEnvCfg:
    ...
    def get_lib_path(self):
        ...
        for param in env_path_param:
            if (param.find('hcc_riscv32') >= 0) and (param.endswith('bin') == True):
                compiler = os.path.join(param, 'riscv32-unknown-elf-gcc.exe')
    ...
    def get_ohos_libs(self):
        ...
        prefix "lib" and surffix ".a", has to remove those parts from a library file name.
        """
        libs = [lib[3:-2] for lib in os.listdir(os.path.join(self.root, 'components', 'OHOS', 'ndk', 'libs'))
                if lib.startswith('lib') and lib.endswith('.a') and not lib.startswith('libActs')
                and not lib.endswith('Test.a')]
        return libs

    def get_xts_libs(self):
        libs = [lib[3:-2] for lib in os.listdir(os.path.join(self.root, 'components', 'OHOS', 'ndk', 'libs'))
                if (lib.startswith('libActs') and lib.endswith('Test.a'))]
        print("the lib path is {}".format(
            os.path.join(self.root, 'components', 'OHOS', 'ndk', 'libs')))
        return libs
