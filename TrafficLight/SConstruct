...
env = Environment(ENV={'PATH':os.environ['PATH']},
                  TARGET_PREFIX = env_cfg.target_name, tools=['mingw'])
...
if scons_utils.scons_get_cfg_val('CONFIG_HILINK') != 'y':
    if scons_utils.scons_get_cfg_val('CONFIG_OHOS') == 'y':
        env['LIBS'] += list(map(lambda x:'-l%s'%x, env_cfg.get_ohos_libs()))
...
if scons_utils.scons_get_cfg_val('CONFIG_HILINK') != 'y':
    if scons_utils.scons_get_cfg_val('CONFIG_OHOS') == 'y':
        env['LIBS'] += ['--whole-archive'] + list(map(lambda x:'-l%s'%x, env_cfg.get_xts_libs())) + ['--no-whole-archive']
...
def burn_bin_builder(target, source, env):
    ...
    #必须为4KB整数倍，需匹配分区表确定烧写地址和大小
    #The value must be 4 KB-aligned. The burning address and size must be determined by matching the partition table.
    ...
    #Adapt the burning address and size based on the partition table
