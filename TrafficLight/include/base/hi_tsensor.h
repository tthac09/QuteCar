...
*
* @attention
* @li By default, the Tsensor function is enabled for the Wi-Fi module. If this function is invoked, the
...
* @li The Tsensor function is strongly coupled with the chip temperature protection service. Tsensor enable status
*     is checked every 100s. If the Tsensor is disabled or destroyed, it will be restarted. Users are not advised
*     to disable the Tsensor by calling hi_tsensor_stop.CNcomment:Tsensor功能和芯片温度保护业务强耦合，系统定时100s
*     周期检查Tsenor开关状态，如果Tsensor被关闭或销毁，则会重启Tsensor，如非必要，建议用户不要调用hi_tsensor_stop接口
*     关闭Tesnsor。CNend
* @param  None
*
* @retval None
...
*
* @attention
* @li After this API is called, if the interrupt callback function is used, you need to invoke the corresponding
...
* @li The Tsensor function is strongly coupled with the chip temperature protection service. Tsensor enable status
*     is checked every 100s. If the Tsensor is disabled or destroyed, it will be restarted. Users are not advised
*     to destroy the Tsensor by calling hi_tsensor_destroy.CNcomment:Tsensor功能和芯片温度保护业务强耦合，系统定时100s
*     周期检查Tsenor开关状态，如果Tsensor被关闭或销毁，则会重启Tsensor，如非必要，建议用户不要调用hi_tsensor_destroy
*     接口销毁Tesnsor。CNend
*
* @param None
*
