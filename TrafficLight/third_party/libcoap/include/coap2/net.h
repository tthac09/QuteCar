...
#ifdef WITH_LWIP
/**
 * This API is called when the caller is not in the same thread with tcpip thread.
 * Creates a new coap_context_t object that will hold the CoAP stack status.
 */
coap_context_t *coap_new_context_lwip(const coap_address_t *listen_addr);
#endif /* WITH_LWIP */
 ...
 * @param context The current coap_context_t object to free off.
 */
void coap_free_context_lwip(coap_context_t *context);
#endif /* WITH_LWIP */
...
#ifdef WITH_LWIP
/**
 * This API is called when the caller is not in the same thread
 * with tcpip thread.
 * Sends an error response with code @p code for request @p request to @p dst.
 * @p opts will be passed to coap_new_error_response() to copy marked options
 * from the request. This function returns the transaction id if the message was
 ...
 *                        COAP_INVALID_TID otherwise.
 */
coap_tid_t coap_send_error_lwip(coap_session_t *session,
                           coap_pdu_t *request,
                           unsigned char code,
                           coap_opt_filter_t opts);
 ...
 *                        COAP_INVALID_TID on error.
 */
coap_tid_t coap_send_ack_lwip(coap_session_t *session, coap_pdu_t *request);
 ...
 */
COAP_STATIC_INLINE coap_tid_t
coap_send_rst_lwip(coap_session_t *session, coap_pdu_t *request) {
  return coap_send_message_type_lwip(session, request, COAP_MESSAGE_RST);
}
#endif
...
* with tcpip thread.
* Sends a CoAP message to given peer. The memory that is
* allocated by pdu will be released by coap_send_lwip().
* The caller must not use the pdu after calling coap_send_lwip().
*
* @param session         The CoAP session.
* @param pdu             The CoAP PDU to send.
*
...
*                        COAP_INVALID_TID on error.
*/
coap_tid_t coap_send_lwip( coap_session_t *session, coap_pdu_t *pdu );
#endif /* WITH_LWIP */
...
#ifdef WITH_LWIP
/**
 * Check if there any sessions which created by server has been idle without send/receive any
 * packets. The duration time of idle can be setted through ctx->session_timeout (unit is second).
 * if ctx->session_timeout is 0, the default duration time is defined by COAP_DEFAULT_SESSION_TIMEOUT.
 */
 ...
 * if ctx->session_timeout is 0, the default duration time is defined by COAP_DEFAULT_SESSION_TIMEOUT.
 */
void coap_server_sessions_check_idle_lwip(coap_context_t *ctx);
#endif
