...
#ifdef WITH_LWIP
coap_pdu_t *
coap_pdu_from_pbuf(struct pbuf *pbuf, coap_pdu_t *recv_pdu)
{
  coap_pdu_t *pdu;

  if ((pbuf == NULL) || (recv_pdu == NULL)) return NULL;

  LWIP_ASSERT("Can only deal with contiguous PBUFs", pbuf->tot_len == pbuf->len);
  LWIP_ASSERT("coap_read needs to receive an exclusive copy of the incoming pbuf", atomic_read(&pbuf->ref) == 1);

  pdu = recv_pdu;

  (void)memset_s(pdu, sizeof(coap_pdu_t), 0, sizeof(coap_pdu_t));
...
coap_pdu_init(uint8_t type, uint8_t code, uint16_t tid, size_t size) {
  ...
  if (pdu == NULL) {
    coap_log(LOG_EMERG, "coap_pdu_init: malloc pdu\n");
    return NULL;
  }
  ...
  if (buf == NULL) {
    coap_log(LOG_EMERG, "coap_pdu_init: malloc pdu buf\n");
    coap_free_type(COAP_PDU, pdu);
    return NULL;
  }
