...
coap_add_attr(coap_resource_t *resource,
  ...
  if (attr) {
    (void)memset_s(attr, sizeof(coap_attr_t), 0, sizeof(coap_attr_t));
...
#ifdef WITH_LWIP
static err_t
coap_check_notify_lwip_fn(struct netif *netif, void *arg)
{
  (void)netif;
...
void
coap_check_notify_lwip(coap_context_t *context) {
  if (context == NULL) {
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_check_notify_lwip: invalid param\n");
#endif
    return;
  }
  (void)netifapi_call_argcb(coap_check_notify_lwip_fn, (void *)context);
}
#endif
