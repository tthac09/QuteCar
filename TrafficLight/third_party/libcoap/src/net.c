...
#ifdef WITH_LWIP
static err_t
do_coap_context_lwip(struct tcpip_api_call_data *m)
{
  struct coapapi_msg *msg = (struct coapapi_msg*)(void*)m;
  if (msg->msg.ctx.cb == coap_new_context) {
    msg->msg.ctx.context = coap_new_context(msg->msg.ctx.addr);
  } else if (msg->msg.ctx.cb == coap_free_context) {
    coap_free_context(msg->msg.ctx.context);
    msg->msg.ctx.context = NULL;
  } else {
    return ERR_ARG;
  }
  return ERR_OK;
...
coap_new_context_lwip(
  ...
  struct coapapi_msg *msg = NULL;
  err_t err;

  msg = (struct coapapi_msg *)mem_malloc(sizeof(struct coapapi_msg));
  if (msg == NULL) {
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_new_context_lwip: malloc\n");
#endif
    return NULL;
  }

  (void)memset_s(msg, sizeof(struct coapapi_msg), 0x0, sizeof(struct coapapi_msg));
  msg->msg.ctx.context = NULL;
  msg->msg.ctx.addr = listen_addr;
  msg->msg.ctx.cb = (void *)coap_new_context;

  err = tcpip_api_call(do_coap_context_lwip, &(msg->call));
  if (err == ERR_OK) {
    c = msg->msg.ctx.context;
  }
...
void
coap_free_context_lwip(coap_context_t *context) {
  struct coapapi_msg *msg = NULL;
  err_t err;
  ...
  if (msg == NULL) {
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_free_context_lwip: malloc\n");
#endif
    return;
  }

  (void)memset_s(msg, sizeof(struct coapapi_msg), 0x0, sizeof(struct coapapi_msg));
  msg->msg.ctx.context = context;
  msg->msg.ctx.addr = NULL;
  msg->msg.ctx.cb = (void *)coap_free_context;
...
err_t do_coap_send_lwip(struct tcpip_api_call_data *m) {
  struct coapapi_msg *msg = (struct coapapi_msg*)(void*)m;

  if (msg->msg.send.cb == coap_send) {
    msg->msg.send.tid = coap_send(msg->msg.send.session, msg->msg.send.pdu);
  } else if (msg->msg.send.cb == coap_send_ack) {
    msg->msg.send.tid = coap_send_ack(msg->msg.send.session, msg->msg.send.pdu);
  } else if (msg->msg.send.cb == coap_send_error) {
    msg->msg.send.tid = coap_send_error(msg->msg.send.session, msg->msg.send.pdu,
      msg->msg.send.type_code, msg->msg.send.opts);
  } else if (msg->msg.send.cb == coap_send_message_type) {
    msg->msg.send.tid = coap_send_message_type(msg->msg.send.session, msg->msg.send.pdu,
      msg->msg.send.type_code);
  } else {
    return ERR_ARG;
  }
...
coap_tid_t
coap_send_lwip(coap_session_t *session, coap_pdu_t *pdu) {
  struct coapapi_msg *msg = NULL;
...
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_send_lwip: invalid param\n");
#endif
  ...
  if (msg == NULL) {
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_send_lwip: malloc\n");
#endif
    return COAP_INVALID_TID;
  }

  (void)memset_s(msg, sizeof(struct coapapi_msg), 0x0, sizeof(struct coapapi_msg));
  msg->msg.send.session = session;
  msg->msg.send.pdu = pdu;
  msg->msg.send.tid = COAP_INVALID_TID;
  msg->msg.send.cb = (void *)coap_send;

  err = tcpip_api_call(do_coap_send_lwip, &(msg->call));
  if (err == ERR_OK) {
    tid = msg->msg.send.tid;
  }
  mem_free(msg);
...
coap_tid_t
coap_send_ack_lwip(coap_session_t *session, coap_pdu_t *request)
{
  struct coapapi_msg *msg = NULL;
  coap_tid_t tid = COAP_INVALID_TID;
  err_t err;
...
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_send_ack_lwip: invalid param\n");
#endif
    return COAP_INVALID_TID;
  }
  ...
  if (msg == NULL) {
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_send_ack_lwip: malloc\n");
#endif
    return COAP_INVALID_TID;
  }
  ...
  (void)memset_s(msg, sizeof(struct coapapi_msg), 0x0, sizeof(struct coapapi_msg));
  msg->msg.send.session = session;
  msg->msg.send.pdu = request;
  msg->msg.send.tid = COAP_INVALID_TID;
  msg->msg.send.cb = (void *)coap_send_ack;
...
coap_tid_t
coap_send_error_lwip(coap_session_t *session,
                     coap_pdu_t *request,
                     unsigned char code,
                     coap_opt_filter_t opts)
{
  ...
  if ((session == NULL) || (request == NULL)) {
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_send_error_lwip: invalid param\n");
#endif
    return COAP_INVALID_TID;
  }
  ...
  if (msg == NULL) {
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_send_error_lwip: malloc\n");
#endif
    return COAP_INVALID_TID;
  }
  ...
  msg->msg.send.pdu = request;
  msg->msg.send.tid = COAP_INVALID_TID;
  msg->msg.send.type_code = code;
  (void)memcpy_s(msg->msg.send.opts, sizeof(coap_opt_filter_t), opts, sizeof(coap_opt_filter_t));
  msg->msg.send.cb = (void *)coap_send_error;
...
coap_tid_t
coap_send_message_type_lwip(coap_session_t *session, coap_pdu_t *request, unsigned char type)
{
  struct coapapi_msg *msg = NULL;
  coap_tid_t tid = COAP_INVALID_TID;
  err_t err;
  ...
  if ((session == NULL) || (request == NULL)) {
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_send_message_type: invalid param\n");
#endif
    return COAP_INVALID_TID;
  }
  ...
  if (msg == NULL) {
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_send_message_type: malloc\n");
#endif
    return COAP_INVALID_TID;
  }
  ...
  msg->msg.send.pdu = request;
  msg->msg.send.tid = COAP_INVALID_TID;
  msg->msg.send.type_code = type;
  msg->msg.send.cb = (void *)coap_send_message_type;
...
coap_new_context(
  ...
  if (!c) {
#ifndef NDEBUG
    coap_log(LOG_EMERG, "coap_new_context: malloc ctx\n");
#endif
    return NULL;
  }
#endif /* not WITH_CONTIKI */
