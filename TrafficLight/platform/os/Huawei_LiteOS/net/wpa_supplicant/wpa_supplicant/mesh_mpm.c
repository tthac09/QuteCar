...
#include "src/drivers/driver_hisi.h"
#include "hi_at.h"
...
static const char * const mplevent[] = {
	...
	[REQ_RJCT] = "REQ_RJCT",
#ifdef LOS_CONFIG_MESH_GTK
	[MGTK_INFO_ACPT] = "MGTK_INFO_ACPT",
	[MGTK_ACK_ACPT] = "MGTK_ACK_ACPT",
#endif /* LOS_CONFIG_MESH_GTK */
};
...
#ifdef LOS_CONFIG_MESH_GTK
		(action_field == PLINK_MGTK_INFO && len != 6) ||
		(action_field == PLINK_MGTK_ACK && len != 6) ||
#endif
...
void mesh_mpm_init_link(struct wpa_supplicant *wpa_s,
			       struct sta_info *sta)
{
	u16 llid;
...
void mesh_mpm_send_plink_action(struct wpa_supplicant *wpa_s,
				       struct sta_info *sta,
				       ...
				       u16 close_reason)
{
	...
	if (!sta)
		return;
/* hisi driver will fill some ies bellow */
			      ...
			      enum mesh_plink_state state)
{
	...
	params.mesh_initiative_peering  = sta->mesh_initiative_peering;
	ret = wpa_drv_sta_add(wpa_s, &params);
	if (ret) {
		wpa_warning_log4(MSG_ERROR, "wpa_mesh_set_plink_state failed to add sta : ""%02x:xx:xx:%02x:%02x:%02x",
...
void plink_timer(void *eloop_ctx, void *user_data)
{
	...
	case PLINK_CNF_RCVD:
		...
		hi_mesh_report_connect_results(sta, false, reason);
		break;
	case PLINK_HOLDING:
		/* holding timer */
#ifndef LOS_CONFIG_HOSTAPD_PMKSA
		if (sta->mesh_sae_pmksa_caching) {
			wpa_error_log4(MSG_DEBUG, "MPM: Peer " "%02x:xx:xx:%02x:%02x:%02x"
			...
			if (g_mesh_connect_flag) {
				mesh_mpm_fsm_restart(wpa_s, sta);
				hi_wpa_rejoin_mesh(wpa_s);
				break;
			}
		}
#endif /* LOS_CONFIG_HOSTAPD_PMKSA */
...
#ifdef LOS_CONFIG_MESH_GTK
	case PLINK_ESTAB:
		if (hisi_mesh_plink_timer_mgtk_process(wpa_s, sta, conf) == HISI_OK) {
			hi_at_printf("+NOTICE:MESH DISCONNECTED\r\n");
			hi_mesh_report_connect_results(sta, false, WLAN_REASON_MESH_GTK_REKEY_TIMEOUT);
		}
		break;
#endif /* LOS_CONFIG_MESH_GTK */
	default:
		    ...
		    enum mesh_plink_state next_state)
{
	...
	eloop_register_timeout(conf->dot11MeshRetryTimeout / 1000,
			       ...
			       plink_timer, wpa_s, sta);
	mesh_mpm_send_plink_action(wpa_s, sta, PLINK_OPEN, 0);
	if ((sta->plink_state == PLINK_ESTAB) && (g_hapd != NULL))
		g_hapd->num_plinks--;
				...
				void *ctx)
{
	...
	if ((hapd == NULL) || (wpa_s == NULL) || (wpa_s->ifmsh == NULL)) {
		wpa_warning_log0(MSG_DEBUG, "mesh_mpm_plink_close : NULL ptr param input");
		return -1;
	}
	conf = wpa_s->ifmsh->mconf;
	if (conf == NULL) {
		wpa_warning_log0(MSG_DEBUG, "mesh_mpm_plink_close : mconf ptr is NULL");
		return -1;
	}

	if ((sta) && (sta->plink_state >= PLINK_IDLE)) {
		...
		hi_at_printf("+NOTICE:MESH DISCONNECTED\r\n");
		hi_mesh_report_connect_results(sta, false, reason);
		wpa_error_log4(MSG_DEBUG, "MPM closing plink sta=" "%02x:xx:xx:%02x:%02x:%02x",
			   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
		eloop_register_timeout(
...
int mesh_mpm_close_peer(struct wpa_supplicant *wpa_s, const u8 *addr)
{
	...
	if (!sta) {
		...
		/* send mesh_close_action */
		if (hi_mesh_mpm_send_plink_close_action(wpa_s, addr) != HISI_OK)
...
void mesh_mpm_auth_peer(struct wpa_supplicant *wpa_s, const u8 *addr)
{
	...
	ret = wpa_drv_sta_add(wpa_s, &params);
	if (ret) {
		wpa_warning_log4(MSG_ERROR, "mesh_mpm_auth_peer failed to add sta : ""%02x:xx:xx:%02x:%02x:%02x",
						 (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
		wpa_msg(wpa_s, MSG_ERROR,
					   ...
					   struct ieee802_11_elems *elems)
{
	...
	sta->mesh_bcn_priority  = elems->mesh_bcn_priority;
	sta->mesh_is_mbr		= elems->mesh_is_mbr;
	...
	if (conf->security != MESH_CONF_SEC_NONE)
		sta->flags |= WLAN_STA_MFP;
#endif /* HISI_CODE_CROP */
	params.mesh_bcn_priority = elems->mesh_bcn_priority;
	params.mesh_is_mbr = elems->mesh_is_mbr;
	ret = wpa_drv_sta_add(wpa_s, &params);
	if (ret) {
		wpa_warning_log4(MSG_ERROR, "mesh_mpm_add_peer failed to add sta : ""%02x:xx:xx:%02x:%02x:%02x",
						 (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
		wpa_msg(wpa_s, MSG_ERROR,
				 ...
				 struct sta_info *sta)
{
	...
	if (conf->security & MESH_CONF_SEC_AMPE) {
		...
		wpa_drv_set_key(wpa_s, wpa_cipher_to_alg(conf->group_cipher),
				...
				sta->mgtk, sta->mgtk_len);
#else
		if (hisi_set_mesh_mgtk(wpa_s->ifname, sta->addr, sta->mgtk, sta->mgtk_len) < 0) {
			wpa_error_log0(MSG_ERROR, "hisi_set_mesh_mgtk fail!");
			if (hi_mesh_mpm_send_plink_close_action(wpa_s, sta->addr) != HISI_OK)
				wpa_error_log0(MSG_DEBUG, "mesh close action send failed");
	...
	/* Send ctrl event */
	wpa_printf(MSG_ERROR, MESH_PEER_CONNECTED MACSTR,
		MAC2STR(sta->addr));
...
#ifdef LOS_CONFIG_MESH_GTK
		case MGTK_INFO_ACPT:
			if (hisi_mesh_mgtk_info_accept_process(wpa_s, sta) != HISI_OK)
				wpa_error_log0(MSG_DEBUG, "mesh gtk install failed");
			break;
		case MGTK_ACK_ACPT:
			if (hisi_mesh_mgtk_ack_accept_process(wpa_s, sta) == HISI_OK)
				wpa_error_log0(MSG_DEBUG, "MPM: MESH GTK rekey success !");
			break;
#endif /* LOS_CONFIG_MESH_GTK */
		default:
			break;
			...
			const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	wpa_error_log1(MSG_DEBUG, "MPM: Received PLINK action %u", action_field);

	if (action_field == PLINK_OPEN || action_field == PLINK_CONFIRM) {
		wpa_warning_log1(MSG_DEBUG, "MPM: Capability 0x%x",
			   WPA_GET_LE16(ies));
		ies += 2;	/* capability */
		ie_len -= 2;
	}
	if (action_field == PLINK_CONFIRM) {
		...
		wpa_warning_log1(MSG_DEBUG, "MPM: AID 0x%x", aid);
		ies += 2;	/* aid */
		ie_len -= 2;
	}
	hisi_mesh_flag = hi_wpa_find_hisi_mesh_ie(ies, ie_len);
	/* check for mesh peering, mesh id and mesh config IEs */
	if (ieee802_11_parse_elems(ies, ie_len, &elems, 0) == ParseFailed) {
		wpa_warning_log0(MSG_DEBUG, "MPM: Failed to parse PLINK IEs");
		return;
	}
	if (!elems.peer_mgmt) {
	...
	ret = mesh_mpm_parse_peer_mgmt(wpa_s, action_field,
				       ...
				       &peer_mgmt_ie);
	if (ret) {
		wpa_warning_log0(MSG_DEBUG, "MPM: Mesh parsing rejected frame");
		return;
	}
	...
	wpa_warning_log2(MSG_DEBUG, "MPM: plid=0x%x llid=0x%x", plid, llid);

	if (action_field == PLINK_CLOSE) {
		wpa_error_log1(MSG_DEBUG, "MPM: close reason=%u",
	...
	if (!sta && action_field == PLINK_OPEN &&
	    (!(mconf->security & MESH_CONF_SEC_AMPE) ||
		 wpa_auth_pmksa_get(hapd->wpa_mesh_auth, mgmt->sa, NULL))) {
#else
	if (!sta && action_field == PLINK_OPEN &&
	    !(mconf->security & MESH_CONF_SEC_AMPE)) {
#endif /* LOS_CONFIG_HOSTAPD_PMKSA */
		if (g_mesh_reject_peer_flag) {
			wpa_error_log0(MSG_ERROR, "mesh_mpm_action_rx: reject mesh peer now.");
			return;
		}
		sta = mesh_mpm_add_peer(wpa_s, mgmt->sa, &elems);
	}
	if (!sta) {
		wpa_warning_log0(MSG_DEBUG, "MPM: No STA entry for peer");
		return;
	}
	/* reset mesh mpm if recv hisi mesh peer close action */
	if ((hisi_mesh_flag == WPA_FLAG_ON) && (action_field == PLINK_CLOSE)) {
		wpa_error_log0(MSG_DEBUG, "MPM: hisi_mesh_flag is on, mpm restart.");
#ifndef LOS_CONFIG_HOSTAPD_PMKSA
		wpa_auth_pmksa_remove(hapd->wpa_mesh_auth, sta->addr);
#endif /* LOS_CONFIG_HOSTAPD_PMKSA */
		if ((sta->plink_state >= PLINK_IDLE) && (sta->plink_state != PLINK_HOLDING)) {
			hi_at_printf("+NOTICE:MESH DISCONNECTED\r\n");
			hi_mesh_report_connect_results(sta, false, WLAN_REASON_MESH_CLOSE_RCVD);
		}
		mesh_mpm_fsm_restart(wpa_s, sta);
		return;
	}
	...
	if (sta->sae && sta->sae->state != SAE_ACCEPTED) {
		wpa_error_log0(MSG_DEBUG, "MPM: SAE not yet accepted for peer");
		return;
	}
#endif /* CONFIG_SAE */
	...
	if (sta->plink_state == PLINK_BLOCKED) {
		wpa_error_log0(MSG_DEBUG, "MPM: PLINK_BLOCKED");
		return;
	}
	...
	case PLINK_MGTK_ACK:
		...
		event = (action_field == PLINK_MGTK_INFO) ? MGTK_INFO_ACPT : MGTK_ACK_ACPT;
		break;
#endif /* LOS_CONFIG_MESH_GTK */
	default:
		/*
