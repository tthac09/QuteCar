				 ...
				 const u8 *ssid_val, size_t ssid_len, int freq)
{
	...
	if (wpa_config_set(ssid, "phase1", val, 0) < 0) {
		wpa_warning_buf(MSG_DEBUG, "WPS: Failed to set phase1 '%s'", val, strlen(val));
		return -1;
	}
	if (wpa_s->wps_fragment_size)
		ssid->eap.fragment_size = wpa_s->wps_fragment_size;
				   ...
				   const struct wps_device_data *dev)
{
	...
	if (uuid_bin2str(uuid_e, uuid, sizeof(uuid)))
		return;
	wpa_warning_buf(MSG_DEBUG, "WPS: PIN needed for UUID-E %s", uuid, strlen(uuid));
	...
	if (!os_snprintf_error(sizeof(txt), len)) {
		wpa_warning_buf(MSG_INFO, "%s", txt,strlen(txt));
	}
}
			      ...
			      struct wps_context *wps)
{
	...
	wpa_warning_two_buf(MSG_DEBUG, "WPS: UUID %s: %s", src, strlen(src), buf, strlen(buf));
}
