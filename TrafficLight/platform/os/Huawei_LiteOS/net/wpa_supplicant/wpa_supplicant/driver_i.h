				  ...
				  const char *ifname)
{
	...
	if ((wpa_s == NULL) || (wpa_s->driver == NULL))
		return NULL;
	if (wpa_s->driver->init2)
		return wpa_s->driver->init2(wpa_s, ifname, NULL);
...
static inline int wpa_drv_set_delay_report(struct wpa_supplicant *wpa_s, int enable_flag)
{
	if ((wpa_s == NULL) || (wpa_s->driver == NULL))
		return -1;
	if (wpa_s->driver->set_delay_report)
	 	return wpa_s->driver->set_delay_report(wpa_s->ifname, enable_flag);
	return -1;
}
...
#ifdef CONFIG_DRIVER_HISILICON
static inline int wpa_drv_get_p2p_mac_addr(struct wpa_supplicant *wpa_s,
										   enum wpa_driver_if_type type,
										   unsigned char *mac_addr)
{
	if ((wpa_s == NULL) || (wpa_s->driver == NULL))
		return -1;
	if (wpa_s->driver->get_p2p_mac_addr)
		return wpa_s->driver->get_p2p_mac_addr(wpa_s->drv_priv, type, mac_addr);
	return -1;
}
#endif
...
static inline int wpa_drv_get_drv_flags(struct wpa_supplicant *wpa_s, unsigned long long *dev_flags)
{
	if ((wpa_s == NULL) || (wpa_s->driver == NULL))
		return -1;
	if (wpa_s->driver->get_drv_flags)
		return wpa_s->driver->get_drv_flags(wpa_s->drv_priv, dev_flags);
	return -1;
}
