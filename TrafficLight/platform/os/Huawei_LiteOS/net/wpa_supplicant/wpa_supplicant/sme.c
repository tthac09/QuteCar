				    ...
				    int start)
{
	...
	if (!skip_auth && params.auth_alg == WPA_AUTH_ALG_SAE &&
	    pmksa_cache_set_current(wpa_s->wpa, NULL, bss->bssid, ssid, 0,
				    ...
				    WPA_KEY_MGMT_SAE) == 0) {
#else
				    wpa_s->key_mgmt == WPA_KEY_MGMT_SAE) == 0) {
					  ...
					  union wpa_event_data *data)
{
	...
	/* Get the SSID conf from the ssid string or bssid obtained */
	for (ssid = wpa_s->conf->ssid; ssid; ssid = ssid->next) {
		if (!wpas_network_disabled(wpa_s, ssid) &&
		    ...
		    ((ssid_str_len == ssid->ssid_len &&
		     os_memcmp(ssid_str, ssid->ssid, ssid_str_len) == 0) ||
		     os_memcmp(data->external_auth.bssid, ssid->bssid, ETH_ALEN) == 0) &&
#endif /* CONFIG_WPA3 */
#ifndef HISI_CODE_CROP
...
#ifdef CONFIG_SAE_ONE_ECC_CURVE
	int default_groups[] = { 19, 0 };
	groups = default_groups;
#endif /* CONFIG_DRIVER_HISILICON */
