				       ...
				       u8 **spos, const u8 *end)
{
	while (*spos < end) {
		...
		if (val == 1 || val == 2) {
			...
			wpa_warning_log1(MSG_DEBUG, "P2P: Invalid domain name "
		...
		if (val == 3) {
			...
			/* Offset */
			if (end - *spos < 2) {
				wpa_warning_log0(MSG_DEBUG, "P2P: No room for full "
					   "DNS offset field");
				return -1;
			}
			...
			if (offset >= *spos - start) {
				wpa_warning_log1(MSG_DEBUG, "P2P: Invalid DNS "
				...
				struct wpabuf *resp, u8 srv_trans_id)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: SD Request for all Bonjour services");

	if (dl_list_empty(&wpa_s->global->p2p_srv_bonjour)) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Bonjour protocol not available");
		return;
	}
				...
				const u8 *query, size_t query_len)
{
	...
	if (matches == 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Requested Bonjour service not "
			   "available");
		if (wpabuf_tailroom(resp) < 5)
			return;
			     ...
			     struct wpabuf *resp, u8 srv_trans_id)
{
	...
	wpa_warning_log0(MSG_DEBUG, "P2P: SD Request for all UPnP services");

	if (dl_list_empty(&wpa_s->global->p2p_srv_upnp)) {
		wpa_warning_log0(MSG_DEBUG, "P2P: UPnP protocol not available");
		return;
	}
	...
	dl_list_for_each(usrv, &wpa_s->global->p2p_srv_upnp,
		...
		wpa_warning_buf(MSG_DEBUG, "P2P: Matching UPnP Service: %s",
			   usrv->service, strlen(usrv->service));
			     ...
			     const u8 *query, size_t query_len)
{
	...
	if (count == 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Requested UPnP service not "
			   "available");
		/* Status Code */
		wpabuf_put_u8(resp, P2P_SD_REQUESTED_INFO_NOT_AVAILABLE);
		/* Response Data: empty */
			    ...
			    const u8 *query, size_t query_len)
{
	...
	if (!wpa_s->global->wifi_display) {
		wpa_warning_log0(MSG_DEBUG, "P2P: WFD protocol not available");
	...
	if (query_len < 1) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Missing WFD Requested Device "
			   "Role");
		return;
	}
	...
	pos = query;
	role = *pos++;
	wpa_warning_log1(MSG_DEBUG, "P2P: WSD for device role 0x%x", role);
	...
	while (pos < query + query_len) {
		if (*pos < MAX_WFD_SUBELEMS &&
			...
			wpa_warning_log1(MSG_DEBUG, "P2P: Add WSD response "
				   "subelement %u", *pos);
			    ...
			    const u8 *query, size_t query_len)
{
	...
	if (!wpa_s->global->p2p) {
		wpa_warning_log0(MSG_DEBUG, "P2P: ASP protocol not available");
	...
	if (svc_len + (info_len ? info_len + 2 : 1) > query_len) {
		wpa_warning_log0(MSG_DEBUG, "P2P: ASP bad request");
		...
		if (os_memcmp(adv_data->svc_name, svc, svc_len) == 0 &&
			...
			if (svc_info_len) {
				wpa_warning_two_buf(MSG_DEBUG,
					   ...
					   adv_data->svc_name, strlen(adv_data->svc_name),
					   adv_data->svc_info, strlen(adv_data->svc_info));
			} else {
				wpa_warning_buf(MSG_DEBUG, "P2P: Add Svc: %s",
					   adv_data->svc_name, strlen(adv_data->svc_name));
			}
	...
	if (count_pos == NULL) {
		wpa_warning_log0(MSG_DEBUG, "P2P: ASP service not found");
		     ...
		     u16 update_indic, const u8 *tlvs, size_t tlvs_len)
{
	...
	while (end - pos > 1) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Service Request TLV");
		slen = WPA_GET_LE16(pos);
		pos += 2;
		if (slen > end - pos || slen < 2) {
			wpa_warning_log0(MSG_DEBUG, "P2P: Unexpected Query Data "
				   "length");
			wpabuf_free(resp);
			return;
		}
		...
		srv_proto = *pos++;
		wpa_warning_log1(MSG_DEBUG, "P2P: Service Protocol Type %u",
			   srv_proto);
		srv_trans_id = *pos++;
		wpa_warning_log1(MSG_DEBUG, "P2P: Service Transaction ID %u",
			   srv_trans_id);
		...
		if (wpa_s->force_long_sd) {
			wpa_warning_log0(MSG_DEBUG, "P2P: SD test - force long "
				   "response");
			wpas_sd_all_bonjour(wpa_s, resp, srv_trans_id);
			wpas_sd_all_upnp(wpa_s, resp, srv_trans_id);
		...
		case P2P_SERV_ALL_SERVICES:
			wpa_warning_log0(MSG_DEBUG, "P2P: Service Discovery Request "
				   "for all services");
			if (dl_list_empty(&wpa_s->global->p2p_srv_upnp) &&
			    dl_list_empty(&wpa_s->global->p2p_srv_bonjour) &&
			    !p2p_get_p2ps_adv_list(wpa_s->global->p2p)) {
				wpa_warning_log0(MSG_DEBUG, "P2P: No service "
		...
		case P2P_SERV_P2PS:
			...
			break;
		default:
			wpa_warning_log1(MSG_DEBUG, "P2P: Unavailable service "
		      ...
		      const u8 *tlvs, size_t tlvs_len)
{
	...
	while (end - pos >= 2) {
		...
		wpa_warning_log0(MSG_DEBUG, "P2P: Service Response TLV");
		slen = WPA_GET_LE16(pos);
		pos += 2;
		if (slen > end - pos || slen < 3) {
			wpa_warning_log0(MSG_DEBUG, "P2P: Unexpected Response Data "
				   "length");
			return;
		}
		tlv_end = pos + slen;
...
#ifndef HISI_CODE_CROP
		status = *pos++;
		wpa_warning_log1(MSG_DEBUG, "P2P: Status Code ID %u",
			   status);
#else
		pos++;
#endif /* HISI_CODE_CROP */
