	...
	if (wpa_s->last_scan_res) {
		...
		for (i = 0; i < wpa_s->last_scan_res_used; i++) {
			if (wpa_s->last_scan_res[i] == bss) {
				(void)os_memmove(&wpa_s->last_scan_res[i],
...
{
	dst->flags = src->flags;
	(void)os_memcpy(dst->bssid, src->bssid, ETH_ALEN);
	dst->freq = src->freq;
	dst->caps = src->caps;
...
static int wpa_bss_remove_oldest_unknown(struct wpa_supplicant *wpa_s)
{
	...
	dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
		if (!wpa_bss_known(wpa_s, bss) &&
			...
			wpa_bss_remove(wpa_s, bss, __FUNCTION__);
			return 0;
		}
	}
				    ...
				    struct os_reltime *fetch_time)
{
	...
	(void)os_memcpy(bss->ssid, ssid, ssid_len);
	bss->ssid_len = ssid_len;
	bss->ie_len = res->ie_len;
#ifndef HISI_SCAN_SIZE_CROP
	bss->beacon_ie_len = res->beacon_ie_len;
	(void)os_memcpy(bss + 1, res + 1, res->ie_len + res->beacon_ie_len);
#else
	(void)os_memcpy(bss + 1, res + 1, res->ie_len);
#endif /* HISI_SCAN_SIZE_CROP */
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_error_log1(MSG_ERROR, "Increasing the MAX BSS count to %d "
			     ...
			     struct os_reltime *fetch_time)
{
...
#ifndef CONFIG_DRIVER_HISILICON
	/* HISILICON driver do not support mesh id IEs */
...
#ifdef CONFIG_MESH
	u8 *hi_mesh = NULL;
	if ((g_mesh_flag && (wpa_s->ifmsh != NULL)) ||
		(g_mesh_sta_flag && (wpa_s->ifmsh == NULL))) {
		hi_mesh = hi_wpa_scan_get_mesh_id(res);
		if (hi_mesh && (hi_mesh[1] <= SSID_MAX_LEN)) {
			ssid = hi_mesh;
		}
	}
#endif /* CONFIG_MESH */
	...
	if (p2p == NULL &&
		 ...
		 */
		wpa_warning_log4(MSG_DEBUG, "BSS: No P2P IE - skipping BSS " "%02x:xx:xx:%02x:%02x:%02x"
			   " update for P2P interface", (res->bssid)[0], (res->bssid)[3], (res->bssid)[4], (res->bssid)[5]);
		return;
	}
#endif /* CONFIG_P2P */
			...
			int new_scan)
{
	...
	(void)os_get_reltime(&wpa_s->last_scan);
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
...
#ifndef CONFIG_PRINT_NOUSE
	wpa_warning_log2(MSG_DEBUG, "BSS: last_scan_res_used=%u/%u",
