			      ...
			      struct hostapd_config *conf)
{
	...
	if (conf->hw_mode == NUM_HOSTAPD_MODES) {
		wpa_error_log1(MSG_ERROR, "Unsupported AP mode frequency: %d MHz",
			   ssid->frequency);
		return -1;
	}
				  ...
				  struct hostapd_config *conf)
{
	...
	(void)os_strlcpy(bss->iface, wpa_s->ifname, sizeof(bss->iface));
...
#ifndef HISI_CODE_CROP
	if (ssid->pbss > 1) {
		wpa_error_log1(MSG_ERROR, "Invalid pbss value(%d) for AP mode",
			   ssid->pbss);
		return -1;
	}
	bss->pbss = ssid->pbss;
	...
	if (ssid->acs) {
		...
		conf->channel = 0;
		wpa_warning_log0(MSG_DEBUG, "Use automatic channel selection");
	}
#endif /* CONFIG_ACS */
	...
	if (ssid->ssid_len == 0) {
		wpa_error_log0(MSG_ERROR, "No SSID configured for AP mode");
		return -1;
	}
	(void)os_memcpy(bss->ssid.ssid, ssid->ssid, ssid->ssid_len);
	...
	if (ssid->psk_set) {
		...
		if (bss->ssid.wpa_psk == NULL)
			return -1;
		(void)os_memcpy(bss->ssid.wpa_psk->psk, ssid->psk, PMK_LEN);
			     ...
			     struct wpa_ssid *ssid)
{
	...
	wpa_warning_buf(MSG_DEBUG, "Setting up AP (SSID='%s')",
	...
	if (wpa_s->pairwise_cipher < 0) {
		wpa_warning_log0(MSG_WARNING, "WPA: Failed to select pairwise "
			   "cipher.");
		return -1;
	}
	...
	if (params.p2p)
		(void)wpa_drv_get_ext_capa(wpa_s, WPA_IF_P2P_GO);
	else
		(void)wpa_drv_get_ext_capa(wpa_s, WPA_IF_AP_BSS);
	...
	(void)os_memcpy(wpa_s->ap_iface->conf->wmm_ac_params,
	...
	if (wpa_supplicant_conf_ap(wpa_s, ssid, conf)) {
		wpa_error_log0(MSG_ERROR, "Failed to create AP configuration");
		wpa_supplicant_ap_deinit(wpa_s);
		return -1;
	}
...
#ifndef HISI_CODE_CROP
		(void)hostapd_register_probereq_cb(hapd_iface->bss[i],
	...
	(void)os_memcpy(hapd_iface->bss[0]->own_addr, wpa_s->own_addr, ETH_ALEN);
	hapd_iface->bss[0]->driver = wpa_s->driver;
	hapd_iface->bss[0]->drv_priv = wpa_s->drv_priv;
	g_hapd = hapd_iface->bss[0];

	wpa_s->current_ssid = ssid;
	eapol_sm_notify_config(wpa_s->eapol, NULL, NULL);
	(void)os_memcpy(wpa_s->bssid, wpa_s->own_addr, ETH_ALEN);
	...
	if (hostapd_setup_interface(wpa_s->ap_iface)) {
		wpa_error_log0(MSG_ERROR, "Failed to initialize AP interface");
		wpa_supplicant_ap_deinit(wpa_s);
		return -1;
	}
...
{
#ifdef CONFIG_WPS_AP
	(void)eloop_cancel_timeout(wpas_wps_ap_pin_timeout, wpa_s, NULL);
#endif /* CONFIG_WPS_AP */
...
#ifndef HISI_CODE_CROP
	wpa_s->ap_iface->driver_ap_teardown = 0;
...
void ap_mgmt_rx(void *ctx, struct rx_mgmt *rx_mgmt)
{
	...
	(void)ieee802_11_mgmt(wpa_s->ap_iface->bss[0], rx_mgmt->frame,
...
int wpa_supplicant_ap_wps_cancel(struct wpa_supplicant *wpa_s)
{
	...
	if (!reg_sel && !wps_sta) {
		wpa_warning_log0(MSG_DEBUG, "No WPS operation in progress at this "
			   "time");
		return -1;
	}
...
const char * wpas_wps_ap_pin_random(struct wpa_supplicant *wpa_s, int timeout)
{
	...
	if (wps_generate_pin(&pin) < 0)
		return NULL;
	(void)os_snprintf(pin_txt, sizeof(pin_txt), "%08u", pin);
	os_free(hapd->conf->ap_pin);
...
void wpa_supplicant_ap_pwd_auth_fail(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WPS: AP PIN authentication failure number %u",
		   hapd->ap_pin_failures);
	if (hapd->ap_pin_failures < 3)
		return;

	wpa_warning_log0(MSG_DEBUG, "WPS: Disable AP PIN");
	hapd->ap_pin_failures = 0;
	os_free(hapd->conf->ap_pin);
	hapd->conf->ap_pin = NULL;
}
...
int wpa_supplicant_ap_update_beacon(struct wpa_supplicant *wpa_s)
{
	...
	if (hapd->drv_priv == NULL)
		return -1;
	(void)ieee802_11_set_beacons(iface);
				      ...
				      const u8 *addr)
{
	...
	if (conf->accept_mac == NULL)
		return -1;
	(void)os_memcpy(conf->accept_mac[0].addr, addr, ETH_ALEN);
				      ...
				      struct dfs_event *radar)
{
	...
	(void)hostapd_dfs_radar_detected(wpa_s->ap_iface, radar->freq,
				   ...
				   struct dfs_event *radar)
{
	...
	if (!iface || !iface->bss[0])
		return;
	wpa_warning_log1(MSG_DEBUG, "DFS CAC started on %d MHz", radar->freq);
	(void)hostapd_dfs_start_cac(wpa_s->ap_iface, radar->freq,
			      radar->ht_enabled, radar->chan_offset,
				    ...
				    struct dfs_event *radar)
{
	...
	(void)hostapd_dfs_complete_cac(wpa_s->ap_iface, 1, radar->freq,
				 radar->ht_enabled, radar->chan_offset,
				 radar->chan_width, radar->cf1, radar->cf2);
}
				   ...
				   struct dfs_event *radar)
{
	...
	(void)hostapd_dfs_complete_cac(wpa_s->ap_iface, 0, radar->freq,
				 radar->ht_enabled, radar->chan_offset,
				 radar->chan_width, radar->cf1, radar->cf2);
}
					...
					struct dfs_event *radar)
{
	...
	(void)hostapd_dfs_nop_finished(wpa_s->ap_iface, radar->freq,
				 radar->ht_enabled, radar->chan_offset,
				 radar->chan_width, radar->cf1, radar->cf2);
}
#endif /* NEED_AP_MLME */
