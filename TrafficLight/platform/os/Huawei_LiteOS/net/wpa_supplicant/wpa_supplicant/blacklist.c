...
int wpa_blacklist_add(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	if (e) {
		e->count++;
		wpa_warning_log4(MSG_DEBUG, "BSSID " "%02x:xx:xx:%02x:%02x:%02x" " blacklist count ",
			   (bssid)[0], (bssid)[3], (bssid)[4], (bssid)[5]);
		wpa_warning_log1(MSG_DEBUG, "incremented to %d", e->count);
		return e->count;
	}
	...
	wpa_s->blacklist = e;
	wpa_warning_log4(MSG_DEBUG, "Added BSSID " "%02x:xx:xx:%02x:%02x:%02x" " into blacklist",
		   (bssid)[0], (bssid)[3], (bssid)[4], (bssid)[5]);
...
int wpa_blacklist_del(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	while (e) {
		if (os_memcmp(e->bssid, bssid, ETH_ALEN) == 0) {
			...
			wpa_warning_log4(MSG_DEBUG, "Removed BSSID " "%02x:xx:xx:%02x:%02x:%02x" " from "
				   "blacklist", (bssid)[0], (bssid)[3], (bssid)[4], (bssid)[5]);
			os_free(e);
			return 0;
		}
		prev = e;
...
void wpa_blacklist_clear(struct wpa_supplicant *wpa_s)
{
	...
	while (e) {
		...
		e = e->next;
		wpa_warning_log4(MSG_DEBUG, "Removed BSSID " "%02x:xx:xx:%02x:%02x:%02x" " from "
			   "blacklist (clear)", (prev->bssid)[0], (prev->bssid)[3], (prev->bssid)[4], (prev->bssid)[5]);
		os_free(prev);
	}
