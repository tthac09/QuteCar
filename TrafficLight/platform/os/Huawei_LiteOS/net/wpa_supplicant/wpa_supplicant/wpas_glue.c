					   ...
					   struct wpa_config_blob *blob)
{
	...
	if (wpa_s->conf->update_config) {
		...
		if (ret) {
			wpa_warning_log0(MSG_DEBUG, "Failed to update config after "
				   "blob set");
		}
	}
}
			    ...
			    size_t *msg_len, void **data_pos)
{
	...
	*msg_len = sizeof(*hdr) + data_len;
	...
	if (data)
		(void)os_memcpy(hdr + 1, data, data_len);
	else
		(void)os_memset(hdr + 1, 0, data_len);
				     ...
				     size_t len)
{
	...
	if (wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt) ||
		 ...
		 * machines. */
		wpa_warning_log2(MSG_DEBUG, "WPA: drop TX EAPOL in non-IEEE 802.1X "
	...
	if (is_zero_ether_addr(wpa_s->bssid)) {
#ifndef HISI_CODE_CROP
		wpa_warning_log0(MSG_DEBUG, "BSSID not set when trying to send an "
			   "EAPOL frame");
#endif /* HISI_CODE_CROP */
		if (wpa_drv_get_bssid(wpa_s, bssid) == 0 &&
			...
			dst = bssid;
#ifndef HISI_CODE_CROP
			wpa_warning_log4(MSG_DEBUG, "Using current BSSID " "%02x:xx:xx:%02x:%02x:%02x"
...
#ifndef HISI_CODE_CROP
			wpa_warning_log4(MSG_DEBUG, "Using the source address of the"
				   " last received EAPOL frame " "%02x:xx:xx:%02x:%02x:%02x" " as "
				    ...
				    void *ctx)
{
	...
	int res, pmk_len;
	u8 pmk[PMK_LEN] = {0};
	wpa_warning_buf(MSG_DEBUG, "EAPOL authentication completed - result=%s",
		   result_str(result), strlen(result_str(result)));
#else
	(void)eapol;
#endif /* HISI_CODE_CROP */
...
#ifndef HISI_CODE_CROP
		(void)ieee802_1x_notify_create_actor(wpa_s, wpa_s->last_eapol_src);
#endif /* HISI_CODE_CROP */
	}
	...
	wpa_warning_log0(MSG_DEBUG, "Configure PMK for driver-based RSN 4-way "
		   "handshake");
	...
	if (wpa_key_mgmt_ft(wpa_s->key_mgmt)) {
		...
		u8 buf[2 * PMK_LEN];
		wpa_warning_log0(MSG_DEBUG, "RSN: Use FT XXKey as PMK for "
	...
	if (res) {
		wpa_warning_log0(MSG_DEBUG, "Failed to get PMK from EAPOL state "
			   "machines");
		return;
	}
	...
	if (wpa_drv_set_key(wpa_s, WPA_ALG_PMK, NULL, 0, 0, NULL, 0, pmk,
			    pmk_len)) {
		wpa_warning_log0(MSG_DEBUG, "Failed to set PMK to the driver");
	}
					 ...
					 const u8 *ies, size_t ies_len)
{
	...
	if (action != 1) {
		wpa_error_log1(MSG_ERROR, "Unsupported send_ft_action action %d",
			   action);
		return -1;
	}
					       ...
					       const char **txt)
{
	...
	*txt = default_txt;
	...
	if (*txt == NULL) {
		wpa_warning_log1(MSG_WARNING, "No message for request %d", field);
		ret = NULL;
	}
					    ...
					    const char *default_txt)
{
	...
	if (field_name == NULL) {
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log1(MSG_WARNING, "Unhandled EAP param %d needed",
			   field);
#endif /* CONFIG_PRINT_NOUSE */
		return;
	}
					   ...
					   enum eap_proxy_sim_state sim_state)
{
	...
	wpa_warning_log1(MSG_DEBUG, "eap_proxy: SIM card status %u", sim_state);
	switch (sim_state) {
	case SIM_STATE_ERROR:
		wpa_sm_sim_state_error_handler(wpa_s);
		break;
	default:
		wpa_warning_log0(MSG_DEBUG, "eap_proxy: SIM card status unknown");
		break;
	}
}
...
static void wpa_supplicant_port_cb(void *ctx, int authorized)
{
	...
	wpa_printf(MSG_DEBUG, "EAPOL: Supplicant port status: %s",
		   authorized ? "Authorized" : "Unauthorized");
	(void)wpa_drv_set_supp_port(wpa_s, authorized);
}
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
...
static void wpa_supplicant_set_anon_id(void *ctx, const u8 *id, size_t len)
{
	...
	if (id == NULL) {
		...
		if (str == NULL)
			return;
		(void)wpa_snprintf_hex(str, len * 2 + 1, id, len);
