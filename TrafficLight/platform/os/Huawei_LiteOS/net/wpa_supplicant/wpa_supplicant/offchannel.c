...
wpas_get_tx_interface(struct wpa_supplicant *wpa_s, const u8 *src)
{
		...
		if (wpa_s->p2p_mgmt && wpa_s != wpa_s->parent &&
		    ...
		    *wpabuf_head_u8(wpa_s->pending_action_tx) !=
			...
			wpa_warning_two_buf(MSG_DEBUG,
				   ...
				   wpa_s->parent->ifname, strlen(wpa_s->parent->ifname), wpa_s->ifname, strlen(wpa_s->ifname));
			return wpa_s->parent;
		}
#endif /* CONFIG_P2P */
		return wpa_s;
	...
	if (iface) {
		wpa_warning_two_buf(MSG_DEBUG, "P2P: Use group interface %s "
			   "instead of interface %s for Action TX",
			   iface->ifname, strlen(iface->ifname), wpa_s->ifname, strlen(wpa_s->ifname));
		return iface;
	}
...
static void wpas_send_action_cb(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_s->off_channel_freq != wpa_s->pending_action_freq &&
		...
		wpa_warning_log3(MSG_DEBUG, "Off-channel: Pending Action frame TX "
		...
		if (without_roc && wpa_s->off_channel_freq == 0) {
			 ...
			 */
			wpa_warning_log0(MSG_DEBUG, "Off-channel: Schedule "
			...
			if (wpa_drv_remain_on_channel(
				    ...
				    duration) < 0) {
				wpa_warning_log1(MSG_DEBUG, "Off-channel: Failed to "
	...
	wpa_warning_log4(MSG_DEBUG, "Off-channel: Sending pending Action frame to "
		   "%02x:xx:xx:%02x:%02x:%02x",
		   (wpa_s->pending_action_dst)[0], (wpa_s->pending_action_dst)[3], (wpa_s->pending_action_dst)[4], (wpa_s->pending_action_dst)[5]);
	wpa_warning_buf(MSG_DEBUG, " using interface %s",
		   iface->ifname, strlen(iface->ifname));
	wpa_warning_log1(MSG_DEBUG, "(pending_action_tx=%p)", wpa_s->pending_action_tx);
	...
	if (res) {
		wpa_warning_log0(MSG_DEBUG, "Off-channel: Failed to send the "
	...
	size_t data_len, enum offchannel_send_action_result result)
{
	if (wpa_s->pending_action_tx == NULL) {
		wpa_warning_log0(MSG_DEBUG, "Off-channel: Ignore Action TX status - "
	...
	wpa_warning_log4(MSG_DEBUG,
		   ...
		   "%02x:xx:xx:%02x:%02x:%02x", dst[0], dst[3], dst[4], dst[5]);
	wpa_warning_log1(MSG_DEBUG,
		   " pending_action_tx=%p)", wpa_s->pending_action_tx);
	wpa_hexdump_buf(MSG_MSGDUMP, "Pending TX frame",
			wpa_s->pending_action_tx);
	wpabuf_free(wpa_s->pending_action_tx);
	wpa_s->pending_action_tx = NULL;

	wpa_warning_log2(MSG_DEBUG, "Off-channel: TX status result=%d cb=%p",
...
int offchannel_send_action(struct wpa_supplicant *wpa_s, unsigned int freq,
			   ...
			   int no_cck)
{
	wpa_warning_log2(MSG_DEBUG, "Off-channel: Send action frame: freq=%d" " len=%d", freq, (int) len);
	wpa_warning_log4(MSG_DEBUG, "dst="
		   "%02x:xx:xx:%02x:%02x:%02x", dst[0], dst[3], dst[4], dst[5]);
	wpa_warning_log4(MSG_DEBUG, " src=" "%02x:xx:xx:%02x:%02x:%02x",
		   (src)[0], (src)[3], (src)[4], (src)[5]);
	wpa_warning_log4(MSG_DEBUG," bssid=" "%02x:xx:xx:%02x:%02x:%02x",
		   (bssid)[0], (bssid)[3], (bssid)[4], (bssid)[5]);
	wpa_s->pending_action_tx_status_cb = tx_cb;

	if (wpa_s->pending_action_tx) {
		wpa_warning_log4(MSG_DEBUG, "Off-channel: Dropped pending Action "
			   "frame TX to " "%02x:xx:xx:%02x:%02x:%02x",
			   (wpa_s->pending_action_dst)[0], (wpa_s->pending_action_dst)[3], 
			   (wpa_s->pending_action_dst)[4], (wpa_s->pending_action_dst)[5]);
		wpa_warning_log1(MSG_DEBUG, " (pending_action_tx=%p)",
			   wpa_s->pending_action_tx);
		wpa_hexdump_buf(MSG_MSGDUMP, "Pending TX frame",
				wpa_s->pending_action_tx);
		wpabuf_free(wpa_s->pending_action_tx);
	...
	if (wpa_s->pending_action_tx == NULL) {
		wpa_warning_log1(MSG_DEBUG, "Off-channel: Failed to allocate Action "
	...
	wpa_warning_log4(MSG_DEBUG,
		   "Off-channel: Stored pending action frame (dst=" "%02x:xx:xx:%02x:%02x:%02x",
	...
	if (freq) {
		...
		if (tx_iface->assoc_freq == freq) {
			wpa_warning_log0(MSG_DEBUG, "Off-channel: Already on "
	...
	if (wpa_s->off_channel_freq == freq || freq == 0) {
		wpa_warning_log0(MSG_DEBUG, "Off-channel: Already on requested "
	...
	if (wpa_s->roc_waiting_drv_freq == freq) {
		wpa_warning_log1(MSG_DEBUG, "Off-channel: Already waiting for "
			   "driver to get to frequency %u MHz; continue "
			   "waiting to send the Action frame", freq);
		return 0;
	}

	wpa_warning_log0(MSG_DEBUG, "Off-channel: Schedule Action frame to be "
	...
	if (wpa_drv_remain_on_channel(wpa_s, freq, wait_time) < 0) {
		wpa_warning_log1(MSG_DEBUG, "Off-channel: Failed to request driver "
...
{
	wpa_warning_log4(MSG_DEBUG,
		   "Off-channel: Action frame sequence done notification: pending_action_tx=%p action_tx_wait_time=%d off_channel_freq=%d roc_waiting_drv_freq=%d",
		   wpa_s->pending_action_tx,
#ifndef HISI_CODE_CROP
