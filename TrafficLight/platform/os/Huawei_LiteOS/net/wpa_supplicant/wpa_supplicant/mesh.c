...
#else
	static int default_groups[] = { 19, -1 };
...
#else
	if (!ssid || !ifmsh) {
		wpa_error_log0(MSG_ERROR, "mesh: wpas_mesh_complete called without active mesh.");
		return -1;
	}
#endif /* LOS_CONFIG_MESH_TRIM */
...
#endif
{
	...
	if (wpa_drv_init_mesh(wpa_s)) {
		wpa_error_log0(MSG_ERROR, "Failed to init mesh in driver");
		goto out_free;
	}
	wifi_dev->iftype = HI_WIFI_IFTYPE_MESH_POINT;
			     ...
			     struct wpa_ssid *ssid)
{
	...
	if (hi_get_wifi_dev_by_iftype(HI_WIFI_IFTYPE_AP) != NULL) {
		wpa_error_log0(MSG_ERROR, "wpa_supplicant_join_mesh: AP is in use, should not start mesh.\n");
		ret = -ENOENT;
#ifndef LOS_CONFIG_MESH_TRIM
		os_free(params);
#endif /* LOS_CONFIG_MESH_TRIM */
