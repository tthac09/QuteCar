...
static void wpa_supplicant_update_current_bss(struct wpa_supplicant *wpa_s)
{
	...
	if (!bss) {
		(void)wpa_supplicant_update_scan_results(wpa_s);
...
void wpa_supplicant_mark_disassoc(struct wpa_supplicant *wpa_s)
{
	...
	/*
	 * clear current_ssid only when reconnection is disabled.
	 * In reconnection scenario, current_ssid should NOT be cleared as it is used in hidden ssid scanning.
	 */
	if (g_reconnect_set.pending_flag == WPA_FLAG_OFF)
...
int wpa_supplicant_match_privacy(struct wpa_bss *bss,
					struct wpa_ssid *ssid)
{
	int privacy = 0;
#ifndef HISI_CODE_CROP
...
#ifdef CONFIG_MESH
			ssid = hi_wpa_scan_res_match(wpa_s, i, bss, group, only_first_ssid);
#else
...
#endif /* HISI_CODE_CROP */
#ifdef CONFIG_MESH
		*selected_ssid = hi_wpa_scan_res_match(wpa_s, i, bss, group, only_first_ssid);
#else
...
#else
	if ((wpa_s != NULL) && (wpa_s->conf != NULL) && (wpa_s->conf->ssid != NULL)) {
		selected = wpa_supplicant_select_bss(
...
static inline void wpa_reconnect_freq_check(struct wpa_supplicant *wpa_s)
{
	if ((wpa_s == NULL) || (wpa_s->ifmsh != NULL))
		return;
	if (g_reconnect_set.pending_flag && g_reconnect_set.try_freq_scan_count) {
		g_reconnect_set.try_freq_scan_count--;
		if ((g_reconnect_set.try_freq_scan_count == 0) && (g_connecting_ssid != NULL))
					      ...
					      int own_request, int update_only)
{
	...
	scan_res = wpa_supplicant_get_scan_results(wpa_s,
						   ...
						   NULL, 1);
#else
	scan_res = wpa_supplicant_get_scan_results(wpa_s, NULL, 1);
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	/* report to usr scan_done event here, since wpa have higher task priority than usr task */
	...
	if (g_scan_flag == WPA_FLAG_ON) {
		if (wpa_s->ifmsh != NULL)
			wpa_events.event = HI_WIFI_EVT_MESH_SCAN_DONE;
		else
			wpa_events.event = g_mesh_sta_flag ? HI_WIFI_EVT_MESH_STA_SCAN_DONE : HI_WIFI_EVT_SCAN_DONE;
		wpa_events.info.wifi_scan_done.bss_num = scan_res ? scan_res->num : 0;
	...
	/* register timer task to clean scan results */
	wpa_register_timeout_periodic_cleanup(wpa_s);
					      ...
					      int new_scan, int own_request)
{
	...
	if (time_to_reenable > 0) {
		...
		(void)eloop_register_timeout(time_to_reenable, 0,
...
#endif /* HISI_CODE_CROP */
#ifdef LOS_WPA_PATCH
		/* do not reconnect if sta is connecting or connected */
		if ((wpa_s->current_ssid != NULL) && (wpa_s->current_ssid->mode == WPAS_MODE_INFRA) &&
			(wpa_s->wpa_state >= WPA_ASSOCIATING)) {
			return 0;
		}
#endif /* LOS_WPA_PATCH */
	...
	if (selected) {
		...
		if (wpa_s->ifmsh != NULL) {
			if (g_mesh_connect_flag == WPA_FLAG_ON) {
				hi_wpa_meshap_req_scan(wpa_s, WPA_MESH_PEER_SCAN_TIMEOUT, 0);
			}
			return 0;
		}
#endif /* CONFIG_MESH */
		if (wpa_supplicant_enabled_networks(wpa_s)) {
			hi_wifi_event wpa_events = {0};
			wpa_error_log0(MSG_ERROR, "No suitable network found\n");
			hi_at_printf("+NOTICE:NETWORK NOT FIND\r\n");
			wpa_events.event = HI_WIFI_EVT_STA_FCON_NO_NETWORK;
			if (g_wpa_event_cb != NULL)
				wifi_new_task_event_cb(&wpa_events);
		}

		if ((g_fast_connect_flag == WPA_FLAG_ON) && (g_fast_connect_scan_flag == WPA_FLAG_ON)) {
			wpas_request_disconnection(wpa_s);
			g_fast_connect_scan_flag = WPA_FLAG_OFF;
			(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_QUICK_CONNECT_ERROR);
			return -1;
		}
		wpa_reconnect_freq_check(wpa_s);
#ifndef HISI_CODE_CROP
...
#else
			int timeout_sec = (g_reconnect_set.pending_flag == WPA_FLAG_ON) ? 0 : 5;
#endif /* LOS_WPA_PATCH */
			int timeout_usec = 0;
#ifdef CONFIG_P2P
			int res;
					     ...
					     union wpa_event_data *data)
{
	...
	dl_list_for_each(ifs, &wpa_s->radio->ifaces, struct wpa_supplicant,
			 radio_list) {
		if (ifs != wpa_s) {
			wpa_warning_buf(MSG_DEBUG, "%s: Updating scan results from "
				   "sibling", ifs->ifname, strlen(ifs->ifname));
...
int wpa_supplicant_fast_associate(struct wpa_supplicant *wpa_s)
{
	...
	if (os_reltime_expired(&now, &wpa_s->last_scan, 5)) {
		wpa_warning_log0(MSG_DEBUG, "Fast associate: Old scan results");
		return -1;
	}
			    ...
			    size_t len)
{
	...
	if (res) {
		wpa_warning_log0(MSG_DEBUG, "Interworking: Failed to configure QoS Map Set to the driver");
	}
				...
				const u8 *ie, size_t ie_len)
{
	...
	while (ie_len >= 2 && mb_ies.nof_ies < MAX_NOF_MB_IES_SUPPORTED) {
		...
		if (ie[0] == WLAN_EID_MULTI_BAND) {
			wpa_warning_log1(MSG_DEBUG, "MB IE of %u bytes found",
				   (unsigned int) len);
...
#ifndef LOS_CONFIG_80211_IES_CROP
...
#endif /*LOS_CONFIG_80211_IES_CROP*/
...
no_pfs:
	...
	while (p && l >= 2) {
		...
		if (!wpa_found &&
		    ...
		    os_memcmp(&p[2], "\x00\x50\xF2\x01\x01\x00", 6) == 0) {
			wpa_found = 1;
			(void)wpa_sm_set_ap_wpa_ie(wpa_s->wpa, p, len);
		}

		if (!rsn_found &&
		    p[0] == WLAN_EID_RSN && p[1] >= 2) {
			rsn_found = 1;
			(void)wpa_sm_set_ap_rsn_ie(wpa_s->wpa, p, len);
		}
	...
	if (!wpa_found && data->assoc_info.beacon_ies)
		(void)wpa_sm_set_ap_wpa_ie(wpa_s->wpa, NULL, 0);
	if (!rsn_found && data->assoc_info.beacon_ies)
		(void)wpa_sm_set_ap_rsn_ie(wpa_s->wpa, NULL, 0);
				     ...
				     union wpa_event_data *data)
{
	...
	if (!wpas_fst_update_mbie(wpa_s, ieprb, bss->ie_len))
		wpa_warning_log0(MSG_DEBUG, "FST: MB IEs updated from bss IE");
	else if (!wpas_fst_update_mbie(wpa_s, iebcn, bss->beacon_ie_len))
		wpa_warning_log0(MSG_DEBUG, "FST: MB IEs updated from bss beacon IE");
#endif /* CONFIG_FST */
}
				       ...
				       union wpa_event_data *data)
{
	...
	if (wpa_s->ap_iface) {
		if (!data)
			return;
		(void)hostapd_notif_assoc(wpa_s->ap_iface->bss[0],
...
#endif /* CONFIG_NO_RANDOM_POOL */
		(void)os_memcpy(wpa_s->bssid, bssid, ETH_ALEN);
	...
	if (wpa_s->current_ssid) {
		...
		(void)wpa_supplicant_scard_init(wpa_s, wpa_s->current_ssid);
	}
#endif /* HISI_CODE_CROP */
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_NONE ||
		...
		if (wpa_s->current_ssid &&
			 ...
			 * from the driver.
			 */
			(void)wpa_supplicant_set_wpa_none_key(wpa_s,
							wpa_s->current_ssid);
		}
#endif /* HISI_WPA_KEY_MGMT_CROP */
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_NONE) {
		...
		(void)wpa_set_wep_keys(wpa_s, wpa_s->current_ssid);
	}
	...
	if (data) {
		...
		if (wpa_s->reassoc_same_bss)
			(void)wmm_ac_restore_tspecs(wpa_s);
	}
#endif /* CONFIG_WMM_AC */
#ifdef CONFIG_FILS
						 ...
						 int locally_generated)
{
	...
	(void)os_memcpy(prev_pending_bssid, wpa_s->pending_bssid, ETH_ALEN);
#ifndef HISI_WPA_KEY_MGMT_CROP
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPA_NONE) {
		/*
		 * At least Host AP driver and a Prism3 card seemed to be
	...
	if (!wpa_s->disconnected &&
		...
		if (wpa_s->wpa_state == WPA_COMPLETED &&
			...
			wpa_dbg(wpa_s, MSG_DEBUG, "Do not request new "
				"immediate scan");
#else
		if (wpa_check_reconnect_timeout_match(wpa_s)) {
			(void)eloop_register_timeout(g_reconnect_set.timeout, 0,
				wpa_supplicant_reconnect_timeout, wpa_s, NULL);
			g_reconnect_set.pending_flag = WPA_FLAG_ON;
			g_connecting_flag = WPA_FLAG_ON;
			wpa_error_log0(MSG_DEBUG, "Auto connect enabled: try to reconnect");
			if ((g_connecting_ssid != NULL) && (g_connecting_ssid->frequency == 0)) {
				g_reconnect_set.try_freq_scan_count = WPA_MAX_TRY_FREQ_SCAN_CNT;
				g_connecting_ssid->frequency = wpa_s->assoc_freq;
			}
		}

		if (wpa_s->wpa_state >= WPA_ASSOCIATING) {
			if ((wpa_s->key_mgmt == WPA_KEY_MGMT_WPS) || (g_reconnect_set.pending_flag == WPA_FLAG_ON) ||
				(g_reconnect_set.timeout == WIFI_MAX_RECONNECT_TIMEOUT)) {
				wpa_supplicant_req_scan(wpa_s, 0, 50000);
			}
		} else
			wpa_dbg(wpa_s, MSG_DEBUG, "Do not request new "
				"immediate scan");
	...
	if (wpa_s->wpa_state >= WPA_AUTHENTICATING) {
		...
		if (g_sta_delay_report_flag == WPA_FLAG_OFF)
			hi_at_printf("+NOTICE:DISCONNECTED\r\n");
		else
			wpa_save_disconnect_event(wpa_s, bssid, reason_code);
		if ((g_wpa_event_cb != NULL) && (g_sta_delay_report_flag == WPA_FLAG_OFF)) {
			hi_wifi_event wpa_events = {0};
			wpa_events.event = HI_WIFI_EVT_DISCONNECTED;
			(void)os_memcpy(wpa_events.info.wifi_disconnected.ifname, wpa_s->ifname, WIFI_IFNAME_MAX_SIZE);
			(void)os_memcpy(wpa_events.info.wifi_disconnected.bssid, bssid, ETH_ALEN);
			wpa_events.info.wifi_disconnected.reason_code = reason_code;
			wifi_new_task_event_cb(&wpa_events);
		}
	}
					 ...
					 union wpa_event_data *data)
{
	...
	if ((wpa_s->last_michael_mic_error.sec &&
		...
		(void)wpa_blacklist_add(wpa_s, wpa_s->bssid);
		...
		(void)wpa_drv_set_countermeasures(wpa_s, 1);
		wpa_supplicant_deauthenticate(wpa_s,
					      WLAN_REASON_MICHAEL_MIC_FAILURE);
		(void)eloop_cancel_timeout(wpa_supplicant_stop_countermeasures,
				     wpa_s, NULL);
		(void)eloop_register_timeout(60, 0,
				  ...
				  const u8 *ie, size_t ie_len, int deauth)
{
	...
	if (wpa_s->ap_iface && addr) {
		(void)hostapd_notif_disassoc(wpa_s->ap_iface->bss[0], addr);
		return;
	}
				...
				struct disassoc_info *info)
{
	...
	if (wpa_s->ap_iface && info && info->addr) {
		(void)hostapd_notif_disassoc(wpa_s->ap_iface->bss[0], info->addr);
		return;
	}
...
#if defined (CONFIG_P2P) || defined(CONFIG_SME)
	const u8 *payload;
	size_t plen;
	u8 category;
#endif /* CONFIG_P2P || CONFIG_SME */
				    ...
				    union wpa_event_data *data)
{
	...
	if (data->assoc_reject.status_code ==
		...
		wpa_warning_log0(MSG_DEBUG, "OWE: Try next supported DH group");
...
#ifndef LTOS_CONFIG_NO_INTERNAL_SME
	if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) {
		sme_event_assoc_reject(wpa_s, data);
		return;
	}
#endif /* LTOS_CONFIG_NO_INTERNAL_SME */
	...
	if (wpa_s->current_ssid &&
		...
		sae_pmksk_is_used = eapol_sm_pmkid_is_used(wpa_s->eapol);
	...
	wpa_supplicant_mark_disassoc(wpa_s);
#endif /* HISI_CODE_CROP */
	if (sae_pmksk_is_used) {
		wpas_connection_failed(wpa_s, bssid);
		wpa_supplicant_mark_disassoc(wpa_s);
		wpa_supplicant_req_scan(wpa_s, 0, 50000);
	} else {
		wpa_supplicant_event_disassoc_finish(wpa_s, data->assoc_reject.status_code, 0);
	}
			  ...
			  union wpa_event_data *data)
{
	...
	int peer_disconnect_flag = WPA_FLAG_OFF;
#endif /* LOS_CONFIG_P2P */
	void hostapd_event(void *ctx, enum wpa_event_type event, union wpa_event_data *data);
	wifi_dev = hi_get_wifi_dev_by_priv(ctx);
	if (wifi_dev == NULL) {
		return;
	}
	if ((wifi_dev->iftype == HI_WIFI_IFTYPE_AP) || (wifi_dev->iftype == HI_WIFI_IFTYPE_P2P_GO)) {
		hostapd_event(g_hapd, event, data);
		return;
	}
#ifdef CONFIG_MESH
	if ((wifi_dev->iftype == HI_WIFI_IFTYPE_MESH_POINT) && (wpa_sta_event_check(event) == 0)) {
		hostapd_event(g_hapd, event, data);
		return;
	}
#endif /* CONFIG_MESH */
#endif /* CONFIG_LITEOS_WPA */
	wpa_warning_log1(MSG_DEBUG, "WPA Event (%d)", event);
	...
	wpa_dbg(wpa_s, level, "Event %s (%d) received",
		event_to_string(event), event);
#else
	wpa_dbg(wpa_s, level, "Event (%d) received", event);
#endif /* HISI_CODE_CROP */
#endif /* CONFIG_NO_STDOUT_DEBUG */
...
#ifdef LOS_CONFIG_P2P
		if (wpa_s->wpa_state == WPA_COMPLETED) {
			peer_disconnect_flag = WPA_FLAG_ON;
		}
#endif /* LOS_CONFIG_P2P */
		wpas_event_disassoc(wpa_s,
				    data ? &data->disassoc_info : NULL);
#ifdef LOS_CONFIG_P2P
		if ((wifi_dev->iftype == HI_WIFI_IFTYPE_P2P_CLIENT) && (peer_disconnect_flag == WPA_FLAG_ON)) {
			wpas_p2p_group_remove((struct wpa_supplicant *)(wifi_dev->priv), wifi_dev->ifname);
		}
		peer_disconnect_flag = WPA_FLAG_OFF;
#endif /* LOS_CONFIG_P2P */
		if (g_wpa_rm_network == HI_WPA_RM_NETWORK_WORKING)
	...
	case EVENT_SCAN_STARTED:
		if (wpa_s->own_scan_requested ||
			...
			(void)os_get_reltime(&wpa_s->scan_start_time);
...
#ifndef CONFIG_P2P
		if (!(wpa_s) || !(wpa_s->radio) || !(wpa_s->scan_work)) {
#ifndef HISI_CODE_CROP
			wpa_error_log1(MSG_ERROR, "wpa_supplicant_event: interface may have been removed");
#endif /* HISI_CODE_CROP */
			break;
		}
#endif /* CONFIG_P2P */
	...
	case EVENT_ASSOCINFO:
		(void)wpa_supplicant_event_associnfo(wpa_s, data);
		break;
	...
	case EVENT_CH_SWITCH:
		...
		if (data && wpa_s->ifmsh && ssid_temp) {
			ssid_temp->frequency = data->ch_switch.freq;
			wpa_s->ifmsh->freq = data->ch_switch.freq;
			break;
		} else if (wpa_s->ifmsh) {
			break;
		}
		...
		if (!data || !wpa_s->current_ssid)
			break;
#ifdef LOS_WPA_PATCH
		if (hi_freq_to_channel(data->ch_switch.freq, &channel) != HISI_OK) {
			break;
		}
#endif /* LOS_WPA_PATCH */
#ifndef HISI_CODE_CROP
...
#ifdef CONFIG_AP
		if (wpa_s->ap_iface) {
			(void)hostapd_probe_req_rx(wpa_s->ap_iface->bss[0],
...
#endif /* CONFIG_AP */
#ifdef CONFIG_P2P
		(void)wpas_p2p_probe_req_rx(wpa_s, data->rx_probe_req.sa,
				      data->rx_probe_req.da,
				      data->rx_probe_req.bssid,
				      data->rx_probe_req.ie,
				      data->rx_probe_req.ie_len,
	...
	case EVENT_INTERFACE_ENABLED:
		...
		if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED) {
			(void)wpa_supplicant_update_mac_addr(wpa_s);
	...
	case EVENT_SCHED_SCAN_STOPPED:
		...
		if (wpa_s->sched_scan_timed_out) {
			(void)wpa_supplicant_req_sched_scan(wpa_s);
		} else if (wpa_s->pno_sched_pending) {
			wpa_s->pno_sched_pending = 0;
			(void)wpas_start_pno(wpa_s);
		} else if (resched) {
			wpa_supplicant_req_scan(wpa_s, 0, 0);
		}
...
#ifdef CONFIG_WPS
		(void)wpas_wps_start_pbc(wpa_s, NULL, 0);
#endif /* CONFIG_WPS */
		break;
	...
	case EVENT_NEW_PEER_CANDIDATE:
		...
		if (!wpa_s->ifmsh || !data)
			break;
		if (g_mesh_auto_peer_flag) {
			hi_event_new_mesh_peer(wpa_s, data->hi_mesh_peer.peer,
				data->hi_mesh_peer.mesh_bcn_priority, data->hi_mesh_peer.mesh_is_mbr);
		}
#endif /* CONFIG_MESH */
		break;
...
#ifdef CONFIG_SAE
		if (!wpa_s->current_ssid) {
			wpa_warning_log0(MSG_DEBUG, "SAE: current_ssid is NULL");
			break;
		}
