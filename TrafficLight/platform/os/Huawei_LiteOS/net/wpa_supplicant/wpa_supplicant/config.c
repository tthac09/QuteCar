				...
				int line, const char *value)
{
	...
	if (tmp == NULL) {
		wpa_error_two_buf(MSG_ERROR, "wpa_config_parse_str failed to parse %s '%s'.",
			   data->name, strlen(data->name),
			   data->key_data ? "[KEY DATA REMOVED]" : value, data->key_data ? strlen("[KEY DATA REMOVED]") : strlen(value));
		return -1;
	}
	...
	if (data->param3 && res_len < (size_t) data->param3) {
		...
		wpa_printf(MSG_ERROR, "too short %s", data->name);
#endif /* CONFIG_PRINT_NOUSE */
		os_free(tmp);
		return -1;
	}
...
{
#ifndef CONFIG_NO_PBKDF2
	if (g_quick_conn_psk_flag == WPA_FLAG_OFF) {
		(void)pbkdf2_sha1(ssid->passphrase, ssid->ssid, ssid->ssid_len, 4096,
			ssid->psk, PMK_LEN);
	} else {
		os_memcpy(ssid->psk, g_quick_conn_psk, PMK_LEN);
	}
#ifndef CONFIG_PRINT_NOUSE
