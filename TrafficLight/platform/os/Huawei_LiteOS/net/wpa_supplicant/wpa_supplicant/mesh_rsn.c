			   ...
			   const u8 *cat, struct wpabuf *buf)
{
	...
	if (cat == NULL)
		return -EINVAL;
	u8 *pcat = hi_mesh_rsn_protect_frame_set_cat(cat, ie, &pcat_len);
	if ((pcat == NULL) || (pcat_len == 0))
		return -EINVAL;
	...
	pos = (u8 *) (ampe + 1);
#else
	pos = hi_mesh_rsn_protect_frame_set_ampe_ie(rsn, &ampe_ie, &len, cat, buf, sta);
	if ((pos == NULL) || (ampe_ie == NULL) || (len == 0))
		return -EINVAL;
#endif /* LOS_CONFIG_MESH_GTK */
			  ...
			  const u8 *start, size_t elems_len)
{
	...
	size_t crypt_len;
	if ((elems == NULL) || (sta == NULL) || (wpa_s == NULL) || (cat == NULL))
		return -1;
#ifndef LOS_CONFIG_MESH
...
#else
	u8 const *pcat = hi_mesh_rsn_process_ampe_set_cat(elems);
	if (pcat == NULL)
		return -1;
	const u8 *aad[] = { sta->addr, wpa_s->own_addr, pcat - 2};
	const size_t aad_len[] = { ETH_ALEN, ETH_ALEN, elems->mic ? elems->mic - pcat : 0};
#endif /* LOS_CONFIG_MESH */
	size_t key_len;
	...
	if (!elems->mic || elems->mic_len < AES_BLOCK_SIZE) {
		wpa_error_log0(MSG_DEBUG, "Mesh RSN: missing mic ie or mesh id ie");
		return -1;
	}
	 ...
	 * frames */
#ifdef LOS_CONFIG_MESH_GTK
	if (hi_mesh_mesh_rsn_process_ampe_check(&pos, cat, sta) != HISI_OK) {
		ret = -1;
		goto free;
	}
