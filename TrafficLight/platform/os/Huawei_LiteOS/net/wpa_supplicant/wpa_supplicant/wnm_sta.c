...
{
	wpa_warning_log1(MSG_DEBUG, "ieee80211_11_get_tfs_ie: TFS get operation %d", oper);
				   ...
				   enum wnm_oper oper)
{
	...
	wpa_warning_log1(MSG_DEBUG, "ieee80211_11_set_tfs_ie: TFS set operation %d", oper);
				 ...
				 u8 action, u16 intval, struct wpabuf *tfs_req)
{
	...
	wpa_warning_buf(MSG_DEBUG, "WNM: Request to send WNM-Sleep Mode Request "
		   "action=%s to ",
		   action == 0 ? "enter" : "exit", action == 0 ? strlen("enter") : strlen("exit"));
	wpa_warning_log4(MSG_DEBUG, "WNM: Request to send WNM-Sleep Mode Request " "%02x:xx:xx:%02x:%02x:%02x",
		   (wpa_s->bssid)[0], (wpa_s->bssid)[3], (wpa_s->bssid)[4], (wpa_s->bssid)[5]);
	/* WNM-Sleep Mode IE */
	...
	mgmt = os_zalloc(sizeof(*mgmt) + wnmsleep_ie_len + wnmtfs_ie_len +
			 oci_ie_len);
	if (mgmt == NULL) {
		wpa_warning_log0(MSG_DEBUG, "MLME: Failed to allocate buffer for "
	...
	if (res < 0)
		wpa_warning_log2(MSG_DEBUG, "Failed to send WNM-Sleep Request "
					 ...
					 const u8 *tfsresp_ie_end)
{
	...
	if (tfsresp_ie_start && tfsresp_ie_end &&
		...
		tfsresp_ie_len = (tfsresp_ie_end + tfsresp_ie_end[1] + 2) -
			tfsresp_ie_start;
		wpa_warning_log0(MSG_DEBUG, "TFS Resp IE(s) found");
		/* pass the TFS Resp IE(s) to driver for processing */
		if (ieee80211_11_set_tfs_ie(wpa_s, wpa_s->bssid,
					    tfsresp_ie_start,
					    tfsresp_ie_len,
					    WNM_SLEEP_TFS_RESP_IE_SET))
			wpa_warning_log0(MSG_DEBUG, "WNM: Fail to set TFS Resp IE");
	}
}
					...
					const u8 *frm, u16 key_len_total)
{
	...
	while (end - ptr > 1) {
		if (2 + ptr[1] > end - ptr) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Invalid Key Data element "
				   "length");
			if (end > ptr) {
		...
		if (*ptr == WNM_SLEEP_SUBELEM_GTK) {
			if (ptr[1] < 11 + 5) {
				wpa_warning_log0(MSG_DEBUG, "WNM: Too short GTK "
					   "subelem");
				break;
			}
			gtk_len = *(ptr + 4);
			if (ptr[1] < 11 + gtk_len ||
				...
				wpa_warning_log0(MSG_DEBUG, "WNM: Invalid GTK "
					   "subelem");
				break;
			}
			...
			if (ptr[1] < 2 + 6 + WPA_IGTK_LEN) {
				wpa_warning_log0(MSG_DEBUG, "WNM: Too short IGTK "
					   "subelem");
				break;
			}
					...
					const u8 *frm, int len)
{
	...
	wpa_warning_log2(MSG_DEBUG, "WNM-Sleep Mode Response token=%u key_len_total=%d",
		   frm[0], key_len_total);
	left = len - 3;
	if (key_len_total > left) {
		wpa_warning_log0(MSG_INFO, "WNM: Too short frame for Key Data field");
		return;
	}
	pos += 3 + key_len_total;
	while (pos - frm + 1 < len) {
		...
		if (2 + ie_len > frm + len - pos) {
			wpa_warning_log1(MSG_INFO, "WNM: Invalid IE len %u", ie_len);
			break;
		}
		...
		if (*pos == WLAN_EID_WNMSLEEP && ie_len >= 4)
			...
			wpa_warning_log1(MSG_DEBUG, "EID %d not recognized", *pos);
		pos += ie_len + 2;
	}

	if (!wnmsleep_ie) {
		wpa_warning_log0(MSG_DEBUG, "No WNM-Sleep IE found");
		return;
	}
	...
	if (wnmsleep_ie->status == WNM_STATUS_SLEEP_ACCEPT ||
		...
		wpa_warning_log2(MSG_DEBUG, "Successfully recv WNM-Sleep Response "
		...
		wpa_warning_log2(MSG_DEBUG, "Reject recv WNM-Sleep Response frame "
			   "(action=%d, intval=%d)",
			   wnmsleep_ie->action_type, wnmsleep_ie->intval);
		if (wnmsleep_ie->action_type == WNM_SLEEP_MODE_ENTER)
					   ...
					   u8 id, u8 elen, const u8 *pos)
{
	...
	case WNM_NEIGHBOR_TSF:
		if (elen < 2 + 2) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Too short TSF");
			break;
		}
	...
	case WNM_NEIGHBOR_CONDENSED_COUNTRY_STRING:
		if (elen < 2) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Too short condensed "
				   "country string");
			break;
		}
	...
	case WNM_NEIGHBOR_BSS_TRANSITION_CANDIDATE:
		if (elen < 1) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Too short BSS transition "
				   "candidate");
			break;
		}
	...
	case WNM_NEIGHBOR_BSS_TERMINATION_DURATION:
		if (elen < 10) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Too short bss_term_tsf");
			break;
		}
	...
	case WNM_NEIGHBOR_BEARING:
		if (elen < 8) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Too short neighbor "
				   "bearing");
			break;
		}
	...
	case WNM_NEIGHBOR_MEASUREMENT_PILOT:
		if (elen < 1) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Too short measurement "
				   "pilot");
			break;
		}
	...
	case WNM_NEIGHBOR_RRM_ENABLED_CAPABILITIES:
		if (elen < 5) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Too short RRM enabled "
				   "capabilities");
			break;
		}
	...
	case WNM_NEIGHBOR_MULTIPLE_BSSID:
		if (elen < 1) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Too short multiple BSSID");
			break;
		}
				      ...
				      struct neighbor_report *rep)
{
	...
	if (left < 13) {
		wpa_warning_log0(MSG_DEBUG, "WNM: Too short neighbor report");
		return;
	}
	...
	while (left >= 2) {
		...
		id = *pos++;
		elen = *pos++;
		wpa_warning_log2(MSG_DEBUG, "WNM: Subelement id=%u len=%u", id, elen);
		left -= 2;
		if (elen > left) {
			wpa_warning_log0(MSG_DEBUG,
			      ...
			      enum mbo_transition_reject_reason *reason)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WNM: Current BSS " "%02x:xx:xx:%02x:%02x:%02x",
		   (wpa_s->bssid)[0], (wpa_s->bssid)[3], (wpa_s->bssid)[4], (wpa_s->bssid)[5]);
	wpa_warning_log1(MSG_DEBUG, "WNM: Current BSS " " RSSI %d", bss->level);
	...
	for (i = 0; i < wpa_s->wnm_num_neighbor_report; i++) {
		...
		if (nei->preference_present && nei->preference == 0) {
			wpa_warning_log4(MSG_DEBUG, "Skip excluded BSS " "%02x:xx:xx:%02x:%02x:%02x",
				   (nei->bssid)[0], (nei->bssid)[3], (nei->bssid)[4], (nei->bssid)[5]);
			continue;
		}

		target = wpa_bss_get_bssid(wpa_s, nei->bssid);
		if (!target) {
			wpa_warning_log4(MSG_DEBUG, "Candidate BSS " "%02x:xx:xx:%02x:%02x:%02x",
				   (nei->bssid)[0], (nei->bssid)[3], (nei->bssid)[4], (nei->bssid)[5]);
			wpa_warning_log1(MSG_DEBUG, " (pref %d) not found in scan results",
		...
		if (wpa_s->current_ssid &&
			...
			wpa_warning_log4(MSG_DEBUG, "Candidate BSS " "%02x:xx:xx:%02x:%02x:%02x",
				   (nei->bssid)[0], (nei->bssid)[3], (nei->bssid)[4], (nei->bssid)[5]);
			wpa_warning_log1(MSG_DEBUG, " (pref %d) does not match the current network profile",
				   nei->preference_present ? nei->preference :
				   -1);
			continue;
		}
		...
		if (wpa_is_bss_tmp_disallowed(wpa_s, target->bssid)) {
			wpa_warning_log4(MSG_DEBUG,
				   "MBO: Candidate BSS " "%02x:xx:xx:%02x:%02x:%02x"
		...
		if (target->level < bss->level && target->level < -80) {
			wpa_warning_log4(MSG_DEBUG, "Candidate BSS " "%02x:xx:xx:%02x:%02x:%02x",
				   (nei->bssid)[0], (nei->bssid)[3], (nei->bssid)[4], (nei->bssid)[5]);
			wpa_warning_log2(MSG_DEBUG, "Candidate BSS "
	...
	if (target) {
		wpa_warning_log4(MSG_DEBUG,
			   ...
			   (target->bssid)[0], (target->bssid)[3], (target->bssid)[4], (target->bssid)[5]);
		wpa_warning_log1(MSG_DEBUG, " (RSSI %d)", target->level);
	}
	...
	u8 delay, const u8 *target_bssid)
{
	...
	wpa_warning_log4(MSG_DEBUG,
		   "WNM: Send BSS Transition Management Response to " "%02x:xx:xx:%02x:%02x:%02x",
		   (wpa_s->bssid)[0], (wpa_s->bssid)[3], (wpa_s->bssid)[4], (wpa_s->bssid)[5]);
	wpa_warning_log4(MSG_DEBUG,
		   "WNM: Send BSS Transition Management Response to " " dialog_token=%u status=%u reason=%u delay=%d",
		   dialog_token, status, reason, delay);
			       ...
			       int after_new_scan)
{
	...
	wpa_s->reassociate = 1;
	wpa_warning_log0(MSG_DEBUG, "WNM: Issuing connect");
...
int wnm_scan_process(struct wpa_supplicant *wpa_s, int reply_on_fail)
{
	...
	if (os_reltime_before(&wpa_s->wnm_cand_valid_until,
		...
		wpa_warning_log0(MSG_DEBUG, "WNM: Previously stored BSS transition candidate list is not valid anymore - drop it");
		wnm_deallocate_memory(wpa_s);
		return 0;
	}
	...
	if (!wpa_s->current_bss ||
	    os_memcmp(wpa_s->wnm_cand_from_bss, wpa_s->current_bss->bssid,
		      ETH_ALEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "WNM: Stored BSS transition candidate list not from the current BSS - ignore it");
		return 0;
	}
	...
	if (!bss) {
		wpa_warning_log0(MSG_DEBUG, "WNM: No BSS transition candidate match found");
...
static void wnm_dump_cand_list(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WNM: BSS Transition Candidate List");
	if (!wpa_s->wnm_neighbor_report_elements)
		return;
	for (i = 0; i < wpa_s->wnm_num_neighbor_report; i++) {
		struct neighbor_report *nei;
		...
		wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x",
			   (nei->bssid)[0], (nei->bssid)[3], (nei->bssid)[4], (nei->bssid)[5]);
		wpa_warning_log4(MSG_DEBUG, "%u: " " info=0x%x op_class=%u chan=%u",
			   i, nei->bssid_info, nei->regulatory_class, nei->channel_number);
		wpa_warning_log3(MSG_DEBUG, " phy=%u pref=%d freq=%d",
			   nei->phy_type, nei->preference_present ? nei->preference : -1, nei->freq);
	...
	for (i = 0; i < wpa_s->wnm_num_neighbor_report; i++) {
		...
		if (nei->freq <= 0) {
			wpa_warning_log4(MSG_DEBUG,
				   ...
				   "%02x:xx:xx:%02x:%02x:%02x" " - scan all channels",
				   (nei->bssid)[0], (nei->bssid)[3], (nei->bssid)[4], (nei->bssid)[5]);
			os_free(freqs);
			return;
		}
					     ...
					     int reply)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WNM: BSS Transition Management Request: "
	...
	if (wpa_s->wnm_mode & WNM_BSS_TM_REQ_BSS_TERMINATION_INCLUDED) {
		if (end - pos < 12) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Too short BSS TM Request");
			return;
		}
	...
	if (wpa_s->wnm_mode & WNM_BSS_TM_REQ_ESS_DISASSOC_IMMINENT) {
		...
		if (end - pos < 1 || 1 + pos[0] > end - pos) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Invalid BSS Transition "
	...
	if (wpa_s->wnm_mode & WNM_BSS_TM_REQ_DISASSOC_IMMINENT) {
		...
		if (wpa_s->wnm_dissoc_timer && !wpa_s->scanning) {
			 ...
			 * selection */
			wpa_warning_log0(MSG_DEBUG, "Trying to find another BSS");
	...
	if (wpa_s->wnm_mode & WNM_BSS_TM_REQ_PREF_CAND_LIST_INCLUDED) {
		       ...
		       wpa_s->wnm_num_neighbor_report < WNM_MAX_NEIGHBOR_REPORT)
		{
			...
			wpa_warning_log1(MSG_DEBUG, "WNM: Neighbor report tag %u",
				   tag);
			if (len > end - pos) {
				wpa_warning_log0(MSG_DEBUG, "WNM: Truncated request");
				return;
			}
		...
		wpa_warning_log1(MSG_DEBUG, "WNM: Candidate list valid for %u ms",
			   valid_ms);
		...
		if (wpa_s->wnm_num_neighbor_report == 1) {
			...
			wpa_warning_log4(MSG_DEBUG,
				   ...
				   "%02x:xx:xx:%02x:%02x:%02x", (wpa_s->next_scan_bssid)[0], (wpa_s->next_scan_bssid)[3], (wpa_s->next_scan_bssid)[4], (wpa_s->next_scan_bssid)[5]);
		}
		wpa_supplicant_req_scan(wpa_s, 0, 0);
	} else if (reply) {
		enum bss_trans_mgmt_status_code status;
				       ...
				       int cand_list)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WNM: Send BSS Transition Management Query to "
		   "%02x:xx:xx:%02x:%02x:%02x",
		   (wpa_s->bssid)[0], (wpa_s->bssid)[3], (wpa_s->bssid)[4], (wpa_s->bssid)[5]);
	wpa_warning_log1(MSG_DEBUG, "WNM: Send BSS Transition Management Query to "" query_reason=%u",
		   query_reason);
	wpa_warning_buf(MSG_DEBUG, "WNM: Send BSS Transition Management Query to " "%s",
		   cand_list ? " candidate list" : "", cand_list ? strlen(" candidate list") : strlen(""));
					    ...
					    int len)
{
	...
	while (end - pos > 1) {
		ie = *pos++;
		ie_len = *pos++;
		wpa_warning_log2(MSG_DEBUG, "WNM: WFA subelement %u len %u",
			   ie, ie_len);
		if (ie_len > end - pos) {
			wpa_warning_log0(MSG_DEBUG, "WNM: Not enough room for "
				   "subelement");
			break;
		}
		next = pos + ie_len;
		...
		wpa_warning_log2(MSG_DEBUG, "WNM: Subelement OUI %06x type %u",
		...
		if (ie == WLAN_EID_VENDOR_SPECIFIC && ie_len >= 5 &&
			...
			wpa_warning_log0(MSG_DEBUG, "WNM: Subscription Remediation "
				   "subelement");
			ie_end = pos + ie_len;
			pos += 4;
			url_len = *pos++;
			if (url_len == 0) {
				wpa_warning_log0(MSG_DEBUG, "WNM: No Server URL included");
				url = NULL;
				osu_method = 1;
			} else {
			      ...
			      const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WNM: RX action %u from ", act);
	wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x",
					(mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
	if (wpa_s->wpa_state < WPA_ASSOCIATED ||
	    os_memcmp(mgmt->sa, wpa_s->bssid, ETH_ALEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "WNM: Ignore unexpected WNM Action "
			   "frame");
		return;
	}
	...
	case WNM_COLLOCATED_INTERFERENCE_REQ:
		...
		break;
	default:
		wpa_error_log0(MSG_ERROR, "WNM: Unknown request");
		break;
	}
}
			       ...
			       const struct wpabuf *elems)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WNM: Send Collocated Interference Report to "
		   "%02x:xx:xx:%02x:%02x:%02x",
		   (wpa_s->bssid)[0], (wpa_s->bssid)[3], (wpa_s->bssid)[4], (wpa_s->bssid)[5]);
	wpa_warning_log1(MSG_DEBUG, "WNM: Send Collocated Interference Report to " " (dialog token %u)",
		   dialog_token);
