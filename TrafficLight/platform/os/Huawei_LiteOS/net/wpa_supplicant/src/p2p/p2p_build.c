...
void p2p_buf_add_action_hdr(struct wpabuf *buf, u8 subtype, u8 dialog_token)
{
	...
	wpa_warning_log1(MSG_DEBUG, "P2P: * Dialog Token: %d", dialog_token);
}
...
u8 * p2p_buf_add_ie_hdr(struct wpabuf *buf)
{
	...
	wpabuf_put_be32(buf, P2P_IE_VENDOR_TYPE);
	wpa_warning_log0(MSG_DEBUG, "P2P: * P2P IE header");
	return len;
}
...
void p2p_buf_add_capability(struct wpabuf *buf, u8 dev_capab, u8 group_capab)
{
	...
	wpa_warning_log2(MSG_DEBUG, "P2P: * Capability dev=%02x group=%02x",
...
void p2p_buf_add_go_intent(struct wpabuf *buf, u8 go_intent)
{
	...
	wpa_warning_log2(MSG_DEBUG, "P2P: * GO Intent: Intent %u Tie breaker %u",
				...
				u8 reg_class, u8 channel)
{
	...
	wpa_warning_log2(MSG_DEBUG, "P2P: * Listen Channel: Regulatory Class %u "
				   ...
				   u8 reg_class, u8 channel)
{
	...
	wpabuf_put_u8(buf, reg_class); /* Regulatory Class */
	wpabuf_put_u8(buf, channel); /* Channel Number */
	wpa_warning_log2(MSG_DEBUG, "P2P: * Operating Channel: Regulatory Class %u "
		   "Channel %u", reg_class, channel);
}
				   ...
				   unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		if (p2p_freq_to_channel(preferred_freq_list[i], &op_class,
					&op_channel) < 0) {
			wpa_warning_log1(MSG_DEBUG, "Unsupported frequency %u MHz",
...
void p2p_buf_add_status(struct wpabuf *buf, u8 status)
{
	...
	wpa_warning_log1(MSG_DEBUG, "P2P: * Status: %d", status);
}
			     ...
			     struct p2p_device *peer)
{
	...
	/* Update attribute length */
	WPA_PUT_LE16(len, (u8 *) wpabuf_put(buf, 0) - len - 2);
	wpa_warning_log0(MSG_DEBUG, "P2P: * Device Info");
}
				...
				u8 client_timeout)
{
	...
	wpa_warning_log2(MSG_DEBUG, "P2P: * Configuration Timeout: GO %d (*10ms)  "
...
void p2p_buf_add_intended_addr(struct wpabuf *buf, const u8 *interface_addr)
{
	...
	wpa_warning_log4(MSG_DEBUG, "P2P: * Intended P2P Interface Address " "%02x:xx:xx:%02x:%02x:%02x",
		   interface_addr[0], interface_addr[3], interface_addr[4], interface_addr[5]);
}
			  ...
			  const u8 *ssid, size_t ssid_len)
{
	...
	wpa_warning_log4(MSG_DEBUG, "P2P: * P2P Group ID " "%02x:xx:xx:%02x:%02x:%02x",
		   dev_addr[0], dev_addr[3], dev_addr[4], dev_addr[5]);
...
void p2p_buf_add_invitation_flags(struct wpabuf *buf, u8 flags)
{
	...
	wpa_warning_log1(MSG_DEBUG, "P2P: * Invitation Flags: bitmap 0x%x", flags);
}
				   ...
				   u16 interval)
{
	...
	wpa_warning_log2(MSG_DEBUG, "P2P: * Extended Listen Timing (period %u msec  "
				    ...
				    enum p2p_role_indication role)
{
	...
	wpa_warning_log3(MSG_DEBUG, "P2P: * OOB GO Negotiation Channel: Operating "
...
void p2p_buf_add_session_info(struct wpabuf *buf, const char *info)
{
	...
	if (info) {
		...
		wpa_warning_buf(MSG_DEBUG, "P2P: * Session Info Data (%s)", info, strlen(info));
	}
}
...
void p2p_buf_add_session_id(struct wpabuf *buf, u32 id, const u8 *mac)
{
	...
	wpabuf_put_le32(buf, id);
	wpabuf_put_data(buf, mac, ETH_ALEN);
	wpa_warning_log1(MSG_DEBUG, "P2P: * Session ID Info (%x) ", id);
	wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x",
		   mac[0], mac[3], mac[4], mac[5]);
}
...
void p2p_buf_add_feature_capability(struct wpabuf *buf, u16 len, const u8 *mask)
{
	...
	wpa_warning_log1(MSG_DEBUG, "P2P: * Feature Capability (%d)", len);
}
		     ...
		     int all_attr)
{
	...
	if (pw_id >= 0) {
		...
		wpa_warning_log1(MSG_DEBUG, "P2P: WPS IE Device Password ID: %d",
			   pw_id);
