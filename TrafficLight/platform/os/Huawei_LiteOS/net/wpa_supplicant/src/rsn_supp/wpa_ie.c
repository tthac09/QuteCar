			      ...
			      int key_mgmt)
{
	...
	if (suite == 0) {
		...
		wpa_warning_log1(MSG_WARNING, "Invalid group cipher (%d).",
			   group_cipher);
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log1(MSG_WARNING, "Invalid pairwise cipher (%d).",
			   pairwise_cipher);
#endif /* CONFIG_PRINT_NOUSE */
		return -1;
	}
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log1(MSG_WARNING, "Invalid key management type (%d).",
			   key_mgmt);
#endif /* CONFIG_PRINT_NOUSE */
		return -1;
	}
			      ...
			      struct wpa_sm *sm)
{
	...
	if (rsn_ie_len < sizeof(*hdr) + RSN_SELECTOR_LEN +
		...
		wpa_warning_log1(MSG_DEBUG, "RSN: Too short IE buffer (%lu bytes)",
				     ...
				     struct wpa_eapol_ie_parse *ie)
{
	...
	if (pos[1] < 4) {
#ifndef CONFIG_PRINT_NOUSE
		wpa_msgdump_log1(MSG_MSGDUMP, "Too short vendor specific IE ignored (len=%u)",
			   pos[1]);
#endif /* CONFIG_PRINT_NOUSE */
		return 1;
	}
