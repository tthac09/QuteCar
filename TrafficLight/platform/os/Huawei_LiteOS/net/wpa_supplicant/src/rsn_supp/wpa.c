...
void wpa_sm_key_request(struct wpa_sm *sm, int error, int pairwise)
{
	...
	(void)os_memcpy(reply->replay_counter, sm->request_counter,
...
static void wpa_supplicant_key_mgmt_set_pmk(struct wpa_sm *sm)
{
	...
	if (sm->key_mgmt == WPA_KEY_MGMT_FT_IEEE8021X) {
		...
		wpa_sm_key_mgmt_set_pmk(sm, sm->pmk, sm->pmk_len);
				  ...
				  const u8 *pmkid)
{
	    ...
	    !wpa_key_mgmt_ft(sm->key_mgmt) && sm->key_mgmt != WPA_KEY_MGMT_OSEN)
	{
		...
		buf = wpa_sm_alloc_eapol(sm, IEEE802_1X_TYPE_EAPOL_START,
					 NULL, 0, &buflen, NULL);
		if (buf) {
			(void)wpa_sm_ether_send(sm, sm->bssid, ETH_P_EAPOL,
					  buf, buflen);
			os_free(buf);
			return -2;
		}
			       ...
			       struct wpa_ptk *ptk)
{
	...
	if (sm->proto == WPA_PROTO_RSN)
		...
		(void)os_memcpy(reply->key_length, key->key_length, 2);
	(void)os_memcpy(reply->replay_counter, key->replay_counter,
		  WPA_REPLAY_COUNTER_LEN);
#ifndef CONFIG_PRINT_NOUSE
	...
	(void)os_memcpy(reply->key_nonce, nonce, WPA_NONCE_LEN);

#ifndef CONFIG_PRINT_NOUSE
	wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Sending EAPOL-Key 2/4,mic_len = %d",mic_len);
#endif /* CONFIG_PRINT_NOUSE */
					  ...
					  size_t key_data_len)
{
	...
	if (sm->pairwise_cipher == WPA_CIPHER_TKIP) {
		...
		(void)os_memcpy(buf, &ptk->tk[16], 8);
		(void)os_memcpy(&ptk->tk[16], &ptk->tk[24], 8);
		(void)os_memcpy(&ptk->tk[24], buf, 8);
		(void)forced_memzero(buf, sizeof(buf));
	}
	sm->tptk_set = 1;
	...
	if (sm->p2p) {
		...
		if (kde_buf) {
			u8 *pos;
			wpa_warning_log0(MSG_DEBUG, "P2P: Add IP Address Request KDE "
				   "into EAPOL-Key 2/4");
			os_memcpy(kde_buf, kde, kde_len);
			kde = kde_buf;
			pos = kde + kde_len;
	...
	os_free(kde_buf);
#endif /* CONFIG_P2P */
	(void)os_memcpy(sm->anonce, key->key_nonce, WPA_NONCE_LEN);
	return;
				      ...
				      const struct wpa_eapol_key *key)
{
	...
	if (sm->wpa_ptk_rekey) {
		(void)eloop_cancel_timeout(wpa_sm_rekey_ptk, sm, NULL);
		(void)eloop_register_timeout(sm->wpa_ptk_rekey, 0, wpa_sm_rekey_ptk,
				       sm, NULL);
	}
				      ...
				      const u8 *key_rsc, int wnm_sleep)
{
	...
	if (sm->group_cipher == WPA_CIPHER_TKIP) {
		...
		(void)os_memcpy(gtk_buf, gd->gtk, 16);
		(void)os_memcpy(gtk_buf + 16, gd->gtk + 24, 8);
		(void)os_memcpy(gtk_buf + 24, gd->gtk + 16, 8);
		_gtk = gtk_buf;
	}
	if (sm->pairwise_cipher == WPA_CIPHER_NONE) {
...
#endif
		(void)forced_memzero(gtk_buf, sizeof(gtk_buf));
		return -1;
	}
	(void)forced_memzero(gtk_buf, sizeof(gtk_buf));
#ifndef HISI_CODE_CROP
				       ...
				       int key_info)
{
	...
	(void)os_memcpy(gd.gtk, gtk, gtk_len);
	gd.gtk_len = gtk_len;
				       ...
				       int wnm_sleep)
{
	...
	if (keyidx == 1024 || keyidx == 1280) {
		...
		wpa_printf(MSG_ERROR, "PMF: Workaround - Allow Key ID in big endian format(%d)",
			   keyidx);
...
{
	u16 key_info, keylen;
	struct wpa_eapol_ie_parse ie = {0};;
...
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
		(void)wpa_sm_mlme_setprotection(
...
failed:
	...
	wpa_sm_deauthenticate(sm, WLAN_REASON_4WAY_HANDSHAKE_TIMEOUT);
#else
	wpa_sm_deauthenticate(sm, WLAN_REASON_UNSPECIFIED);
					     ...
					     struct wpa_gtk_data *gd)
{
	...
	(void)os_memcpy(gd->gtk, ie.gtk + 2, ie.gtk_len - 2);
					     ...
					     u16 ver, struct wpa_gtk_data *gd)
{
	...
	if (ver == WPA_KEY_INFO_TYPE_HMAC_MD5_RC4 && sm->ptk.kek_len == 16) {
		...
		(void)os_memcpy(ek, key->key_iv, 16);
		(void)os_memcpy(ek + 16, sm->ptk.kek, sm->ptk.kek_len);
		(void)os_memcpy(gd->gtk, key_data, key_data_len);
		if (rc4_skip(ek, 32, 256, gd->gtk, key_data_len)) {
			(void)forced_memzero(ek, sizeof(ek));
#ifndef CONFIG_PRINT_NOUSE
			wpa_msg(sm->ctx->msg_ctx, MSG_ERROR,
				"WPA: RC4 failed");
#endif /* CONFIG_PRINT_NOUSE */
					       ...
					       const u8 *buf, size_t len)
{
	...
	if (sm->tptk_set) {
		...
		continue_fuzz:
			...
			sm->tptk_set = 0;
			sm->ptk_set = 1;
			(void)os_memcpy(&sm->ptk, &sm->tptk, sizeof(sm->ptk));
			(void)os_memset(&sm->tptk, 0, sizeof(sm->tptk));
			/*
	...
	(void)os_memcpy(sm->rx_replay_counter, key->replay_counter,
		  WPA_REPLAY_COUNTER_LEN);
					   ...
					   u8 *key_data, size_t *key_data_len)
{
...
#endif /* #ifndef CONFIG_PRINT_NOUSE */
	...
	continue_fuzz:
#endif /* TEST_FUZZ */
		(void)os_memcpy(key_data, buf, *key_data_len);
		bin_clear_free(buf, *key_data_len);
				 ...
				 size_t *key_data_len)
{
	...
	if (*key_data_len < AES_BLOCK_SIZE) {
		wpa_warning_log0(MSG_INFO, "No room for AES-SIV data in the frame");
		return -1;
	}
	...
	if (aes_siv_decrypt(ptk->kek, ptk->kek_len, pos, *key_data_len,
			    1, aad, aad_len, tmp) < 0) {
		wpa_warning_log0(MSG_INFO, "Invalid AES-SIV data in the frame");
				 ...
				 void *ctx, enum pmksa_free_reason reason)
{
	...
	if (sm->cur_pmksa == entry) {
		 ...
		 */
#ifdef LOS_CONFIG_NO_PMKSA
		if (reason != PMKSA_REPLACE)
			deauth = 1;
#else
		/* if pmksa lifetime expires, sta do not deauth from ap, just clear pmksa cache */
		if ((reason != PMKSA_REPLACE) && (reason != PMKSA_EXPIRE))
			deauth = 1;
#endif /* LOS_CONFIG_NO_PMKSA */
	}
	...
	if (deauth) {
		sm->pmk_len = 0;
		(void)os_memset(sm->pmk, 0, sizeof(sm->pmk));
		wpa_sm_deauthenticate(sm, WLAN_REASON_UNSPECIFIED);
	}
}
#endif /* LOS_CONFIG_NO_PMKSA */
...
#endif /* LOS_CONFIG_NO_PMKSA */
#ifndef HISI_CODE_CROP
	(void)eloop_cancel_timeout(wpa_sm_start_preauth, sm, NULL);
#endif /* HISI_CODE_CROP */
	(void)eloop_cancel_timeout(wpa_sm_rekey_ptk, sm, NULL);
...
void wpa_sm_notify_assoc(struct wpa_sm *sm, const u8 *bssid)
{
...
#endif /* CONFIG_PRINT_NOUSE */
	(void)os_memcpy(sm->bssid, bssid, ETH_ALEN);
	(void)os_memset(sm->rx_replay_counter, 0, WPA_REPLAY_COUNTER_LEN);
	...
	if (clear_keys) {
		...
		sm->tptk_set = 0;
		(void)os_memset(&sm->tptk, 0, sizeof(sm->tptk));
		(void)os_memset(&sm->gtk, 0, sizeof(sm->gtk));
#ifndef HISI_CODE_CROP
		(void)os_memset(&sm->gtk_wnm_sleep, 0, sizeof(sm->gtk_wnm_sleep));
#endif /* HISI_CODE_CROP */
#ifdef CONFIG_IEEE80211W
		(void)os_memset(&sm->igtk, 0, sizeof(sm->igtk));
#ifndef HISI_CODE_CROP
		(void)os_memset(&sm->igtk_wnm_sleep, 0, sizeof(sm->igtk_wnm_sleep));
#endif /* HISI_CODE_CROP */
#endif /* CONFIG_IEEE80211W */
	}
...
void wpa_sm_set_pmk_from_pmksa(struct wpa_sm *sm)
{
	...
	if (sm->cur_pmksa) {
		...
		(void)os_memcpy(sm->pmk, sm->cur_pmksa->pmk, sm->pmk_len);
	} else {
...
void wpa_sm_set_config(struct wpa_sm *sm, struct rsn_supp_config *config)
{
	...
	if (config) {
		...
		if (config->ssid) {
			(void)os_memcpy(sm->ssid, config->ssid, config->ssid_len);
...
{
	if (sm)
		(void)os_memcpy(sm->own_addr, addr, ETH_ALEN);
}
...
#ifndef HISI_CODE_CROP
void wpa_sm_get_chipher_txt(struct wpa_sm *sm, const char **pairwise_cipher, const char **group_cipher)
{
	*pairwise_cipher = wpa_cipher_txt(sm->pairwise_cipher);
	*group_cipher = wpa_cipher_txt(sm->group_cipher);
	return;
}
...
#endif /* CONFIG_IEEE80211W */
#ifdef CONFIG_IEEE80211R
	(void)os_memset(sm->xxkey, 0, sizeof(sm->xxkey));
#ifndef HISI_CODE_CROP
	sm->xxkey_len = 0;
#endif /* HISI_CODE_CROP */
	(void)os_memset(sm->pmk_r0, 0, sizeof(sm->pmk_r0));
#ifndef HISI_CODE_CROP
	sm->pmk_r0_len = 0;
#endif /* HISI_CODE_CROP */
	(void)os_memset(sm->pmk_r1, 0, sizeof(sm->pmk_r1));
#ifndef HISI_CODE_CROP
	sm->pmk_r1_len = 0;
#endif /* HISI_CODE_CROP */
#endif /* CONFIG_IEEE80211R */
...
{
	(void)os_memcpy(sm->rx_replay_counter, replay_ctr, WPA_REPLAY_COUNTER_LEN);
}
...
int wpa_wnmsleep_install_key(struct wpa_sm *sm, u8 subelem_id, u8 *buf)
{
	...
	if (subelem_id == WNM_SLEEP_SUBELEM_GTK) {
		...
		if (gd.alg == WPA_ALG_NONE) {
			wpa_warning_log0(MSG_DEBUG, "Unsupported group cipher suite");
			return -1;
		}
		...
		if (wpa_supplicant_install_gtk(sm, &gd, key_rsc, 1)) {
			forced_memzero(&gd, sizeof(gd));
			wpa_warning_log0(MSG_DEBUG, "Failed to install the GTK in "
				   "WNM mode");
			return -1;
		}
		forced_memzero(&gd, sizeof(gd));
		...
		wpa_warning_log0(MSG_DEBUG, "Unknown element id");
		return -1;
	}
...
void wpa_sm_set_rx_replay_ctr(struct wpa_sm *sm, const u8 *rx_replay_counter)
{
	...
	sm->rx_replay_counter_set = 1;
#ifndef CONFIG_PRINT_NOUSE
	wpa_warning_log0(MSG_DEBUG, "Updated key replay counter");
#endif /* CONFIG_PRINT_NOUSE */
}
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log0(MSG_DEBUG, "Updated PTK KCK");
#endif /* CONFIG_PRINT_NOUSE */
	}
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log0(MSG_DEBUG, "Updated PTK KEK");
#endif /* CONFIG_PRINT_NOUSE */
	}
	sm->ptk_set = 1;
}
...
struct wpabuf * fils_build_auth(struct wpa_sm *sm, int dh_group, const u8 *md)
{
	...
	if (!sm->assoc_wpa_ie) {
		wpa_warning_log0(MSG_INFO, "FILS: No own RSN IE set for FILS");
		goto fail;
	}
		      ...
		      size_t len)
{
	...
	if (sm->fils_dh_group) {
		...
		if (!sm->fils_ecdh) {
			wpa_warning_log0(MSG_DEBUG, "FILS: No ECDH state available");
			goto fail;
		}
	...
	if (!elems.rsn_ie ||
	    wpa_parse_wpa_ie_rsn(elems.rsn_ie - 2, elems.rsn_ie_len + 2,
				 &rsn) < 0) {
		wpa_warning_log0(MSG_DEBUG, "FILS: No RSN element");
		goto fail;
	}
	...
	if (wpa_key_mgmt_ft(sm->key_mgmt)) {
		...
		if (!elems.mdie || !elems.ftie) {
			wpa_warning_log0(MSG_DEBUG, "FILS+FT: No MDE or FTE");
			goto fail;
		}

		if (wpa_ft_parse_ies(pos, end - pos, &parse,
				     wpa_key_mgmt_sha384(sm->key_mgmt)) < 0) {
			wpa_warning_log0(MSG_DEBUG, "FILS+FT: Failed to parse IEs");
			goto fail;
		}
	...
	if (!sm->cur_pmksa && elems.fils_wrapped_data) {
		...
		if (!sm->fils_erp_pmkid_set) {
			wpa_warning_log0(MSG_DEBUG, "FILS: PMKID not available");
			goto fail;
		}
		wpa_hexdump(MSG_DEBUG, "FILS: PMKID", sm->fils_erp_pmkid,
			    PMKID_LEN);
		wpa_warning_log0(MSG_DEBUG, "FILS: ERP processing succeeded - add PMKSA cache entry for the result");
	...
	if (fils_pmk_to_ptk(sm->pmk, sm->pmk_len, sm->own_addr, sm->bssid,
		...
		wpa_warning_log0(MSG_DEBUG, "FILS: Failed to derive PTK");
		goto fail;
	}
	...
	if (sm->fils_dh_group) {
		if (!sm->fils_ecdh) {
			wpa_warning_log0(MSG_INFO, "FILS: ECDH not initialized");
			goto fail;
		}
		pub = crypto_ecdh_get_pubkey(sm->fils_ecdh, 1);
		if (!pub)
		...
		if (!g_ap) {
			wpa_warning_log0(MSG_INFO, "FILS: gAP not available");
			goto fail;
		}
...
static int fils_ft_build_assoc_req_rsne(struct wpa_sm *sm, struct wpabuf *buf)
{
	...
	if (!wpa_cipher_valid_group(sm->group_cipher)) {
		wpa_warning_log1(MSG_WARNING, "FT: Invalid group cipher (%d)",
			   sm->group_cipher);
		return -1;
	}
	...
	/* Pairwise Suite List */
	if (!wpa_cipher_valid_pairwise(sm->pairwise_cipher)) {
		wpa_warning_log1(MSG_WARNING, "FT: Invalid pairwise cipher (%d)",
			   sm->pairwise_cipher);
		return -1;
	}
	pos = wpabuf_put(buf, RSN_SELECTOR_LEN);
	...
	if (wpa_derive_pmk_r0(sm->fils_ft, sm->fils_ft_len, sm->ssid,
		...
		wpa_warning_log0(MSG_WARNING, "FILS+FT: Could not derive PMK-R0");
		return -1;
	}
...
int fils_process_assoc_resp(struct wpa_sm *sm, const u8 *resp, size_t len)
{
	...
	if (!sm || !sm->ptk_set) {
		wpa_warning_log0(MSG_DEBUG, "FILS: No KEK available");
		return -1;
	}

	if (!wpa_key_mgmt_fils(sm->key_mgmt)) {
		wpa_warning_log0(MSG_DEBUG, "FILS: Not a FILS AKM");
		return -1;
	}
	...
	if (os_memcmp(elems.fils_session, sm->fils_session,
		      FILS_SESSION_LEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "FILS: FILS Session mismatch");
	...
	if (!elems.key_delivery) {
		wpa_warning_log0(MSG_DEBUG, "FILS: No Key Delivery element");
		goto fail;
	}
	...
	if (wpa_supplicant_parse_ies(elems.key_delivery + WPA_KEY_RSC_LEN,
				     ...
				     &kde) < 0) {
		wpa_warning_log0(MSG_DEBUG, "FILS: Failed to parse KDEs");
		goto fail;
	}
	if (!kde.gtk) {
		wpa_warning_log0(MSG_DEBUG, "FILS: No GTK KDE");
		goto fail;
	}
	...
	if (kde.gtk_len - 2 > sizeof(gd.gtk)) {
		wpa_warning_log1(MSG_DEBUG, "FILS: Too long GTK in GTK KDE (len=%lu)",
			   (unsigned long) kde.gtk_len - 2);
		goto fail;
	}
	os_memcpy(gd.gtk, kde.gtk + 2, kde.gtk_len - 2);

	wpa_warning_log0(MSG_DEBUG, "FILS: Set GTK to driver");
	if (wpa_supplicant_install_gtk(sm, &gd, elems.key_delivery, 0) < 0) {
		wpa_warning_log0(MSG_DEBUG, "FILS: Failed to set GTK");
		goto fail;
	}

	if (ieee80211w_set_keys(sm, &kde) < 0) {
		wpa_warning_log0(MSG_DEBUG, "FILS: Failed to set IGTK");
		goto fail;
	}
	...
	wpa_warning_log0(MSG_DEBUG, "FILS: Auth+Assoc completed successfully");
	sm->fils_completed = 1;
	forced_memzero(&gd, sizeof(gd));
			   ...
			   const u8 *resp_ies, size_t resp_ies_len)
{
	...
	if (sm->cur_pmksa && elems.rsn_ie &&
		...
		wpa_warning_log0(MSG_DEBUG, "OWE: Use PMKSA caching");
		wpa_sm_set_pmk_from_pmksa(sm);
		return 0;
	}
	...
	if (!sm->owe_ecdh) {
		wpa_warning_log0(MSG_INFO, "OWE: No ECDH state available");
		return -1;
	}
