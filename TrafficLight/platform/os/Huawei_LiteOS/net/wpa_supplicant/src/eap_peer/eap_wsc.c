				   ...
				   const char *params)
{
	...
	if ((len & 1) || len > 2 * sizeof(cred->ssid) ||
		...
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Invalid new_ssid");
		return -1;
	}
	cred->ssid_len = len / 2;
	...
	if (pos == NULL) {
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Missing new_auth");
		return -1;
	}
	if (os_strncmp(pos + 9, "OPEN", 4) == 0)
		...
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Unknown new_auth");
		return -1;
	}

	pos = os_strstr(params, "new_encr=");
	if (pos == NULL) {
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Missing new_encr");
		return -1;
	}
	if (os_strncmp(pos + 9, "NONE", 4) == 0)
		...
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Unknown new_encr");
		return -1;
	}
	...
	if ((len & 1) || len > 2 * sizeof(cred->key) ||
		...
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Invalid new_key");
		return -1;
	}
	cred->key_len = len / 2;
...
static void * eap_wsc_init(struct eap_sm *sm)
{
	...
	wps = sm->wps;
	if (wps == NULL) {
		wpa_error_log0(MSG_ERROR, "EAP-WSC: WPS context not available");
		return NULL;
	}
	...
	if (phase1 == NULL) {
		wpa_warning_log0(MSG_INFO, "EAP-WSC: phase1 configuration data not "
			   "set");
		os_free(data);
		return NULL;
	}
	...
	if (cfg.pin == NULL && !cfg.pbc && !nfc) {
		wpa_warning_log0(MSG_INFO, "EAP-WSC: PIN or PBC not set in phase1 "
			   "configuration data");
		os_free(data);
		return NULL;
	}
	...
	if (pos) {
		...
		if (len != 2 * WPS_OOB_PUBKEY_HASH_LEN ||
			...
			wpa_warning_log0(MSG_INFO, "EAP-WSC: Invalid pkhash");
			os_free(data);
			return NULL;
		}
	...
	if (res < 0) {
		os_free(data);
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Failed to parse new AP "
			   "settings");
		return NULL;
	}
	if (res == 1) {
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Provide new AP settings for "
			   "WPS");
	...
	if (data->wps == NULL) {
		os_free(data);
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: wps_init failed");
		return NULL;
	}
	...
	wpa_warning_log1(MSG_DEBUG, "EAP-WSC: Fragment size limit %u",
		   (unsigned int) data->fragment_size);

	if (registrar && cfg.pin) {
#ifdef CONFIG_WPS_AP
		(void)wps_registrar_add_pin(data->wps_ctx->registrar, NULL, NULL,
				      cfg.pin, cfg.pin_len, 0);
#else
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: wps not support hostapd mode now.");
		return NULL;
#endif /* CONFIG_WPS_AP */
	}
					 ...
					 struct eap_method_ret *ret, u8 id)
{
	...
	ret->ignore = FALSE;
	wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Generating Response");
	...
	if (data->out_used == wpabuf_len(data->out_buf)) {
		wpa_warning_log1(MSG_DEBUG, "EAP-WSC: Sending out %lu bytes "
		...
		wpa_warning_log2(MSG_DEBUG, "EAP-WSC: Sending out %lu bytes "
				...
				const u8 *buf, size_t len, u8 op_code)
{
	...
	if (op_code != data->in_op_code) {
		wpa_warning_log2(MSG_DEBUG, "EAP-WSC: Unexpected Op-Code %d in "
			   "fragment (expected %d)",
	...
	if (len > wpabuf_tailroom(data->in_buf)) {
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Fragment overflow");
		eap_wsc_state(data, FAIL);
		return -1;
	}

	wpabuf_put_data(data->in_buf, buf, len);
	wpa_warning_log2(MSG_DEBUG, "EAP-WSC: Received %lu bytes, waiting "
						...
						const u8 *buf, size_t len)
{
	...
	if (data->in_buf == NULL && !(flags & WSC_FLAGS_LF)) {
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: No Message Length field in a "
			   "fragmented packet");
		ret->ignore = TRUE;
		return NULL;
	}
	...
	if (data->in_buf == NULL) {
		...
		if (data->in_buf == NULL) {
			wpa_warning_log0(MSG_DEBUG, "EAP-WSC: No memory for "
				   "message");
			ret->ignore = TRUE;
			return NULL;
		}
		...
		wpabuf_put_data(data->in_buf, buf, len);
		wpa_warning_log2(MSG_DEBUG, "EAP-WSC: Received %lu bytes in first "
				       ...
				       const struct wpabuf *reqData)
{
	...
	if (flags & WSC_FLAGS_LF) {
		if (end - pos < 2) {
			wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Message underflow");
			ret->ignore = TRUE;
			return NULL;
		}
		...
		if (message_length < end - pos || message_length > 50000) {
			wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Invalid Message "
				   "Length");
			ret->ignore = TRUE;
			return NULL;
		}
	...
	wpa_warning_log3(MSG_DEBUG, "EAP-WSC: Received packet: Op-Code %d "
		   "Flags 0x%x Message Length %d",
		   op_code, flags, message_length);

	if (data->state == WAIT_FRAG_ACK) {
		if (op_code != WSC_FRAG_ACK) {
			wpa_warning_log1(MSG_DEBUG, "EAP-WSC: Unexpected Op-Code %d "
				   "in WAIT_FRAG_ACK state", op_code);
			ret->ignore = TRUE;
			return NULL;
		}
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Fragment acknowledged");
		eap_wsc_state(data, MESG);
	...
	if (data->state == WAIT_START) {
		...
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Received start");
		eap_wsc_state(data, MESG);
	...
	switch (res) {
	case WPS_DONE:
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: WPS processing completed "
	...
	case WPS_FAILURE:
	case WPS_PENDING:
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: WPS processing failed");
		eap_wsc_state(data, FAIL);
		break;
	}
...
send_msg:
	if (data->out_buf == NULL) {
		...
		if (data->out_buf == NULL) {
			wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Failed to receive "
				   "message from WPS");
			eap_wsc_state(data, FAIL);
			ret->methodState = METHOD_DONE;
			ret->decision = DECISION_FAIL;
