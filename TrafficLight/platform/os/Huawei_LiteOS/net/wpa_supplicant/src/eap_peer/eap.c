				    ...
				    int mnc_len)
{
	...
	if (*imsi_len + 36 > max_len) {
		wpa_warning_log0(MSG_WARNING, "No room for realm in IMSI buffer");
		return -1;
	}
...
SM_STATE(EAP, INITIALIZE)
{
	...
	if (sm->fast_reauth && sm->m && sm->m->has_reauth_data &&
		...
		wpa_warning_log0(MSG_DEBUG, "EAP: maintaining EAP method data for "
...
#else
		eap_deinit_prev_method(sm, NULL);
#endif /* HISI_EAP_TRIM */
	}
...
static char * eap_get_realm(struct eap_sm *sm, struct eap_peer_config *config)
{
	...
	if (!config->identity && !config->anonymous_identity &&
		...
		wpa_warning_log0(MSG_DEBUG, "EAP: Build realm from IMSI (eap_proxy)");
		...
		if (eap_sm_append_3gpp_realm(sm, imsi, sizeof(imsi), &imsi_len,
					     mnc_len) < 0) {
			wpa_warning_log0(MSG_WARNING, "Could not append realm");
			return NULL;
		}
		...
		wpa_warning_buf(MSG_DEBUG, "EAP: Generated realm '%s'", realm, strlen(realm));
		return realm;
	}
...
static void eap_erp_remove_keys_realm(struct eap_sm *sm, const char *realm)
{
	...
	while ((erp = eap_erp_get_key(sm, realm)) != NULL) {
		wpa_warning_buf(MSG_DEBUG, "EAP: Delete old ERP key %s",
		       ...
		       size_t ext_emsk_len)
{
	...
	wpa_warning_buf(MSG_DEBUG, "EAP: Realm for ERP keyName-NAI: %s", realm, strlen(realm));
	...
	wpa_warning_buf(MSG_DEBUG, "EAP: Stored ERP keys %s", erp->keyname_nai, strlen(erp->keyname_nai));
...
static int eap_peer_erp_reauth_start(struct eap_sm *sm, u8 eap_id)
{
	...
	wpa_warning_log0(MSG_DEBUG, "EAP: Sending EAP-Initiate/Re-auth");
	wpabuf_free(sm->eapRespData);
	sm->eapRespData = msg;
	sm->reauthInit = TRUE;
	return 0;
	...
	size_t count)
{
	...
	wpa_warning_log0(MSG_DEBUG, "EAP: Building expanded EAP-Nak");
...
void eap_peer_finish(struct eap_sm *sm, const struct eap_hdr *hdr, size_t len)
{
	...
	if (!erp) {
		wpa_warning_buf(MSG_DEBUG, "EAP: No matching ERP key found for %s",
			   nai, strlen(nai));
		return;
	}
...
static void eap_sm_parseEapReq(struct eap_sm *sm, const struct wpabuf *req)
{
	...
	if (sm->workaround) {
		...
		(void)sha1_vector(1, addr, &plen, sm->req_sha1);
	}
	...
	case EAP_CODE_SUCCESS:
		wpa_warning_log0(MSG_DEBUG, "EAP: Received EAP-Success");
#ifndef HISI_EAP_TRIM
	...
	case EAP_CODE_FAILURE:
		wpa_warning_log0(MSG_DEBUG, "EAP: Received EAP-Failure");
#ifndef HISI_EAP_TRIM
				  ...
				  union tls_event_data *data)
{
	...
	case TLS_PEER_CERTIFICATE:
		...
		if (data->peer_cert.hash) {
			...
			if (hash_hex) {
				(void)wpa_snprintf_hex(hash_hex, len,
						 data->peer_cert.hash,
