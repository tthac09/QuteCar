...
#else
/**
 * Trimmed version of struct eap_method_ret.
 */
struct eap_method_ret {
	unsigned int ignore:1;
	unsigned int allowNotifications:1;
	unsigned int decision:2;
	unsigned int methodState:4;
};
...
#else
/**
 * Trimmed version of struct eap_sm. The main modifications:
 * 1, change the type of all Boolean variables to "unsigned int:1".
 * 2, change some params with "int" type to "unsigned int:1", include
 *	fast_reauth, prev_failure, workaround, force_disabled.
 * 3, change some params with "enum" type to "unsigned int:n", include
 *	methodState and decision.
 * 4, change reqMethod and selectedMethod from "EapType" to "unsigned char".
 * 5, change some "int" type to "unsigned char", include ClientTimeout and num_rounds.
 * 6, delete some unused params, include init_phase2, erp_seq, scard_ctx
 *	ssl_ctx, ssl_ctx2, peer_challenge, auth_challenge, ext_pw, ext_pw_buf,
 *	external_sim, ext_cert_check, waiting_ext_cert_check,  erp_keys.
 */
struct eap_sm {
	enum {
		EAP_INITIALIZE,
	...
	unsigned char reqMethod;
	unsigned char ClientTimeout;
	unsigned char num_rounds;

	int lastId;
	int reqId;
	unsigned int methodState:3;
	unsigned int decision:2;
	unsigned int rxReq:1;
	unsigned int rxSuccess:1;
	unsigned int rxFailure:1;
	unsigned int ignore:1;
	unsigned int allowNotifications:1; /* peer state machine <-> methods */
	unsigned int eapKeyAvailable:1; /* peer to lower layer */
	unsigned int changed:1;
	unsigned int reauthInit:1; /* send EAP-Identity/Re-auth */
	unsigned int rxResp:1 /* LEAP only */;
	unsigned int leap_done:1;
	unsigned int peap_done:1;
	unsigned int expected_failure:1;
	unsigned int force_disabled:1;
	unsigned int fast_reauth:1;
	unsigned int prev_failure:1;
	unsigned int workaround:1;
	int reqVendor;
	u32 reqVendorMethod;
	struct wpabuf *lastRespData;
	struct wpabuf *eapRespData; /* peer to lower layer */
