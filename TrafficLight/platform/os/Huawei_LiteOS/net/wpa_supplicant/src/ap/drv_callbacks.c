...
#ifndef HISI_CODE_CROP
#ifndef  LOS_HOSTAPD_HT_CONFIG_CROP
...
#endif /* LOS_HOSTAPD_HT_CONFIG_CROP */
			      ...
			      struct rx_mgmt *drv_mgmt)
{
	...
	wpa_warning_log3(MSG_DEBUG, "RX_ACTION category %u action %u plen %d",
		   mgmt->u.action.category, *action, (int) plen);
	...
	wpa_warning_log4(MSG_DEBUG, " da %02x:xx:xx:%02x:%02x:%02x",
			(mgmt->da)[0], (mgmt->da)[3], (mgmt->sa)[4], (mgmt->da)[5]);
...
static int hostapd_event_new_sta(struct hostapd_data *hapd, const u8 *addr)
{
	...
	if (sta)
		return 0;
   wpa_warning_log4(MSG_DEBUG, " Data frame from unknown STA %02x:xx:xx:%02x:%02x:%02x",
			(addr)[0], (addr)[3], (addr)[4], (addr)[5]);
	sta = ap_sta_add(hapd, addr);
	if (sta) {
		hostapd_new_assoc_sta(hapd, sta, 0);
	} else {
		wpa_warning_log4(MSG_DEBUG, " DFailed to add STA entry for %02x:xx:xx:%02x:%02x:%02x",
				(addr)[0], (addr)[3], (addr)[4], (addr)[5]);
		return -1;
	}
				      ...
				      size_t ie_len)
{
	...
	if (!hapd || !hapd->wpa_auth) {
		wpa_warning_log0(MSG_DEBUG, "OWE: Invalid hapd context");
		return -1;
	}
	if (!peer) {
	...
	if (!(hapd->conf->wpa_key_mgmt & WPA_KEY_MGMT_OWE)) {
		wpa_warning_log0(MSG_DEBUG, "OWE: No OWE AKM configured");
		status = WLAN_STATUS_AKMP_NOT_VALID;
		goto err;
	}
	if (ieee802_11_parse_elems(ie, ie_len, &elems, 1) == ParseFailed) {
		wpa_printf(MSG_DEBUG, "OWE: Failed to parse OWE IE for %02x:xx:xx:%02x:%02x:%02x",
...
#endif /* LOS_WPA_PATCH */
{
	...
	case EVENT_INTERFACE_ENABLED:
		...
		if (hapd->disabled && hapd->started) {
			...
			wpa_warning_log0(MSG_DEBUG, "DFS: restarting pending CAC");
...
/* LiteOS wpa_supplicant_event_global->hostapd_event_global */
#ifdef LOS_WPA_PATCH
