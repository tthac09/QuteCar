					 ...
					 u8 action_type, u16 intval)
{
	...
	if (sta == NULL) {
		wpa_warning_log0(MSG_DEBUG, "ieee802_11_send_wnmsleep_resp: station not found");
		return -EINVAL;
	}
...
#define MAX_IGTK_SUBELEM_LEN 26
	...
	if (!wpa_auth_uses_mfp(sta->wpa_sm) ||
		...
		pos += gtk_elem_len;
		wpa_warning_log1(MSG_DEBUG, "Pass 4, gtk_len = %d",
			   (int) gtk_elem_len);
		...
		igtk_elem_len = res;
		pos += igtk_elem_len;
		wpa_warning_log1(MSG_DEBUG, "Pass 4 igtk_len = %d",
			   (int) igtk_elem_len);
	...
	if (!res) {
		wpa_warning_log0(MSG_DEBUG, "Successfully send WNM-Sleep Response "
			   "frame");
		...
		wpa_warning_log0(MSG_DEBUG, "Fail to send WNM-Sleep Response frame");
				       ...
				       const u8 *addr, const u8 *frm, int len)
{
	...
	if (!hapd->conf->wnm_sleep_mode) {
		wpa_warning_log4(MSG_DEBUG, "Ignore WNM-Sleep Mode Request from "
			   "%02x:xx:xx:%02x:%02x:%02x" " since WNM-Sleep Mode is disabled",
			   addr[0], addr[3], addr[4], addr[5]);
		return;
	}

	if (len < 1) {
		wpa_warning_log4(MSG_DEBUG, "WNM: Ignore too short WNM-Sleep Mode Request from %02x:xx:xx:%02x:%02x:%02x",
		   			(addr)[0], (addr)[3], (addr)[4], (addr)[5]);
		return;
	}
	...
	while (pos + 1 < frm + len) {
		...
		if (*pos == WLAN_EID_WNMSLEEP &&
			...
			wpa_warning_log1(MSG_DEBUG, "WNM: EID %d not recognized",
				   *pos);
		pos += ie_len + 2;
	}
	...
	if (wnmsleep_ie->action_type == WNM_SLEEP_MODE_ENTER &&
		...
		tfsreq_ie_len = (tfsreq_ie_end + tfsreq_ie_end[1] + 2) -
			tfsreq_ie_start;
		wpa_warning_log0(MSG_DEBUG, "TFS Req IE(s) found");
		...
		if (ieee80211_11_set_tfs_ie(hapd, addr, tfsreq_ie_start,
			...
			wpa_warning_log0(MSG_DEBUG, "Fail to set TFS Req IE");
	}
						  ...
						  u8 dialog_token)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WNM: Send BSS Transition Management Request to "
		   "%02x:xx:xx:%02x:%02x:%02x",
		   addr[0], addr[3], addr[4], addr[5]);
	wpa_warning_log4(MSG_DEBUG, "WNM: Send BSS Transition Management Request to "
		   " dialog_token=%u req_mode=0x%x disassoc_timer=%u "
		   "validity_interval=%u", dialog_token,
					       ...
					       size_t len)
{
	...
	if (len < 2) {
		wpa_warning_log4(MSG_DEBUG, "WNM: Ignore too short BSS Transition Management Query from "
	...
	wpa_warning_log4(MSG_DEBUG, "WNM: BSS Transition Management Query from "
		   "%02x:xx:xx:%02x:%02x:%02x",
		   addr[0], addr[3], addr[4], addr[5]);
	wpa_warning_log2(MSG_DEBUG, "WNM: BSS Transition Management Query from "
			" dialog_token=%u reason=%u", dialog_token, reason);
...
void ap_sta_reset_steer_flag_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (sta->agreed_to_steer) {
		wpa_warning_buf(MSG_DEBUG, "%s: Reset steering flag for STA ",
					      ...
					      size_t len)
{
	...
	if (len < 3) {
		wpa_warning_log4(MSG_DEBUG, "WNM: Ignore too short BSS Transition Management Response from "
			   "%02x:xx:xx:%02x:%02x:%02x", addr[0], addr[3], addr[4], addr[5]);
		return;
	}
	...
	wpa_warning_log4(MSG_DEBUG, "WNM: BSS Transition Management Response from "
		   "%02x:xx:xx:%02x:%02x:%02x", addr[0], addr[3], addr[4], addr[5]);
	wpa_warning_log3(MSG_DEBUG, "WNM: BSS Transition Management Response from "
		   " dialog_token=%u status_code=%u " "bss_termination_delay=%u", dialog_token,
		   status_code, bss_termination_delay);

	sta = ap_get_sta(hapd, addr);
	if (!sta) {
		wpa_warning_log4(MSG_DEBUG, "Station " "%02x:xx:xx:%02x:%02x:%02x"
	...
	if (status_code == WNM_BSS_TM_ACCEPT) {
		if (end - pos < ETH_ALEN) {
			wpa_warning_log0(MSG_DEBUG, "WNM: not enough room for Target BSSID field");
			return;
		}
		...
		eloop_register_timeout(2, 0, ap_sta_reset_steer_flag_timer,
				       hapd, sta);
		wpa_warning_log4(MSG_DEBUG, "WNM: Target BSSID: " "%02x:xx:xx:%02x:%02x:%02x",
					       ...
					       size_t len)
{
	...
	wpa_warning_log2(MSG_DEBUG,
		   "WNM: Received WNM Notification Request frame from "
		   " (dialog_token=%u type=%u)", dialog_token, type);
						...
						size_t len)
{
	...
	wpa_warning_log1(MSG_DEBUG,
		   "WNM: Received Collocated Interference Report frame from " " (dialog_token=%u)",
		   dialog_token);
				...
				const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WNM: Unsupported WNM Action %u from ",
		   action);
	wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x",
			       ...
			       struct sta_info *sta, int disassoc_timer)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WNM: Send BSS Transition Management Request frame to indicate imminent disassociation (disassoc_timer=%d) to ", 
					disassoc_timer);
	wpa_warning_log4(MSG_DEBUG, "WNM: Send BSS Transition Management Request frame to indicate imminent disassociation to "
		   "%02x:xx:xx:%02x:%02x:%02x", (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	if (hostapd_drv_send_mlme(hapd, buf, pos - buf, 0) < 0) {
		wpa_warning_log0(MSG_DEBUG, "Failed to send BSS Transition "
			       ...
			       int disassoc_timer)
{
	...
	wpa_warning_log4(MSG_DEBUG, "Disassociation timer for " "%02x:xx:xx:%02x:%02x:%02x", 
		(sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	wpa_warning_log1(MSG_DEBUG, "Disassociation timer for "
		   " set to %d ms", timeout);
			...
			const u8 *mbo_attrs, size_t mbo_len)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WNM: Send BSS Transition Management Request to "
		   "%02x:xx:xx:%02x:%02x:%02x",
		   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	wpa_warning_log3(MSG_DEBUG, "WNM: Send BSS Transition Management Request to "
					" req_mode=0x%x disassoc_timer=%d valid_int=0x%x",
					req_mode, disassoc_timer, valid_int);
			    ...
			    unsigned int auto_report, unsigned int timeout)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WNM: Sending Collocated Interference Request to "
		   "%02x:xx:xx:%02x:%02x:%02x",
		   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	wpa_warning_log3(MSG_DEBUG, "WNM: Sending Collocated Interference Request to " 
