...
static void hostapd_lci_rep_timeout_handler(void *eloop_data, void *user_ctx)
{
	...
	(void)user_ctx;
	wpa_warning_log1(MSG_DEBUG, "RRM: LCI request (token %u) timed out",
		   hapd->lci_req_token);
				      ...
				      const u8 *pos, size_t len)
{
	...
	if (!hapd->lci_req_active || hapd->lci_req_token != token) {
		wpa_warning_log1(MSG_DEBUG, "Unexpected LCI report, token %u", token);
		return;
	}

	hapd->lci_req_active = 0;
	(void)eloop_cancel_timeout(hostapd_lci_rep_timeout_handler, hapd, NULL);
	wpa_warning_log2(MSG_DEBUG, "LCI report token %u len %zu", token, len);
}
...
static void hostapd_range_rep_timeout_handler(void *eloop_data, void *user_ctx)
{
	...
	(void)user_ctx;
	wpa_warning_log1(MSG_DEBUG, "RRM: Range request (token %u) timed out",
					...
					const u8 *pos, size_t len)
{
	...
	if (!hapd->range_req_active || hapd->range_req_token != token) {
		wpa_warning_log1(MSG_DEBUG, "Unexpected range report, token %u",
			   token);
		return;
	}

	hapd->range_req_active = 0;
	(void)eloop_cancel_timeout(hostapd_range_rep_timeout_handler, hapd, NULL);
	wpa_warning_log2(MSG_DEBUG, "Range report token %u len %zu", token, len);
}
					 ...
					 const u8 *pos, size_t len)
{
	...
	wpa_warning_log2(MSG_DEBUG, "Beacon report token %u len %zu from ",
		   token, len);
					     ...
					     const u8 *buf, size_t len)
{
	...
	while ((ie = get_ie(pos, end - pos, WLAN_EID_MEASURE_REPORT))) {
		if (ie[1] < 3) {
			wpa_warning_log0(MSG_DEBUG, "Bad Measurement Report element");
			break;
		}

		rep_mode = ie[3];
		wpa_warning_log2(MSG_DEBUG, "Measurement report mode 0x%x type %u",
			   rep_mode, ie[4]);
					 ...
					 u8 civic, u16 lci_max_age)
{
	...
	dl_list_for_each(nr, &hapd->nr_db, struct hostapd_neighbor_entry,
		...
		if (len - 2 > 0xff) {
			wpa_warning_log4(MSG_DEBUG,
				   "NR entry for " "%02x:xx:xx:%02x:%02x:%02x" " exceeds 0xFF bytes",
				   (nr->bssid)[0], (nr->bssid)[3], (nr->bssid)[4], (nr->bssid)[5]);
			continue;
		}
	...
	(void)hostapd_drv_send_action(hapd, hapd->iface->freq, 0, addr,
				      ...
				      const u8 *buf, size_t len)
{
	...
	wpa_warning_log1(MSG_DEBUG, "Radio measurement frame, action %u from ",
	...
	case WLAN_RRM_NEIGHBOR_REPORT_REQUEST:
		...
		break;
	default:
		wpa_warning_log1(MSG_DEBUG, "RRM action %u is not supported",
			   mgmt->u.action.u.rrm.action);
		break;
	}
}
...
int hostapd_send_lci_req(struct hostapd_data *hapd, const u8 *addr)
{
	...
	if (hapd->lci_req_active) {
		...
		hapd->lci_req_active = 0;
		(void)eloop_cancel_timeout(hostapd_lci_rep_timeout_handler, hapd,
				     NULL);
	}
	...
	(void)eloop_register_timeout(HOSTAPD_RRM_REQUEST_TIMEOUT, 0,
			   ...
			   const u8 *responders, unsigned int n_responders)
{
	...
	wpa_warning_log4(MSG_DEBUG, "Request range: dest addr " "%02x:xx:xx:%02x:%02x:%02x",
		addr[0], addr[3], addr[4], addr[5]);
	wpa_warning_log3(MSG_DEBUG, "Request range: dest addr "
		   " rand interval %u min AP %u n_responders %u",
		   random_interval, min_ap, n_responders);

	if (min_ap == 0 || min_ap > n_responders) {
		wpa_warning_log0(MSG_INFO, "Request range: Wrong min AP count");
		return -1;
	}
	...
	for (i = 0; i < n_responders; i++) {
		...
		nr = hostapd_neighbor_get(hapd, responders + ETH_ALEN * i,
					  NULL);
		if (!nr) {
			wpa_warning_log4(MSG_INFO, "Missing neighbor report for "
				   "%02x:xx:xx:%02x:%02x:%02x",
				   (responders + ETH_ALEN * i)[0], (responders + ETH_ALEN * i)[3],
				   (responders + ETH_ALEN * i)[4], (responders + ETH_ALEN * i)[5]);
			wpabuf_free(buf);
			return -1;
		}

		if (wpabuf_tailroom(buf) < 2 + wpabuf_len(nr->nr)) {
			wpa_error_log0(MSG_ERROR, "Too long range request");
			wpabuf_free(buf);
			return -1;
		}
			    ...
			    u8 req_mode, const struct wpabuf *req)
{
	...
	if (wpabuf_len(req) < 13) {
		wpa_warning_log0(MSG_INFO, "Beacon request: Too short request data");
		return -1;
	}
	pos = wpabuf_head(req);
	mode = pos[6];
	...
	if (!sta || !(sta->flags & WLAN_STA_AUTHORIZED)) {
		wpa_warning_log4(MSG_INFO,
			   "Beacon request: " "%02x:xx:xx:%02x:%02x:%02x" " is not connected",
			   addr[0], addr[3], addr[4], addr[5]);
		return -1;
	}
	...
	case BEACON_REPORT_MODE_PASSIVE:
		if (!(sta->rrm_enabled_capa[0] &
			...
			wpa_warning_log4(MSG_INFO,
				   "Beacon request: " "%02x:xx:xx:%02x:%02x:%02x"
