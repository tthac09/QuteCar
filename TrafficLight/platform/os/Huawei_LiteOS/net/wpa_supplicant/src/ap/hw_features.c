...
static char * dfs_info(struct hostapd_channel_data *chan)
{
	...
	(void)os_snprintf(info, sizeof(info), " (DFS state = %s)", state);
...
#else
			wpa_msgdump_log3(MSG_MSGDUMP, "Allowed channel: mode=%d "
				   "chan=%d freq=%d MHz",
				   feature->mode,
				   feature->channels[j].chan,
				   feature->channels[j].freq);
			  ...
			  struct hostapd_hw_modes *mode)
{
	...
	if (!iface->current_rates) {
		wpa_error_log0(MSG_ERROR, "Failed to allocate memory for rate "
			   "table.");
		return -1;
	}
	...
	for (i = 0; i < mode->num_rates; i++) {
		...
		wpa_warning_log3(MSG_DEBUG, "RATE[%d] rate=%d flags=0x%x",
...
#endif /* HISI_CODE_CROP */
		wpa_error_log2(MSG_ERROR, "No rates remaining in supported/basic "
			   "rate sets (%d,%d).",
...
static void ieee80211n_check_scan(struct hostapd_iface *iface)
{
	...
	if (scan_res == NULL) {
		(void)hostapd_setup_interface_complete(iface, 1);
		return;
	}
#ifndef HISI_CODE_CROP
	...
	if (!oper40) {
		wpa_warning_log2(MSG_INFO, "20/40 MHz operation not permitted on "
	...
	if (!res) {
		...
		res = 1;
		wpa_warning_log0(MSG_INFO, "Fallback to 20 MHz");
	}
#endif /* HISI_CODE_CROP */

	(void)hostapd_setup_interface_complete(iface, !res);
}
					 ...
					 struct wpa_driver_scan_params *params)
{
	...
	wpa_warning_log2(MSG_DEBUG, "40 MHz affected channel range: [%d,%d] MHz",
...
#define HT2040_COEX_SCAN_RETRY 15
	...
	if (ret == -EBUSY &&
		...
		wpa_error_buf(MSG_ERROR,
			   "Failed to request a scan of neighboring BSSes (%s)",
			   strerror(-ret), strlen(strerror(-ret)));
		wpa_error_log2(MSG_ERROR,
			   "Failed to request a scan of neighboring BSSes ret=%d - try to scan again (attempt %d)",
			   ret, iface->num_ht40_scan_tries);
		(void)eloop_register_timeout(1, 0, ap_ht40_scan_retry, iface, NULL);
		return;
	}
	...
	(void)hostapd_setup_interface_complete(iface, 0);
}


void hostapd_stop_setup_timers(struct hostapd_iface *iface)
{
	(void)eloop_cancel_timeout(ap_ht40_scan_retry, iface, NULL);
}
...
static int ieee80211n_check_40mhz(struct hostapd_iface *iface)
{
	...
	wpa_warning_log0(MSG_DEBUG, "Scan for neighboring BSSes prior to enabling "
		   "40 MHz channel");
	os_memset(&params, 0, sizeof(params));
	if (iface->current_mode->mode == HOSTAPD_MODE_IEEE80211G)
		ieee80211n_scan_channels_2g4(iface, &params);
	...
	if (ret == -EBUSY) {
		wpa_error_log1(MSG_ERROR,
			   "Failed to request a scan of neighboring BSSes ret=%d - try to scan again",
			   ret);
		wpa_error_buf(MSG_ERROR,
			   "Failed to request a scan of neighboring BSSes (%s) - try to scan again",
			   strerror(-ret), strlen(strerror(-ret)));
	...
	if (ret < 0) {
		wpa_error_log1(MSG_ERROR,
			   "Failed to request a scan of neighboring BSSes ret=%d",
			   ret);
		wpa_error_buf(MSG_ERROR,
			   "Failed to request a scan of neighboring BSSes (%s)",
			   strerror(-ret), strlen(strerror(-ret)));
...
static int ieee80211n_supported_ht_capab(struct hostapd_iface *iface)
{
	...
	if ((conf & HT_CAP_INFO_LDPC_CODING_CAP) &&
		...
		wpa_error_log0(MSG_ERROR, "Driver does not support configured "
			   "HT capability [LDPC]");
		return 0;
	}
#ifdef CONFIG_ACS
				  ...
				  int channel, int primary)
{
	...
	for (i = 0; i < iface->current_mode->num_channels; i++) {
		...
		wpa_warning_buf(MSG_DEBUG,
			   "%s", primary ? "" : "Configured HT40 secondary ", primary ? strlen("") : strlen("Configured HT40 secondary "));
		wpa_warning_log3(MSG_DEBUG,
			   "channel [%i] (%i) is disabled for use in AP mode, flags: 0x%x",
			   i, chan->chan, chan->flag);
		wpa_warning_two_buf(MSG_DEBUG,
			   "%s%s", chan->flag & HOSTAPD_CHAN_NO_IR ? " NO-IR" : "", chan->flag & HOSTAPD_CHAN_NO_IR ? strlen(" NO-IR") : strlen(""),
			   chan->flag & HOSTAPD_CHAN_RADAR ? " RADAR" : "", chan->flag & HOSTAPD_CHAN_RADAR ? strlen(" RADAR") : strlen(""));
	}
#ifndef HISI_CODE_CROP
...
int hostapd_acs_completed(struct hostapd_iface *iface, int err)
{
	...
	case HOSTAPD_CHAN_VALID:
		...
		break;
	case HOSTAPD_CHAN_ACS:
		wpa_error_log0(MSG_ERROR, "ACS error - reported complete, but no result available");
		wpa_msg(iface->bss[0]->msg_ctx, MSG_INFO, ACS_EVENT_FAILED);
		hostapd_notify_bad_chans(iface);
		goto out;
	case HOSTAPD_CHAN_INVALID:
	default:
		wpa_error_log0(MSG_ERROR, "ACS picked unusable channels");
		wpa_msg(iface->bss[0]->msg_ctx, MSG_INFO, ACS_EVENT_FAILED);
		hostapd_notify_bad_chans(iface);
		goto out;
	}
	...
	if (ret < 0)
		goto out;
	if (ret == 1) {
		wpa_warning_log0(MSG_DEBUG, "Interface initialization will be completed in a callback");
		return 0;
	}
...
int hostapd_select_hw_mode(struct hostapd_iface *iface)
{
	...
	if (iface->num_hw_features < 1)
		return -1;
#ifdef LOS_WPA_PATCH
	if (g_ap_opt_set.hw_mode != HI_WIFI_PHY_MODE_BUTT) {
		if (g_ap_opt_set.hw_mode == HI_WIFI_PHY_MODE_11B) {
			iface->conf->hw_mode = HOSTAPD_MODE_IEEE80211B;
			iface->conf->ieee80211n = 0;
		} else if (g_ap_opt_set.hw_mode == HI_WIFI_PHY_MODE_11BG) {
			iface->conf->hw_mode = HOSTAPD_MODE_IEEE80211G;
			iface->conf->ieee80211n = 0;
		} else if (g_ap_opt_set.hw_mode == HI_WIFI_PHY_MODE_11BGN) {
			iface->conf->hw_mode = HOSTAPD_MODE_IEEE80211G;
	...
	if ((iface->conf->hw_mode == HOSTAPD_MODE_IEEE80211G ||
		...
		wpa_warning_log0(MSG_INFO, "Disable OFDM/HT/VHT on channel 14");
		iface->conf->hw_mode = HOSTAPD_MODE_IEEE80211B;
		iface->conf->ieee80211n = 0;
#ifndef HISI_CODE_CROP
