				  ...
				  size_t psk_len)
{
	...
	if (psk_len != PMK_LEN) {
		wpa_warning_log1(MSG_DEBUG, "Unexpected PSK length %lu",
	...
	if (ssid->wpa_psk_file) {
		...
		if (f == NULL) {
			wpa_warning_buf(MSG_DEBUG, "Failed to add the PSK to "
				   "'%s'", ssid->wpa_psk_file, strlen(ssid->wpa_psk_file));
			return -1;
		}

		(void)wpa_snprintf_hex(hex, sizeof(hex), psk, psk_len);
				      ...
				      const struct wps_device_data *dev)
{
	...
	if (uuid_bin2str(uuid_e, uuid, sizeof(uuid)))
		return;
	wpa_warning_buf(MSG_DEBUG, "WPS: PIN needed for E-UUID %s", uuid, strlen(uuid));
				       ...
				       size_t dev_pw_len)
{
	...
	(void)hostapd_wps_for_each(hapd, wps_stop_registrar, &data);
}
...
static void wps_reload_config(void *eloop_data, void *user_ctx)
{
	...
	(void)user_ctx;
	wpa_warning_log0(MSG_DEBUG, "WPS: Reload configuration data");
	if (iface->interfaces == NULL ||
	    iface->interfaces->reload_config(iface) < 0) {
		wpa_warning_log0(MSG_WARNING, "WPS: Failed to reload the updated "
			   "configuration");
	}
}
...
void hostapd_wps_eap_completed(struct hostapd_data *hapd)
{
	...
	if (eloop_deplete_timeout(0, 0, wps_reload_config, hapd->iface, NULL) ==
		1) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Reschedule immediate configuration reload");
	}
}
			      ...
			      size_t attr_len)
{
	...
	if (buf) {
		(void)wpa_snprintf_hex(buf, blen, attr, attr_len);
				       ...
				       const struct wps_credential *cred)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Updating in-memory configuration");
	 ...
	 */
	(void)eloop_register_timeout(0, 100000, wps_reload_config, hapd->iface,
			       NULL);
...
static int hapd_wps_cred_cb(struct hostapd_data *hapd, void *ctx)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Received new AP Settings");
	...
	if (cred->key_len == 0) {
		...
		wpa_warning_log1(MSG_INFO, "WPS: Invalid key length %lu for WPA/WPA2",
	...
	if (tmp_fname == NULL)
		return -1;
	(void)os_snprintf(tmp_fname, len, "%s-new", hapd->iface->config_fname);
#ifndef HISI_CODE_CROP
	oconf = fopen(hapd->iface->config_fname, "r");
	if (oconf == NULL) {
		wpa_warning_log0(MSG_WARNING, "WPS: Could not open current "
			   "configuration file");
		os_free(tmp_fname);
		return -1;
	}
	...
	if (nconf == NULL) {
		wpa_warning_log0(MSG_WARNING, "WPS: Could not write updated "
			   "configuration file");
		os_free(tmp_fname);
		fclose(oconf);
		return -1;
	...
	if (wpa) {
		...
		if (cred->key_len >= 8 && cred->key_len < 64) {
			...
			wpa_warning_log1(MSG_WARNING, "WPS: Invalid key length %lu "
				   "for WPA/WPA2",
				   (unsigned long) cred->key_len);
		}
	...
	if (rename(tmp_fname, hapd->iface->config_fname) < 0) {
		wpa_warning_buf(MSG_WARNING, "WPS: Failed to rename the updated "
			   "configuration file: %s", strerror(errno), strlen(strerror(errno)));
		os_free(tmp_fname);
		return -1;
	}
#endif /* LOS_WPA_PATCH */
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: AP configuration updated");
...
static void hostapd_wps_reenable_ap_pin(void *eloop_data, void *user_ctx)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Re-enable AP PIN");
	wpa_msg(hapd->msg_ctx, MSG_INFO, WPS_EVENT_AP_SETUP_UNLOCKED);
	hapd->wps->ap_setup_locked = 0;
	(void)wps_registrar_update_ie(hapd->wps->registrar);
}
...
static int wps_pwd_auth_fail(struct hostapd_data *hapd, void *ctx)
{
	...
	wpa_warning_log2(MSG_DEBUG, "WPS: AP PIN authentication failure number %u "
		   "(%u consecutive)",
	...
	if (!hapd->conf->ap_setup_locked &&
		 ...
		 * reenablement.
		 */
		(void)eloop_cancel_timeout(hostapd_wps_reenable_ap_pin, hapd, NULL);
		wpa_warning_log0(MSG_DEBUG, "WPS: AP PIN disabled indefinitely");
		...
		wpa_warning_log1(MSG_DEBUG, "WPS: Disable AP PIN for %u seconds",
			   hapd->ap_pin_lockout_time);
		(void)eloop_cancel_timeout(hostapd_wps_reenable_ap_pin, hapd, NULL);
		(void)eloop_register_timeout(hapd->ap_pin_lockout_time, 0,
...
static int wps_ap_pin_success(struct hostapd_data *hapd, void *ctx)
{
	...
	wpa_warning_log2(MSG_DEBUG, "WPS: Clear consecutive AP PIN failure counter "
...
{
	(void)hostapd_wps_for_each(hapd, wps_ap_pin_success, NULL);
}
...
static void hostapd_wps_clear_ies(struct hostapd_data *hapd, int deinit_only)
{
	...
	if (deinit_only) {
		if (hapd->drv_priv)
			(void)hostapd_reset_ap_wps_ie(hapd);
		return;
	}

	(void)hostapd_set_ap_wps_ie(hapd);
#endif /* CONFIG_WPS_AP */
}
...
static const u8 * get_own_uuid(struct hostapd_iface *iface)
{
	...
	if (iface->interfaces == NULL ||
		...
		return NULL;
	uuid = NULL;
	(void)iface->interfaces->for_each_interface(iface->interfaces, get_uuid_cb,
					      &uuid);
	return uuid;
}
...
static int interface_count(struct hostapd_iface *iface)
{
	...
	if (iface->interfaces == NULL ||
	    iface->interfaces->for_each_interface == NULL)
		return 0;
	(void)iface->interfaces->for_each_interface(iface->interfaces,
		     ...
		     struct hostapd_bss_config *conf)
{
	...
	if (conf->ssid.wpa_passphrase) {
		...
		if (wps->network_key == NULL)
			goto fail;
		(void)wpa_snprintf_hex((char *) wps->network_key, 2 * PMK_LEN + 1,
	...
	if ((wps->dev.rf_bands & (WPS_RF_50GHZ | WPS_RF_24GHZ)) ==
		...
		cfg.dualband = 1;
	if (cfg.dualband) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Dualband AP");
	}
	cfg.force_per_enrollee_psk = conf->force_per_enrollee_psk;

	wps->registrar = wps_registrar_init(wps, &cfg);
	if (wps->registrar == NULL) {
		wpa_error_log0(MSG_ERROR, "Failed to initialize WPS Registrar");
		goto fail;
	}
...
#ifndef HISI_CODE_CROP
	(void)hostapd_register_probereq_cb(hapd, hostapd_wps_probe_req_rx, hapd);
#endif /* HISI_CODE_CROP */
...
int hostapd_init_wps_complete(struct hostapd_data *hapd)
{
	...
	if (hostapd_wps_upnp_init(hapd, wps) < 0) {
		wpa_error_log0(MSG_ERROR, "Failed to initialize WPS UPnP");
...
{
	(void)eloop_cancel_timeout(hostapd_wps_reenable_ap_pin, hapd, NULL);
	(void)eloop_cancel_timeout(hostapd_wps_ap_pin_timeout, hapd, NULL);
	(void)eloop_cancel_timeout(wps_reload_config, hapd->iface, NULL);
	if (hapd->wps == NULL) {
...
void hostapd_update_wps(struct hostapd_data *hapd)
{
	...
	(void)hostapd_wps_set_vendor_ext(hapd, hapd->wps);
...
static int wps_cancel(struct hostapd_data *hapd, void *ctx)
{
	...
	if (hapd->wps) {
		data->count++;
		(void)wps_registrar_wps_cancel(hapd->wps->registrar);
		(void)ap_for_each_sta(hapd, ap_sta_wps_cancel, NULL);
	}
				    ...
				    int ssi_signal)
{
	...
	if (ieee802_11_parse_elems(ie, ie_len, &elems, 0) == ParseFailed) {
		wpa_warning_log4(MSG_DEBUG, "WPS: Could not parse ProbeReq from "
	...
	enum wps_msg_type msg_type)
{
	...
	if (!sta || !(sta->flags & WLAN_STA_WPS)) {
		wpa_warning_log0(MSG_DEBUG, "WPS UPnP: No matching STA found");
		return 0;
	}
	...
	if (!sta->eapol_sm) {
		 ...
		 * deauthenticated).
		 */
		wpa_warning_log0(MSG_DEBUG, "WPS UPnP: Matching STA did not have EAPOL state machine initialized");
		return 0;
	}
...
static void hostapd_wps_ap_pin_timeout(void *eloop_data, void *user_ctx)
{
	struct hostapd_data *hapd = eloop_data;
	(void)user_ctx;
	wpa_warning_log0(MSG_DEBUG, "WPS: AP PIN timed out");
...
{
	wpa_warning_log1(MSG_DEBUG, "WPS: Enabling AP PIN (timeout=%d)", timeout);
	...
	(void)eloop_cancel_timeout(hostapd_wps_ap_pin_timeout, hapd, NULL);
	if (timeout > 0)
		(void)eloop_register_timeout(timeout, 0,
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS: Disabling AP PIN");
	(void)hostapd_wps_for_each(hapd, wps_ap_pin_disable, NULL);
}
...
const char * hostapd_wps_ap_pin_random(struct hostapd_data *hapd, int timeout)
{
	...
	if (wps_generate_pin(&pin) < 0)
		return NULL;
	(void)os_snprintf(data.pin_txt, sizeof(data.pin_txt), "%08u", pin);
	data.timeout = timeout;
	(void)hostapd_wps_for_each(hapd, wps_ap_pin_set, &data);
...
{
	(void)hostapd_wps_for_each(hapd, wps_update_ie, NULL);
}
				    ...
				    const struct wpabuf *sel)
{
	...
	if (wsc_len > wpabuf_len(wps) - 2) {
		wpa_warning_log1(MSG_DEBUG, "WPS: Invalid WSC attribute length (%u) "
