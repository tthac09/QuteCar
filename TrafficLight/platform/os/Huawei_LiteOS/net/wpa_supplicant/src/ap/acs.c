...
{
	wpa_error_log0(MSG_ERROR, "ACS: Failed to start");
	acs_cleanup(iface);
...
acs_survey_interference_factor(struct freq_survey *survey, s8 min_nf)
{
	...
	if (survey->filled & SURVEY_HAS_CHAN_TIME_BUSY)
		 ...
		 * acs_sanity_check() */
		wpa_error_log0(MSG_ERROR, "ACS: Survey data missing");
		return 0;
	}
				    ...
				    struct hostapd_channel_data *chan)
{
	...
	dl_list_for_each(survey, &chan->survey_list, struct freq_survey, list)
	{
		...
		if (!acs_survey_is_sufficient(survey)) {
			wpa_warning_log1(MSG_DEBUG, "ACS: %d: insufficient data", i);
			continue;
		}
		...
		wpa_warning_log3(MSG_DEBUG, "ACS: %d: min_nf=%d interference_factor=%Lg",
			   i, chan->min_nf, int_factor);
		wpa_warning_log4(MSG_DEBUG, "nf=%d time=%lu busy=%lu rx=%lu",
...
static int acs_survey_is_sufficient(struct freq_survey *survey)
{
	if (!(survey->filled & SURVEY_HAS_NF)) {
		wpa_warning_log0(MSG_INFO, "ACS: Survey is missing noise floor");
		return 0;
	}

	if (!(survey->filled & SURVEY_HAS_CHAN_TIME)) {
		wpa_warning_log0(MSG_INFO, "ACS: Survey is missing channel time");
		return 0;
	}
	...
	struct hostapd_iface *iface)
{
	...
	for (i = 0; i < iface->current_mode->num_channels; i++) {
		...
		wpa_warning_log2(MSG_DEBUG, "ACS: Survey analysis for channel %d (%d MHz)",
			   chan->chan, chan->freq);

		acs_survey_chan_interference_factor(iface, chan);

		wpa_warning_log1(MSG_DEBUG, "ACS:  * interference factor average: %Lg",
...
acs_find_ideal_chan(struct hostapd_iface *iface)
{
	...
	if (iface->conf->ieee80211n &&
		...
		wpa_error_log0(MSG_ERROR, "ACS: HT40- is not supported yet. Please try HT40+");
		return NULL;
	}
	...
	wpa_warning_log1(MSG_DEBUG, "ACS: Survey analysis for selected bandwidth %d MHz",
		   n_chans == 1 ? 20 :
		   n_chans == 2 ? 40 :
		   80);
	...
	for (i = 0; i < iface->current_mode->num_channels; i++) {
		...
		if (iface->current_mode->mode == HOSTAPD_MODE_IEEE80211A &&
			...
			wpa_warning_log1(MSG_DEBUG, "ACS: Channel %d: not allowed as primary channel for HT40",
				   chan->chan);
			continue;
		}
		...
		if (iface->current_mode->mode == HOSTAPD_MODE_IEEE80211A &&
			...
			if (hostapd_get_oper_chwidth(iface->conf) ==
				...
				wpa_warning_log1(MSG_DEBUG, "ACS: Channel %d: not allowed as primary channel for VHT80",
				   chan->chan);
				continue;
			}
		...
		if (j != n_chans) {
			wpa_warning_log1(MSG_DEBUG, "ACS: Channel %d: not enough bandwidth",
				   chan->chan);
			continue;
		}
	...
	if (ideal_chan) {
		wpa_warning_log3(MSG_DEBUG, "ACS: Ideal channel is %d (%d MHz) with total interference factor of %Lg",
...
static void acs_adjust_center_freq(struct hostapd_iface *iface)
{
	...
	wpa_warning_log0(MSG_DEBUG, "ACS: Adjusting VHT center frequency");
	...
	default:
		 ...
		 * acs_find_ideal_chan() */
		wpa_warning_log0(MSG_INFO, "ACS: Only VHT20/40/80 is supported now");
		return;
	}
...
{
	wpa_warning_log0(MSG_DEBUG, "ACS: Trying survey-based ACS");

	if (!iface->chans_surveyed) {
		wpa_error_log0(MSG_ERROR, "ACS: Unable to collect survey data");
		return -1;
	}

	if (!acs_surveys_are_sufficient(iface)) {
		wpa_error_log0(MSG_ERROR, "ACS: Surveys have insufficient data");
		return -1;
	}
...
static void acs_study(struct hostapd_iface *iface)
{
	...
	if (err < 0) {
		wpa_error_log0(MSG_ERROR, "ACS: All study options have failed");
		goto fail;
	}

	ideal_chan = acs_find_ideal_chan(iface);
	if (!ideal_chan) {
		wpa_error_log0(MSG_ERROR, "ACS: Failed to compute ideal channel");
		err = -1;
		goto fail;
	}
...
fail:
	...
	wpa_error_log0(MSG_ERROR, "ACS: Possibly channel configuration is invalid, please report this along with your config file.");
	acs_fail(iface);
}
...
static void acs_scan_complete(struct hostapd_iface *iface)
{
	...
	wpa_warning_log1(MSG_DEBUG, "ACS: Using survey based algorithm (acs_num_scans=%d)",
		   iface->conf->acs_num_scans);

	err = hostapd_drv_get_survey(iface->bss[0], 0);
	if (err) {
		wpa_error_log0(MSG_ERROR, "ACS: Failed to get survey data");
		goto fail;
	}
	...
	if (++iface->acs_num_completed_scans < iface->conf->acs_num_scans) {
		...
		if (err) {
			wpa_error_log0(MSG_ERROR, "ACS: Failed to request scan");
			goto fail;
		}
...
static int acs_request_scan(struct hostapd_iface *iface)
{
	...
	*freq = 0;
	...
	wpa_warning_log2(MSG_DEBUG, "ACS: Scanning %d / %d",
		   iface->acs_num_completed_scans + 1,
		   iface->conf->acs_num_scans);

	if (hostapd_driver_scan(iface->bss[0], &params) < 0) {
		wpa_error_log0(MSG_ERROR, "ACS: Failed to request initial scan");
		acs_cleanup(iface);
...
{
	wpa_warning_log0(MSG_INFO, "ACS: Automatic channel selection started, this may take a bit");

	if (iface->drv_flags & WPA_DRIVER_FLAGS_ACS_OFFLOAD) {
		wpa_warning_log0(MSG_INFO, "ACS: Offloading to driver");
