...
#endif /* CONFIG_MBO */
#ifdef LOS_WPA_PATCH
	if (g_ap_opt_set.wpa_group_rekey != 0) {
		bss->wpa_group_rekey = g_ap_opt_set.wpa_group_rekey;
	}
	if(g_ap_opt_set.dtim_period != 0) {
	   bss->dtim_period = g_ap_opt_set.dtim_period;
	}
#ifdef LOS_CONFIG_MESH
	if (g_mesh_flag && (g_mesh_opt_set.wpa_group_rekey != 0)) {
		bss->wpa_group_rekey = g_mesh_opt_set.wpa_group_rekey;
	}
	if (g_mesh_flag) {
		bss->dtim_period = g_mesh_opt_set.dtim_period ? g_mesh_opt_set.dtim_period : 1;
	}
...
#ifdef LOS_WPA_PATCH
	if (g_ap_opt_set.beacon_period != 0) {
		conf->beacon_int = g_ap_opt_set.beacon_period;
	}
#ifdef LOS_CONFIG_MESH
	if (g_mesh_flag) {
		conf->beacon_int = g_mesh_opt_set.beacon_period ? g_mesh_opt_set.beacon_period : 1000;	// 1000 = 1s
	}
#endif /* LOS_CONFIG_MESH */
#endif /* LOS_WPA_PATCH */
	return conf;
				       ...
				       struct hostapd_ssid *ssid)
{
	...
	f = fopen(fname, "r");
	if (!f) {
		wpa_error_buf(MSG_ERROR, "WPA PSK file '%s' not found.", fname, strlen(fname));
		return -1;
	}
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		if (hwaddr_aton(token, addr)) {
			wpa_error_two_buf(MSG_ERROR, "Invalid MAC address '%s' "
				   "in '%s'", token, strlen(token), fname, strlen(fname));
			ret = -1;
			break;
		}
		...
		if (psk == NULL) {
			wpa_error_log0(MSG_ERROR, "WPA PSK allocation failed");
			ret = -1;
			break;
		}
		psk->vlan_id = vlan_id;
		...
		if (!pos) {
			wpa_error_buf(MSG_ERROR, "No PSK in '%s'", fname, strlen(fname));
			os_free(psk);
			ret = -1;
			break;
		}
		...
		if (!ok) {
			wpa_error_two_buf(MSG_ERROR, "Invalid PSK '%s' in '%s'", pos, strlen(pos), fname, strlen(fname));
			os_free(psk);
			ret = -1;
			break;
		}
		...
		if (keyid) {
			...
			if ((size_t) len >= sizeof(psk->keyid)) {
				wpa_error_buf(MSG_ERROR,
					   "PSK keyid too long in '%s'", fname, strlen(fname));
				os_free(psk);
				ret = -1;
				break;
			}
...
static int hostapd_derive_psk(struct hostapd_ssid *ssid)
{
	...
	if (ssid->wpa_psk == NULL) {
		wpa_error_log0(MSG_ERROR, "Unable to alloc space for PSK");
		return -1;
	}
...
int hostapd_setup_wpa_psk(struct hostapd_bss_config *conf)
{
	...
	if (ssid->wpa_passphrase != NULL) {
		if (ssid->wpa_psk != NULL) {
			wpa_warning_log0(MSG_DEBUG, "Using pre-configured WPA PSK "
				   "instead of passphrase");
		} else {
			wpa_warning_log0(MSG_DEBUG, "Deriving WPA PSK based on "
				   "passphrase");
...
{
#if defined(CONFIG_WPS_AP) || defined(CONFIG_HS20)
	size_t i;
#endif
...
#endif /* LOS_CONFIG_NO_VLAN */
{
	...
	if (p2p_dev_addr && !is_zero_ether_addr(p2p_dev_addr)) {
		wpa_warning_log4(MSG_DEBUG, "Searching a PSK for " "%02x:xx:xx:%02x:%02x:%02x",
			   addr[0], addr[3], addr[4], addr[5]);
		wpa_warning_log4(MSG_DEBUG, " p2p_dev_addr=" "%02x:xx:xx:%02x:%02x:%02x",
			   p2p_dev_addr[0], p2p_dev_addr[3], p2p_dev_addr[4], p2p_dev_addr[5]);
		wpa_warning_log1(MSG_DEBUG, " prev_psk=%p", prev_psk);
				    ...
				    int full_config)
{
	...
	if (full_config && bss->ieee802_1x && !bss->eap_server &&
		...
		wpa_error_log0(MSG_ERROR, "Invalid IEEE 802.1X configuration (no "
	...
	if (bss->wpa) {
		...
		if (wep) {
			wpa_error_log0(MSG_ERROR, "WEP configuration in a WPA network is not supported");
			return -1;
		}
	}
#endif
	...
	if (full_config && bss->wpa &&
		...
		wpa_error_log0(MSG_ERROR, "WPA-PSK using RADIUS enabled, but no "
	...
	if (full_config && bss->wpa && (bss->wpa_key_mgmt & WPA_KEY_MGMT_PSK) &&
	    ...
	    (bss->wpa_psk_radius != PSK_RADIUS_REQUIRED ||
	     bss->macaddr_acl != USE_EXTERNAL_RADIUS_AUTH)) {
		wpa_error_log0(MSG_ERROR, "WPA-PSK enabled, but PSK or passphrase "
			   "is not configured.");
#else
	if (full_config && bss->wpa && (bss->wpa_key_mgmt & WPA_KEY_MGMT_PSK) &&
		bss->ssid.wpa_psk == NULL && bss->ssid.wpa_passphrase == NULL) {
		...
		for (i = 0; i < conf->num_bss; i++) {
			if (conf->bss[i] != bss &&
			    (hostapd_mac_comp(conf->bss[i]->bssid,
					      bss->bssid) == 0)) {
				wpa_error_log4(MSG_ERROR, "Duplicate BSSID " "%02x:xx:xx:%02x:%02x:%02x",
					   (bss->bssid)[0], (bss->bssid)[3], (bss->bssid)[4], (bss->bssid)[5]);
				wpa_error_two_buf(MSG_ERROR, " on interface '%s' and '%s'.",
					   conf->bss[i]->iface, strlen(conf->bss[i]->iface), bss->iface, strlen(bss->iface));
				return -1;
			}
		}
	}
	...
	if (full_config && wpa_key_mgmt_ft(bss->wpa_key_mgmt) &&
		...
		wpa_error_log0(MSG_ERROR, "FT (IEEE 802.11r) requires "
	...
	if (full_config && conf->ieee80211n &&
		...
		bss->disable_11n = 1;
		wpa_error_log0(MSG_ERROR, "HT (IEEE 802.11n) in 11b mode is not "
	...
	if (full_config && conf->ieee80211n &&
		...
		bss->disable_11n = 1;
		wpa_error_log0(MSG_ERROR, "HT (IEEE 802.11n) with WEP is not "
			   "allowed, disabling HT capabilities");
	}
#endif
	...
	{
		bss->disable_11n = 1;
		wpa_error_log0(MSG_ERROR, "HT (IEEE 802.11n) with WPA/WPA2 "
	...
	if (full_config && bss->wps_state && bss->ignore_broadcast_ssid) {
		wpa_warning_log0(MSG_INFO, "WPS: ignore_broadcast_ssid "
	...
	if (full_config && bss->wps_state &&
		...
		wpa_warning_log0(MSG_INFO, "WPS: WEP configuration forced WPS to be "
			   "disabled");
		bss->wps_state = 0;
	}
#endif
	if (full_config && bss->wps_state && bss->wpa &&
	    (!(bss->wpa & 2) ||
	     !(bss->rsn_pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP |
				    WPA_CIPHER_CCMP_256 |
				    WPA_CIPHER_GCMP_256)))) {
		wpa_warning_log0(MSG_INFO, "WPS: WPA/TKIP configuration without "
	...
	if (full_config && bss->hs20 &&
	    (!(bss->wpa & 2) ||
	     !(bss->rsn_pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP |
				    WPA_CIPHER_CCMP_256 |
				    WPA_CIPHER_GCMP_256)))) {
		wpa_error_log0(MSG_ERROR, "HS 2.0: WPA2-Enterprise/CCMP "
...
int hostapd_config_check(struct hostapd_config *conf, int full_config)
{
	...
	if (full_config && conf->ieee80211d &&
		...
		wpa_error_log0(MSG_ERROR, "Cannot enable IEEE 802.11d without "
			   "setting the country_code");
		return -1;
	}

	if (full_config && conf->ieee80211h && !conf->ieee80211d) {
		wpa_error_log0(MSG_ERROR, "Cannot enable IEEE 802.11h without "
	...
	if (full_config && conf->local_pwr_constraint != -1 &&
	    !conf->ieee80211d) {
		wpa_error_log0(MSG_ERROR, "Cannot add Power Constraint element without Country element");
		return -1;
	}

	if (full_config && conf->spectrum_mgmt_required &&
	    conf->local_pwr_constraint == -1) {
		wpa_error_log0(MSG_ERROR, "Cannot set Spectrum Management bit without Country and Power Constraint elements");
		return -1;
	}
	...
	if (full_config && conf->airtime_mode > AIRTIME_MODE_STATIC &&
		...
		wpa_error_log0(MSG_ERROR, "Airtime update interval cannot be zero");
		return -1;
	}
#endif /* CONFIG_AIRTIME_POLICY */
#endif /* HISI_CODE_CROP */
