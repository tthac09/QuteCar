...
{
	while (pmksa->pmksa) {
		wpa_warning_log4(MSG_DEBUG, "RSN: Flush PMKSA cache entry for "
			   "%02x:xx:xx:%02x:%02x:%02x", 
			   (pmksa->pmksa->spa)[0], (pmksa->pmksa->spa)[3], (pmksa->pmksa->spa)[4], (pmksa->pmksa->spa)[5]);
...
static void pmksa_cache_expire(void *eloop_ctx, void *timeout_ctx)
{
	...
	while (pmksa->pmksa && pmksa->pmksa->expiration <= now.sec) {
		wpa_warning_log4(MSG_DEBUG, "RSN: expired PMKSA cache entry for "
			   "%02x:xx:xx:%02x:%02x:%02x", 
			   (pmksa->pmksa->spa)[0], (pmksa->pmksa->spa)[3], (pmksa->pmksa->spa)[4], (pmksa->pmksa->spa)[5]);
		pmksa_cache_free_entry(pmksa, pmksa->pmksa);
	}
...
static void pmksa_cache_set_expiration(struct rsn_pmksa_cache *pmksa)
{
	...
	(void)eloop_cancel_timeout(pmksa_cache_expire, pmksa, NULL);
	if (pmksa->pmksa == NULL)
		return;
	(void)os_get_reltime(&now);
	...
	if (sec < 0)
		sec = 0;
	(void)eloop_register_timeout(sec + 1, 0, pmksa_cache_expire, pmksa, NULL);
}
					...
					struct eapol_state_machine *eapol)
{
	...
	if (eapol->identity) {
		...
		if (entry->identity) {
			...
			(void)os_memcpy(entry->identity, eapol->identity,
				  eapol->identity_len);
		}
	}
#ifndef HISI_CODE_CROP
			       ...
			       struct eapol_state_machine *eapol)
{
	...
	if (entry->identity) {
		...
		if (eapol->identity) {
			...
			(void)os_memcpy(eapol->identity, entry->identity,
				  entry->identity_len);
		}
	...
	if (eapol->radius_class.attr) {
		wpa_warning_log1(MSG_DEBUG, "Copied %lu Class attribute(s) from "
				   ...
				   struct rsn_pmksa_cache_entry *entry)
{
	...
	if (prev == NULL)
		pmksa_cache_set_expiration(pmksa);
	wpa_warning_log4(MSG_DEBUG, "RSN: added PMKSA cache entry for " "%02x:xx:xx:%02x:%02x:%02x",
		   (entry->spa)[0], (entry->spa)[3], (entry->spa)[4], (entry->spa)[5]);
			      ...
			      struct eapol_state_machine *eapol, int akmp)
{
	...
	if (pmkid)
		...
		(void)rsn_pmkid_suite_b_192(kck, kck_len, aa, spa, entry->pmkid);
	else if (wpa_key_mgmt_suite_b(akmp))
		(void)rsn_pmkid_suite_b(kck, kck_len, aa, spa, entry->pmkid);
#endif /* HISI_WPA_KEY_MGMT_CROP */
	else
			       ...
			       struct rsn_pmksa_cache_entry *entry)
{
	...
	if (pmksa->pmksa_count >= pmksa_cache_max_entries && pmksa->pmksa) {
		...
		wpa_warning_log4(MSG_DEBUG, "RSN: removed the oldest PMKSA cache "
			   "entry (for " "%02x:xx:xx:%02x:%02x:%02x" ") to make room for new one",
			   (pmksa->pmksa->spa)[0], (pmksa->pmksa->spa)[3], (pmksa->pmksa->spa)[4], (pmksa->pmksa->spa)[5]);
		pmksa_cache_free_entry(pmksa, pmksa->pmksa);
	}
			  ...
			  struct radius_das_attrs *attr)
{
	...
	if (attr->acct_multi_session_id) {
		...
		if (attr->acct_multi_session_id_len != 16)
			return 0;
		(void)os_snprintf(buf, sizeof(buf), "%016llX",
