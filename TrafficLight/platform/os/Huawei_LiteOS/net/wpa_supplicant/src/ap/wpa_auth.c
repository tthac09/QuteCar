			       ...
			       const u8 *addr, u16 reason)
{
	if (wpa_auth->cb->disconnect == NULL)
		return;
	wpa_error_log4(MSG_DEBUG, "wpa_sta_disconnect STA " "%02x:xx:xx:%02x:%02x:%02x",
		   addr[0], addr[3], addr[4], addr[5]);
	wpa_error_log1(MSG_DEBUG, "wpa_sta_disconnect STA " " (reason %u)", reason);
...
static void wpa_rekey_gmk(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (random_get_bytes(wpa_auth->group->GMK, WPA_GMK_LEN)) {
		wpa_error_log0(MSG_ERROR, "Failed to get random data for WPA "
			   "initialization.");
	} else {
	...
	if (wpa_auth->conf.wpa_gmk_rekey) {
		(void)eloop_register_timeout(wpa_auth->conf.wpa_gmk_rekey, 0,
...
static void wpa_rekey_gtk(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_auth->conf.wpa_group_rekey) {
		(void)eloop_register_timeout(wpa_auth->conf.wpa_group_rekey,
...
#ifndef LOS_CONFIG_HOSTAPD_PMKSA
				   ...
				   void *ctx)
{
	...
	(void)wpa_auth_for_each_sta(wpa_auth, wpa_auth_pmksa_clear_cb, entry);
}
#endif /* LOS_CONFIG_HOSTAPD_PMKSA */
					  ...
					  struct wpa_group *group)
{
	 ...
	 */
	(void)os_memcpy(buf, wpa_auth->addr, ETH_ALEN);
	wpa_get_ntp_timestamp(buf + ETH_ALEN);
	ptr = (unsigned long) group;
	(void)os_memcpy(buf + ETH_ALEN + 8, &ptr, sizeof(ptr));
#ifdef TEST_FUZZ
					 ...
					 int vlan_id, int delay_init)
{
	...
	if (random_pool_ready() != 1) {
		wpa_error_log0(MSG_INFO, "WPA: Not enough entropy in random pool "
	...
	group->GInit = TRUE;
	if (delay_init) {
		wpa_error_log0(MSG_DEBUG, "WPA: Delay group state machine start "
				    ...
				    void *cb_ctx)
{
	...
	if (wpa_auth_gen_wpa_ie(wpa_auth)) {
		wpa_error_log0(MSG_ERROR, "Could not generate WPA IE.");
		os_free(wpa_auth);
		return NULL;
	}
	...
	if (wpa_auth->pmksa == NULL) {
		wpa_error_log0(MSG_ERROR, "PMKSA cache initialization failed.");
	...
	if (wpa_auth->ft_pmk_cache == NULL) {
		wpa_error_log0(MSG_ERROR, "FT PMK cache initialization failed.");
		os_free(wpa_auth->group);
		os_free(wpa_auth->wpa_ie);
	...
	if (wpa_auth->conf.wpa_group_rekey) {
		(void)eloop_register_timeout(wpa_auth->conf.wpa_group_rekey, 0,
				       wpa_rekey_gtk, wpa_auth, NULL);
		wpa_error_log0(MSG_DEBUG, "wpa_rekey_gtk register start\n");
	}
...
int wpa_init_keys(struct wpa_authenticator *wpa_auth)
{
	...
	wpa_error_log0(MSG_DEBUG, "WPA: Start group state machine to set initial "
		   "keys");
	wpa_group_sm_step(wpa_auth, group);
	group->GInit = FALSE;
	wpa_group_sm_step(wpa_auth, group);
...
void wpa_deinit(struct wpa_authenticator *wpa_auth)
{
	...
	(void)eloop_cancel_timeout(wpa_rekey_gmk, wpa_auth, NULL);
	(void)eloop_cancel_timeout(wpa_rekey_gtk, wpa_auth, NULL);
		 ...
		 struct wpa_auth_config *conf)
{
	...
	(void)os_memcpy(&wpa_auth->conf, conf, sizeof(*conf));
	if (wpa_auth_gen_wpa_ie(wpa_auth)) {
		wpa_error_log0(MSG_ERROR, "Could not generate WPA IE.");
		return -1;
	}
		  ...
		  const u8 *p2p_dev_addr)
{
	...
	if (sm == NULL)
		return NULL;
	(void)os_memcpy(sm->addr, addr, ETH_ALEN);
	if (p2p_dev_addr)
		(void)os_memcpy(sm->p2p_dev_addr, p2p_dev_addr, ETH_ALEN);
...
static void wpa_free_sta_sm(struct wpa_state_machine *sm)
{
	...
	if (WPA_GET_BE32(sm->ip_addr)) {
		u32 start;
		wpa_error_log4(MSG_DEBUG, "P2P: Free assigned IP "
			   "address %u.%u.%u.%u from ",
...
void wpa_auth_sta_deinit(struct wpa_state_machine *sm)
{
	...
	(void)eloop_cancel_timeout(wpa_send_eapol_timeout, sm->wpa_auth, sm);
	sm->pending_1_of_4_timeout = 0;
	(void)eloop_cancel_timeout(wpa_sm_call_step, sm, NULL);
	(void)eloop_cancel_timeout(wpa_rekey_ptk, sm->wpa_auth, sm);
#ifdef CONFIG_IEEE80211R_AP
	...
	if (sm->in_step_loop) {
		...
		wpa_error_log4(MSG_DEBUG, "WPA: Registering pending STA state "
			   "machine deinit for " "%02x:xx:xx:%02x:%02x:%02x",
			   (sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
		sm->pending_deinit = 1;
	} else
		wpa_free_sta_sm(sm);
			       ...
			       struct wpa_eapol_ie_parse *kde)
{
	...
	if (wpa_parse_wpa_ie_rsn(kde->rsn_ie, kde->rsn_ie_len, &ie) < 0 ||
		...
		wpa_warning_log0(MSG_DEBUG, "FT: No PMKR1Name in "
	...
	if (kde->mdie[1] < sizeof(struct rsn_mdie) ||
		...
		wpa_warning_log0(MSG_DEBUG, "FT: MDIE mismatch");
		return -1;
	}
	...
	if (sm->assoc_resp_ftie &&
		...
		wpa_warning_log0(MSG_DEBUG, "FT: FTIE mismatch");
		 ...
		 u8 *data, size_t data_len)
{
	...
	if (data_len < sizeof(*hdr) + keyhdrlen) {
		wpa_error_log0(MSG_DEBUG, "WPA: Ignore too short EAPOL-Key frame");
		return;
	}
	...
	wpa_error_log4(MSG_DEBUG, "WPA: Received EAPOL-Key from " "%02x:xx:xx:%02x:%02x:%02x",
		   (sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
	wpa_error_log4(MSG_DEBUG, "WPA: Received EAPOL-Key from "
		   " key_info=0x%x type=%u mic_len=%u key_data_length=%u",
		   key_info, key->type, (unsigned int) mic_len, key_data_length);
	...
	if (key_data_length > data_len - sizeof(*hdr) - keyhdrlen) {
		wpa_error_log2(MSG_INFO, "WPA: Invalid EAPOL-Key frame - "
	...
	if (sm->wpa == WPA_VERSION_WPA2) {
		if (key->type == EAPOL_KEY_TYPE_WPA) {
			 ...
			 */
			wpa_error_log0(MSG_DEBUG, "Workaround: Allow EAPOL-Key "
				   "with unexpected WPA type in RSN mode");
		} else if (key->type != EAPOL_KEY_TYPE_RSN) {
			wpa_error_log1(MSG_DEBUG, "Ignore EAPOL-Key with "
	...
	if (key_info & WPA_KEY_INFO_SMK_MESSAGE) {
		wpa_error_log0(MSG_DEBUG, "WPA: Ignore SMK message");
		return;
	}
...
continue_processing:
	...
	case PAIRWISE_2:
		...
		if (sm->group->reject_4way_hs_for_entropy) {
			 ...
			 * continue.
			 */
			wpa_error_log0(MSG_DEBUG, "WPA: Reject 4-way handshake to "
...
#endif /* TEST_FUZZ */
		sm->MICVerified = TRUE;
		(void)eloop_cancel_timeout(wpa_send_eapol_timeout, wpa_auth, sm);
		sm->pending_1_of_4_timeout = 0;
	}
	...
	if (key_info & WPA_KEY_INFO_REQUEST) {
		if (sm->MICVerified) {
			...
			(void)os_memcpy(sm->req_replay_counter, key->replay_counter,
				  WPA_REPLAY_COUNTER_LEN);
		} else {
			wpa_auth_logger(wpa_auth, sm->addr, LOGGER_INFO,
					"received EAPOL-Key request with "
		...
		if (msg == PAIRWISE_2) {
			 ...
			 */
			(void)os_memcpy(sm->prev_key_replay, sm->key_replay,
				  sizeof(sm->key_replay));
		} else {
			(void)os_memset(sm->prev_key_replay, 0,
	...
	(void)os_memcpy(sm->SNonce, key->key_nonce, WPA_NONCE_LEN);
	(void)wpa_sm_step(sm);
}
		      ...
		      int keyidx, int encr, int force_version)
{
	...
	wpa_error_log4(MSG_DEBUG, "WPA: Send EAPOL(version=%d secure=%d mic=%d ack=%d",
		   version,
		   (key_info & WPA_KEY_INFO_SECURE) ? 1 : 0,
		   (key_info & WPA_KEY_INFO_MIC) ? 1 : 0,
		   (key_info & WPA_KEY_INFO_ACK) ? 1 : 0);
	wpa_error_log4(MSG_DEBUG, "install=%d pairwise=%d kde_len=%lu keyidx=%d",
		   (key_info & WPA_KEY_INFO_INSTALL) ? 1 : 0,
		   pairwise, (unsigned long) kde_len, keyidx);
	wpa_error_log1(MSG_DEBUG, "encr=%d", encr);
	...
	for (i = RSNA_MAX_EAPOL_RETRIES - 1; i > 0; i--) {
		...
		(void)os_memcpy(sm->key_replay[i].counter,
			  sm->key_replay[i - 1].counter,
			  WPA_REPLAY_COUNTER_LEN);
	}
	inc_byte_array(sm->key_replay[0].counter, WPA_REPLAY_COUNTER_LEN);
	(void)os_memcpy(key->replay_counter, sm->key_replay[0].counter,
		  WPA_REPLAY_COUNTER_LEN);
	...
	if (nonce)
		(void)os_memcpy(key->key_nonce, nonce, WPA_NONCE_LEN);

	if (key_rsc)
		(void)os_memcpy(key->key_rsc, key_rsc, WPA_KEY_RSC_LEN);
	...
	if (kde && !encr) {
		...
		if (aes_siv_encrypt(sm->PTK.kek, sm->PTK.kek_len, kde, kde_len,
			...
			wpa_warning_log0(MSG_DEBUG, "WPA: AES-SIV encryption failed");
			return;
		}
		...
		pos = buf;
		(void)os_memcpy(pos, kde, kde_len);
		pos += kde_len;
	...
	wpa_auth_set_eapol(sm->wpa_auth, sm->addr, WPA_EAPOL_inc_EapolFramesTx,
			   1);
	(void)wpa_auth_send_eapol(wpa_auth, sm->addr, (u8 *) hdr, len,
			    sm->pairwise_set);
	os_free(hdr);
}
			   ...
			   int keyidx, int encr)
{
	...
	timeout_ms = 1;
#endif /* TEST_FUZZ */
	wpa_error_log2(MSG_DEBUG, "WPA: Use EAPOL-Key timeout of %u ms (retry "
...
int wpa_auth_sm_event(struct wpa_state_machine *sm, enum wpa_event event)
{
	...
	case WPA_REAUTH_EAPOL:
		if (!sm->started) {
			 ...
			 */
			wpa_error_log0(MSG_DEBUG, "WPA state machine had not been "
	...
	case WPA_ASSOC_FT:
#ifdef CONFIG_IEEE80211R_AP
		wpa_warning_log0(MSG_DEBUG, "FT: Retry PTK configuration "
			   "after association");
	...
	if (remove_ptk) {
		sm->PTK_valid = FALSE;
		(void)os_memset(&sm->PTK, 0, sizeof(sm->PTK));
				  ...
				  struct wpa_group *group)
{
	 ...
	 */
	wpa_error_log0(MSG_DEBUG, "WPA: Re-initialize GMK/Counter on first "
		   "station");
#ifndef CONFIG_NO_RANDOM_POOL
	if (random_pool_ready() != 1) {
		wpa_warning_log0(MSG_INFO, "WPA: Not enough entropy in random pool "
	...
	if (wpa_group_init_gmk_and_counter(wpa_auth, group) < 0 ||
		...
		wpa_error_log0(MSG_INFO, "WPA: GMK/GTK setup failed");
...
SM_STATE(WPA_PTK, AUTHENTICATION2)
{
	...
	if (random_get_bytes(sm->ANonce, WPA_NONCE_LEN)) {
		wpa_error_log0(MSG_ERROR, "WPA: Failed to get random data for "
			   "ANonce.");
		sm->Disconnect = TRUE;
		return;
	}
...
#endif /* CONFIG_IEEE80211R_AP */
	if (sm->pmksa) {
		wpa_warning_log0(MSG_DEBUG, "WPA: PMK from PMKSA cache");
		(void)os_memcpy(sm->PMK, sm->pmksa->pmk, sm->pmksa->pmk_len);
...
#endif /* HISI_WPA_KEY_MGMT_CROP */
			pmk_len = PMK_LEN;
		wpa_warning_log2(MSG_DEBUG, "WPA: PMK from EAPOL state machine "
		...
		(void)os_memcpy(sm->PMK, msk, pmk_len);
		sm->pmk_len = pmk_len;
#ifdef CONFIG_IEEE80211R_AP
		if (len >= 2 * PMK_LEN) {
			if (wpa_key_mgmt_sha384(sm->wpa_key_mgmt)) {
		...
		wpa_warning_log1(MSG_DEBUG, "WPA: Could not get PMK, get_msk: %p",
...
SM_STATE(WPA_PTK, INITPSK)
{
	...
	if (wpa_auth_uses_sae(sm) && sm->pmksa) {
		wpa_warning_log0(MSG_DEBUG, "SAE: PMK from PMKSA cache");
...
#endif /* CONFIG_PRINT_NOUSE */
			(void)os_memcpy(&pmkid[2 + RSN_SELECTOR_LEN],
				  sm->pmksa->pmkid, PMKID_LEN);
#else
		if (sm->wpa != WPA_VERSION_WPA2) {
#endif /* LOS_CONFIG_NO_PMKSA */
					  ...
					  const u8 *fils_session)
{
	...
	if (os_memcmp(fils_session, session + 3, FILS_SESSION_LEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "FILS: Session mismatch");
				  ...
				  size_t ies_len)
{
	...
	if (!elems.fils_key_confirm) {
		wpa_warning_log0(MSG_DEBUG, "FILS: No FILS Key Confirm element");
		return -1;
	}
	...
	if (os_memcmp(elems.fils_key_confirm, sm->fils_key_auth_sta,
		...
		wpa_warning_log0(MSG_DEBUG, "FILS: Key-Auth mismatch");
		       ...
		       u8 *pos, size_t left)
{
	...
	session = wpa_fils_validate_fils_session(sm, ie, end - ie,
						 fils_session);
	if (!session) {
		wpa_warning_log0(MSG_DEBUG, "FILS: Session validation failed");
		return -1;
	}
	...
	if (wpa_fils_validate_key_confirm(sm, pos, left - AES_BLOCK_SIZE) < 0) {
		wpa_warning_log0(MSG_DEBUG, "FILS: Key Confirm validation failed");
		return -1;
	}
		       ...
		       const struct wpabuf *hlp)
{
	...
	if (!plain) {
		wpa_warning_log0(MSG_DEBUG, "FILS: Plain buffer prep failed");
		return -1;
	}
...
int fils_set_tk(struct wpa_state_machine *sm)
{
	...
	if (!sm || !sm->PTK_valid) {
		wpa_warning_log0(MSG_DEBUG, "FILS: No valid PTK available to set TK");
		return -1;
	}
	if (sm->tk_already_set) {
		wpa_warning_log0(MSG_DEBUG, "FILS: TK already set to the driver");
		return -1;
	}
	...
	wpa_warning_log0(MSG_DEBUG, "FILS: Configure TK to the driver");
	if (wpa_auth_set_key(sm->wpa_auth, 0, alg, sm->addr, 0,
			     sm->PTK.tk, klen)) {
		wpa_warning_log0(MSG_DEBUG, "FILS: Failed to set TK to the driver");
		return -1;
	}
...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING)
{
	...
	if (kde.ip_addr_req && kde.ip_addr_req[0] &&
		...
		if (idx >= 0) {
			...
			wpa_warning_log4(MSG_DEBUG,
				   "P2P: Assigned IP address %u.%u.%u.%u to ", sm->ip_addr[0], sm->ip_addr[1],
				   sm->ip_addr[2], sm->ip_addr[3]);
			wpa_warning_log4(MSG_DEBUG,
				   "%02x:xx:xx:%02x:%02x:%02x",
				   (sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
	...
	if (wpa_key_mgmt_wpa_psk(sm->wpa_key_mgmt)) {
		 ...
		 */
		(void)os_memcpy(sm->PMK, pmk, PMK_LEN);
		sm->pmk_len = PMK_LEN;
	}
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING)
{
	 ...
	 */
	(void)os_memset(rsc, 0, WPA_KEY_RSC_LEN);
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
	(void)wpa_auth_get_seqnum(sm->wpa_auth, NULL, gsm->GN, rsc);
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	...
	pos = kde;
	(void)os_memcpy(pos, wpa_ie, wpa_ie_len);
	pos += wpa_ie_len;
#ifdef CONFIG_IEEE80211R_AP
	if (wpa_key_mgmt_ft(sm->wpa_key_mgmt)) {
		int res;
		...
		if (res < 0) {
			wpa_error_log0(MSG_ERROR, "FT: Failed to insert "
	...
	if (wpa_key_mgmt_ft(sm->wpa_key_mgmt)) {
		...
		if (res < 0) {
			wpa_error_log0(MSG_ERROR, "FT: Failed to insert FTIE "
			  ...
			  struct wpa_group *group)
{
	...
	(void)os_memcpy(group->GNonce, group->Counter, WPA_NONCE_LEN);
...
{
#ifndef LOS_CONFIG_NO_VLAN
	wpa_warning_log1(MSG_DEBUG, "WPA: group state machine entering state "
		   "GTK_INIT (VLAN-ID %d)", group->vlan_id);
#else
	wpa_error_log0(MSG_DEBUG, "WPA: group state machine entering state GTK_INIT");
#endif /* LOS_CONFIG_NO_VLAN */
	group->changed = FALSE; /* GInit is not cleared here; avoid loop */
	group->wpa_group_state = WPA_GROUP_GTK_INIT;

	/* GTK[0..N] = 0 */
	(void)os_memset(group->GTK, 0, sizeof(group->GTK));
	group->GN = 1;
	group->GM = 2;
#ifdef CONFIG_IEEE80211W
	group->GN_igtk = 4;
...
#endif /* CONFIG_IEEE80211W */
	/* GTK[GN] = CalcGTK() */
	(void)wpa_gtk_update(wpa_auth, group);
}
...
int wpa_wnmsleep_gtk_subelem(struct wpa_state_machine *sm, u8 *pos)
{
	...
	*pos++ = gsm->GTK_len;
	...
	wpa_warning_log1(MSG_DEBUG, "WNM: GTK Key ID %u in WNM-Sleep Mode exit",
		   gsm->GN);
...
int wpa_wnmsleep_igtk_subelem(struct wpa_state_machine *sm, u8 *pos)
{
	...
	*pos++ = 2 + 6 + len;
	...
	wpa_warning_log1(MSG_DEBUG, "WNM: IGTK Key ID %u in WNM-Sleep Mode exit",
		   gsm->GN_igtk);
			      ...
			      struct wpa_group *group)
{
...
#else
	wpa_error_log0(MSG_DEBUG, "WPA: group state machine entering state SETKEYS");
#endif
	group->changed = TRUE;
	...
	if (group->GKeyDoneStations) {
		wpa_error_log1(MSG_DEBUG, "wpa_group_setkeys: Unexpected "
			   "GKeyDoneStations=%d when starting new GTK rekey",
			   group->GKeyDoneStations);
		group->GKeyDoneStations = 0;
	}
	(void)wpa_auth_for_each_sta(wpa_auth, wpa_group_update_sta, group);
	wpa_error_log1(MSG_DEBUG, "wpa_group_setkeys: GKeyDoneStations=%d",
		   group->GKeyDoneStations);
}
...
{
	if (sm->group == ctx) {
		wpa_error_log4(MSG_DEBUG, "WPA: Mark STA " "%02x:xx:xx:%02x:%02x:%02x"
...
static void wpa_group_fatal_failure(struct wpa_authenticator *wpa_auth,
				    struct wpa_group *group)
{
	wpa_error_log0(MSG_DEBUG, "WPA: group state machine entering state FATAL_FAILURE");
	group->changed = TRUE;
	group->wpa_group_state = WPA_GROUP_FATAL_FAILURE;
	(void)wpa_auth_for_each_sta(wpa_auth, wpa_group_disconnect_cb, group);
}
...
#else
	wpa_error_log0(MSG_DEBUG, "WPA: group state machine entering state SETKEYSDONE");
#endif
	group->changed = TRUE;
			      ...
			      struct wpa_group *group)
{
	if (group->GInit) {
		...
		(void)wpa_group_setkeysdone(wpa_auth, group);
...
static int wpa_sm_step(struct wpa_state_machine *sm)
{
	...
	if (sm->in_step_loop) {
		 ...
		 * recursive call. */
		wpa_error_log0(MSG_ERROR, "WPA: wpa_sm_step() called recursively");
		return 0;
	}
	...
	if (sm->pending_deinit) {
		wpa_error_log4(MSG_DEBUG, "WPA: Completing pending STA state "
			   "machine deinit for " "%02x:xx:xx:%02x:%02x:%02x", (sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
		wpa_free_sta_sm(sm);
		return 1;
	}
	return 0;
...
void wpa_auth_sm_notify(struct wpa_state_machine *sm)
{
	if (sm == NULL)
		return;
	(void)eloop_register_timeout(0, 0, wpa_sm_call_step, sm, NULL);
}
...
#endif /* CONFIG_IEEE80211W */
		(void)wpa_gtk_update(wpa_auth, group);
		(void)wpa_group_config_group_keys(wpa_auth, group);
	}
}
...
int wpa_get_mib(struct wpa_authenticator *wpa_auth, char *buf, size_t buflen)
{
	...
	(void)wpa_snprintf_hex(pmkid_txt, sizeof(pmkid_txt),
			   ...
			   const u8 *sta_addr)
{
	...
	if (pmksa) {
		wpa_warning_log4(MSG_DEBUG, "WPA: Remove PMKSA cache entry for "
			   "%02x:xx:xx:%02x:%02x:%02x" " based on request", (sta_addr)[0], (sta_addr)[3], (sta_addr)[4], (sta_addr)[5]);
...
#ifndef LOS_CONFIG_NO_VLAN
	wpa_warning_log1(MSG_DEBUG, "WPA: Remove group state machine for VLAN-ID %d",
		   group->vlan_id);
#else
	wpa_error_log0(MSG_DEBUG, "WPA: Remove group state machine");
#endif
	while (prev) {
		if (prev->next == group) {
...
wpa_auth_add_group(struct wpa_authenticator *wpa_auth, int vlan_id)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WPA: Add group state machine for VLAN-ID %d",
		   vlan_id);
...
int wpa_auth_sta_set_vlan(struct wpa_state_machine *sm, int vlan_id)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WPA: Moving STA " "%02x:xx:xx:%02x:%02x:%02x",
						   (sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
	wpa_warning_log1(MSG_DEBUG, "WPA: Moving STA " "machine for VLAN ID %d", vlan_id);
				  ...
				  struct wpa_state_machine *sm, int ack)
{
	if (wpa_auth == NULL || sm == NULL)
		return;
	wpa_error_log4(MSG_DEBUG, "WPA: EAPOL-Key TX status for STA " "%02x:xx:xx:%02x:%02x:%02x",
					 (sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
	wpa_error_log1(MSG_DEBUG, "WPA: EAPOL-Key TX status for STA " " ack=%d", ack);
	if (sm->pending_1_of_4_timeout && ack) {
		...
		wpa_error_log1(MSG_DEBUG, "WPA: Increase initial EAPOL-Key 1/4 "
			   "timeout by %u ms because of acknowledged frame",
			   timeout_ms);
		(void)eloop_cancel_timeout(wpa_send_eapol_timeout, wpa_auth, sm);
		(void)eloop_register_timeout(timeout_ms / 1000,
