...
static int dfs_is_chan_allowed(struct hostapd_channel_data *chan, int n_chans)
{
	...
	case 8:
		...
		break;
	default:
		wpa_warning_log1(MSG_DEBUG, "Unknown width for %d channels", n_chans);
		break;
	}
			    ...
			    int idx, int skip_radar)
{
	...
	wpa_warning_log1(MSG_DEBUG, "DFS new chan checking %d channels", n_chans);
	for (i = 0; i < mode->num_channels; i++) {
		...
		if (ret_chan && idx == channel_idx) {
			wpa_warning_log1(MSG_DEBUG, "Selected ch. #%d", chan->chan);
			*ret_chan = chan;
			return idx;
		}
		wpa_warning_log1(MSG_DEBUG, "Adding channel: %d", chan->chan);
		channel_idx++;
	}
	return channel_idx;
}
	...
	*oper_centr_freq_seg1_idx = 0;
	...
	case CHANWIDTH_160MHZ:
		*oper_centr_freq_seg0_idx = chan->chan + 14;
		break;
	default:
		wpa_warning_log0(MSG_INFO, "DFS only VHT20/40/80/160 is supported now");
		*oper_centr_freq_seg0_idx = 0;
		break;
	}

	wpa_warning_log2(MSG_DEBUG, "DFS adjusting VHT center frequency: %d, %d",
	...
	*seg1_start = -1;
	...
	if (res == -1) {
		wpa_warning_log4(MSG_DEBUG,
			   "DFS chan_idx seems wrong; num-ch: %d ch-no: %d conf-ch-no: %d 11n: %d ",
			   mode->num_channels, channel_no, iface->conf->channel,
			   iface->conf->ieee80211n);
		wpa_warning_log2(MSG_DEBUG,
			   "DFS chan_idx seems wrong; sec-ch: %d vht-oper-width: %d",
			   iface->conf->secondary_channel,
			   hostapd_get_oper_chwidth(iface->conf));

		for (i = 0; i < mode->num_channels; i++) {
			wpa_warning_log1(MSG_DEBUG, "Available channel: %d",
		      ...
		      int skip_radar)
{
	...
	wpa_warning_log0(MSG_DEBUG, "DFS: Selecting random channel");
	*secondary_channel = 0;
	*oper_centr_freq_seg0_idx = 0;
	*oper_centr_freq_seg1_idx = 0;
...
static int set_dfs_state_freq(struct hostapd_iface *iface, int freq, u32 state)
{
	...
	wpa_warning_log2(MSG_DEBUG, "set_dfs_state 0x%X for %d MHz", state, freq);
	...
	wpa_warning_log1(MSG_WARNING, "Can't set DFS state for freq %d MHz", freq);
	return 0;
}
			 ...
			 int cf2, u32 state)
{
	...
	if (mode->mode != HOSTAPD_MODE_IEEE80211A) {
		wpa_warning_log0(MSG_WARNING, "current_mode != IEEE80211A");
		return 0;
	}
	...
	case CHAN_WIDTH_160:
		...
		break;
	default:
		wpa_warning_log1(MSG_INFO, "DFS chan_width %d not supported",
			   chan_width);
		break;
	}

	wpa_warning_log2(MSG_DEBUG, "DFS freq: %dMHz, n_chans: %d", frequency,
		   n_chans);
	for (i = 0; i < n_chans; i++) {
				       ...
				       int chan_width, int cf1, int cf2)
{
	...
	for (i = 0; i < n_chans; i++) {
		...
		for (j = 0; j < radar_n_chans; j++) {
			wpa_warning_log2(MSG_DEBUG, "checking our: %d, radar: %d",
	...
	wpa_warning_log1(MSG_DEBUG, "overlapped: %d", res);
...
int hostapd_handle_dfs(struct hostapd_iface *iface)
{
	...
	do {
		...
		if (res) {
			...
			channel = dfs_get_valid_channel(iface, &sec, &cf1, &cf2,
							skip_radar);
			if (!channel) {
				wpa_error_log0(MSG_ERROR, "could not get valid channel");
	...
	/* Finally start CAC */
	hostapd_set_state(iface, HAPD_IFACE_DFS);
	wpa_warning_log1(MSG_DEBUG, "DFS start CAC on %d MHz", iface->freq);
	...
	if (res) {
		wpa_error_log1(MSG_ERROR, "DFS start_dfs_cac() failed, %d", res);
		return -1;
	}
...
static int hostapd_dfs_start_channel_switch_cac(struct hostapd_iface *iface)
{
	...
	if (!channel) {
		wpa_error_log0(MSG_ERROR, "No valid channel available");
		return err;
	}

	wpa_warning_log1(MSG_DEBUG, "DFS will switch to a new channel %d",
		   channel->chan);
...
static int hostapd_dfs_start_channel_switch(struct hostapd_iface *iface)
{
	...
	if (!channel) {
		...
		channel = dfs_get_valid_channel(iface, &secondary_channel,
						...
						skip_radar);
		if (!channel) {
			wpa_warning_log0(MSG_INFO, "no DFS channels left, waiting for NOP to finish");
			return err;
		}
...
err = hostapd_set_freq_params(&csa_settings.freq_params,
...
#endif /* LOS_WPA_PATCH */
	if (err) {
		wpa_error_log0(MSG_ERROR, "DFS failed to calculate CSA freq params");
		hostapd_disable_iface(iface);
		return err;
	}
	...
	if (err) {
		wpa_warning_log1(MSG_WARNING, "DFS failed to schedule CSA (%d) - trying fallback",
			   err);
		iface->freq = channel->freq;
		iface->conf->channel = channel->chan;
		iface->conf->secondary_channel = secondary_channel;
	...
	wpa_warning_log0(MSG_DEBUG, "DFS waiting channel switch event");
	return 0;
}
...
{
	wpa_warning_log1(MSG_DEBUG, "iface->cac_started: %d", iface->cac_started);
	...
	if (ieee80211_is_dfs(iface->freq, iface->hw_features,
		...
		wpa_warning_log1(MSG_DEBUG, "freq %d MHz requires DFS", iface->freq);
		return 0;
	}

	wpa_warning_log1(MSG_DEBUG, "freq %d MHz does not require DFS. Continue channel/AP setup", iface->freq);
	return 2;
}
