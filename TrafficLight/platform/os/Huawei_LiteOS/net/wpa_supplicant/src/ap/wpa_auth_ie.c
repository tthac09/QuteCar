...
static int wpa_write_wpa_ie(struct wpa_auth_config *conf, u8 *buf, size_t len)
{
	...
	if (suite == 0) {
		wpa_warning_log1(MSG_DEBUG, "Invalid group cipher (%d).",
			   conf->wpa_group);
		return -1;
	}
	...
	if (num_suites == 0) {
		wpa_warning_log1(MSG_DEBUG, "Invalid pairwise cipher (%d).",
			   conf->wpa_pairwise);
		return -1;
	}
	...
	if (num_suites == 0) {
		wpa_warning_log1(MSG_DEBUG, "Invalid key management type (%d).",
			   conf->wpa_key_mgmt);
		return -1;
	}
	WPA_PUT_LE16(count, num_suites);
...
#endif /* LOS_CONFIG_80211_IES_CROP */
{
	...
	if (!(wpa_auth->conf.wpa & version)) {
		wpa_warning_log1(MSG_DEBUG, "Invalid WPA proto (%d) from ", version);
	...
	if (res) {
		wpa_warning_log4(MSG_DEBUG, "Failed to parse WPA/RSN IE from "
			   "%02x:xx:xx:%02x:%02x:%02x", (sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
		wpa_warning_log1(MSG_DEBUG, "Failed to parse WPA/RSN IE from "" (res=%d)", res);
		wpa_hexdump(MSG_DEBUG, "WPA/RSN IE", wpa_ie, wpa_ie_len);
		return WPA_INVALID_IE;
	}

	if (data.group_cipher != wpa_auth->conf.wpa_group) {
		wpa_warning_log1(MSG_DEBUG, "Invalid WPA group cipher (0x%x) from ", data.group_cipher);
		wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x", (sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
		return WPA_INVALID_GROUP;
	}

	key_mgmt = data.key_mgmt & wpa_auth->conf.wpa_key_mgmt;
	if (!key_mgmt) {
		wpa_warning_log1(MSG_DEBUG, "Invalid WPA key mgmt (0x%x) from ", data.key_mgmt);
		wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x", (sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
	...
	if (!ciphers) {
		wpa_warning_buf(MSG_DEBUG, "Invalid %s pairwise",
			   version == WPA_PROTO_RSN ? "RSN" : "WPA",
			   version == WPA_PROTO_RSN ? strlen("RSN") : strlen("WPA"));
		wpa_warning_log1(MSG_DEBUG, "cipher (0x%x) ", data.pairwise_cipher);
	...
	if (wpa_auth->conf.ieee80211w == MGMT_FRAME_PROTECTION_REQUIRED) {
		if (!(data.capabilities & WPA_CAPABILITY_MFPC)) {
			wpa_warning_log0(MSG_DEBUG, "Management frame protection "
		...
		{
			wpa_warning_log1(MSG_DEBUG, "Unsupported management group "
	...
	if (wpa_key_mgmt_ft(sm->wpa_key_mgmt)) {
		if (mdie == NULL || mdie_len < MOBILITY_DOMAIN_ID_LEN + 1) {
			wpa_warning_log0(MSG_DEBUG, "RSN: Trying to use FT, but "
				   "MDIE not included");
...
int wpa_parse_kde_ies(const u8 *buf, size_t len, struct wpa_eapol_ie_parse *ie)
{
	...
	for (pos = buf, end = pos + len; end - pos > 1; pos += 2 + pos[1]) {
		...
		if (2 + pos[1] > end - pos) {
			wpa_warning_log3(MSG_DEBUG, "WPA: EAPOL-Key Key Data "
