...
int hostapd_eid_wmm_valid(struct hostapd_data *hapd, const u8 *eid, size_t len)
{
	...
	if (len < sizeof(struct wmm_information_element)) {
		wpa_warning_log1(MSG_DEBUG, "Too short WMM IE (len=%lu)",
			   (unsigned long) len);
		return 0;
	}

	wmm = (struct wmm_information_element *) eid;
	wpa_warning_log3(MSG_DEBUG, "Validating WMM IE: OUI %02x:%02x:%02x  ",
		   wmm->oui[0], wmm->oui[1], wmm->oui[2]);
	wpa_warning_log4(MSG_DEBUG, "OUI type %d  OUI sub-type %d  version %d  QoS info 0x%x",
		   wmm->oui_type, wmm->oui_subtype, wmm->version, wmm->qos_info);
	if (wmm->oui_subtype != WMM_OUI_SUBTYPE_INFORMATION_ELEMENT ||
	    wmm->version != WMM_VERSION) {
		wpa_warning_log0(MSG_DEBUG, "Unsupported WMM IE Subtype/Version");
		return 0;
	}
			    ...
			    u8 action_code, u8 dialogue_token, u8 status_code)
{
	...
	if (hostapd_drv_send_mlme(hapd, m, len, 0) < 0)
		wpa_warning_log0(MSG_INFO, "wmm_send_action: send failed");
}
...
int wmm_process_tspec(struct wmm_tspec_element *tspec)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WMM: TS Info: UP=%d PSB=%d Direction=%d TID=%d",
		   up, psb, dir, tid);
	val = le_to_host16(tspec->nominal_msdu_size);
	wpa_warning_log1(MSG_DEBUG, "WMM: Nominal MSDU Size: %d",
		   val & 0x7fff);
	wpa_warning_buf(MSG_DEBUG, "WMM: Nominal MSDU Size: %s",
		   val & 0x8000 ? " (fixed)" : "", val & 0x8000 ? strlen(" (fixed)") : strlen(""));
	wpa_warning_log1(MSG_DEBUG, "WMM: Mean Data Rate: %u bps",
		   le_to_host32(tspec->mean_data_rate));
	wpa_warning_log1(MSG_DEBUG, "WMM: Minimum PHY Rate: %u bps",
		   le_to_host32(tspec->minimum_phy_rate));
	val = le_to_host16(tspec->surplus_bandwidth_allowance);
	wpa_warning_log2(MSG_DEBUG, "WMM: Surplus Bandwidth Allowance: %u.%04u",
		   val >> 13, 10000 * (val & 0x1fff) / 0x2000);

	val = le_to_host16(tspec->nominal_msdu_size);
	if (val == 0) {
		wpa_warning_log0(MSG_DEBUG, "WMM: Invalid Nominal MSDU Size (0)");
	...
	wpa_warning_log1(MSG_DEBUG, "WMM: Packets-per-second estimate for TSPEC: %d",
		   pps);

	if (le_to_host32(tspec->minimum_phy_rate) < 1000000) {
		wpa_warning_log0(MSG_DEBUG, "WMM: Too small Minimum PHY Rate");
		return WMM_ADDTS_STATUS_INVALID_PARAMETERS;
	}
	...
	if (surplus <= 0x2000) {
		wpa_warning_log0(MSG_DEBUG, "WMM: Surplus Bandwidth Allowance not "
			   "greater than unity");
		return WMM_ADDTS_STATUS_INVALID_PARAMETERS;
	}

	medium_time = (u64) surplus * pps * duration / 0x2000;
	wpa_warning_log1(MSG_DEBUG, "WMM: Estimated medium time: %lu",
	...
	if (medium_time > 750000) {
		wpa_warning_log0(MSG_DEBUG, "WMM: Refuse TSPEC request for over "
			  ...
			  struct wmm_tspec_element *tspec, size_t len)
{
	...
	if ((const u8 *) (tspec + 1) > end) {
		wpa_warning_log0(MSG_DEBUG, "WMM: TSPEC overflow in ADDTS Request");
		return;
	}

	wpa_warning_log1(MSG_DEBUG, "WMM: ADDTS Request (Dialog Token %d) for TSPEC ",
		   mgmt->u.action.u.wmm_action.dialog_token);
	wpa_warning_log4(MSG_DEBUG, "from " "%02x:xx:xx:%02x:%02x:%02x",
		   mgmt->u.action.u.wmm_action.dialog_token,
		   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
	res = wmm_process_tspec(tspec);
	wpa_warning_log1(MSG_DEBUG, "WMM: ADDTS processing result: %d", res);
