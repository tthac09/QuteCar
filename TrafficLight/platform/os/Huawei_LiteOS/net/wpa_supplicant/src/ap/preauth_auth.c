				...
				const u8 *buf, size_t len)
{
	...
	wpa_warning_log1(MSG_DEBUG, "RSN: receive pre-auth packet "
		   "from interface '%s'", piface->ifname);
	if (len < sizeof(*ethhdr) + sizeof(*hdr)) {
		wpa_warning_log1(MSG_DEBUG, "RSN: too short pre-auth packet "
	...
	if (os_memcmp(ethhdr->h_dest, hapd->own_addr, ETH_ALEN) != 0) {
		wpa_warning_log4(MSG_DEBUG, "RSN: pre-auth for foreign address %02x:xx:xx:%02x:%02x:%02x",
				(ethhdr->h_dest)[0], (ethhdr->h_dest)[3], (ethhdr->h_dest)[4], (ethhdr->h_dest)[5]));
		return;
	}

	sta = ap_get_sta(hapd, ethhdr->h_source);
	if (sta && (sta->flags & WLAN_STA_ASSOC)) {
		wpa_warning_log4(MSG_DEBUG, "RSN: pre-auth for already association %02x:xx:xx:%02x:%02x:%02x",
...
static int rsn_preauth_iface_add(struct hostapd_data *hapd, const char *ifname)
{
	...
	wpa_warning_log1(MSG_DEBUG, "RSN pre-auth interface '%s'", ifname);
	...
	if (piface->l2 == NULL) {
		wpa_error_log0(MSG_ERROR, "Failed to open register layer 2 access "
			   "to ETH_P_PREAUTH");
		goto fail2;
	}
...
static void rsn_preauth_finished_cb(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_warning_log4(MSG_DEBUG, "RSN: Removing pre-authentication STA entry for %02x:xx:xx:%02x:%02x:%02x",
			(sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]));
	ap_free_sta(hapd, sta);
}
		      ...
		      u8 *buf, size_t len)
{
	...
	if (piface == NULL) {
		wpa_warning_log4(MSG_DEBUG, "RSN: Could not find pre-authentication interface for"
	...
	if (l2_packet_send(piface->l2, sta->addr, ETH_P_PREAUTH, (u8 *) ethhdr,
			   sizeof(*ethhdr) + len) < 0) {
		wpa_warning_log0(MSG_ERROR, "Failed to send preauth packet using "
			   "l2_packet_send\n");
	}
	os_free(ethhdr);
}
