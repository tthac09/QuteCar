...
{
	struct hostapd_data *hapd = ctx;
	wpa_warning_log4(MSG_DEBUG, "hostapd_wpa_auth_disconnect: WPA authenticator requests disconnect: "
		   "STA " "%02x:xx:xx:%02x:%02x:%02x",
			addr[0], addr[3], addr[4], addr[5]);
	wpa_warning_log1(MSG_DEBUG, "hostapd_wpa_auth_disconnect: WPA authenticator requests disconnect: " " reason %d",
		   reason);
	...
	if (sta && sta->psk && !psk) {
		...
		for (pos = sta->psk; pos; pos = pos->next) {
			if (pos->is_passphrase) {
				(void)pbkdf2_sha1(pos->passphrase,
					    hapd->conf->ssid.ssid,
				    ...
				    size_t *len)
{
	...
	if (sta == NULL) {
		wpa_warning_log0(MSG_DEBUG, "AUTH_GET_MSK: Cannot find STA");
		return -1;
	}

	key = ieee802_1x_get_key(sta->eapol_sm, &keylen);
	if (key == NULL) {
		wpa_warning_log1(MSG_DEBUG, "AUTH_GET_MSK: Key is null, eapol_sm: %p",
			   sta->eapol_sm);
		return -1;
	}
	...
	void *cb_ctx)
{
	...
	for (sta = hapd->sta_list; sta; sta = sta->next) {
		...
		/* stas with MGTK process in other way */
		if (sta->plink_state >= PLINK_IDLE) {
			wpa_error_log0(MSG_DEBUG, "mesh gtk rekey: stas with MGTK process in other way");
			continue;
		}
...
int hostapd_setup_wpa(struct hostapd_data *hapd)
{
	...
	if (hapd->wpa_auth == NULL) {
		wpa_error_log0(MSG_ERROR, "WPA initialization failed.");
		return -1;
	}

#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
	if (hostapd_set_privacy(hapd, 1)) {
		wpa_error_buf(MSG_ERROR, "Could not set PrivacyInvoked "
			   "for interface %s", hapd->conf->iface, strlen(hapd->conf->iface));
		return -1;
	}
	...
	if (rsn_preauth_iface_init(hapd)) {
		wpa_error_log0(MSG_ERROR, "Initialization of RSN "
	...
	if (!hostapd_drv_none(hapd) &&
		...
		if (hapd->l2 == NULL &&
			...
			wpa_error_log0(MSG_ERROR, "Failed to open l2_packet "
				   "interface");
			return -1;
		}
...
void hostapd_reconfig_wpa(struct hostapd_data *hapd)
{
	...
	(void)wpa_reconfig(hapd->wpa_auth, &wpa_auth_conf);
}
...
#ifdef CONFIG_MESH
	if (hapd->wpa_mesh_auth != NULL) {
		wpa_warning_log0(MSG_DEBUG, "wpa_mesh_auth deinit");
		wpa_deinit(hapd->wpa_mesh_auth);
		hapd->wpa_mesh_auth = NULL;
	}
#endif /* CONFIG_MESH */
	if (hapd->wpa_auth) {
		...
		if (hapd->drv_priv && hostapd_set_privacy(hapd, 0)) {
			wpa_warning_buf(MSG_DEBUG, "Could not disable "
		...
		if (hapd->drv_priv &&
			...
			wpa_warning_buf(MSG_DEBUG, "Could not remove generic "
