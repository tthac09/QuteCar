...
static void hostapd_reload_bss(struct hostapd_data *hapd)
{
	...
	if (hostapd_setup_wpa_psk(hapd->conf)) {
		wpa_error_log0(MSG_ERROR, "Failed to re-configure WPA PSK "
			   "after reloading configuration");
	}
#ifndef LOS_CONFIG_HOSTAPD_SECURITY
	if (hapd->conf->ieee802_1x || hapd->conf->wpa)
		(void)hostapd_set_drv_ieee8021x(hapd, hapd->conf->iface, 1);
	else
		(void)hostapd_set_drv_ieee8021x(hapd, hapd->conf->iface, 0);
#endif
#ifndef HISI_CODE_CROP
	...
	if ((hapd->conf->wpa) && hapd->wpa_auth == NULL) {
#endif /* HISI_CODE_CROP */
		(void)hostapd_setup_wpa(hapd);
		if (hapd->wpa_auth)
			(void)wpa_init_keys(hapd->wpa_auth);
		...
		if (hostapd_set_generic_elem(hapd, wpa_ie, wpa_ie_len))
			wpa_error_log0(MSG_ERROR, "Failed to configure WPA IE for "
				   "the kernel driver.");
#else
		hostapd_reconfig_wpa(hapd);
		...
		hapd->wpa_auth = NULL;
#ifndef LOS_CONFIG_HOSTAPD_SECURITY
		(void)hostapd_set_privacy(hapd, 0);
		(void)hostapd_setup_encryption(hapd->conf->iface, hapd);
		(void)hostapd_set_generic_elem(hapd, (u8 *) "", 0);
#endif
	}

	(void)ieee802_11_set_beacon(hapd);
#ifdef CONFIG_WPS_AP
	...
	if (hapd->conf->ssid.ssid_set &&
		...
		wpa_error_log0(MSG_ERROR, "Could not set SSID for kernel driver");
		/* try to continue */
	}
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	wpa_warning_buf(MSG_DEBUG, "Reconfigured interface %s", hapd->conf->iface, strlen(hapd->conf->iface));
}
...
static void hostapd_clear_old(struct hostapd_iface *iface)
{
	...
	for (j = 0; j < iface->num_bss; j++) {
		(void)hostapd_flush_old_stations(iface->bss[j],
					   WLAN_REASON_PREV_AUTH_NOT_VALID);
		(void)hostapd_broadcast_wep_clear(iface->bss[j]);
					      ...
					      const char *ifname)
{
	...
	for (i = 0; i < NUM_WEP_KEYS; i++) {
		if (hostapd_drv_set_key(ifname, hapd, WPA_ALG_NONE, NULL, i,
					0, NULL, 0, NULL, 0)) {
			wpa_warning_log1(MSG_DEBUG, "Failed to clear default "
				   "encryption keys (keyidx=%d)", i);
			wpa_warning_buf(MSG_DEBUG, "Failed to clear default "
				   "encryption keys (ifname=%s)",
	...
	if (hapd->conf->ieee80211w) {
		for (i = NUM_WEP_KEYS; i < NUM_WEP_KEYS + 2; i++) {
			if (hostapd_drv_set_key(ifname, hapd, WPA_ALG_NONE,
						NULL, i, 0, NULL,
						0, NULL, 0)) {
				wpa_warning_log1(MSG_DEBUG, "Failed to clear "
					   "default mgmt encryption keys "
					   "keyidx=%d)", i);
				wpa_warning_buf(MSG_DEBUG, "Failed to clear "
					   "default mgmt encryption keys "
					   "(ifname=%s)", ifname, strlen(ifname));
			}
		}
	}
...
static int hostapd_broadcast_wep_set(struct hostapd_data *hapd)
{
	...
	if (ssid->wep.default_len &&
	    hostapd_drv_set_key(hapd->conf->iface,
				...
				ssid->wep.len[idx])) {
		wpa_warning_log0(MSG_WARNING, "Could not set WEP encryption.");
		errors++;
	}
...
static void hostapd_free_hapd_data(struct hostapd_data *hapd)
{
	...
	if (!hapd->started) {
		wpa_error_buf(MSG_ERROR, "hostapd_free_hapd_data: Interface %s wasn't started",
	...
	wpa_warning_buf(MSG_DEBUG, "hostapd_free_hapd_data(%s)", hapd->conf->iface, strlen(hapd->conf->iface));
#ifndef HISI_CODE_CROP
...
#endif /* HISI_CODE_CROP */
#if (defined(LOS_CONFIG_HOSTAPD_MGMT) && defined(CONFIG_SAE))
	{
...
#endif /* LOS_CONFIG_HOSTAPD_MGMT && CONFIG_SAE */
}
...
{
	wpa_warning_buf(MSG_DEBUG, "hostapd_cleanup((%s))", hapd->conf->iface, strlen(hapd->conf->iface));
...
{
#ifndef LOS_HOSTAPD_CONFIG_CROP
	wpa_warning_log1(MSG_DEBUG, "hostapd_cleanup_iface_partial(%p)", iface);
...
{
	wpa_warning_log0(MSG_DEBUG, "hostapd_cleanup_iface");
#ifndef HISI_CODE_CROP
	(void)eloop_cancel_timeout(channel_list_update_timeout, iface, NULL);
#endif /* HISI_CODE_CROP */
#ifdef CONFIG_P2P
...
#else
	if (hapd->drv_priv && hapd->conf) {
#endif /* HISI_CODE_CROP */
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
		(void)hostapd_set_privacy(hapd, 0);
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
		(void)hostapd_broadcast_wep_clear(hapd);
	}
}
#ifndef LOS_CONFIG_HOSTAPD_SECURITY
...
static int hostapd_setup_encryption(char *iface, struct hostapd_data *hapd)
{
	...
	(void)hostapd_broadcast_wep_set(hapd);

	if (hapd->conf->ssid.wep.default_len) {
		(void)hostapd_set_privacy(hapd, 1);
		return 0;
	}
	 ...
	 */
	(void)hostapd_drv_set_authmode(hapd, hapd->conf->auth_algs);
	...
	for (i = 0; i < 4; i++) {
		if (hapd->conf->ssid.wep.key[i] &&
			...
			wpa_warning_log0(MSG_WARNING, "Could not set WEP "
				   "encryption.");
			return -1;
		}
		if (hapd->conf->ssid.wep.key[i] &&
...
static int hostapd_flush_old_stations(struct hostapd_data *hapd, u16 reason)
{
	...
	if (hapd->conf) {
#endif /* HISI_CODE_CROP */
	wpa_warning_log0(MSG_DEBUG, "Deauthenticate all stations");
	(void)os_memset(addr, 0xff, ETH_ALEN);
#ifndef HISI_CODE_CROP
...
{
	hostapd_free_stas(hapd);
	(void)hostapd_flush_old_stations(hapd, WLAN_REASON_DEAUTH_LEAVING);
...
static int hostapd_validate_bssid_configuration(struct hostapd_iface *iface)
{
	...
	if (bits > 40) {
		wpa_error_log1(MSG_ERROR, "Too many bits in the BSSID mask (%u)",
			   bits);
		return -1;
	}

	(void)os_memset(mask, 0xff, ETH_ALEN);
	j = bits / 8;
	for (i = 5; i > 5 - j; i--)
		mask[i] = 0;
	j = bits % 8;
...
skip_mask_ext:
	wpa_warning_log2(MSG_DEBUG, "BSS count %lu, BSSID mask " " (%d bits)",
		   (unsigned long) iface->conf->num_bss, bits);
	wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x",
			mask[0], mask[3], mask[4], mask[5]);
	if (!auto_addr)
		return 0;

	for (i = 0; i < ETH_ALEN; i++) {
		if ((hapd->own_addr[i] & mask[i]) != hapd->own_addr[i]) {
			wpa_warning_log4(MSG_ERROR, "Invalid BSSID mask " "%02x:xx:xx:%02x:%02x:%02x",
				   mask[0], mask[3], mask[4], mask[5]);
			wpa_warning_log4(MSG_ERROR, " for start address " "%02x:xx:xx:%02x:%02x:%02x" ".",
				   (hapd->own_addr)[0], (hapd->own_addr)[3], (hapd->own_addr)[4], (hapd->own_addr)[5]);
			wpa_error_log0(MSG_ERROR, "Start address must be the "
				    ...
				    struct radius_das_attrs *attr)
{
	if (attr->nas_identifier &&
		...
		wpa_warning_log0(MSG_DEBUG, "RADIUS DAS: NAS-Identifier mismatch");
		return 1;
	}
	...
	if (attr->nas_ip_addr &&
	    (hapd->conf->own_ip_addr.af != AF_INET ||
	     ...
	     0)) {
		wpa_warning_log0(MSG_DEBUG, "RADIUS DAS: NAS-IP-Address mismatch");
		return 1;
	}
	...
	if (attr->nas_ipv6_addr &&
	    (hapd->conf->own_ip_addr.af != AF_INET6 ||
	     ...
	     != 0)) {
		wpa_warning_log0(MSG_DEBUG, "RADIUS DAS: NAS-IPv6-Address mismatch");
		return 1;
	}
#endif /* CONFIG_IPV6 */
					      ...
					      int *multi)
{
	...
	*multi = 0;
	...
	if (attr->sta_addr) {
		...
		wpa_warning_log0(MSG_DEBUG, "RADIUS DAS: Calling-Station-Id match");
	}
	...
	if (attr->acct_session_id) {
		...
		wpa_warning_log0(MSG_DEBUG, "RADIUS DAS: Acct-Session-Id match");
	}
...
hostapd_das_disconnect(void *ctx, struct radius_das_attrs *attr)
{
	...
	if (sta == NULL) {
		...
		wpa_warning_log0(MSG_DEBUG, "RADIUS DAS: No matching session found");
		return RADIUS_DAS_SESSION_NOT_FOUND;
	}

	wpa_warning_log4(MSG_DEBUG, "RADIUS DAS: Found a matching session %02x:xx:xx:%02x:%02x:%02x"
		   " - disconnecting", (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
...
hostapd_das_coa(void *ctx, struct radius_das_attrs *attr)
{
	...
	wpa_warning_log4(MSG_DEBUG, "RADIUS DAS: Found a matching session %02x:xx:xx:%02x:%02x:%02x"
		   " - CoA", (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	wpa_warning_buf(MSG_DEBUG, "hostapd_setup_bss((%s)",
		   conf->iface, strlen(conf->iface));

#ifdef EAP_SERVER_TNC
	if (conf->tnc && tncs_global_init() < 0) {
		wpa_error_log0(MSG_ERROR, "Failed to initialize TNCS");
		return -1;
	}
#endif /* EAP_SERVER_TNC */

	if (hapd->started) {
		wpa_error_buf(MSG_ERROR, "hostapd_setup_bss: Interface %s was already started",
			   conf->iface, strlen(conf->iface));
		return -1;
	}
	hapd->started = 1;
	...
	if (!first || first == -1) {
		...
		if (!is_zero_ether_addr(conf->bssid)) {
			...
			(void)os_memcpy(hapd->own_addr, conf->bssid, ETH_ALEN);

			if (hostapd_mac_comp(hapd->own_addr,
					     hapd->iface->bss[0]->own_addr) ==
			    0) {
				wpa_error_buf(MSG_ERROR, "BSS '%s' may not have "
					   "BSSID set to the MAC address of "
					   "the radio", conf->iface, strlen(conf->iface));
				return -1;
			}
		...
		if (hostapd_if_add(hapd->iface->bss[0], WPA_IF_AP_BSS,
				   ...
				   first == -1)) {
			wpa_error_log4(MSG_ERROR, "Failed to add BSS (BSSID="
				   "%02x:xx:xx:%02x:%02x:%02x" ")", (hapd->own_addr)[0], (hapd->own_addr)[3], (hapd->own_addr)[4], (hapd->own_addr)[5]);
			hapd->interface_added = 0;
			return -1;
		}
	...
	if (flush_old_stations)
		(void)hostapd_flush_old_stations(hapd,
					   WLAN_REASON_PREV_AUTH_NOT_VALID);
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
	(void)hostapd_set_privacy(hapd, 0);
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	...
	if (ssid_len < 0) {
		wpa_error_log0(MSG_ERROR, "Could not read SSID from system");
		return -1;
	}
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	/* ssid has been set in hostapd_config_read2() */
#ifndef HISI_CODE_CROP
	if (conf->ssid.ssid_set) {
		...
		(void)os_memcpy(conf->ssid.ssid, ssid, conf->ssid.ssid_len);
	}
#endif /* HISI_CODE_CROP */
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
	if (!hostapd_drv_none(hapd)) {
		wpa_error_log4(MSG_ERROR, "%02x:xx:xx:%02x:%02x:%02x",
			   (hapd->own_addr)[0], (hapd->own_addr)[3], (hapd->own_addr)[4], (hapd->own_addr)[5]);
		wpa_error_two_buf(MSG_ERROR, "Using interface %s with hwaddr "
			   " and ssid \"%s\"", conf->iface, strlen(conf->iface),
			   wpa_ssid_txt(conf->ssid.ssid, conf->ssid.ssid_len), strlen(wpa_ssid_txt(conf->ssid.ssid, conf->ssid.ssid_len)));
	}
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	if (hostapd_setup_wpa_psk(conf)) {
		wpa_error_log0(MSG_ERROR, "WPA-PSK setup failed.");
		return -1;
	}
	...
	if (hapd->radius == NULL) {
		wpa_error_log0(MSG_ERROR, "RADIUS client initialization failed.");
		return -1;
	}
	...
	if (conf->radius_das_port) {
		...
		if (hapd->radius_das == NULL) {
			wpa_error_log0(MSG_ERROR, "RADIUS DAS initialization "
				   "failed.");
			return -1;
		}
	}
	...
	if (hostapd_acl_init(hapd)) {
		wpa_error_log0(MSG_ERROR, "ACL initialization failed.");
		return -1;
	}
#endif /* HISI_CODE_CROP */
#ifdef CONFIG_WPS_AP
	...
	if (ieee802_1x_init(hapd)) {
		wpa_error_log0(MSG_ERROR, "IEEE 802.1X initialization failed.");
		return -1;
	}
#ifndef HISI_CODE_CROP
	...
	if (accounting_init(hapd)) {
		wpa_error_log0(MSG_ERROR, "Accounting initialization failed.");
		return -1;
	}
	...
	if (conf->ieee802_11f &&
		...
		wpa_error_log0(MSG_ERROR, "IEEE 802.11F (IAPP) initialization "
			   "failed.");
		return -1;
	}
	...
	if (gas_serv_init(hapd)) {
		wpa_error_log0(MSG_ERROR, "GAS server initialization failed");
		return -1;
	}
	...
	if (conf->qos_map_set_len &&
		...
		wpa_error_log0(MSG_ERROR, "Failed to initialize QoS Map");
		return -1;
	}
#endif /* CONFIG_INTERWORKING */

	if (conf->bss_load_update_period && bss_load_update_init(hapd)) {
		wpa_error_log0(MSG_ERROR, "BSS Load initialization failed");
		return -1;
	}
	...
	if (!hostapd_drv_none(hapd) && vlan_init(hapd)) {
		wpa_error_log0(MSG_ERROR, "VLAN initialization failed.");
		return -1;
	}
#endif
#ifndef HISI_CODE_CROP
...
static void hostapd_tx_queue_params(struct hostapd_iface *iface)
{
	...
	for (i = 0; i < NUM_TX_QUEUES; i++) {
		...
		if (hostapd_set_tx_queue_params(hapd, i, p->aifs, p->cwmin,
						p->cwmax, p->burst)) {
			wpa_warning_log1(MSG_DEBUG, "Failed to set TX queue "
				...
				int n_entries, u8 accept_acl)
{
	...
	for (i = 0; i < n_entries; i++)
		(void)os_memcpy(acl_params->mac_acl[i].addr, mac_acl[i].addr,
			  ETH_ALEN);
...
static void hostapd_set_acl(struct hostapd_data *hapd)
{
	...
	if (conf->bss[0]->macaddr_acl == DENY_UNLESS_ACCEPTED) {
		...
		err = hostapd_set_acl_list(hapd, conf->bss[0]->accept_mac,
					   ...
					   accept_acl);
		if (err) {
			wpa_warning_log0(MSG_DEBUG, "Failed to set accept acl");
			return;
		}
		...
		err = hostapd_set_acl_list(hapd, conf->bss[0]->deny_mac,
					   ...
					   accept_acl);
		if (err) {
			wpa_warning_log0(MSG_DEBUG, "Failed to set deny acl");
			return;
		}
	}
}
...
static void channel_list_update_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (!iface->wait_channel_update) {
		wpa_warning_log0(MSG_INFO, "Channel list update timeout, but interface was not waiting for it");
		return;
	}
	 ...
	 * to proceed.
	 */
	wpa_warning_log0(MSG_DEBUG, "Channel list update timeout - try to continue anyway");
	(void)setup_interface2(iface);
}
...
void hostapd_channel_list_updated(struct hostapd_iface *iface, int initiator)
{
	...
	wpa_warning_log0(MSG_DEBUG, "Channel list updated - continue setup");
	(void)eloop_cancel_timeout(channel_list_update_timeout, iface, NULL);
	(void)setup_interface2(iface);
}
#endif /* HISI_CODE_CROP */
...
static int setup_interface(struct hostapd_iface *iface)
{
	...
	if (!iface->phy[0]) {
		...
		if (phy) {
			wpa_warning_buf(MSG_DEBUG, "phy: %s", phy, strlen(phy));
			(void)os_strlcpy(iface->phy, phy, sizeof(iface->phy));
		}
	}
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	...
	if (hapd->iconf->country[0] && hapd->iconf->country[1]) {
		...
		(void)os_memcpy(country, hapd->iconf->country, 3);
		country[3] = '\0';
		if (hostapd_set_country(hapd, country) < 0) {
			wpa_error_log0(MSG_ERROR, "Failed to set country code");
			return -1;
		}

		wpa_warning_two_buf(MSG_DEBUG, "Previous country code %s, new country code %s",
			   previous_country, strlen(previous_country), country, strlen(country));

		if (os_strncmp(previous_country, country, 2) != 0) {
			wpa_warning_log0(MSG_DEBUG, "Continue interface setup after channel list update");
			iface->wait_channel_update = 1;
			(void)eloop_register_timeout(5, 0,
...
static int setup_interface2(struct hostapd_iface *iface)
{
	...
	if (hostapd_get_hw_features(iface)) {
		...
		if (ret < 0) {
			wpa_error_log1(MSG_ERROR, "Could not select hw_mode and "
				   "channel. (%d)", ret);
			goto fail;
		}
		if (ret == 1) {
			wpa_warning_log0(MSG_DEBUG, "Interface initialization will be completed in a callback (ACS)");
			return 0;
		}
		...
		if (ret < 0)
			goto fail;
		if (ret == 1) {
			wpa_warning_log0(MSG_DEBUG, "Interface initialization will "
		...
		if (iface->conf->ieee80211h) {
			wpa_warning_log0(MSG_DEBUG, "DFS support is enabled");
		}
#endif /* HISI_CODE_CROP */
	}
...
fail:
	...
	if (iface->interfaces && iface->interfaces->terminate_on_error) {
		...
		eloop_terminate();
#else
		wpa_error_log1(MSG_ERROR, "eloop_terminate : %d", ELOOP_TASK_HOSTAPD);
		eloop_terminate(ELOOP_TASK_HOSTAPD);
#endif /* LOS_WPA_PATCH */
	}
	return -1;
}
...
static void fst_hostapd_set_ies_cb(void *ctx, const struct wpabuf *fst_ies)
{
	...
	if (hapd->iface->fst_ies != fst_ies) {
		...
		if (ieee802_11_set_beacon(hapd))
			wpa_warning_log0(MSG_WARNING, "FST: Cannot set beacon");
	}
}
...
static int hostapd_owe_iface_iter(struct hostapd_iface *iface, void *ctx)
{
	...
	for (i = 0; i < iface->num_bss; i++) {
		...
		wpa_warning_two_buf(MSG_DEBUG,
			   "OWE: ifname=%s found transition mode ifname=%s",
			   hapd->conf->iface, strlen(hapd->conf->iface),
			   bss->conf->iface, strlen(bss->conf->iface));
		wpa_warning_log4(MSG_DEBUG,
			   "OWE: found transition mode BSSID %02x:xx:xx:%02x:%02x:%02x",
				(bss->own_addr)[0], (bss->own_addr)[3], (bss->own_addr)[4], (bss->own_addr)[5]);
		wpa_warning_buf(MSG_DEBUG,
			   "OWE: found transition mode SSID",
			   wpa_ssid_txt(bss->conf->ssid.ssid, bss->conf->ssid.ssid_len),
			   strlen(wpa_ssid_txt(bss->conf->ssid.ssid, bss->conf->ssid.ssid_len)));
						 ...
						 int err)
{
	...
	wpa_warning_log0(MSG_DEBUG, "Completing interface initialization");
	if (iface->conf->channel) {

		iface->freq = hostapd_hw_get_freq(hapd, iface->conf->channel);
		wpa_warning_log2(MSG_DEBUG, "Channel: %d  "
			   "Frequency: %d MHz",
			   iface->conf->channel, iface->freq);
		wpa_warning_buf(MSG_DEBUG, "Mode: %s",
			   hostapd_hw_mode_txt(iface->conf->hw_mode),
			   strlen(hostapd_hw_mode_txt(iface->conf->hw_mode)));
#ifndef HISI_CODE_CROP
#ifdef NEED_AP_MLME
		...
		if (iface->mconf != NULL) {
			wpa_warning_buf(MSG_DEBUG,
				   ...
				   iface->bss[0]->conf->iface, strlen(iface->bss[0]->conf->iface));
		}
#endif /* CONFIG_MESH */
		...
		if (!delay_apply_cfg &&
			...
			wpa_error_log0(MSG_ERROR, "Could not set channel for "
				   "kernel driver");
	...
	if (iface->current_mode) {
		if (hostapd_prepare_rates(iface, iface->current_mode)) {
			wpa_error_log0(MSG_ERROR, "Failed to prepare rates "
				   "table.");
#ifndef HISI_CODE_CROP
	...
	if (hapd->iconf->rts_threshold >= -1 &&
		...
		wpa_error_log0(MSG_ERROR, "Could not set RTS threshold for "
			   "kernel driver");
		goto fail;
	}
	...
	if (hapd->iconf->fragm_threshold >= -1 &&
		...
		wpa_error_log0(MSG_ERROR, "Could not set fragmentation threshold "
			   "for kernel driver");
		goto fail;
	}
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	...
	for (j = 0; j < iface->num_bss; j++) {
		hapd = iface->bss[j];
		if (j)
			(void)os_memcpy(hapd->own_addr, prev_addr, ETH_ALEN);
...
#endif
#ifndef LOS_HOSTAPD_CONFIG_CROP
	(void)ap_list_init(iface);
#endif /* LOS_HOSTAPD_CONFIG_CROP */
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
	hostapd_set_acl(hapd);

	if (hostapd_driver_commit(hapd) < 0) {
		wpa_error_log0(MSG_ERROR, "hostapd_setup_interface_complete_sync: Failed to commit driver "
			   "configuration");
		goto fail;
	}
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
...
dfs_offload:
...
#ifdef LOS_WPA_PATCH
	wpa_events.event = HI_WIFI_EVT_AP_START;
	...
	wpa_warning_buf(MSG_DEBUG, "%s: Setup of interface done.",
		   iface->bss[0]->conf->iface, strlen(iface->bss[0]->conf->iface));
...
fail:
	wpa_error_log0(MSG_ERROR, "Interface initialization failed");
	hostapd_set_state(iface, HAPD_IFACE_DISABLED);
...
int hostapd_setup_interface_complete(struct hostapd_iface *iface, int err)
{
	...
	wpa_warning_buf(MSG_INFO,
		   ...
		   iface->bss[0]->conf->iface, strlen(iface->bss[0]->conf->iface));
	iface->need_to_start_in_sync = 0;
	(void)hostapd_setup_interface_complete_sync(iface, err);
	for (i = 0; i < interfaces->count; i++) {
		if (interfaces->iface[i]->need_to_start_in_sync &&
		    interfaces->iface[i]->ready_to_start_in_sync) {
			(void)hostapd_setup_interface_complete_sync(
...
int hostapd_setup_interface(struct hostapd_iface *iface)
{
	...
	if (ret) {
		wpa_error_buf(MSG_ERROR, "%s: Unable to setup interface.",
			   iface->bss[0]->conf->iface, strlen(iface->bss[0]->conf->iface));
		return -1;
	}
...
static void hostapd_bss_deinit(struct hostapd_data *hapd)
{
	if (!hapd)
		return;
	wpa_warning_buf(MSG_DEBUG, "hostapd_bss_deinit: deinit bss %s",
		   hapd->conf->iface, strlen(hapd->conf->iface));
	hostapd_bss_deinit_no_free(hapd);
	wpa_msg(hapd->msg_ctx, MSG_INFO, AP_EVENT_DISABLED);
#ifdef CONFIG_SQLITE
...
void hostapd_interface_deinit(struct hostapd_iface *iface)
{
	...
	wpa_warning_log1(MSG_DEBUG, "hostapd_interface_deinit(%p)", iface);
	if (iface == NULL)
		return;
...
{
	size_t j;
	wpa_warning_log1(MSG_DEBUG, "hostapd_interface_free(%p)", iface);
	for (j = 0; j < iface->num_bss; j++) {
		if (!iface->bss)
			break;
		wpa_warning_log1(MSG_DEBUG, "hostapd_interface_free: free hapd %p",
			   iface->bss[j]);
		os_free(iface->bss[j]);
	}
...
fail:
	wpa_error_buf(MSG_ERROR, "Failed to set up interface with %s",
	...
	if (hapd_iface) {
		...
		wpa_warning_log1(MSG_DEBUG, "hostapd_init: free iface %p",
			   hapd_iface);
		os_free(hapd_iface);
	}
	return NULL;
			   ...
			   const char *config_fname, int debug)
{
	...
	wpa_warning_two_buf(MSG_INFO, "Configuration file: %s (phy %s)",
		   config_fname, strlen(config_fname), phy, strlen(phy));
	wpa_warning_buf(MSG_INFO, "Configuration file: %s",
		   iface ? "" : " --> new PHY", iface ? strlen("") : strlen(" --> new PHY"));
	if (iface) {
		struct hostapd_config *conf;
		...
		if (conf == NULL)
			return NULL;
		if (conf->num_bss > 1) {
			wpa_error_log0(MSG_ERROR, "Multiple BSSes specified in BSS-config");
			hostapd_config_free(conf);
			return NULL;
		}
		...
		if (ifname[0] != '\0' && ifname_in_use(interfaces, ifname)) {
			wpa_error_buf(MSG_ERROR,
				   "Interface name %s already in use", ifname, strlen(ifname));
			hostapd_config_free(conf);
			return NULL;
		}
	...
	if (iface->conf->bss[bss_idx]->iface[0] == '\0' &&
		...
		wpa_error_buf(MSG_ERROR, "Interface name not specified in %s",
			   config_fname, strlen(config_fname));
		if (new_iface)
...
void hostapd_interface_deinit_free(struct hostapd_iface *iface)
{
	...
	wpa_warning_log1(MSG_DEBUG, "hostapd_interface_deinit_free (%p)", iface);
	if (iface == NULL)
		return;
	wpa_warning_log2(MSG_DEBUG, "hostapd_interface_deinit_free: num_bss=%u conf->num_bss=%u",
		   (unsigned int) iface->num_bss,
		   (unsigned int) iface->conf->num_bss);
	driver = iface->bss[0]->driver;
	drv_priv = iface->bss[0]->drv_priv;
	hostapd_interface_deinit(iface);
	wpa_warning_log2(MSG_DEBUG, "hostapd_interface_deinit_free: driver=%p drv_priv=%p -> hapd_deinit",
		   driver, drv_priv);
				  ...
				  struct hostapd_iface *hapd_iface)
{
	...
	wpa_warning_log2(MSG_DEBUG, "hostapd_deinit_driver: driver=%p drv_priv=%p -> hapd_deinit",
		   driver, drv_priv);
	if (driver && driver->hapd_deinit && drv_priv) {
		driver->hapd_deinit(drv_priv);
		for (j = 0; j < hapd_iface->num_bss; j++) {
			wpa_warning_log2(MSG_DEBUG, "hostapd_deinit_driver:bss[%d]->drv_priv=%p",
				   (int) j, hapd_iface->bss[j]->drv_priv);
...
int hostapd_enable_iface(struct hostapd_iface *hapd_iface)
{
	...
	if (hapd_iface->bss[0]->drv_priv != NULL) {
		wpa_error_buf(MSG_ERROR, "Interface %s already enabled",
			   hapd_iface->conf->bss[0]->iface, strlen(hapd_iface->conf->bss[0]->iface));
		return -1;
	}

	wpa_warning_buf(MSG_DEBUG, "Enable interface %s",
		   hapd_iface->conf->bss[0]->iface, strlen(hapd_iface->conf->bss[0]->iface));

	for (j = 0; j < hapd_iface->num_bss; j++)
		hostapd_set_security_params(hapd_iface->conf->bss[j], 1);
	if (hostapd_config_check(hapd_iface->conf, 1) < 0) {
		wpa_warning_log0(MSG_INFO, "Invalid configuration - cannot enable");
		return -1;
	}
...
int hostapd_reload_iface(struct hostapd_iface *hapd_iface)
{
	...
	wpa_warning_buf(MSG_DEBUG, "Reload interface %s",
		   hapd_iface->conf->bss[0]->iface, strlen(hapd_iface->conf->bss[0]->iface));
	for (j = 0; j < hapd_iface->num_bss; j++)
		hostapd_set_security_params(hapd_iface->conf->bss[j], 1);
	if (hostapd_config_check(hapd_iface->conf, 1) < 0) {
		wpa_error_log0(MSG_ERROR, "Updated configuration is invalid");
		return -1;
	}
...
int hostapd_disable_iface(struct hostapd_iface *hapd_iface)
{
	...
	if (hapd_iface->bss[0]->drv_priv == NULL) {
		wpa_warning_buf(MSG_INFO, "Interface %s already disabled",
			   hapd_iface->conf->bss[0]->iface, strlen(hapd_iface->conf->bss[0]->iface));
		return -1;
	}
	...
	wpa_warning_buf(MSG_DEBUG, "Interface %s disabled",
		   hapd_iface->bss[0]->conf->iface, strlen(hapd_iface->bss[0]->conf->iface));
...
hostapd_iface_alloc(struct hapd_interfaces *interfaces)
{
	...
	hapd_iface = interfaces->iface[interfaces->count] =
		hostapd_alloc_iface();
	if (hapd_iface == NULL) {
		wpa_error_log0(MSG_ERROR, "hostapd_iface_alloc: Failed to allocate memory for "
			   "the interface");
		return NULL;
	}
	interfaces->count++;
		     ...
		     const char *ctrl_iface, const char *driver)
{
	...
	if (conf == NULL) {
		 wpa_error_log0(MSG_ERROR, "hostapd_config_alloc: Failed to allocate memory for "
				"configuration");
		 return NULL;
	}
...
skip:
	...
	(void)os_strlcpy(bss->iface, ifname, sizeof(bss->iface));
	...
	if (os_strncmp(buf, "bss_config=", 11) == 0) {
		...
		*pos++ = '\0';
		...
		if (new_iface) {
			...
			(void)os_memcpy(hapd->own_addr, hapd_iface->bss[0]->own_addr,
				  ETH_ALEN);
			...
			if (start_ctrl_iface_bss(hapd) < 0 ||
				...
				hapd_iface->num_bss--;
				wpa_warning_buf(MSG_DEBUG, "hostapd_add_iface: free hapd %s",
					   hapd->conf->iface, strlen(hapd->conf->iface));
				wpa_warning_log1(MSG_DEBUG, "hostapd_add_iface: free hapd %p",
					   hapd);
	...
	*ptr++ = '\0';
	...
	for (i = 0; i < interfaces->count; i++) {
		if (!os_strcmp(interfaces->iface[i]->conf->bss[0]->iface,
			       buf)) {
			wpa_warning_log0(MSG_INFO, "Cannot add interface - it "
				   "already exists");
			return -1;
		}
	}
	...
	if (hapd_iface == NULL) {
		wpa_error_log0(MSG_ERROR, "hostapd_add_iface: Failed to allocate memory "
			   "for interface");
		goto fail;
	}
	new_iface = hapd_iface;
	...
	if (conf_file && interfaces->config_read_cb) {
		...
		if (conf && conf->bss)
			(void)os_strlcpy(conf->bss[0]->iface, buf,
	...
	wpa_warning_buf(MSG_INFO, "Add interface '%s'",
		   hapd_iface->conf->bss[0]->iface, strlen(hapd_iface->conf->bss[0]->iface));
...
static int hostapd_remove_bss(struct hostapd_iface *iface, unsigned int idx)
{
	...
	wpa_warning_buf(MSG_INFO, "Remove BSS '%s'", iface->conf->bss[idx]->iface, strlen(iface->conf->bss[idx]->iface));
	...
	if (idx < iface->num_bss) {
		...
		wpa_warning_buf(MSG_DEBUG, "hostapd_remove_bss: free hapd (%s)",
			   hapd->conf->iface, strlen(hapd->conf->iface));
		wpa_warning_log1(MSG_DEBUG, "hostapd_remove_bss: free hapd %p",
			   hapd);
		hostapd_config_free_bss(hapd->conf);
		hapd->conf = NULL;
		os_free(hapd);
...
int hostapd_remove_iface(struct hapd_interfaces *interfaces, char *buf)
{
	...
	for (i = 0; i < interfaces->count; i++) {
		...
		if (!os_strcmp(hapd_iface->conf->bss[0]->iface, buf)) {
			wpa_warning_buf(MSG_INFO, "Remove interface '%s'", buf, strlen(buf));
#ifndef HISI_CODE_CROP
			hapd_iface->driver_ap_teardown =
				!!(hapd_iface->drv_flags &
				   WPA_DRIVER_FLAGS_AP_TEARDOWN_SUPPORT);
			   ...
			   int reassoc)
{
	...
	if (!hapd->conf->wpa) {
		...
		(void)os_get_reltime(&sta->connected_time);
	...
	if (reassoc) {
		if (sta->auth_alg != WLAN_AUTH_FT &&
			...
			(void)wpa_auth_sm_event(sta->wpa_sm, WPA_REAUTH);
	} else
		(void)wpa_auth_sta_associated(hapd->wpa_auth, sta->wpa_sm);

#ifndef HISI_CODE_CROP
	if (hapd->iface->drv_flags & WPA_DRIVER_FLAGS_WIRED) {
		if (eloop_cancel_timeout(ap_handle_timer, hapd, sta) > 0) {
			wpa_warning_buf(MSG_DEBUG,
				   "%s: hostapd_new_assoc_sta: canceled wired ap_handle_timer timeout for ",
				   hapd->conf->iface, strlen(hapd->conf->iface));
			wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x",
	...
	if (!(hapd->iface->drv_flags &
		...
		wpa_warning_buf(MSG_DEBUG,
			   "%s: hostapd_new_assoc_sta: reschedule ap_handle_timer timeout for ",
			   hapd->conf->iface, strlen(hapd->conf->iface));
		wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x",
				(sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
		wpa_warning_log1(MSG_DEBUG, " (%d seconds - ap_max_inactivity)", hapd->conf->ap_max_inactivity);
...
{
	wpa_warning_buf(MSG_INFO, "%s",
		   iface->conf ? iface->conf->bss[0]->iface : "N/A", iface->conf ? strlen(iface->conf->bss[0]->iface) : strlen("N/A"));
	wpa_warning_two_buf(MSG_INFO, "interface state %s->%s",
		   hostapd_state_text(iface->state), strlen(hostapd_state_text(iface->state)),
		   hostapd_state_text(s), strlen(hostapd_state_text(s)));
	iface->state = s;
}
				     ...
				     struct csa_settings *settings)
{
	...
	(void)hostapd_change_config_freq(iface->bss[0], iface->conf,
				   &old_freq, NULL);
			   ...
			   struct csa_settings *settings)
{
	...
	if (!(hapd->iface->drv_flags & WPA_DRIVER_FLAGS_AP_CSA)) {
		wpa_warning_log0(MSG_INFO, "CSA is not supported");
		return -1;
	}
				...
				const struct hostapd_freq_params *freq_params)
{
	...
	wpa_warning_log0(MSG_DEBUG, "Restarting all CSA-related BSSes");
	...
	case 160:
		...
		break;
	default:
		wpa_warning_log1(MSG_WARNING, "Unknown CSA bandwidth: %d",
	 ...
	 */
	(void)hostapd_disable_iface(iface);
	(void)hostapd_enable_iface(iface);
}
