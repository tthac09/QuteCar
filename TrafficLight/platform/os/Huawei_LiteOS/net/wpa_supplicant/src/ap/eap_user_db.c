...
static void set_user_methods(struct hostapd_eap_user *user, const char *methods)
{
	...
	while (*start) {
		...
		if (user->methods[num_methods].vendor == EAP_VENDOR_IETF &&
			...
			wpa_warning_buf(MSG_INFO, "DB: Unsupported EAP type '%s'",
				   start, strlen(start));
			os_free(buf);
			return;
		}
		    ...
		    size_t identity_len, int phase2)
{
	...
	if (identity_len >= sizeof(id_str)) {
		wpa_warning_log2(MSG_DEBUG, "identity len too big: %d >= %d",
			   (int) identity_len,
			   (int) (sizeof(id_str)));
		return NULL;
	}
	...
	for (i = 0; i < identity_len; i++) {
		...
		if (id_str[i] == '-' || id_str[i] == '_' || id_str[i] == '.' ||
		    ...
		    id_str[i] == '=' || id_str[i] == ' ')
			continue;
		wpa_warning_log0(MSG_INFO, "DB: Unsupported character in identity");
		return NULL;
	}
	...
	if (sqlite3_open(hapd->conf->eap_user_sqlite, &db)) {
		wpa_warning_two_buf(MSG_INFO, "DB: Failed to open database %s: %s",
			   hapd->conf->eap_user_sqlite, strlen(hapd->conf->eap_user_sqlite),
			   sqlite3_errmsg(db), strlen(sqlite3_errmsg(db)));
		sqlite3_close(db);
		return NULL;
	}
	...
	if (user == NULL && !phase2) {
		...
		wpa_warning_buf(MSG_DEBUG, "DB: %s", cmd, strlen(cmd));
