...
static int prune_associations(struct hostapd_iface *iface, void *ctx)
{
	...
	for (j = 0; j < iface->num_bss; j++) {
		...
		wpa_warning_buf(MSG_INFO, "%s: Prune association for ",
			   ohapd->conf->iface, strlen(ohapd->conf->iface));
		wpa_warning_log4(MSG_INFO, "%02x:xx:xx:%02x:%02x:%02x",
			   (osta->addr)[0], (osta->addr)[3], (osta->addr)[4], (osta->addr)[5]);
...
void hostapd_prune_associations(struct hostapd_data *hapd, const u8 *addr)
{
	...
	if (hapd->iface->interfaces &&
		...
		(void)hapd->iface->interfaces->for_each_interface(
