...
int hostapd_p2p_set_noa(struct hostapd_data *hapd, u8 count, int start,
			int duration)
{
	wpa_warning_log3(MSG_DEBUG, "P2P: Set NoA parameters: count=%u start=%d "
	...
	if (count != 255) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Non-periodic NoA - set "
			   "NoA parameters");
	...
	if (hapd->num_sta_no_p2p == 0) {
		wpa_warning_log0(MSG_DEBUG, "P2P: No legacy STAs connected - update "
			   "periodic NoA parameters");
		return hostapd_driver_set_noa(hapd, count, start, duration);
	}

	wpa_warning_log0(MSG_DEBUG, "P2P: Legacy STA(s) connected - do not enable "
		   "periodic NoA");
...
{
	wpa_warning_log0(MSG_DEBUG, "P2P: First non-P2P device connected");

	if (hapd->noa_enabled) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Disable periodic NoA");
...
{
	wpa_warning_log0(MSG_DEBUG, "P2P: Last non-P2P device disconnected");

	if (hapd->noa_enabled) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Enable periodic NoA");
