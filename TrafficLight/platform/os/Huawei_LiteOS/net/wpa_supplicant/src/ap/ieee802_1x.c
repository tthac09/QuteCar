			    ...
			    u8 type, const u8 *data, size_t datalen)
{
	...
	buf = os_zalloc(len);
	if (buf == NULL) {
		wpa_error_log1(MSG_ERROR, "malloc() failed for "
	...
	if (datalen > 0 && data != NULL)
		(void)os_memcpy(xhdr + 1, data, datalen);
	...
	if (sta->flags & WLAN_STA_PREAUTH) {
		...
		(void)hostapd_drv_hapd_send_eapol(
				  ...
				  u8 *key_data, size_t key_len)
{
	...
	if (random_get_bytes(key->key_iv, sizeof(key->key_iv))) {
		wpa_error_log0(MSG_ERROR, "Could not get random numbers");
		os_free(buf);
		return;
	}
	...
	if (sm->eap_if->eapKeyData == NULL || sm->eap_if->eapKeyDataLen < 64) {
		wpa_error_log0(MSG_ERROR, "No eapKeyData available for encrypting "
			   "and signing EAPOL-Key");
		os_free(buf);
		return;
	}
	(void)os_memcpy((u8 *) (key + 1), key_data, key_len);
	ekey_len = sizeof(key->key_iv) + 32;
	ekey = os_malloc(ekey_len);
	if (ekey == NULL) {
		wpa_error_log0(MSG_ERROR, "Could not encrypt key");
		os_free(buf);
		return;
	}
	(void)os_memcpy(ekey, key->key_iv, sizeof(key->key_iv));
	(void)os_memcpy(ekey + sizeof(key->key_iv), sm->eap_if->eapKeyData, 32);
	(void)rc4_skip(ekey, ekey_len, 0, (u8 *) (key + 1), key_len);
	os_free(ekey);
	...
	(void)hmac_md5(sm->eap_if->eapKeyData + 32, 32, buf, sizeof(*hdr) + len,
		 key->key_signature);

	wpa_warning_log4(MSG_DEBUG, "IEEE 802.1X: Sending EAPOL-Key to " "%02x:xx:xx:%02x:%02x:%02x",
					(sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
	wpa_warning_buf(MSG_DEBUG, "IEEE 802.1X: Sending EAPOL-Key to "
		   " (%s)", broadcast ? "broadcast" : "unicast",
		   broadcast ? strlen("broadcast") : strlen("unicast"));
	wpa_warning_log1(MSG_DEBUG, "IEEE 802.1X: Sending EAPOL-Key to "
		   " (index=%d)", idx);
...
static void ieee802_1x_tx_key(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	wpa_warning_log4(MSG_DEBUG, "IEEE 802.1X: Sending EAPOL-Key(s) to " "%02x:xx:xx:%02x:%02x:%02x",
		   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);

#ifndef CONFIG_NO_VLAN
	if (sta->vlan_id > 0) {
		wpa_error_log0(MSG_ERROR, "Using WEP with vlans is not supported.");
		return;
	}
	...
	if (hapd->conf->individual_wep_key_len > 0) {
		...
		{
			wpa_error_log0(MSG_ERROR, "Could not generate random "
				   "individual WEP key.");
			os_free(ikey);
			return;
		}
		...
		if (hostapd_drv_set_key(hapd->conf->iface, hapd, WPA_ALG_WEP,
					...
					hapd->conf->individual_wep_key_len)) {
			wpa_error_log0(MSG_ERROR, "Could not set individual WEP "
				   "encryption.");
		}
					  ...
					  struct radius_msg *msg)
{
	...
	if (val != -1 &&
	    ...
	    !radius_msg_add_attr_int32(msg, RADIUS_ATTR_WLAN_PAIRWISE_CIPHER,
				       suite)) {
		wpa_error_log0(MSG_ERROR, "Could not add WLAN-Pairwise-Cipher");
		return -1;
	}
	...
	if (!hostapd_config_get_radius_attr(req_attr,
	    ...
	    !radius_msg_add_attr_int32(msg, RADIUS_ATTR_WLAN_GROUP_CIPHER,
				       suite)) {
		wpa_error_log0(MSG_ERROR, "Could not add WLAN-Group-Cipher");
		return -1;
	}
	...
	if (val != -1 &&
	    ...
	    !radius_msg_add_attr_int32(msg, RADIUS_ATTR_WLAN_AKM_SUITE,
				       suite)) {
		wpa_error_log0(MSG_ERROR, "Could not add WLAN-AKM-Suite");
		return -1;
	}
				      ...
				      struct radius_msg *msg)
{
	...
	if (!hostapd_config_get_radius_attr(req_attr,
		...
		wpa_error_log0(MSG_ERROR, "Could not add Service-Type");
		return -1;
	}
	...
	if (!hostapd_config_get_radius_attr(req_attr,
		...
		wpa_error_log0(MSG_ERROR, "Could not add NAS-Port");
		return -1;
	}
	...
	if (!radius_msg_add_attr(msg, RADIUS_ATTR_CALLING_STATION_ID,
		...
		wpa_error_log0(MSG_ERROR, "Could not add Calling-Station-Id");
		return -1;
	}
	...
	if (!hostapd_config_get_radius_attr(req_attr,
	    ...
	    !radius_msg_add_attr(msg, RADIUS_ATTR_CONNECT_INFO,
				 (u8 *) buf, os_strlen(buf))) {
		wpa_error_log0(MSG_ERROR, "Could not add Connect-Info");
		return -1;
	}
	...
	if (sta->acct_session_id) {
		...
		if (!radius_msg_add_attr(msg, RADIUS_ATTR_ACCT_SESSION_ID,
					 (u8 *) buf, os_strlen(buf))) {
			wpa_error_log0(MSG_ERROR, "Could not add Acct-Session-Id");
			return -1;
		}
	}
	...
	if (hapd->conf->wpa && wpa_key_mgmt_ft(hapd->conf->wpa_key_mgmt) &&
		...
		wpa_error_log0(MSG_ERROR, "Could not add Mobility-Domain-Id");
		return -1;
	}
			   ...
			   struct radius_msg *msg)
{
	...
	if (!hostapd_config_get_radius_attr(req_attr,
		...
		wpa_error_log0(MSG_ERROR, "Could not add NAS-IP-Address");
		return -1;
	}
	...
	if (!hostapd_config_get_radius_attr(req_attr,
		...
		wpa_error_log0(MSG_ERROR, "Could not add NAS-IPv6-Address");
		return -1;
	}
#endif /* CONFIG_IPV6 */
	...
	if (!hostapd_config_get_radius_attr(req_attr,
		...
		wpa_error_log0(MSG_ERROR, "Could not add NAS-Identifier");
		return -1;
	}
	...
	if (!hostapd_config_get_radius_attr(req_attr,
	    ...
	    !radius_msg_add_attr(msg, RADIUS_ATTR_CALLED_STATION_ID,
				 (u8 *) buf, len)) {
		wpa_error_log0(MSG_ERROR, "Could not add Called-Station-Id");
		return -1;
	}
	...
	if (!hostapd_config_get_radius_attr(req_attr,
		...
		wpa_error_log0(MSG_ERROR, "Could not add NAS-Port-Type");
		return -1;
	}
	...
	if (hapd->conf->interworking &&
		...
		if (!hostapd_config_get_radius_attr(req_attr,
		    ...
		    !radius_msg_add_attr(msg, RADIUS_ATTR_WLAN_HESSID,
					 (u8 *) buf, os_strlen(buf))) {
			wpa_error_log0(MSG_ERROR, "Could not add WLAN-HESSID");
			return -1;
		}
	}
	...
	for (attr = req_attr; attr; attr = attr->next) {
		if (!radius_msg_add_attr(msg, attr->type,
					 ...
					 wpabuf_len(attr->val))) {
			wpa_error_log0(MSG_ERROR, "Could not add RADIUS "
				   "attribute");
			return -1;
		}
	}
				   ...
				   const u8 *eap, size_t len)
{
	...
	wpa_warning_log0(MSG_DEBUG, "Encapsulating EAP message into a RADIUS "
		   "packet");
	...
	if (msg == NULL) {
		wpa_warning_log0(MSG_INFO, "Could not create new RADIUS packet");
		return;
	}

	if (radius_msg_make_authenticator(msg) < 0) {
		wpa_warning_log0(MSG_INFO, "Could not make Request Authenticator");
		goto fail;
	}
	...
	if (sm->identity &&
		...
		wpa_warning_log0(MSG_INFO, "Could not add User-Name");
		goto fail;
	}
	...
	if (!hostapd_config_get_radius_attr(hapd->conf->radius_auth_req_attr,
		...
		wpa_warning_log0(MSG_INFO, "Could not add Framed-MTU");
		goto fail;
	}

	if (!radius_msg_add_eap(msg, eap, len)) {
		wpa_warning_log0(MSG_INFO, "Could not add EAP-Message");
		goto fail;
	}
	...
	if (sm->last_recv_radius &&
		...
		int res = radius_msg_copy_attr(msg, sm->last_recv_radius,
					       RADIUS_ATTR_STATE);
		if (res < 0) {
			wpa_warning_log0(MSG_INFO, "Could not copy State attribute from previous Access-Challenge");
			goto fail;
		}
		if (res > 0) {
			wpa_warning_log0(MSG_DEBUG, "Copied RADIUS State Attribute");
		}
	}
	...
	if (hapd->conf->radius_request_cui) {
		...
		if (!radius_msg_add_attr(msg,
					 ...
					 cui, cui_len)) {
			wpa_error_log0(MSG_ERROR, "Could not add CUI");
			goto fail;
		}
	}
	...
	if (hapd->conf->hs20) {
		...
		if (!radius_msg_add_wfa(
			    ...
			    &ver, 1)) {
			wpa_error_log0(MSG_ERROR, "Could not add HS 2.0 AP "
				   "version");
			goto fail;
		}
		...
		if (sta->hs20_ie && wpabuf_len(sta->hs20_ie) > 0) {
			...
			if (!radius_msg_add_wfa(
				    ...
				    buf, sizeof(buf))) {
				wpa_error_log0(MSG_ERROR, "Could not add HS 2.0 "
					   "STA version");
				goto fail;
			}
		}
				...
				size_t len)
{
	...
	if (len < sizeof(*eap) + 1) {
		wpa_warning_log0(MSG_INFO, "handle_eap_response: too short response data");
		return;
	}
	...
	hostapd_logger(hapd, sm->addr, HOSTAPD_MODULE_IEEE8021X,
			   ...
			   eap_server_get_name(0, (EapType)type), type);
		       ...
		       u8 *buf, size_t len)
{
	...
	if (len < sizeof(*eap)) {
		wpa_warning_log0(MSG_INFO, "   too short EAP packet");
		return;
	}
	...
	wpa_warning_log3(MSG_DEBUG, "EAP: code=%d identifier=%d length=%d",
		   eap->code, eap->identifier, eap_len);
	if (eap_len < sizeof(*eap)) {
		wpa_warning_log0(MSG_DEBUG, "   Invalid EAP length");
		return;
	} else if (eap_len > len) {
		wpa_warning_log0(MSG_DEBUG, "   Too short frame to contain this EAP "
			   "packet");
		return;
	} else if (eap_len < len) {
		wpa_warning_log1(MSG_DEBUG, "   Ignoring %lu extra bytes after EAP "
			   "packet", (unsigned long) len - eap_len);
	}

	switch (eap->code) {
	case EAP_CODE_REQUEST:
		wpa_warning_log0(MSG_DEBUG, " (request)");
		return;
	case EAP_CODE_RESPONSE:
		wpa_warning_log0(MSG_DEBUG, " (response)");
		handle_eap_response(hapd, sta, eap, eap_len);
		break;
	case EAP_CODE_SUCCESS:
		wpa_warning_log0(MSG_DEBUG, " (success)");
		return;
	case EAP_CODE_FAILURE:
		wpa_warning_log0(MSG_DEBUG, " (failure)");
		return;
	case EAP_CODE_INITIATE:
		wpa_warning_log0(MSG_DEBUG, " (initiate)");
		handle_eap_initiate(hapd, sta, eap, eap_len);
		break;
	case EAP_CODE_FINISH:
		wpa_warning_log0(MSG_DEBUG, " (finish)");
		break;
	default:
		wpa_warning_log0(MSG_DEBUG, " (unknown code)");
		return;
	}
}
#endif /* CONFIG_WPS_AP */
				  ...
				  size_t len)
{
	...
	(void)os_get_reltime(&sta->pending_eapol_rx->rx_time);
}
#endif /* HISI_CODE_CROP */
			...
			size_t len)
{
...
#ifndef HISI_CODE_CROP
		wpa_warning_log0(MSG_DEBUG, "IEEE 802.1X data frame from not "
	...
	if (len < sizeof(*hdr)) {
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log0(MSG_INFO, "   too short IEEE 802.1X packet");
#endif /* CONFIG_PRINT_NOUSE */
		return;
	}
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log0(MSG_INFO, "   frame too short for this IEEE 802.1X packet");
#endif /* CONFIG_PRINT_NOUSE */
		if (sta->eapol_sm)
...
#ifndef CONFIG_PRINT_NOUSE
	if (len - sizeof(*hdr) > datalen) {
		wpa_warning_log1(MSG_DEBUG, "   ignoring %lu extra octets after "
			   "IEEE 802.1X packet",
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log0(MSG_DEBUG, "IEEE 802.1X: Ignore EAPOL message - "
	...
	if (!sta->eapol_sm) {
		...
		if (hapd->conf->wps_state) {
			...
			if (wflags == WLAN_STA_MAYBE_WPS ||
				 ...
				 * 2.0.
				 */
				wpa_warning_log0(MSG_DEBUG, "WPS: Do not start "
	...
	case IEEE802_1X_TYPE_EAPOL_START:
		...
		if (pmksa) {
			...
			(void)wpa_auth_sta_clear_pmksa(sta->wpa_sm, pmksa);
		}
#endif /* LOS_CONFIG_HOSTAPD_PMKSA */
		sta->eapol_sm->eapolStart = TRUE;
		sta->eapol_sm->dot1xAuthEapolStartFramesRx++;
		eap_server_clear_identity(sta->eapol_sm->eap);
		(void)wpa_auth_sm_event(sta->wpa_sm, WPA_REAUTH_EAPOL);
		break;
	...
	case IEEE802_1X_TYPE_EAPOL_KEY:
		wpa_warning_log0(MSG_DEBUG, "   EAPOL-Key");
		if (!ap_sta_is_authorized(sta)) {
			wpa_warning_log0(MSG_DEBUG, "   Dropped key data from "
	...
	case IEEE802_1X_TYPE_EAPOL_ENCAPSULATED_ASF_ALERT:
		wpa_warning_log0(MSG_DEBUG, "   EAPOL-Encapsulated-ASF-Alert");
	...
	default:
		wpa_warning_log0(MSG_DEBUG, "   unknown IEEE 802.1X packet type");
...
void ieee802_1x_new_station(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	if (key_mgmt != -1 && wpa_key_mgmt_wpa_psk(key_mgmt)) {
#endif /* HISI_WPA_KEY_MGMT_CROP */
		wpa_warning_log0(MSG_DEBUG, "IEEE 802.1X: Ignore STA - using PSK");
		/*
		 * Clear any possible EAPOL authenticator state to support
	...
	if (hapd->conf->wps_state &&
		 ...
		 */
		wpa_warning_log0(MSG_DEBUG, "WPS: Do not start EAPOL until "
	...
	if (pmksa) {
		...
		(void)ap_sta_bind_vlan(hapd, sta);
#endif
	} else {
#endif /* LOS_CONFIG_HOSTAPD_PMKSA */
		if (reassoc) {
					  ...
					  struct radius_msg *msg)
{
	...
	wpa_warning_log1(MSG_DEBUG, "IEEE 802.1X: Stored %lu RADIUS Class attributes ",
		   (unsigned long) sm->radius_class.count);
	wpa_warning_log4(MSG_DEBUG, "IEEE 802.1X: Stored RADIUS Class attributes for %02x:xx:xx:%02x:%02x:%02x",
		   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
}
					 ...
					 size_t len, int session_timeout)
{
	...
	if (session_timeout < 0) {
		wpa_warning_log0(MSG_DEBUG, "HS 2.0: No Session-Timeout set - ignore session info URL");
		return;
	}
	if (swt == 255)
			...
			void *data)
{
	...
	if (sm == NULL) {
		wpa_warning_log0(MSG_DEBUG, "IEEE 802.1X: Could not find matching "
	...
	if (hdr->code == RADIUS_CODE_ACCESS_REJECT &&
		...
		wpa_warning_log0(MSG_DEBUG, "Allowing RADIUS Access-Reject without "
			   "Message-Authenticator since it does not include "
			   "EAP-Message");
	} else if (radius_msg_verify(msg, shared_secret, shared_secret_len,
				     req, 1)) {
		wpa_warning_log0(MSG_INFO, "Incoming RADIUS packet did not have correct Message-Authenticator - dropped");
	...
	if (hdr->code != RADIUS_CODE_ACCESS_ACCEPT &&
		...
		wpa_warning_log0(MSG_INFO, "Unknown RADIUS message code");
		return RADIUS_RX_UNKNOWN;
	}

	sm->radius_identifier = -1;
	wpa_warning_log4(MSG_DEBUG, "RADIUS packet matching with station %02x:xx:xx:%02x:%02x:%02x",
		   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]));
...
static int ieee802_1x_rekey_broadcast(struct hostapd_data *hapd)
{
	...
	if (eapol->default_wep_key == NULL ||
	    random_get_bytes(eapol->default_wep_key,
			     hapd->conf->default_wep_key_len)) {
		wpa_warning_log0(MSG_INFO, "Could not generate random WEP key");
		os_free(eapol->default_wep_key);
...
static void ieee802_1x_rekey(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_warning_log1(MSG_DEBUG, "IEEE 802.1X: New default WEP key index %d",
		   eapol->default_wep_key_idx);
	...
	(void)ap_for_each_sta(hapd, ieee802_1x_sta_key_available, NULL);

	if (hapd->conf->wep_rekeying_period > 0) {
		(void)eloop_register_timeout(hapd->conf->wep_rekeying_period, 0,
				  ...
				  const u8 *data, size_t datalen)
{
	...
	if ((sta->flags & (WLAN_STA_WPS | WLAN_STA_MAYBE_WPS)) ==
		...
		if (identity &&
			...
			wpa_warning_log0(MSG_DEBUG, "WPS: WLAN_STA_MAYBE_WPS -> "
				   "WLAN_STA_WPS");
			sta->flags |= WLAN_STA_WPS;
		}
	}
				   ...
				   struct eap_user *user)
{
	...
	(void)os_memset(user, 0, sizeof(*user));
	user->phase2 = phase2;
...
out:
	if (rv) {
		wpa_warning_log0(MSG_DEBUG, "ieee802_1x_get_eap_user: Failed to find user");
	}
...
int ieee802_1x_init(struct hostapd_data *hapd)
{
	...
	(void)os_memset(&conf, 0, sizeof(conf));
	conf.ctx = hapd;
	...
	(void)os_memset(&cb, 0, sizeof(cb));
	cb.eapol_send = ieee802_1x_eapol_send;
	cb.aaa_send = NULL; /* ieee802_1x_aaa_send; */
	...
	if (hapd->conf->default_wep_key_len) {
		for (i = 0; i < 4; i++)
			(void)hostapd_drv_set_key(hapd->conf->iface, hapd,
...
{
#ifndef LOS_CONFIG_HOSTAPD_SECURITY
	(void)eloop_cancel_timeout(ieee802_1x_rekey, hapd, NULL);
#endif
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
			       ...
			       const u8 *buf, int len, int ack)
{
	...
	if (len < (int) sizeof(*xhdr))
		return 0;
	wpa_warning_log4(MSG_DEBUG, "IEEE 802.1X: " "%02x:xx:xx:%02x:%02x:%02x",
		   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	wpa_warning_log4(MSG_DEBUG, "IEEE 802.1X: " " TX status - version=%d "
		   "type=%d length=%d - ack=%d",
		   xhdr->version, xhdr->type,
