...
int hostapd_ht_operation_update(struct hostapd_iface *iface)
{
	...
	wpa_warning_log1(MSG_DEBUG, "hostapd_ht_operation_update current operation mode=0x%X",
		   iface->ht_op_mode);
	...
	wpa_warning_log2(MSG_DEBUG, "hostapd_ht_operation_update new operation mode=0x%X changes=%d",
...
static int is_40_allowed(struct hostapd_iface *iface, int channel)
{
	...
	wpa_error_log2(MSG_ERROR, "40 MHz affected channel range: [%d,%d] MHz",
		   affected_start, affected_end);
	wpa_error_log1(MSG_ERROR, "Neighboring BSS: freq=%d", pri);
	return 0;
}
			      ...
			      const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (bc_ie->element_id != WLAN_EID_20_40_BSS_COEXISTENCE ||
	    bc_ie->length < 1) {
		wpa_warning_log2(MSG_DEBUG, "Unexpected IE (%u,%u) in coex report",
	...
	wpa_warning_log1(MSG_DEBUG, "20/40 BSS Coexistence Information field: 0x%x",
		   bc_ie->coex_param);
	...
	wpa_warning_log2(MSG_DEBUG, "is_ht40_allowed=%d num_sta_ht40_intolerant=%d",
	...
	if (!is_ht40_allowed &&
		...
		if (!iface->num_sta_ht40_intolerant &&
			...
			delay_time = OVERLAPPING_BSS_TRANS_DELAY_FACTOR *
				iface->conf->obss_interval;
			(void)eloop_cancel_timeout(ap_ht2040_timeout, hapd->iface,
					     NULL);
			(void)eloop_register_timeout(delay_time, 0, ap_ht2040_timeout,
					       hapd->iface, NULL);
			wpa_warning_log1(MSG_DEBUG,
...
void ht40_intolerant_add(struct hostapd_iface *iface, struct sta_info *sta)
{
	...
	wpa_warning_log4(MSG_INFO, "HT: Forty MHz Intolerant is set by STA " "%02x:xx:xx:%02x:%02x:%02x"
		   " in Association Request", (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	...
	(void)eloop_cancel_timeout(ap_ht2040_timeout, iface, NULL);
...
static void update_sta_ht(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	wpa_warning_log4(MSG_DEBUG, "HT: STA " "%02x:xx:xx:%02x:%02x:%02x" " HT Capabilities Info: ",
		(sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	wpa_warning_log1(MSG_DEBUG, "0x%04x", ht_capab);
	if ((ht_capab & HT_CAP_INFO_GREEN_FIELD) == 0) {
		if (!sta->no_ht_gf_set) {
			sta->no_ht_gf_set = 1;
			hapd->iface->num_sta_ht_no_gf++;
		}
		wpa_warning_log4(MSG_DEBUG, "update_sta_ht STA " "%02x:xx:xx:%02x:%02x:%02x",
			   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
		wpa_warning_log1(MSG_DEBUG, "update_sta_ht STA " " - no greenfield, num "
			   "of non-gf stations %d", hapd->iface->num_sta_ht_no_gf);
	}
	if ((ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET) == 0) {
		...
		wpa_warning_log4(MSG_DEBUG, "update_sta_ht STA " "%02x:xx:xx:%02x:%02x:%02x",
			   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
		wpa_warning_log1(MSG_DEBUG, " - 20 MHz HT, num of "
			   "20MHz HT STAs %d", hapd->iface->num_sta_ht_20mhz);
	}
#ifndef HISI_CODE_CROP
...
static void update_sta_no_ht(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	if (hapd->iconf->ieee80211n) {
		wpa_warning_log4(MSG_DEBUG, "update_sta_no_ht STA " "%02x:xx:xx:%02x:%02x:%02x",
				(sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
		wpa_warning_log1(MSG_DEBUG, " - no HT, num of "
			   "non-HT stations %d", hapd->iface->num_sta_no_ht);
	}
}
...
void ap_ht2040_timeout(void *eloop_data, void *user_data)
{
	...
	(void)user_data;
	wpa_warning_log0(MSG_INFO, "Switching to 40 MHz operation");
