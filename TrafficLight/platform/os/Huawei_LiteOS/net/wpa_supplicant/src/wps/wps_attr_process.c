			      ...
			      const struct wpabuf *msg)
{
	...
	if (authenticator == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Authenticator attribute "
			   "included");
		return -1;
	}
	...
	if (os_memcmp_const(hash, authenticator, WPS_AUTHENTICATOR_LEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Incorrect Authenticator");
		return -1;
	}
			      ...
			      const u8 *key_wrap_auth)
{
	...
	if (key_wrap_auth == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No KWA in decrypted attribute");
		return -1;
	}
	...
	if (head + len != key_wrap_auth - 4) {
		wpa_warning_log0(MSG_DEBUG, "WPS: KWA not in the end of the "
			   "decrypted attribute");
		return -1;
	}

	(void)hmac_sha256(wps->authkey, WPS_AUTHKEY_LEN, head, len, hash);
	if (os_memcmp_const(hash, key_wrap_auth, WPS_KWA_LEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Invalid KWA");
		return -1;
	}
					...
					const u8 *idx)
{
	(void)cred;
	if (idx == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Credential did not include "
			   "Network Index");
		return -1;
	}

	wpa_warning_log1(MSG_DEBUG, "WPS: Network Index: %d", *idx);
...
{
	if (ssid == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Credential did not include SSID");
		return -1;
	}
					    ...
					    const u8 *key_idx)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WPS: Network Key Index: %d", *key_idx);
					...
					const u8 *key, size_t key_len)
{
	if (key == NULL) {
		...
		if (cred->auth_type == WPS_AUTH_OPEN &&
			...
			wpa_warning_log0(MSG_DEBUG, "WPS: Workaround - Allow "
				     ...
				     const u8 *mac_addr)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WPS: MAC Address " "%02x:xx:xx:%02x:%02x:%02x", (mac_addr)[0], (mac_addr)[3], (mac_addr)[4], (mac_addr)[5]);
...
static int wps_workaround_cred_key(struct wps_credential *cred)
{
	if (cred->auth_type & (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK) &&
		...
		wpa_warning_log0(MSG_INFO, "WPS: WPA/WPA2-Personal passphrase uses "
		 ...
		 * to fix the encoding.
		 */
		wpa_warning_log0(MSG_DEBUG, "WPS: Workaround - remove NULL "
	...
	if (cred->auth_type & (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK) &&
		...
		wpa_warning_log0(MSG_INFO, "WPS: Reject credential with invalid WPA/WPA2-Personal passphrase");
		wpa_hexdump_ascii_key(MSG_INFO, "WPS: Network Key",
				      cred->key, cred->key_len);
		return -1;
	}
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS: Process Credential");
...
int wps_process_ap_settings(struct wps_parse_attr *attr,
			    struct wps_credential *cred)
{
	wpa_warning_log0(MSG_DEBUG, "WPS: Processing AP Settings");
