...
int wps_build_public_key(struct wps_data *wps, struct wpabuf *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Public Key");
	wpabuf_clear_free(wps->dh_privkey);
	wps->dh_privkey = NULL;
	if (wps->dev_pw_id != DEV_PW_DEFAULT && wps->wps->dh_privkey &&
	    wps->wps->dh_ctx) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Using pre-configured DH keys");
		...
		pubkey = wpabuf_dup(wps->wps->dh_pubkey);
		   ...
		   (wps->dev_pw_id == wps->wps->ap_nfc_dev_pw_id ||
		    wps->wps->ap_nfc_dh_pubkey)) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Using NFC password token DH keys");
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Generate new DH keys");
		dh5_free(wps->dh_ctx);
		wps->dh_ctx = dh5_init(&wps->dh_privkey, &pubkey);
		pubkey = wpabuf_zeropad(pubkey, 192);
	}
	if (wps->dh_ctx == NULL || wps->dh_privkey == NULL || pubkey == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Failed to initialize "
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Request Type");
...
{
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Response Type (%d)", type);
...
int wps_build_uuid_e(struct wpabuf *msg, const u8 *uuid)
{
	if (wpabuf_tailroom(msg) < 4 + WPS_UUID_LEN)
		return -1;
	wpa_warning_log0(MSG_DEBUG, "WPS:  * UUID-E");
...
{
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Configuration Error (%d)", err);
...
int wps_build_authenticator(struct wps_data *wps, struct wpabuf *msg)
{
	...
	if (wps->last_msg == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Last message not available for "
	...
	(void)hmac_sha256_vector(wps->authkey, WPS_AUTHKEY_LEN, 2, addr, len, hash);

	wpa_warning_log0(MSG_DEBUG, "WPS:  * Authenticator");
...
int wps_build_version(struct wpabuf *msg)
{
	...
	if (wpabuf_tailroom(msg) < 5)
		return -1;
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Version (hardcoded 0x10)");
		      ...
		      u8 multi_ap_subelem)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Version2 (0x%x)", WPS_VERSION);
	wpabuf_put_u8(msg, WFA_ELEM_VERSION2);
	wpabuf_put_u8(msg, 1);
	wpabuf_put_u8(msg, WPS_VERSION);

	if (req_to_enroll) {
		wpa_warning_log0(MSG_DEBUG, "WPS:  * Request to Enroll (1)");
	...
	if (auth_macs && auth_macs_count) {
		size_t i;
		wpa_warning_log1(MSG_DEBUG, "WPS:  * AuthorizedMACs (count=%d)",
			   (int) auth_macs_count);
		...
		for (i = 0; i < auth_macs_count; i++)
			wpa_warning_log4(MSG_DEBUG, "WPS:    AuthorizedMAC: " "%02x:xx:xx:%02x:%02x:%02x",
				   (&auth_macs[i * ETH_ALEN])[0], (&auth_macs[i * ETH_ALEN])[3], (&auth_macs[i * ETH_ALEN])[4], (&auth_macs[i * ETH_ALEN])[5]);
	}
	...
	if (WPS_VERSION > 0x20) {
		if (wpabuf_tailroom(msg) < 5)
			return -1;
		wpa_warning_log0(MSG_DEBUG, "WPS:  * Extensibility Testing - extra "
			   "attribute");
...
{
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Message Type (%d)", msg_type);
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Enrollee Nonce");
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Registrar Nonce");
...
#endif /* CONFIG_WPS_TESTING */
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Authentication Type Flags (0x%x)",
		   auth_types);
...
#endif /* CONFIG_WPS_TESTING */
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Encryption Type Flags (0x%x)",
		   encr_types);
...
{
	(void)wps;
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Connection Type Flags");
...
{
	(void)wps;
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Association State");
...
int wps_build_key_wrap_auth(struct wps_data *wps, struct wpabuf *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Key Wrap Authenticator");
	(void)hmac_sha256(wps->authkey, WPS_AUTHKEY_LEN, wpabuf_head(msg),
		    wpabuf_len(msg), hash);
			    ...
			    struct wpabuf *plain)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS:  * Encrypted Settings");
			 ...
			 size_t dev_pw_len)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WPS:  * OOB Device Password (dev_pw_id=%u)",
		   dev_pw_id);
	...
	if (wps_corrupt_pkhash) {
		...
		wpa_warning_log0(MSG_INFO, "WPS: Testing - corrupt public key hash");
...
{
	wpa_warning_log1(MSG_DEBUG, "WPS:  * RF Bands (%x)", rf_bands);
...
{
	wpa_warning_log1(MSG_DEBUG, "WPS:  * AP Channel (%u)", ap_channel);
