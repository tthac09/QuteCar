	     ...
	     const char *label, u8 *res, size_t res_len)
{
	...
	for (i = 1; i <= iter; i++) {
		...
		(void)hmac_sha256_vector(key, SHA256_MAC_LEN, 4, addr, len, hash);
		if (i < iter) {
			os_memcpy(opos, hash, SHA256_MAC_LEN);
			opos += SHA256_MAC_LEN;
			left -= SHA256_MAC_LEN;
		} else
			(void)os_memcpy(opos, hash, left);
	}
}
...
int wps_derive_keys(struct wps_data *wps)
{
	...
	if (wps->dh_privkey == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Own DH private key not available");
		return -1;
	}

	pubkey = wps->registrar ? wps->dh_pubkey_e : wps->dh_pubkey_r;
	if (pubkey == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Peer DH public key not available");
		return -1;
	}
	...
	if (dh_shared == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Failed to derive DH shared key");
		return -1;
	}
	...
	(void)sha256_vector(1, addr, len, dhkey);
	...
	addr[2] = wps->nonce_r;
	len[2] = WPS_NONCE_LEN;
	(void)hmac_sha256_vector(dhkey, sizeof(dhkey), 3, addr, len, kdk);
					  ...
					  size_t encr_len)
{
	...
	{
		wpa_warning_log0(MSG_DEBUG, "WPS: No Encrypted Settings received");
		return NULL;
	}
	...
	if (pad > wpabuf_len(decrypted)) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Invalid PKCS#5 v2.0 pad value");
		wpabuf_clear_free(decrypted);
		return NULL;
	}
	for (i = 0; i < pad; i++) {
		if (*pos-- != pad) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Invalid PKCS#5 v2.0 pad "
				   "string");
			wpabuf_clear_free(decrypted);
			return NULL;
		}
				 ...
				 int channel)
{
	...
	if (plain == NULL) {
		wpa_error_log0(MSG_ERROR, "WPS: Failed to allocate memory for OOB "
			   "credential");
		return NULL;
	}
				       ...
				       const struct wpabuf *dev_pw)
{
	...
	if (wps_build_oob_dev_pw(data, dev_pw_id, pubkey,
		...
		wpa_error_log0(MSG_ERROR, "WPS: Failed to build NFC password "
			   "token");
...
int wps_oob_use_cred(struct wps_context *wps, struct wps_parse_attr *attr)
{
	...
	for (i = 0; i < attr->num_cred; i++) {
		...
		if (wps_parse_msg(&msg, &cattr) < 0 ||
			...
			wpa_error_log0(MSG_ERROR, "WPS: Failed to parse OOB "
				   "credential");
			return -1;
		}
...
void uuid_gen_mac_addr(const u8 *mac_addr, u8 *uuid)
{
	...
	addr[1] = mac_addr;
	len[1] = 6;
	(void)sha1_vector(2, addr, len, hash);
...
struct wpabuf * wps_build_wsc_ack(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message WSC_ACK");
...
struct wpabuf * wps_build_wsc_nack(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message WSC_NACK");
					   ...
					   struct wpabuf *nfc_dh_pubkey)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building attributes for NFC connection "
		   "handover request");

	if (nfc_dh_pubkey == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No NFC OOB Device Password "
			   "configured");
		return NULL;
	}
					       ...
					       struct wpabuf *nfc_dh_pubkey)
{
	...
	if (nfc_dh_pubkey == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No NFC DH Public Key configured");
		return NULL;
	}
