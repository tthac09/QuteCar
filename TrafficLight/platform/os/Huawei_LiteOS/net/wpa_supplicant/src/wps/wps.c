...
struct wps_data * wps_init(const struct wps_config *cfg)
{
	...
	if (cfg->assoc_wps_ie) {
		...
		if (wps_parse_msg(cfg->assoc_wps_ie, &attr) < 0) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Failed to parse WPS IE "
				   "from (Re)AssocReq");
		} else if (attr.request_type == NULL) {
			wpa_warning_log0(MSG_DEBUG, "WPS: No Request Type attribute "
				   "in (Re)AssocReq WPS IE");
		} else {
			wpa_warning_log1(MSG_DEBUG, "WPS: Request Type (from WPS IE "
...
void wps_deinit(struct wps_data *data)
{
	...
	if (data->wps_pin_revealed) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Full PIN information revealed and "
			   "negotiation failed");
		if (data->registrar)
			(void)wps_registrar_invalidate_pin(data->wps->registrar,
						     data->uuid_e);
	} else if (data->registrar)
		(void)wps_registrar_unlock_pin(data->wps->registrar, data->uuid_e);
#endif /* CONFIG_WPS_AP */
...
struct wpabuf * wps_build_assoc_req_ie(enum wps_request_type req_type)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building WPS IE for (Re)Association "
		   "Request");
	ie = wpabuf_alloc(100);
	if (ie == NULL)
		return NULL;
				       ...
				       const u8 *req_dev_types)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building WPS IE for Probe Request");
