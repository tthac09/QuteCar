...
static int wps_build_e_hash(struct wps_data *wps, struct wpabuf *msg)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS:  * E-Hash1");
	...
	wpa_warning_log0(MSG_DEBUG, "WPS:  * E-Hash2");
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * E-SNonce1");
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * E-SNonce2");
...
static struct wpabuf * wps_build_m1(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message M1");
...
static struct wpabuf * wps_build_m3(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message M3");

	if (wps->dev_password == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Device Password available");
		return NULL;
	}
...
static struct wpabuf * wps_build_m5(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message M5");
...
static int wps_build_cred_auth_type(struct wps_data *wps, struct wpabuf *msg)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Authentication Type (0x%x)", auth_type);
...
static int wps_build_cred_encr_type(struct wps_data *wps, struct wpabuf *msg)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WPS:  * Encryption Type (0x%x)", encr_type);
...
#else
	random_get_bytes(psk, sizeof(psk));
#endif /* CONFIG_NO_RANDOM_POOL */
		wpa_hexdump_key(MSG_DEBUG, "WPS: Generated per-device PSK",
				psk, sizeof(psk));
		wpa_warning_log1(MSG_DEBUG, "WPS:  * Network Key (len=%u)",
			   (unsigned int) wps->new_psk_len * 2);
		(void)wpa_snprintf_hex(hex, sizeof(hex), psk, sizeof(psk));
		wpabuf_put_be16(msg, ATTR_NETWORK_KEY);
		wpabuf_put_be16(msg, sizeof(psk) * 2);
		wpabuf_put_data(msg, hex, sizeof(psk) * 2);
#ifdef CONFIG_WPS_AP
		if (wps->wps->registrar) {
			(void)wps_cb_new_psk(wps->wps->registrar,
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS:  * MAC Address (AP BSSID)");
...
static int wps_build_ap_settings(struct wps_data *wps, struct wpabuf *plain)
{
	...
	if (wps->wps->ap_settings) {
		wpa_warning_log0(MSG_DEBUG, "WPS:  * AP Settings (pre-configured)");
		wpabuf_put_data(plain, wps->wps->ap_settings,
				wps->wps->ap_settings_len);
		return 0;
	}

	wpa_warning_log0(MSG_DEBUG, "WPS:  * AP Settings based on current configuration");
...
static struct wpabuf * wps_build_m7(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message M7");
...
static struct wpabuf * wps_build_wsc_done(struct wps_data *wps)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Building Message WSC_Done");
...
static int wps_process_enrollee_nonce(struct wps_data *wps, const u8 *e_nonce)
{
	...
	if (os_memcmp(wps->nonce_e, e_nonce, WPS_NONCE_LEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Invalid Enrollee Nonce received");
		return -1;
	}
...
{
	if (uuid_r == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No UUID-R received");
		return -1;
	}
			      ...
			      size_t pk_len)
{
	...
	if (wps->peer_pubkey_hash_set) {
		...
		(void)sha256_vector(1, &pk, &pk_len, hash);
		if (os_memcmp_const(hash, wps->peer_pubkey_hash,
				    WPS_OOB_PUBKEY_HASH_LEN) != 0) {
			wpa_error_log0(MSG_ERROR, "WPS: Public Key hash mismatch");
...
{
	if (r_hash1 == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No R-Hash1 received");
		return -1;
	}
...
{
	if (r_hash2 == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No R-Hash2 received");
		return -1;
	}
...
static int wps_process_r_snonce1(struct wps_data *wps, const u8 *r_snonce1)
{
	...
	if (r_snonce1 == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No R-SNonce1 received");
		return -1;
	}
	...
	if (os_memcmp_const(wps->peer_hash1, hash, WPS_HASH_LEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: R-Hash1 derived from R-S1 does "
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Registrar proved knowledge of the first "
...
static int wps_process_r_snonce2(struct wps_data *wps, const u8 *r_snonce2)
{
	...
	if (r_snonce2 == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No R-SNonce2 received");
		return -1;
	}
	...
	if (os_memcmp_const(wps->peer_hash2, hash, WPS_HASH_LEN) != 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: R-Hash2 derived from R-S2 does "
			   "not match with the pre-committed value");
		wps->config_error = WPS_CFG_DEV_PASSWORD_AUTH_FAILURE;
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Registrar proved knowledge of the second "
		   "half of the device password");
			      ...
			      size_t cred_len, int wps2)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Received Credential");
	...
	if (os_memcmp(wps->cred.mac_addr, wps->wps->dev.mac_addr, ETH_ALEN) !=
	    0) {
		wpa_warning_log4(MSG_DEBUG, "WPS: MAC Address in the Credential ("
			   "%02x:xx:xx:%02x:%02x:%02x" ")", (wps->cred.mac_addr)[0], (wps->cred.mac_addr)[3], (wps->cred.mac_addr)[4], (wps->cred.mac_addr)[5]);
		wpa_warning_log4(MSG_DEBUG, "does not match with own address (" "%02x:xx:xx:%02x:%02x:%02x"
			   ")", (wps->wps->dev.mac_addr)[0], (wps->wps->dev.mac_addr)[3], (wps->wps->dev.mac_addr)[4], (wps->wps->dev.mac_addr)[5]);
		/*
		...
		if (wps2) {
			wpa_warning_log0(MSG_INFO, "WPS: Do not accept incorrect "
	...
	if (!(wps->cred.encr_type &
		...
		if (wps->cred.encr_type & WPS_ENCR_WEP) {
			wpa_warning_log0(MSG_INFO, "WPS: Reject Credential "
				   "due to WEP configuration");
			wps->error_indication = WPS_EI_SECURITY_WEP_PROHIBITED;
			return -2;
		}

		wpa_warning_log1(MSG_INFO, "WPS: Reject Credential due to "
			     ...
			     u16 cred_len[], unsigned int num_cred, int wps2)
{
	...
	if (num_cred == 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No Credential attributes "
			   "received");
		return -1;
	}
	...
	for (i = 0; i < num_cred; i++) {
		...
		if (res == 0) {
			...
			wpa_warning_log0(MSG_DEBUG, "WPS: WEP credential skipped");
		}
		else {
			return -1;
		}
	...
	if (ok == 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No valid Credential attribute "
			   "received");
		return -1;
	}
				     ...
				     struct wpabuf *attrs, int wps2)
{
	...
	wpa_warning_log0(MSG_INFO, "WPS: Received new AP configuration from "
		   "Registrar");

	if (os_memcmp(cred.mac_addr, wps->wps->dev.mac_addr, ETH_ALEN) !=
	    0) {
		wpa_warning_log4(MSG_DEBUG, "WPS: MAC Address in the AP Settings ("
			   "%02x:xx:xx:%02x:%02x:%02x" ")", (cred.mac_addr)[0], (cred.mac_addr)[3], (cred.mac_addr)[4], (cred.mac_addr)[5]);
		wpa_warning_log4(MSG_DEBUG, "does not match with own address (" "%02x:xx:xx:%02x:%02x:%02x"
			   ")", (wps->wps->dev.mac_addr)[0], (wps->wps->dev.mac_addr)[3], (wps->wps->dev.mac_addr)[4], (wps->wps->dev.mac_addr)[5]);
		/*
		 * In theory, this could be consider fatal error, but there are
	...
	if (wps2) {
		if ((cred.encr_type & (WPS_ENCR_TKIP | WPS_ENCR_AES)) ==
		    ...
		    WPS_AUTH_WPAPSK) {
			wpa_warning_log0(MSG_INFO, "WPS-STRICT: Invalid WSC 2.0 "
...
static int wps_process_dev_pw_id(struct wps_data *wps, const u8 *dev_pw_id)
{
	...
	if (dev_pw_id == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Device Password ID");
		return -1;
	}

	id = WPA_GET_BE16(dev_pw_id);
	if (wps->dev_pw_id == id) {
		wpa_warning_log1(MSG_DEBUG, "WPS: Device Password ID %u", id);
		return 0;
	}
	...
	if ((id == DEV_PW_DEFAULT &&
		 ...
		 */
		wpa_warning_log0(MSG_DEBUG, "WPS: Allow PIN Device Password ID "
			   "change");
		return 0;
	}
#endif /* CONFIG_P2P */

	wpa_warning_log2(MSG_DEBUG, "WPS: Registrar trying to change Device Password "
	...
	if (wps->alt_dev_password && wps->alt_dev_pw_id == id) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Found a matching Device Password");
...
static enum wps_process_res wps_process_m2(struct wps_data *wps,
					   ...
					   struct wps_parse_attr *attr)
{
	wpa_warning_log0(MSG_DEBUG, "WPS: Received M2");
	...
	if (wps->wps->ap &&
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: AP Setup is locked - refuse "
	...
	if (wps->peer_pubkey_hash_set) {
		...
		if (decrypted == NULL) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Failed to decrypt "
				   "Encrypted Settings attribute");
			wps->state = SEND_WSC_NACK;
			return WPS_CONTINUE;
		}

		wpa_warning_log0(MSG_DEBUG, "WPS: Processing decrypted Encrypted "
			   "Settings attribute");
...
static enum wps_process_res wps_process_m2d(struct wps_data *wps,
					    struct wps_parse_attr *attr)
{
	wpa_warning_log0(MSG_DEBUG, "WPS: Received M2D");
					   ...
					   struct wps_parse_attr *attr)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Received M4");
					   ...
					   struct wps_parse_attr *attr)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Received M6");
					   ...
					   struct wps_parse_attr *attr)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Received M8");
						...
						const struct wpabuf *msg)
{
	...
	if (wps->state == RECV_ACK && wps->wps->ap) {
		wpa_warning_log0(MSG_DEBUG, "WPS: External Registrar registration "
						 ...
						 const struct wpabuf *msg)
{
	...
	wpa_warning_log1(MSG_DEBUG, "WPS: Registrar terminated negotiation with "
