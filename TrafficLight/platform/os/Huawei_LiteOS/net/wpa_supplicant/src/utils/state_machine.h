...
sm->data ## _ ## state = machine ## _ ## _state;
#else
#define HI_MAC2STR(a) (a)[0], (a)[3], (a)[4], (a)[5]
#define HI_MACSTR "%02x:xx:xx:%02x:%02x:%02x"

#define SM_ENTRY_MA(machine, _state, data) \
if (!global || sm->data ## _ ## state != machine ## _ ## _state) { \
	sm->changed = TRUE; \
	wpa_printf(MSG_DEBUG, STATE_MACHINE_DEBUG_PREFIX ": " HI_MACSTR " " \
		   #machine " entering state " #_state, \
		   HI_MAC2STR(STATE_MACHINE_ADDR)); \
} \
sm->data ## _ ## state = machine ## _ ## _state;
