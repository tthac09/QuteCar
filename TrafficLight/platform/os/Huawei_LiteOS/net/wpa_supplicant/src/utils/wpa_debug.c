...
void wpa_printf(int level, const char *fmt, ...)
{
	...
	if (wpa_debug_tracing_file != NULL) {
		...
		(void)vfprintf(wpa_debug_tracing_file, fmt, ap);
		fprintf(wpa_debug_tracing_file, "\n");
		(void)fflush(wpa_debug_tracing_file);
		va_end(ap);
	}
#endif /* CONFIG_DEBUG_LINUX_TRACING */
}
...
void wpa_msg(void *ctx, int level, const char *fmt, ...)
{
	...
	if (ctx == NULL) {
		wpa_error_log0(MSG_ERROR, "hostapd_ctrl_iface_msg_cb: ptr is null\n");
		return;
	}

	wifi_dev = hi_get_wifi_dev_by_priv(ctx);
	if (wifi_dev == NULL) {
		wpa_warning_log0(MSG_INFO, "wpa_msg: could not get wifi dev\n");
		return;
	}
	ifname = wifi_dev->ifname;
#endif /* LOS_WPA_PATCH */
	va_start(ap, fmt);
	...
	if (buf == NULL) {
		wpa_error_log0(MSG_ERROR, "wpa_msg: Failed to allocate message "
			   "buffer");
		return;
	}
	va_start(ap, fmt);
...
void wpa_msg_ctrl(void *ctx, int level, const char *fmt, ...)
{
	...
	buf = os_malloc(buflen);
	if (buf == NULL) {
		wpa_error_log0(MSG_ERROR, "wpa_msg_ctrl: Failed to allocate "
			   "message buffer");
		return;
	}
	va_start(ap, fmt);
...
void wpa_msg_global(void *ctx, int level, const char *fmt, ...)
{
	...
	buf = os_malloc(buflen);
	if (buf == NULL) {
		wpa_error_log0(MSG_ERROR, "wpa_msg_global: Failed to allocate "
			   "message buffer");
		return;
	}
	va_start(ap, fmt);
...
void wpa_msg_no_global(void *ctx, int level, const char *fmt, ...)
{
	...
	buf = os_malloc(buflen);
	if (buf == NULL) {
		wpa_error_log0(MSG_ERROR, "wpa_msg_no_global: Failed to allocate "
			   "message buffer");
		return;
	}
	va_start(ap, fmt);
...
void wpa_msg_global_only(void *ctx, int level, const char *fmt, ...)
{
	...
	buf = os_malloc(buflen);
	if (buf == NULL) {
		wpa_error_log0(MSG_ERROR, "wpa_msg_global_only: Failed to allocate message buffer");
		return;
	}
	va_start(ap, fmt);
	len = vsnprintf(buf, buflen, fmt, ap);
		    ...
		    const char *fmt, ...)
{
	...
	buf = os_malloc(buflen);
	if (buf == NULL) {
		wpa_error_log0(MSG_ERROR, "hostapd_logger: Failed to allocate "
			   "message buffer");
		return;
	}
	va_start(ap, fmt);
	...
	if (hostapd_logger_cb)
		...
		wpa_warning_log4(MSG_DEBUG, "hostapd_logger: STA " "%02x:xx:xx:%02x:%02x:%02x",
			   addr[0], addr[3], addr[4], addr[5]);
		wpa_warning_buf(MSG_DEBUG, "hostapd_logger: STA "" - %s",
			   buf, strlen(buf));
	}
	else {
		wpa_warning_buf(MSG_DEBUG, "hostapd_logger: %s", buf, strlen(buf));
	}
	bin_clear_free(buf, buflen);
}
