...
static int printf_encode_decode_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "printf encode/decode tests");
	...
	for (i = 0; printf_tests[i].data; i++) {
		...
		wpa_warning_log1(MSG_INFO, "printf_encode_decode_tests : %d", i);
		wpa_warning_buf(MSG_INFO, "printf_encode_decode_tests :\"%s\"", buf, strlen(buf));
	...
	if (buf[5] != 'A') {
		wpa_error_log0(MSG_ERROR, "Error in bounds checking#1");
		errors++;
	}
	...
	for (i = 5; i < 10; i++) {
		...
		if (buf[i] != 'A') {
			wpa_error_log1(MSG_ERROR, "Error in bounds checking#2(%d)",
				   i);
			errors++;
		}
	}
	...
	if (errors) {
		wpa_error_log1(MSG_ERROR, "%d printf test(s) failed", errors);
		return -1;
	}
...
static int bitfield_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "bitfield tests");
	...
	if (errors) {
		wpa_error_log1(MSG_ERROR, "%d bitfield test(s) failed", errors);
		return -1;
	}
...
static int int_array_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "int_array tests");
	...
	if (errors) {
		wpa_error_log1(MSG_ERROR, "%d int_array test(s) failed", errors);
		return -1;
	}
...
static int ext_password_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "ext_password tests");
...
{
	wpa_warning_log0(MSG_INFO, "trace tests");
...
static int base64_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "base64 tests");
	...
	if (errors) {
		wpa_error_log1(MSG_ERROR, "%d base64 test(s) failed", errors);
		return -1;
	}
...
static int common_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "common tests");
	...
	if (errors) {
		wpa_error_log1(MSG_ERROR, "%d common test(s) failed", errors);
		return -1;
	}
...
static int os_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "os tests");
	...
	if (errors) {
		wpa_error_log1(MSG_ERROR, "%d os test(s) failed", errors);
		return -1;
	}
...
static int wpabuf_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "wpabuf tests");
	...
	if (errors) {
		wpa_error_log1(MSG_ERROR, "%d wpabuf test(s) failed", errors);
		return -1;
	}
...
static int ip_addr_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "ip_addr tests");
	...
	if (errors) {
		wpa_error_log1(MSG_ERROR, "%d ip_addr test(s) failed", errors);
		return -1;
	}
...
static void eloop_test_read_2(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	wpa_warning_log1(MSG_INFO, "eloop_test_read_2: sock=%d", sock);

	if (t->magic != 0x12345678) {
		wpa_warning_log1(MSG_INFO, "eloop_test_read_2: unexpected magic 0x%x",
			   t->magic);
	}

	if (t->pipefd2[0] != sock) {
		wpa_warning_log2(MSG_INFO, "eloop_test_read_2: unexpected sock %d != %d",
			   sock, t->pipefd2[0]);
	}

	res = read(sock, buf, sizeof(buf));
	wpa_warning_log2(MSG_INFO, "eloop_test_read_2: sock=%d --> res=%d",
		   sock, (int) res);
}
...
static void eloop_test_read_2_wrong(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	wpa_warning_log1(MSG_INFO, "eloop_test_read_2_wrong: sock=%d", sock);

	if (t->magic != 0x12345678) {
		wpa_warning_log1(MSG_INFO, "eloop_test_read_2_wrong: unexpected magic 0x%x",
			   t->magic);
	}

	if (t->pipefd2[0] != sock) {
		wpa_warning_log2(MSG_INFO, "eloop_test_read_2_wrong: unexpected sock %d != %d",
			   sock, t->pipefd2[0]);
	}
	 ...
	 * read here.
	 */
	wpa_error_log0(MSG_ERROR, "eloop_test_read_2_wrong: FAIL - should not have called this function");
}
...
{
	if (t->pipefd2[0] < 0) {
		wpa_warning_log0(MSG_INFO, "pipefd2 had been closed");
	} else {
		int res;

		wpa_warning_log0(MSG_INFO, "close pipefd2");
		...
		res = pipe(t->pipefd2);
		if (res < 0) {
			wpa_warning_buf(MSG_INFO, "pipe: %s", strerror(errno), strlen(strerror(errno)));
			t->pipefd2[0] = -1;
			t->pipefd2[1] = -1;
			return;
		}
...
static void eloop_test_read_1(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	wpa_warning_log1(MSG_INFO, "eloop_test_read_1: sock=%d", sock);

	if (t->magic != 0x12345678) {
		wpa_warning_log1(MSG_INFO, "eloop_test_read_1: unexpected magic 0x%x",
				t->magic);
	}

	if (t->pipefd1[0] != sock) {
		wpa_warning_log2(MSG_INFO, "eloop_test_read_1: unexpected sock %d != %d",
			   sock, t->pipefd1[0]);
	}

	res = read(sock, buf, sizeof(buf));
	wpa_warning_log2(MSG_INFO, "eloop_test_read_1: sock=%d --> res=%d",
		   sock, (int) res);
...
static void eloop_test_cb(void *eloop_data, void *user_ctx)
{
	...
	wpa_warning_log0(MSG_INFO, "eloop_test_cb");

	if (t->magic != 0x12345678) {
		wpa_warning_log1(MSG_INFO, "eloop_test_cb: unexpected magic 0x%x",
			   t->magic);
	}
...
static void eloop_test_timeout(void *eloop_data, void *user_ctx)
{
	...
	wpa_warning_log0(MSG_INFO, "eloop_test_timeout");

	if (t->magic != 0x12345678) {
		wpa_warning_log1(MSG_INFO, "eloop_test_timeout: unexpected magic 0x%x",
			   t->magic);
	}

	if (t->pipefd1[0] >= 0) {
		wpa_warning_log0(MSG_INFO, "pipefd1 had not been closed");
	...
	if (t->pipefd2[0] >= 0) {
		wpa_warning_log0(MSG_INFO, "pipefd2 had not been closed");
		eloop_unregister_read_sock(t->pipefd2[0]);
		close(t->pipefd2[0]);
		t->pipefd2[0] = -1;
		close(t->pipefd2[1]);
	...
	t->magic = 0;
	wpa_warning_log1(MSG_INFO, "eloop_test_timeout - free(%p)", t);
	os_free(t);
...
static void eloop_tests_start(int close_in_timeout)
{
	...
	wpa_warning_log2(MSG_INFO, "starting eloop tests (%p) (close_in_timeout=%d)",
		   t, close_in_timeout);
	...
	wpa_warning_log4(MSG_INFO, "pipe fds: %d,%d %d,%d",
	...
	if (write(t->pipefd1[1], "HELLO", 5) < 0)
		wpa_warning_buf(MSG_INFO, "write: %s", strerror(errno), strlen(strerror(errno)));
	if (write(t->pipefd2[1], "TEST", 4) < 0)
		wpa_warning_buf(MSG_INFO, "write: %s", strerror(errno), strlen(strerror(errno)));
	os_sleep(0, 50000);
	wpa_warning_log0(MSG_INFO, "waiting for eloop callbacks");
}
...
{
	wpa_warning_log0(MSG_INFO, "schedule eloop tests to be run");
...
static int json_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "JSON tests");
	...
	for (i = 0; i < ARRAY_SIZE(json_test_cases); i++) {
		...
		if ((root && !test->tree) || (!root && test->tree)) {
			wpa_warning_log1(MSG_INFO, "JSON test %u failed", i);
			res = -1;
		} else if (root) {
...
int utils_module_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "utils module tests");
