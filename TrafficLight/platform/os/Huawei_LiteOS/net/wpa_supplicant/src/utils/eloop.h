...
#ifdef LOS_WPA_PATCH
/**
 * eloop_unregister_cli_event - Unregister handler for a generic event
 * @event: Event to cancel (eloop implementation specific)
 * @event_size: Size of event data
 *
 * Unregister a generic event notifier that was previously registered with
 * eloop_unregister_cli_event().
 */
void eloop_unregister_cli_event(void *event, size_t event_size);
#endif /* LOS_WPA_PATCH */
