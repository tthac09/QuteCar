...
EAP_PWD_group * get_eap_pwd_group(u16 num)
{
	...
	if (!grp->group) {
		wpa_warning_log0(MSG_INFO, "EAP-pwd: unable to create EC group");
		os_free(grp);
		return NULL;
	}

	grp->group_num = num;
	wpa_warning_log1(MSG_INFO, "EAP-pwd: provisioned group %d", num);
			     ...
			     const u8 *token)
{
	...
	if ((prfbuf = os_malloc(primebytelen)) == NULL) {
		wpa_warning_log0(MSG_INFO, "EAP-pwd: unable to malloc space for prf "
			   "buffer");
		goto fail;
	}
	...
	if (!crypto_ec_point_is_on_curve(grp->group, grp->pwe)) {
		wpa_warning_log0(MSG_INFO, "EAP-pwd: point is not on curve");
		goto fail;
	}

	wpa_warning_log0(MSG_DEBUG, "EAP-pwd: found a PWE in  tries");
					     ...
					     const u8 *buf)
{
	...
	if (!eap_pwd_element_coord_ok(prime, buf, prime_len) ||
		...
		wpa_warning_log0(MSG_INFO, "EAP-pwd: Invalid coordinate in element");
		return NULL;
	}

	element = crypto_ec_point_from_bin(group->group, buf);
	if (!element) {
		wpa_warning_log0(MSG_INFO, "EAP-pwd: EC point from element failed");
		return NULL;
	}
	...
	if (!crypto_ec_point_is_on_curve(group->group, element) ||
		...
		wpa_warning_log0(MSG_INFO, "EAP-pwd: Invalid element");
		goto fail;
	}
...
struct crypto_bignum * eap_pwd_get_scalar(EAP_PWD_group *group, const u8 *buf)
{
	...
	if (!scalar || crypto_bignum_is_zero(scalar) ||
		...
		wpa_warning_log0(MSG_INFO, "EAP-pwd: received scalar is invalid");
