...
static int ieee802_11_parse_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "ieee802_11_parse tests");
	...
	for (i = 0; parse_tests[i].data; i++) {
		...
		if (res != test->result ||
			...
			wpa_error_log1(MSG_ERROR, "ieee802_11_parse test %d failed",
				   i);
			ret = -1;
		}
	}
...
static int rsn_ie_parse_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "rsn_ie_parse tests");
	...
	for (i = 0; rsn_parse_tests[i].data; i++) {
		...
		if (wpa_parse_wpa_ie_rsn(test->data, test->len, &data) !=
		    test->result) {
			wpa_error_log1(MSG_ERROR, "rsn_ie_parse test %d failed", i);
			ret = -1;
		}
	}
...
static int gas_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "gas tests");
...
static int sae_tests(void)
{
	...
	if (wpabuf_len(buf) != sizeof(local_commit) ||
		...
		wpa_warning_log0(MSG_ERROR, "SAE: Mismatch in local commit");
		goto fail;
	}
	...
	if (os_memcmp(kck, sae.tmp->kck, SAE_KCK_LEN) != 0) {
		wpa_warning_log0(MSG_ERROR, "SAE: Mismatch in KCK");
		goto fail;
	}

	if (os_memcmp(pmk, sae.pmk, SAE_PMK_LEN) != 0) {
		wpa_warning_log0(MSG_ERROR, "SAE: Mismatch in PMK");
		goto fail;
	}

	if (os_memcmp(pmkid, sae.pmkid, SAE_PMKID_LEN) != 0) {
		wpa_warning_log0(MSG_ERROR, "SAE: Mismatch in PMKID");
		goto fail;
	}
	...
	if (wpabuf_len(buf) != sizeof(local_confirm) ||
		...
		wpa_warning_log0(MSG_ERROR, "SAE: Mismatch in local confirm");
		goto fail;
	}
...
int common_module_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "common module tests");
