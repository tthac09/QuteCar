...
int hw_get_freq(struct hostapd_hw_modes *mode, int chan)
{
	...
	(void)hw_get_channel_chan(mode, chan, &freq);
...
int hw_get_chan(struct hostapd_hw_modes *mode, int freq)
{
	...
	(void)hw_get_channel_freq(mode, freq, &chan);
			      ...
			      int sec_chan)
{
	...
	if ((s_chan->flag & HOSTAPD_CHAN_DISABLED) ||
		...
		wpa_error_log1(MSG_ERROR, "HT40 secondary channel %d not allowed",
			   sec_chan);
		return 0;
	}
	...
	if (!ok) {
		wpa_error_log2(MSG_ERROR, "HT40 channel pair (%d, %d) not allowed",
			   pri_chan, sec_chan);
		return 0;
	}
...
void get_pri_sec_chan(struct wpa_scan_res *bss, int *pri_chan, int *sec_chan)
{
	...
	*pri_chan = *sec_chan = 0;
	...
	(void)ieee802_11_parse_elems((u8 *) (bss + 1), bss->ie_len, &elems, 0);
	...
	if (!match) {
		for (i = 0; i < scan_res->num; i++) {
			...
			if (pri_chan == bss_sec_chan &&
				...
				wpa_warning_log4(MSG_INFO, "Switch own primary and "
					   "secondary channel due to BSS "
					   "overlap with " "%02x:xx:xx:%02x:%02x:%02x",
			   ...
			   int end)
{
	...
	if (!elems.ht_capabilities) {
		wpa_warning_log4(MSG_DEBUG, "Found overlapping legacy BSS: "
			   "%02x:xx:xx:%02x:%02x:%02x", (bss->bssid)[0], (bss->bssid)[3], (bss->bssid)[4], (bss->bssid)[5]);
		wpa_warning_log1(MSG_DEBUG, " freq=%d", bss->freq);
		return 1;
	}
	...
	if (elems.ht_operation) {
		...
		wpa_warning_log4(MSG_DEBUG, "Found overlapping 20 MHz HT BSS: "
			   "%02x:xx:xx:%02x:%02x:%02x", (bss->bssid)[0], (bss->bssid)[3], (bss->bssid)[4], (bss->bssid)[5]);
		wpa_warning_log1(MSG_DEBUG," freq=%d", bss->freq);
		return 1;
	}
		    ...
		    int sec_chan)
{
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		wpa_warning_log4(MSG_DEBUG, "Neighboring BSS: " "%02x:xx:xx:%02x:%02x:%02x",
			   (bss->bssid)[0], (bss->bssid)[3], (bss->bssid)[4], (bss->bssid)[5]);
		wpa_warning_log3(MSG_DEBUG,
			   " freq=%d pri=%d sec=%d", bss->freq, pri_chan, sec_chan);
		if (sec_chan) {
		...
		(void)ieee802_11_parse_elems((u8 *) (bss + 1), bss->ie_len, &elems,
				       0);
		if (elems.ht_capabilities) {
			...
			if (le_to_host16(ht_cap->ht_capabilities_info) &
				...
				wpa_warning_log1(MSG_DEBUG,
					   "40 MHz Intolerant is set on channel %d in BSS ", pri);
				wpa_warning_log4(MSG_DEBUG,
					   "%02x:xx:xx:%02x:%02x:%02x", (bss->bssid)[0], (bss->bssid)[3], (bss->bssid)[4], (bss->bssid)[5]);
				return 0;
			}
