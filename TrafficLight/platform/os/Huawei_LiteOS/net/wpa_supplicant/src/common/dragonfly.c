...
{
#define DRAGONFLY_MAX_QR_QNR_CNT 500
	...
	*qr = *qnr = NULL;
	...
	while (!(*qr) || !(*qnr)) {
		...
		count++;
		if (count >= DRAGONFLY_MAX_QR_QNR_CNT) {
			wpa_warning_log0(MSG_DEBUG, "too many loops to get qr and qnr!");
			break;
		}
	...
	*qr = *qnr = NULL;
	return -1;
#undef DRAGONFLY_MAX_QR_QNR_CNT
}
