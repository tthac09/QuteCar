...
/* Do not support IEEE8021X SUITE*/
...
int wpa_use_cmac(int akmp)
{
	...
	return akmp == WPA_KEY_MGMT_OSEN ||
		...
		wpa_key_mgmt_suite_b(akmp);
#else
	return wpa_key_mgmt_sha256(akmp) ||
		wpa_key_mgmt_sae(akmp);
#endif /* HISI_WPA_KEY_MGMT_CROP */
}
		      ...
		      const u8 *buf, size_t len, u8 *mic)
{
	...
	case WPA_KEY_INFO_TYPE_HMAC_MD5_RC4:
		...
		wpa_warning_log0(MSG_DEBUG, "WPA: EAPOL-Key MIC using HMAC-MD5");
#endif /* CONFIG_PRINT_NOUSE */
		return hmac_md5(key, key_len, buf, len, mic);
#endif /* CONFIG_FIPS */
	case WPA_KEY_INFO_TYPE_HMAC_SHA1_AES:
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log0(MSG_DEBUG, "WPA: EAPOL-Key MIC using HMAC-SHA1");
#endif /* CONFIG_PRINT_NOUSE */
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log0(MSG_DEBUG, "WPA: EAPOL-Key MIC using AES-CMAC");
#endif /* CONFIG_PRINT_NOUSE */
...
#define MAX_Z_LEN 66 /* with NIST P-521 */
	...
	if (wpa_key_mgmt_sha384(akmp)) {
		...
		wpa_warning_log0(MSG_DEBUG, "WPA: PTK derivation using PRF(SHA384)");
...
#endif /* HISI_WPA_KEY_MGMT_CROP */
#ifndef HISI_WPA_KEY_MGMT_CROP
	if (wpa_key_mgmt_sha256(akmp) || akmp == WPA_KEY_MGMT_OWE) {
#else
	if (wpa_key_mgmt_sha256(akmp)) {
#endif /* HISI_WPA_KEY_MGMT_CROP */
#if defined(CONFIG_IEEE80211W) || defined(CONFIG_SAE)
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log0(MSG_DEBUG, "WPA: PTK derivation using PRF(SHA256)");
#endif /* CONFIG_PRINT_NOUSE */
		...
		wpa_warning_log0(MSG_DEBUG, "WPA: PTK derivation using PRF(SHA512)");
		if (sha512_prf(pmk, pmk_len, label, data, data_len,
			       tmp, ptk_len) < 0)
			return -1;
	} else if (akmp == WPA_KEY_MGMT_DPP) {
		wpa_warning_log1(MSG_INFO, "DPP: Unknown PMK length %u",
			   (unsigned int) pmk_len);
		return -1;
#endif /* CONFIG_DPP */
	} else {
#ifndef CONFIG_PRINT_NOUSE
		wpa_warning_log0(MSG_DEBUG, "WPA: PTK derivation using PRF(SHA1)");
#endif /* CONFIG_PRINT_NOUSE */
		     ...
		     size_t dh_ss_len, u8 *pmk, size_t *pmk_len)
{
	...
	wpa_warning_log0(MSG_DEBUG, "FILS: rMSK to PMK derivation");
		    ...
		    u8 *fils_ft, size_t *fils_ft_len)
{
	...
	if (wpa_key_mgmt_sha384(akmp)) {
		wpa_warning_log0(MSG_DEBUG, "FILS: PTK derivation using PRF(SHA384)");
		if (sha384_prf(pmk, pmk_len, label, data, data_len,
			       tmp, key_data_len) < 0)
			goto err;
	} else {
		wpa_warning_log0(MSG_DEBUG, "FILS: PTK derivation using PRF(SHA256)");
		if (sha256_prf(pmk, pmk_len, label, data, data_len,
			       tmp, key_data_len) < 0)
			goto err;
	}
	       ...
	       const u8 *ric, size_t ric_len, u8 *mic)
{
	...
	if (kck_len == 16) {
		...
		wpa_warning_log1(MSG_WARNING, "FT: Unsupported KCK length %u",
			     ...
			     struct wpa_ft_ies *parse, int use_sha384)
{
	...
	while (end - pos >= 2) {
		...
		len = *pos++;
		if (len > end - pos) {
			wpa_warning_log0(MSG_DEBUG, "FT: Truncated subelement");
			break;
		}
...
#endif /* CONFIG_OCV */
		default:
			wpa_warning_log1(MSG_DEBUG, "FT: Unknown subelem id %u", id);
			break;
		}
		     ...
		     struct wpa_ft_ies *parse, int use_sha384)
{
	...
	while (end - pos >= 2) {
		...
		case WLAN_EID_RSN:
			...
			ret = wpa_parse_wpa_ie_rsn(parse->rsn - 2,
						   ...
						   &data);
			if (ret < 0) {
				wpa_warning_log1(MSG_DEBUG, "FT: Failed to parse "
					   "RSN IE: %d", ret);
				return -1;
			}
	...
	if (parse->ftie)
		prot_ie_count--;
	if (prot_ie_count < 0) {
		wpa_warning_log0(MSG_DEBUG, "FT: Some required IEs not included in "
			   "the protected IE count");
		return -1;
	}

	if (prot_ie_count == 0 && parse->ric) {
		wpa_warning_log0(MSG_DEBUG, "FT: RIC IE(s) in the frame, but not "
	...
	if (prot_ie_count) {
		wpa_warning_log1(MSG_DEBUG, "FT: %d protected IEs missing from "
		      ...
		      u8 *pmk_r1, u8 *pmk_r1_name)
{
	...
	if (pmk_r0_len != SHA384_MAC_LEN && pmk_r0_len != PMK_LEN) {
		wpa_error_log1(MSG_ERROR, "FT: Unexpected PMK-R0 length %d",
			   (int) pmk_r0_len);
		return -1;
	}
#ifndef HISI_CODE_CROP
	       ...
	       u8 *pmkid, int akmp)
{
	...
	if (0) {
		...
		wpa_warning_log0(MSG_DEBUG, "RSN: Derive PMKID using HMAC-SHA-384");
	...
	(void)akmp;
		...
		wpa_warning_log0(MSG_DEBUG, "RSN: Derive PMKID using HMAC-SHA-256");
		hmac_sha256_vector(pmk, pmk_len, 3, addr, len, hash);
#endif /* CONFIG_IEEE80211W || CONFIG_FILS */
	} else {
		wpa_warning_log0(MSG_DEBUG, "RSN: Derive PMKID using HMAC-SHA-1");
...
int wpa_insert_pmkid(u8 *ies, size_t *ies_len, const u8 *pmkid)
{
	...
	if (start >= end) {
		wpa_error_log0(MSG_ERROR, "FT: Could not find RSN IE in "
			   "IEs data");
		return -1;
	}
	...
	if (rpos == rend) {
		...
		*rpos++ = 0;
		...
		rpos += 2;
		if (rpos > rend) {
			wpa_error_log0(MSG_ERROR, "FT: Could not parse RSN IE in "
				   "IEs data");
			return -1;
		}
	}
...
int wpa_write_ciphers(char *start, char *end, int ciphers, const char *delim)
{
	...
	int ret;
	unsigned int index;
	struct cipher_Types cipherTypes[] = { { WPA_CIPHER_CCMP_256, "\"%sCCMP-256\"" },
										  { WPA_CIPHER_GCMP_256, "\"%sGCMP-256\""},
										  { WPA_CIPHER_CCMP, "\"%sCCMP\"" },
										  { WPA_CIPHER_GCMP, "\"%sGCMP\"" },
										  { WPA_CIPHER_TKIP, "\"%sTKIP\"" },
										  { WPA_CIPHER_AES_128_CMAC, "\"%sAES-128-CMAC\"" },
										  { WPA_CIPHER_BIP_GMAC_128, "\"%sBIP-GMAC-128\"" },
										  { WPA_CIPHER_BIP_GMAC_256, "\"%sBIP-GMAC-256\"" },
										  { WPA_CIPHER_BIP_CMAC_256, "\"%sBIP-CMAC-256\"" },
										  { WPA_CIPHER_NONE, "\"%sNONE\"" } };

	for (index = 0; index < sizeof(cipherTypes) / sizeof(struct cipher_Types); index++) {
		if (ciphers & cipherTypes[index].cipher_type) {
			ret = os_snprintf(pos, end - pos, cipherTypes[index].cipherTypesString,
					  pos == start ? "" : delim);
