...
static void eap_wsc_ext_reg_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Timeout while waiting for an External "
		   "Registrar");
...
static void * eap_wsc_init(struct eap_sm *sm)
{
	...
	if (registrar) {
		if (sm->wps == NULL || sm->wps->registrar == NULL) {
			wpa_warning_log0(MSG_INFO, "EAP-WSC: WPS Registrar not "
				   "initialized");
			os_free(data);
			return NULL;
		}
		...
		if (sm->user == NULL || sm->user->password == NULL) {
			 ...
			 */
			wpa_warning_log0(MSG_DEBUG, "EAP-WSC: No AP PIN (password) "
...
#ifdef CONFIG_P2P
	if (sm->assoc_p2p_ie) {
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Prefer PSK format for P2P "
			   "client");
		cfg.use_psk_key = 1;
...
static void eap_wsc_reset(struct eap_sm *sm, void *priv)
{
	...
	(void)eloop_cancel_timeout(eap_wsc_ext_reg_timeout, sm, data);
					   ...
					   struct eap_wsc_data *data, u8 id)
{
	...
	req = eap_msg_alloc(EAP_VENDOR_WFA, EAP_VENDOR_TYPE_WSC, 2,
			    EAP_CODE_REQUEST, id);
	if (req == NULL) {
		wpa_error_log0(MSG_ERROR, "EAP-WSC: Failed to allocate memory for "
			   "request");
		return NULL;
	}

	wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Send WSC/Start");
...
static struct wpabuf * eap_wsc_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	case MESG:
		if (data->out_buf == NULL) {
			data->out_buf = wps_get_msg(data->wps,
						    &data->out_op_code);
			if (data->out_buf == NULL) {
				wpa_warning_log0(MSG_DEBUG, "EAP-WSC: Failed to "
	...
	default:
		wpa_warning_log1(MSG_DEBUG, "EAP-WSC: Unexpected state %d in "
			   "buildReq", data->state);
		return NULL;
	}
}
			     ...
			     struct wpabuf *respData)
{
	...
	pos = eap_hdr_validate(EAP_VENDOR_WFA, EAP_VENDOR_TYPE_WSC,
			       respData, &len);
	if (pos == NULL || len < 2) {
		wpa_warning_log0(MSG_INFO, "EAP-WSC: Invalid frame");
		return TRUE;
	}
				...
				const u8 *buf, size_t len, u8 op_code)
{
	...
	wpa_warning_log2(MSG_DEBUG, "EAP-WSC: Received %lu bytes, waiting for %lu "
				    ...
				    const u8 *buf, size_t len)
{
	...
	if (data->in_buf == NULL && !(flags & WSC_FLAGS_LF)) {
		wpa_warning_log0(MSG_DEBUG, "EAP-WSC: No Message Length "
	...
	if (data->in_buf == NULL) {
		...
		wpabuf_put_data(data->in_buf, buf, len);
		wpa_warning_log2(MSG_DEBUG, "EAP-WSC: Received %lu bytes in "
			    ...
			    struct wpabuf *respData)
{
	...
	case WPS_PENDING:
		...
		(void)eloop_cancel_timeout(eap_wsc_ext_reg_timeout, sm, data);
		(void)eloop_register_timeout(5, 0, eap_wsc_ext_reg_timeout,
				       sm, data);
		break;
	}
