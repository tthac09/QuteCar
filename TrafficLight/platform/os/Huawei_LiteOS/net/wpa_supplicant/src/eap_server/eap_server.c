	...
	if (eap_sm_Policy_doPickUp(sm, sm->respMethod)) {
		...
		if (sm->m && sm->m->initPickUp) {
			...
			if (sm->eap_method_priv == NULL) {
				wpa_warning_log1(MSG_DEBUG, "EAP: Failed to "
...
SM_STATE(EAP, SEND_REQUEST)
{
	...
	if (sm->eap_if.eapReqData) {
		...
		wpa_warning_log0(MSG_INFO, "EAP: SEND_REQUEST - no eapReqData");
		sm->eap_if.eapResp = FALSE;
		sm->eap_if.eapReq = FALSE;
		sm->eap_if.eapNoReq = TRUE;
	}
...
SM_STATE(EAP, METHOD_REQUEST)
{
	...
	if (sm->m == NULL) {
		wpa_warning_log0(MSG_DEBUG, "EAP: method not initialized");
		return;
	}

	sm->currentId = eap_sm_nextId(sm, sm->currentId);
	wpa_warning_log1(MSG_DEBUG, "EAP: building EAP-Request: Identifier %d",
		   sm->currentId);
...
static void eap_server_erp_init(struct eap_sm *sm)
{
	...
	if (hmac_sha256_kdf(sm->eap_if.eapSessionId, sm->eap_if.eapSessionIdLen,
		...
		wpa_warning_log0(MSG_DEBUG, "EAP: Could not derive EMSKname");
		goto fail;
	}
	...
	if (hmac_sha256_kdf(emsk, emsk_len,
		...
		wpa_warning_log0(MSG_DEBUG, "EAP: Could not derive rRK for ERP");
		goto fail;
	}
	...
	if (hmac_sha256_kdf(erp->rRK, erp->rRK_len,
		...
		wpa_warning_log0(MSG_DEBUG, "EAP: Could not derive rIK for ERP");
		goto fail;
	}
	...
	if (eap_erp_add_key(sm, erp) == 0) {
		wpa_warning_buf(MSG_DEBUG, "EAP: Stored ERP keys %s",
			   erp->keyname_nai, strlen(erp->keyname_nai));
		erp = NULL;
	}
...
try_another_method:
	...
	if (sm->m) {
		...
		if (sm->eap_method_priv == NULL) {
			wpa_warning_log1(MSG_DEBUG, "EAP: Failed to initialize EAP "
	...
	if (sm->m == NULL) {
		wpa_warning_log0(MSG_DEBUG, "EAP: Could not find suitable EAP method");
				   ...
				   u8 flags, u16 seq, const char *nai)
{
	...
	if (hmac_sha256_kdf(erp->rRK, erp->rRK_len,
		...
		wpa_warning_log0(MSG_DEBUG, "EAP: Could not derive rMSK for ERP");
...
SM_STATE(EAP, INITIATE_RECEIVED)
{
	...
	if (pos == NULL) {
		wpa_warning_log0(MSG_INFO, "EAP-Initiate: Invalid frame");
		goto fail;
	}
	...
	if (len < 4) {
		wpa_warning_log0(MSG_INFO, "EAP: Too short EAP-Initiate/Re-auth");
		goto fail;
	}
	end = pos + len;
...
SM_STATE(EAP, SEND_REQUEST2)
{
	...
	if (sm->eap_if.eapReqData) {
		...
		wpa_warning_log0(MSG_INFO, "EAP: SEND_REQUEST2 - no eapReqData");
		sm->eap_if.eapResp = FALSE;
		sm->eap_if.eapReq = FALSE;
		sm->eap_if.eapNoReq = TRUE;
	}
...
SM_STATE(EAP, AAA_REQUEST)
{
	...
	if (sm->eap_if.eapRespData == NULL) {
		wpa_warning_log0(MSG_INFO, "EAP: AAA_REQUEST - no eapRespData");
		return;
	}
	...
	(void)eap_copy_buf(&sm->eap_if.aaaEapRespData, sm->eap_if.eapRespData);
}
...
SM_STATE(EAP, AAA_RESPONSE)
{
	...
	(void)eap_copy_buf(&sm->eap_if.eapReqData, sm->eap_if.aaaEapReqData);
...
SM_STATE(EAP, SUCCESS2)
{
	...
	if (sm->eap_if.aaaEapKeyAvailable) {
		(void)EAP_COPY(&sm->eap_if.eapKeyData, sm->eap_if.aaaEapKeyData);
	} else {
		bin_clear_free(sm->eap_if.eapKeyData, sm->eap_if.eapKeyDataLen);
		sm->eap_if.eapKeyData = NULL;
		sm->eap_if.eapKeyDataLen = 0;
...
SM_STEP(EAP)
{
	if (sm->eap_if.eapRestart && sm->eap_if.portEnabled)
		...
		if (sm->num_rounds == EAP_MAX_AUTH_ROUNDS + 1) {
			wpa_warning_log1(MSG_DEBUG, "EAP: more than %d "
	...
	case EAP_RECEIVED:
		if (sm->rxResp && (sm->respId == sm->currentId) &&
			...
			wpa_warning_log4(MSG_DEBUG, "EAP: RECEIVED->DISCARD: "
				   "rxResp=%d respId=%d currentId=%d "
				   "respMethod=%d",
				   sm->rxResp, sm->respId, sm->currentId,
				   sm->respMethod);
			wpa_warning_log1(MSG_DEBUG, "currentMethod=%d",
				   sm->currentMethod);
				   ...
				   int methodTimeout)
{
	...
	if (methodTimeout) {
		 ...
		 */
		wpa_warning_log1(MSG_DEBUG, "EAP: retransmit timeout %d seconds "
	...
	wpa_warning_log2(MSG_DEBUG, "EAP: retransmit timeout %d seconds "
...
static void eap_sm_parseEapResp(struct eap_sm *sm, const struct wpabuf *resp)
{
	...
	if (resp == NULL || wpabuf_len(resp) < sizeof(*hdr)) {
		wpa_warning_log2(MSG_DEBUG, "EAP: parseEapResp: invalid resp=%p "
			   "len=%lu", resp,
	...
	if (plen > wpabuf_len(resp)) {
		wpa_warning_log2(MSG_DEBUG, "EAP: Ignored truncated EAP-Packet "
			   "(len=%lu plen=%lu)",
	...
	if (plen > sizeof(*hdr)) {
		...
		if (sm->respMethod == EAP_TYPE_EXPANDED) {
			if (plen < sizeof(*hdr) + 8) {
				wpa_warning_log1(MSG_DEBUG, "EAP: Ignored truncated "
	...
	wpa_warning_log4(MSG_DEBUG,
		   "EAP: parseEapResp: rxResp=%d rxInitiate=%d respId=%d respMethod=%u",
		   sm->rxResp, sm->rxInitiate, sm->respId, sm->respMethod);
	wpa_warning_log2(MSG_DEBUG,
		   "respVendor=%u respVendorMethod=%u",
...
static int eap_sm_getId(const struct wpabuf *data)
{
	...
	wpa_warning_log1(MSG_DEBUG, "EAP: getId: id=%d", hdr->identifier);
...
static struct wpabuf * eap_sm_buildSuccess(struct eap_sm *sm, u8 id)
{
	struct wpabuf *msg;
	struct eap_hdr *resp;
	wpa_warning_log1(MSG_DEBUG, "EAP: Building EAP-Success (id=%d)", id);
...
static struct wpabuf * eap_sm_buildFailure(struct eap_sm *sm, u8 id)
{
	struct wpabuf *msg;
	struct eap_hdr *resp;
	wpa_warning_log1(MSG_DEBUG, "EAP: Building EAP-Failure (id=%d)", id);
...
void eap_sm_process_nak(struct eap_sm *sm, const u8 *nak_list, size_t len)
{
	...
	wpa_msgdump_log1(MSG_MSGDUMP, "EAP: processing NAK (current EAP method "
	...
	not_found:
		...
		if (i + 1 < EAP_MAX_METHODS) {
			(void)os_memmove(&sm->user->methods[i],
				   &sm->user->methods[i + 1],
				   (EAP_MAX_METHODS - i - 1) *
				 ...
				 size_t len)
{
	...
	if (sm->user->phase2) {
		wpa_warning_log0(MSG_DEBUG, "EAP: EAP-Nak received after Phase2 user"
...
static EapType eap_sm_Policy_getNextMethod(struct eap_sm *sm, int *vendor)
{
	...
	wpa_warning_log2(MSG_DEBUG, "EAP: getNextMethod: vendor %d type %d",
		   *vendor, next);
	return next;
}
...
static int eap_sm_Policy_getDecision(struct eap_sm *sm)
{
	if (!sm->eap_server && sm->identity && !sm->start_reauth) {
		wpa_warning_log0(MSG_DEBUG, "EAP: getDecision: -> PASSTHROUGH");
		return DECISION_PASSTHROUGH;
	}

	if (sm->m && sm->currentMethod != EAP_TYPE_IDENTITY &&
	    sm->m->isSuccess(sm, sm->eap_method_priv)) {
		wpa_warning_log0(MSG_DEBUG, "EAP: getDecision: method succeeded -> "
			   "SUCCESS");
		sm->update_user = TRUE;
	...
	if (sm->m && sm->m->isDone(sm, sm->eap_method_priv) &&
	    !sm->m->isSuccess(sm, sm->eap_method_priv)) {
		wpa_warning_log0(MSG_DEBUG, "EAP: getDecision: method failed -> "
			   "FAILURE");
		sm->update_user = TRUE;
	...
	if ((sm->user == NULL || sm->update_user) && sm->identity &&
		...
		if (eap_user_get(sm, sm->identity, sm->identity_len, 0) != 0) {
			wpa_warning_log0(MSG_DEBUG, "EAP: getDecision: user not "
		...
		if (id_req && sm->user &&
		    sm->user->methods[0].vendor == EAP_VENDOR_IETF &&
		    sm->user->methods[0].method == EAP_TYPE_IDENTITY) {
			wpa_warning_log0(MSG_DEBUG, "EAP: getDecision: stop "
	...
	if (sm->user && sm->user_eap_method_index < EAP_MAX_METHODS &&
	    (sm->user->methods[sm->user_eap_method_index].vendor !=
	     ...
	     EAP_TYPE_NONE)) {
		wpa_warning_log0(MSG_DEBUG, "EAP: getDecision: another method "
			   "available -> CONTINUE");
	...
	if (sm->identity == NULL || sm->currentId == -1) {
		wpa_warning_log0(MSG_DEBUG, "EAP: getDecision: no identity known "
			   "yet -> CONTINUE");
		return DECISION_CONTINUE;
	}

	wpa_warning_log0(MSG_DEBUG, "EAP: getDecision: no more methods available -> "
		   "FAILURE");
	return DECISION_FAILURE;
}
				   ...
				   struct eap_config *conf)
{
	...
	wpa_warning_log0(MSG_DEBUG, "EAP: Server state machine created");
...
void eap_server_sm_deinit(struct eap_sm *sm)
{
	if (sm == NULL)
		return;
	wpa_warning_log0(MSG_DEBUG, "EAP: Server state machine removed");
	if (sm->m && sm->eap_method_priv)
		sm->m->reset(sm, sm->eap_method_priv);
	wpabuf_free(sm->eap_if.eapReqData);
	bin_clear_free(sm->eap_if.eapKeyData, sm->eap_if.eapKeyDataLen);
...
void eap_sm_pending_cb(struct eap_sm *sm)
{
	if (sm == NULL)
		return;
	wpa_warning_log0(MSG_DEBUG, "EAP: Callback for pending request received");
	if (sm->method_pending == METHOD_PENDING_WAIT)
