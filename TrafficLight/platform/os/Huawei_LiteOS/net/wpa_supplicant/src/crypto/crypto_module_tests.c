...
static int test_siv(void)
{
	...
	if (aes_siv_encrypt(key, sizeof(key), plaintext, sizeof(plaintext),
			    1, addr, len, out)) {
		wpa_error_log0(MSG_ERROR, "AES-SIV mode encryption failed");
		return 1;
	}
	...
	if (aes_siv_decrypt(key, sizeof(key), iv_c, sizeof(iv_c),
			    1, addr, len, out)) {
		wpa_error_log0(MSG_ERROR, "AES-SIV mode decryption failed");
		return 1;
	}
	...
	wpa_warning_log0(MSG_INFO, "AES-SIV test cases passed");
#endif /* CONFIG_MESH */
			     ...
			     unsigned int i)
{
	...
	if (omac1_aes_128(tv->k, tv->msg, tv->msg_len, result) ||
		...
		wpa_error_log1(MSG_ERROR, "OMAC1-AES-128 test vector %u failed", i);
		return 1;
	}
	...
	if (omac1_aes_128(key, msg, sizeof(msg), result) ||
		...
		wpa_error_log0(MSG_ERROR, "OMAC1-AES-128 short test mismatch");
		return 1;
	}
...
static int test_omac1(void)
{
	...
	wpa_warning_log0(MSG_INFO, "OMAC1-AES-128 test cases passed");
...
static int test_eax(void)
{
	...
	if (aes_128_eax_encrypt(key, nonce, sizeof(nonce), hdr, sizeof(hdr),
		...
		wpa_error_log0(MSG_ERROR, "AES-128 EAX mode encryption failed");
		return 1;
	}
	...
	if (aes_128_eax_decrypt(key, nonce, sizeof(nonce), hdr, sizeof(hdr),
				data, sizeof(data), tag)) {
		wpa_error_log0(MSG_ERROR, "AES-128 EAX mode decryption failed");
		return 1;
	}
	...
	wpa_warning_log0(MSG_INFO, "AES-128 EAX mode test cases passed");
#endif /* EAP_PSK */
...
static int test_cbc(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(vectors); i++) {
		...
		if (aes_128_cbc_encrypt(tv->key, tv->iv, buf, tv->len) ||
			...
			wpa_error_log1(MSG_ERROR, "AES-CBC encrypt %d failed", i);
			ret++;
		}
		...
		if (aes_128_cbc_decrypt(tv->key, tv->iv, buf, tv->len) ||
			...
			wpa_error_log1(MSG_ERROR, "AES-CBC decrypt %d failed", i);
			ret++;
		}
...
static int test_ecb(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(vectors); i++) {
		...
		if (hexstr2bin(tv->key, key, sizeof(key)) ||
			...
			wpa_error_log1(MSG_ERROR, "Invalid AES-ECB test vector %u",
				   i);
			ret++;
			continue;
		}
		...
		if (aes_128_encrypt_block(key, plain, out) < 0 ||
			...
			wpa_error_log1(MSG_ERROR, "AES-ECB encrypt %u failed", i);
			ret++;
		}
	}

	if (!ret)
		wpa_warning_log0(MSG_INFO, "AES ECB mode test cases passed");
...
static int test_key_wrap(void)
{
	...
	wpa_warning_log0(MSG_INFO, "RFC 3394 - Test vector 4.1");
	if (aes_wrap(kek41, sizeof(kek41), sizeof(plain41) / 8, plain41,
		     result)) {
		wpa_error_log0(MSG_ERROR, "AES-WRAP-128 reported failure");
		ret++;
	}
	if (os_memcmp(result, crypt41, sizeof(crypt41)) != 0) {
		wpa_error_log0(MSG_ERROR, "AES-WRAP-128 failed");
		ret++;
	}
	if (aes_unwrap(kek41, sizeof(kek41), sizeof(plain41) / 8, crypt41,
		       result)) {
		wpa_error_log0(MSG_ERROR, "AES-UNWRAP-128 reported failure");
		ret++;
	}
	if (os_memcmp(result, plain41, sizeof(plain41)) != 0) {
		wpa_error_log0(MSG_ERROR, "AES-UNWRAP-128 failed");
		ret++;
	}

#ifndef CONFIG_BORINGSSL
	wpa_warning_log0(MSG_INFO, "RFC 3394 - Test vector 4.2");
	if (aes_wrap(kek42, sizeof(kek42), sizeof(plain42) / 8, plain42,
		     result)) {
		wpa_error_log0(MSG_ERROR, "AES-WRAP-192 reported failure");
		ret++;
	}
	if (os_memcmp(result, crypt42, sizeof(crypt42)) != 0) {
		wpa_error_log0(MSG_ERROR, "AES-WRAP-192 failed");
		ret++;
	}
	if (aes_unwrap(kek42, sizeof(kek42), sizeof(plain42) / 8, crypt42,
		       result)) {
		wpa_error_log0(MSG_ERROR, "AES-UNWRAP-192 reported failure");
		ret++;
	}
	if (os_memcmp(result, plain42, sizeof(plain42)) != 0) {
		wpa_error_log0(MSG_ERROR, "AES-UNWRAP-192 failed");
		ret++;
	}
#endif /* CONFIG_BORINGSSL */

	wpa_warning_log0(MSG_INFO, "RFC 3394 - Test vector 4.3");
	if (aes_wrap(kek43, sizeof(kek43), sizeof(plain43) / 8, plain43,
		     result)) {
		wpa_error_log0(MSG_ERROR, "AES-WRAP-256 reported failure");
		ret++;
	}
	if (os_memcmp(result, crypt43, sizeof(crypt43)) != 0) {
		wpa_error_log0(MSG_ERROR, "AES-WRAP-256 failed");
		ret++;
	}
	if (aes_unwrap(kek43, sizeof(kek43), sizeof(plain43) / 8, crypt43,
		       result)) {
		wpa_error_log0(MSG_ERROR, "AES-UNWRAP-256 reported failure");
		ret++;
	}
	if (os_memcmp(result, plain43, sizeof(plain43)) != 0) {
		wpa_error_log0(MSG_ERROR, "AES-UNWRAP-256 failed");
		ret++;
	}

#ifndef CONFIG_BORINGSSL
	wpa_warning_log0(MSG_INFO, "RFC 3394 - Test vector 4.4");
	...
	wpa_warning_log0(MSG_INFO, "RFC 3394 - Test vector 4.5");
	...
	wpa_warning_log0(MSG_INFO, "RFC 3394 - Test vector 4.6");
	...
	if (!ret)
		wpa_warning_log0(MSG_INFO, "AES key wrap/unwrap test cases passed");
...
static int test_md5(void)
{
	...
	if (!errors)
		wpa_warning_log0(MSG_INFO, "MD5 test cases passed");

	return errors;
#else /* CONFIG_FIPS */
	wpa_warning_log0(MSG_INFO, "MD5 test cases skipped due to CONFIG_FIPS");
	return 0;
#endif /* CONFIG_FIPS */
}
...
static int test_eap_fast(void)
{
	...
	wpa_warning_log0(MSG_INFO, "EAP-FAST test cases");

	wpa_warning_log0(MSG_INFO, "- T-PRF (SHA1) test case / master_secret");
	if (sha1_t_prf(pac_key, sizeof(pac_key),
		...
		wpa_warning_log0(MSG_INFO, "T-PRF test - FAILED!");
		errors++;
	}

#ifndef CONFIG_FIPS
	wpa_warning_log0(MSG_INFO, "- PRF (TLS, SHA1/MD5) test case / key_block");
	if (tls_prf_sha1_md5(master_secret, sizeof(master_secret),
		...
		wpa_warning_log0(MSG_INFO, "PRF test - FAILED!");
		errors++;
	}
#endif /* CONFIG_FIPS */

	wpa_warning_log0(MSG_INFO, "- T-PRF (SHA1) test case / IMCK");
	...
	wpa_warning_log0(MSG_INFO, "- T-PRF (SHA1) test case / MSK");
	...
	wpa_warning_log0(MSG_INFO, "- T-PRF (SHA1) test case / EMSK");
	...
	wpa_warning_log0(MSG_INFO, "- Compound MAC test case");
	...
	if (hmac_sha1(cmk, 20, tlv, sizeof(tlv), tlv + sizeof(tlv) - 20) < 0 ||
		...
		wpa_warning_log0(MSG_INFO, "Compound MAC test - FAILED!");
		errors++;
	}
...
#ifndef HISI_CODE_CROP /* This takes quite long to derive.. */
	{
		"password",
		"salt",
		16777216,
...
#ifndef HISI_CODE_CROP /* \0 not currently supported in passphrase parameters.. */
	{
		"pass\0word",
		"sa\0lt",
		4096,
...
static int test_sha1(void)
{
	...
	wpa_warning_log0(MSG_INFO, "PRF-SHA1 test cases:");

	if (sha1_prf(key0, sizeof(key0), "prefix", data0, sizeof(data0) - 1,
		     res, sizeof(prf0)) == 0 &&
	    os_memcmp(res, prf0, sizeof(prf0)) == 0)
		wpa_warning_log0(MSG_INFO, "Test case 0 - OK");
	else {
		wpa_warning_log0(MSG_INFO, "Test case 0 - FAILED!");
		ret++;
	}
	...
	if (sha1_prf(key1, sizeof(key1) - 1, "prefix", data1, sizeof(data1) - 1,
		...
		wpa_warning_log0(MSG_INFO, "Test case 1 - OK");
	else {
		wpa_warning_log0(MSG_INFO, "Test case 1 - FAILED!");
		ret++;
	}
	...
	if (sha1_prf(key2, sizeof(key2), "prefix", data2, sizeof(data2),
		...
		wpa_warning_log0(MSG_INFO, "Test case 2 - OK");
	else {
		wpa_warning_log0(MSG_INFO, "Test case 2 - FAILED!");
		ret++;
	}

	ret += test_eap_fast();

	wpa_warning_log0(MSG_INFO, "PBKDF2-SHA1 Passphrase test cases:");
	for (i = 0; i < NUM_PASSPHRASE_TESTS; i++) {
		...
		if (pbkdf2_sha1(test->passphrase,
			...
			wpa_warning_log1(MSG_INFO, "Test case %d - OK", i);
		else {
			wpa_warning_log1(MSG_INFO, "Test case %d - FAILED!", i);
			ret++;
		}
	}

	wpa_warning_log0(MSG_INFO, "PBKDF2-SHA1 test cases (RFC 6070):");
	...
	if (!ret)
		wpa_warning_log0(MSG_INFO, "SHA1 test cases passed");
	return ret;
}
...
static int test_sha256(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(tests); i++) {
		wpa_warning_log1(MSG_INFO, "SHA256 test case %d:", i + 1);
	...
	for (i = 0; i < ARRAY_SIZE(hmac_tests); i++) {
		...
		wpa_warning_log1(MSG_INFO, "HMAC-SHA256 test case %d:", i + 1);
	...
	wpa_warning_log0(MSG_INFO, "Test IEEE 802.11r KDF");
	...
	if (!errors)
		wpa_warning_log0(MSG_INFO, "SHA256 test cases passed");
	return errors;
}
...
static int test_sha384(void)
{
	...
	if (sha384_vector(1, addr, len, hash) < 0 ||
		...
		wpa_warning_log0(MSG_INFO, "SHA384 test case 1: FAIL");
		errors++;
	} else {
		wpa_warning_log0(MSG_INFO, "SHA384 test case 1: OK");
	}
	...
	if (sha384_vector(2, addr, len, hash) < 0 ||
	    os_memcmp(hash, hash_res, 48) != 0) {
		wpa_warning_log0(MSG_INFO, "SHA384 test case 2: FAIL");
		errors++;
	} else {
		wpa_warning_log0(MSG_INFO, "SHA384 test case 2: OK");
	}
	...
	for (i = 0; i < ARRAY_SIZE(hmac_tests); i++) {
		...
		if (t->hash384[0] == 0 && t->hash384[1] == 0 &&
			...
			continue;
		wpa_warning_log1(MSG_INFO, "HMAC-SHA384 test case %d:", i + 1);
	...
	if (!errors)
		wpa_warning_log0(MSG_INFO, "SHA384 test cases passed");
	return errors;
#else /* CONFIG_SHA384 */
	return 0;
#endif /* CONFIG_SHA384 */
...
static int test_fips186_2_prf(void)
{
	...
	if (fips186_2_prf(xkey, sizeof(xkey), buf, sizeof(buf)) < 0 ||
		...
		wpa_warning_log0(MSG_INFO, "fips186_2_prf failed");
		return 1;
	}
...
static int test_extract_expand_hkdf(void)
{
	...
	wpa_warning_log0(MSG_INFO, "Testing Extract-and-Expand HKDF (RFC 5869)");

	wpa_warning_log0(MSG_INFO, "RFC 5869 - Test Case 1");
	if (hmac_sha256(salt1, sizeof(salt1), ikm1, sizeof(ikm1), prk) < 0)
		return -1;
	if (os_memcmp(prk, prk1, SHA256_MAC_LEN) != 0) {
		wpa_warning_log0(MSG_INFO, "HKDF-Extract mismatch in PRK");
		return -1;
	}

	wpa_warning_log0(MSG_INFO, "RFC 5869 - Test Case 2");
	...
	wpa_warning_log0(MSG_INFO, "Extract-and-Expand HKDF test cases passed");
...
static int test_ms_funcs(void)
{
	...
	if (nt_password_hash((u8 *) password, os_strlen(password), buf) ||
		...
		wpa_error_log0(MSG_ERROR, "nt_password_hash failed");
		errors++;
	}
	...
	if (generate_nt_response(auth_challenge, peer_challenge,
		...
		wpa_error_log0(MSG_ERROR, "generate_nt_response failed");
		errors++;
	}
	...
	if (hash_nt_password_hash(password_hash, buf) ||
		...
		wpa_error_log0(MSG_ERROR, "hash_nt_password_hash failed");
		errors++;
	}
	...
	if (generate_authenticator_response((u8 *) password,
		...
		wpa_error_log0(MSG_ERROR, "generate_authenticator_response failed");
		errors++;
	}

	if (get_master_key(password_hash_hash, nt_response, buf) ||
	    os_memcmp(master_key, buf, sizeof(master_key)) != 0) {
		wpa_error_log0(MSG_ERROR, "get_master_key failed");
		errors++;
	}
	...
	if (get_asymetric_start_key(master_key, buf, sizeof(send_start_key),
		...
		wpa_error_log0(MSG_ERROR, "get_asymetric_start_key failed");
		errors++;
	}

	if (errors) {
		wpa_error_log1(MSG_ERROR, "ms_funcs: %d errors", errors);
	} else {
		wpa_warning_log0(MSG_INFO, "ms_funcs test cases passed");
	}
	return errors;
#else /* CONFIG_FIPS */
	wpa_warning_log0(MSG_INFO, "ms_funcs test cases skipped due to CONFIG_FIPS");
	return 0;
#endif /* CONFIG_FIPS */
}
...
int crypto_module_tests(void)
{
	...
	wpa_warning_log0(MSG_INFO, "crypto module tests");
	if (test_siv() ||
	    test_omac1() ||
	    test_eax() ||
	    test_cbc() ||
