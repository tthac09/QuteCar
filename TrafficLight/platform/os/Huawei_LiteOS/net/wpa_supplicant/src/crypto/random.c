...
{
	return crypto_get_random(buf, len);
}
#else
...
static void random_extract(u8 *out)
{
	...
	(void)hmac_sha1(dummy_key, sizeof(dummy_key), (const u8 *) pool,
		  sizeof(pool), hash);
	...
	(void)hmac_sha1(dummy_key, sizeof(dummy_key), (const u8 *) buf,
		  sizeof(buf), hash);
	/*
...
int random_get_bytes(void *buf, size_t len)
{
	...
	while (left) {
		...
		if (crypto_get_random(tmp, sizeof(tmp)) < 0) {
			wpa_error_log0(MSG_ERROR, "random: No entropy available "
...
int random_pool_ready(void)
{
	...
	if (own_pool_ready >= MIN_READY_MARK ||
		...
		wpa_warning_log0(MSG_INFO, "random: Allow operation to proceed "
			   "based on internal entropy");
		return 1;
	}

	wpa_warning_log0(MSG_INFO, "random: Not enough entropy pool available for "
		   "secure operations");
	return 0;
#else /* __linux__ */
...
void random_init(const char *entropy_file)
{
	...
	(void)eloop_register_read_sock(random_fd, random_read_fd, NULL, NULL);
#endif /* __linux__ */
