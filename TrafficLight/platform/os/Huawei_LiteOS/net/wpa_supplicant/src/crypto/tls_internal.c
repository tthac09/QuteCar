			      ...
			      const struct tls_connection_params *params)
{
	...
	if (params->subject_match) {
		wpa_warning_log0(MSG_INFO, "TLS: subject_match not supported");
		tlsv1_cred_free(cred);
		return -1;
	}

	if (params->altsubject_match) {
		wpa_warning_log0(MSG_INFO, "TLS: altsubject_match not supported");
		tlsv1_cred_free(cred);
		return -1;
	}

	if (params->suffix_match) {
		wpa_warning_log0(MSG_INFO, "TLS: suffix_match not supported");
		tlsv1_cred_free(cred);
		return -1;
	}

	if (params->domain_match) {
		wpa_warning_log0(MSG_INFO, "TLS: domain_match not supported");
		tlsv1_cred_free(cred);
		return -1;
	}

	if (params->openssl_ciphers) {
		wpa_warning_log0(MSG_INFO, "TLS: openssl_ciphers not supported");
		tlsv1_cred_free(cred);
		return -1;
	}
	...
	if (tlsv1_set_ca_cert(cred, params->ca_cert,
			      ...
			      params->ca_path)) {
		wpa_warning_log0(MSG_INFO, "TLS: Failed to configure trusted CA "
			   "certificates");
		tlsv1_cred_free(cred);
		return -1;
	}
	...
	if (tlsv1_set_cert(cred, params->client_cert,
		...
		wpa_warning_log0(MSG_INFO, "TLS: Failed to configure client "
			   "certificate");
		tlsv1_cred_free(cred);
		return -1;
	}
	...
	if (tlsv1_set_private_key(cred, params->private_key,
		...
		wpa_warning_log0(MSG_INFO, "TLS: Failed to load private key");
		tlsv1_cred_free(cred);
		return -1;
	}
	...
	if (tlsv1_set_dhparams(cred, params->dh_file, params->dh_blob,
			       params->dh_blob_len)) {
		wpa_warning_log0(MSG_INFO, "TLS: Failed to load DH parameters");
		tlsv1_cred_free(cred);
		return -1;
	}
			  ...
			  const struct tls_connection_params *params)
{
	...
	if (tlsv1_set_cert(cred, params->client_cert, params->client_cert_blob,
			   params->client_cert_blob_len)) {
		wpa_warning_log0(MSG_INFO, "TLS: Failed to configure server "
			   "certificate");
		return -1;
	}
